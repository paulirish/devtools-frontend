 ...
 */
WebInspector.TransformController =
    function(element, disableRotate) {
  this._shortcuts = {};
  ...
  WebInspector.installDragHandle(
      element, this._onDragStart.bind(this), this._onDrag.bind(this),
      this._onDragEnd.bind(this), 'move', null);
  ...
  this._controlPanelToolbar =
      new WebInspector.Toolbar('transform-control-panel');

  /** @type {!Object<string, !WebInspector.ToolbarToggle>} */
  this._modeButtons = {};
  if (!disableRotate) {
    var panModeButton = new WebInspector.ToolbarToggle(
        WebInspector.UIString('Pan mode (X)'), 'pan-toolbar-item');
    panModeButton.addEventListener(
        'click',
        this._setMode.bind(this, WebInspector.TransformController.Modes.Pan));
    this._modeButtons[WebInspector.TransformController.Modes.Pan] =
        panModeButton;
    ...
    rotateModeButton.addEventListener(
        'click', this._setMode.bind(
                     this, WebInspector.TransformController.Modes.Rotate));
    this._modeButtons[WebInspector.TransformController.Modes.Rotate] =
        rotateModeButton;
  ...
  resetButton.addEventListener(
      'click', this.resetAndNotify.bind(this, undefined));
  ...
  _onKeyDown: function(event) {
    ...
    var shortcutKey =
        WebInspector.KeyboardShortcut.makeKeyFromEventIgnoringModifiers(event);
  ...
  _onScale: function(scaleFactor, x, y) {
    scaleFactor =
        Number.constrain(
            this._scale * scaleFactor, this._minScale, this._maxScale) /
        this._scale;
  ...
  _onKeyboardZoom: function(zoomFactor) {
    this._onScale(
        zoomFactor, this.element.clientWidth / 2,
  ...
  _onKeyboardPanOrRotate: function(xMultiplier, yMultiplier) {
    ...
    if (this._mode === WebInspector.TransformController.Modes.Rotate) {
      // Sic! _onRotate treats X and Y as "rotate around X" and "rotate around
      // Y", so swap X/Y multiplers.
      this._onRotate(
  ...
  _onDrag: function(event) {
    if (this._mode === WebInspector.TransformController.Modes.Rotate) {
      this._onRotate(
          this._oldRotateX +
              (this._originY - event.clientY) / this.element.clientHeight * 180,
          this._oldRotateY -
              (this._originX - event.clientX) / this.element.clientWidth * 180);
    } else {
