...
WebInspector.LayerView = function() {}
     ...
     */
    WebInspector.LayerView.Selection =
        function(type, layer) {
      this._type = type;
      this._layer = layer;
    }
 ...
 */
     ...
     */
      _isEqual: function(other) { return false; }
                              ...
                              * @param {!WebInspector.Layer} layer
                              */
                             WebInspector.LayerView.LayerSelection =
                function(layer) {
      console.assert(layer, 'LayerSelection with empty layer');
      WebInspector.LayerView.Selection.call(
          this, WebInspector.LayerView.Selection.Type.Layer, layer);
    }
     ...
     */
      ...
      __proto__: WebInspector.LayerView.Selection.prototype
                                  ...
                                  */
                                 WebInspector.LayerView.ScrollRectSelection =
                    function(layer, scrollRectIndex) {
      WebInspector.LayerView.Selection.call(
     ...
     */
      _isEqual: function(other) {
        return other._type ===
            WebInspector.LayerView.Selection.Type.ScrollRect &&
            this.layer().id() === other.layer().id() &&
      ...
      __proto__: WebInspector.LayerView.Selection.prototype
                                      ...
                                      * traceEvent
                                      */
                                     WebInspector.LayerView.TileSelection =
                        function(layer, traceEvent) {
      WebInspector.LayerView.Selection.call(
          this, WebInspector.LayerView.Selection.Type.Tile, layer);
     ...
     */
      _isEqual: function(other) {
        return other._type === WebInspector.LayerView.Selection.Type.Tile &&
            this.layer().id() === other.layer().id() &&
            this.traceEvent === other.traceEvent;
      },
     ...
     */
      ...
      this._showInternalLayersSetting = WebInspector.settings.createSetting(
          'layersShowInternalLayers', false);
    }
      ...
      setLayerTree: function(layerTree) {
        ...
        var selectedLayer =
            this._selectedObject && this._selectedObject.layer();
        if (selectedLayer &&
            (!layerTree || !layerTree.layerById(selectedLayer.id())))
          this.selectObject(null);
        var hoveredLayer = this._hoveredObject && this._hoveredObject.layer();
        if (hoveredLayer &&
            (!layerTree || !layerTree.layerById(hoveredLayer.id())))
       ...
       */
      hoverObject: function(selection) {
        if (WebInspector.LayerView.Selection.isEqual(
     ...
     */
      ...
      showContextMenu: function(contextMenu, selection) {
        contextMenu.appendCheckboxItem(
            WebInspector.UIString('Show internal layers'),
            this._toggleShowInternalLayers.bind(this),
            this._showInternalLayersSetting.get());
        var node = selection && selection.layer() &&
            selection.layer().nodeForSelfOrAncestor();
        if (node)
     ...
     */
      ...
      _toggleShowInternalLayers: function() {
        this._showInternalLayersSetting.set(
            !this._showInternalLayersSetting.get());
      },
