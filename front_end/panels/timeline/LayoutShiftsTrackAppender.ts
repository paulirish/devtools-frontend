  ...
  */
  ...
  getDrawOverride(event: Trace.Types.Events.Event): DrawOverride|undefined {
    if (!Root.Runtime.experiments.isEnabled(Root.Runtime.ExperimentName.TIMELINE_LAYOUT_SHIFT_DETAILS)) {
      ...
      if (Trace.Types.Events.isLayoutShift(event)) {
        return (context, x, y, _width, levelHeight, isVisible, timeToPosition) => {
    ...
    if (Trace.Types.Events.isLayoutShift(event)) {
      ...
      return (context, x, y, _width, levelHeight, isVisible) => {
        ...
        const halfSize = levelHeight / 2;
        if (!isVisible(levelHeight - buffer * 2)) {
          return null;
        }
    ...
    if (Trace.Types.Events.isSyntheticLayoutShiftCluster(event)) {
      return (context, x, y, width, levelHeight, isVisible, timeToPosition) => {
        if (!isVisible(width)) {
          return null;
        }
        const m = timeToPosition(x);
