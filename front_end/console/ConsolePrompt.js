...
WebInspector.ConsolePrompt =
  ...
  self.runtime.extension(WebInspector.TextEditorFactory)
      .instance()
  ...
  function gotFactory(factory) {
    ...
    this._editor.widget().element.addEventListener(
        'keydown', this._editorKeyDown.bind(this), true);
  ...
  _isCaretAtEndOfPrompt: function() {
    return !!this._editor &&
        this._editor.selection().collapseToEnd().equal(
  ...
  moveCaretToEndOfPrompt: function() {
    if (this._editor)
      this._editor.setSelection(
          WebInspector.TextRange.createFromLocation(Infinity, Infinity));
  },
  ...
  _editorKeyDown: function(event) {
    ...
    switch (keyboardEvent.keyCode) {
      ...
      case WebInspector.KeyboardShortcut.Keys.P.code:  // Ctrl+P = Previous
        if (WebInspector.isMac() && keyboardEvent.ctrlKey &&
    ...
    if (isPrevious)
      this._editor.setSelection(
          WebInspector.TextRange.createFromLocation(0, Infinity));
    else
  ...
  _appendCommand: function(text, useCommandLineAPI) {
    ...
    if (currentExecutionContext) {
      WebInspector.ConsoleModel.evaluateCommandInConsole(
          currentExecutionContext, text, useCommandLineAPI);
      if (WebInspector.inspectorView.currentPanel() &&
          WebInspector.inspectorView.currentPanel().name === 'console')
        WebInspector.userMetrics.actionTaken(
            WebInspector.UserMetrics.Action.CommandEvaluatedInConsolePanel);
    }
  },
  ...
  _substituteRange: function(lineNumber, columnNumber) {
    ...
    return new WebInspector.TextRange(
        lineNumber, index + 1, lineNumber, columnNumber);
  },
  ...
  _wordsWithPrefix: function(prefixRange, substituteRange) {
    ...
    var before = this._editor.text(new WebInspector.TextRange(
        0, 0, prefixRange.startLine, prefixRange.startColumn));
