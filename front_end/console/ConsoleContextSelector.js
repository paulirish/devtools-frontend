 ...
 */
WebInspector.ConsoleContextSelector =
  ...
  WebInspector.targetManager.addModelListener(
      WebInspector.RuntimeModel,
      WebInspector.RuntimeModel.Events.ExecutionContextChanged,
  ...
  this._selectElement.addEventListener(
      'change', this._executionContextChanged.bind(this), false);
  WebInspector.context.addFlavorChangeListener(
      WebInspector.ExecutionContext, this._executionContextChangedExternally,
      this);
}
  ...
  _titleFor: function(executionContext) {
    ...
    if (executionContext.isDefault) {
      if (executionContext.frameId) {
        var resourceTreeModel = WebInspector.ResourceTreeModel.fromTarget(
            executionContext.target());
        var frame = resourceTreeModel &&
            resourceTreeModel.frameForId(executionContext.frameId);
        result = frame ? frame.displayName() : executionContext.label();
      } else {
        result =
            executionContext.target().decorateLabel(executionContext.label());
      }
    } else {
      result = '\u00a0\u00a0\u00a0\u00a0' +
          (executionContext.label() || executionContext.origin);
    }
  ...
  _executionContextCreated: function(executionContext) {
    // FIXME(413886): We never want to show execution context for the main
    // thread of shadow page in service/shared worker frontend.
    ...
    var index =
        contexts
            .lowerBound(
                executionContext,
                executionContext.runtimeModel.executionContextComparator()) this
            ._selectElement.insertBefore(newOption, options[index]);
    ...
    /**
         ...
         */
    function mapping(option) { return option.__executionContext; }
  },
  ...
  _onExecutionContextCreated: function(event) {
    var executionContext =
        /** @type {!WebInspector.ExecutionContext} */ (event.data);
  ...
  _updateSelectionWarning: function() {
    ...
    this._selectElement.parentElement.classList.toggle(
        'warning',
        !this._isTopContext(executionContext) && this._hasTopContext())
  },
  ...
  _isTopContext: function(executionContext) {
    ...
    var resourceTreeModel =
        WebInspector.ResourceTreeModel.fromTarget(executionContext.target());
    var frame = executionContext.frameId && resourceTreeModel &&
        resourceTreeModel.frameForId(executionContext.frameId);
    if (!frame)
      return false;
  ...
  targetAdded: function(target) {
    target.runtimeModel.executionContexts().forEach(
