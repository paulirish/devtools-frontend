 ...
 */
WebInspector.NetworkLogViewColumns = function(
                                         networkLogView,
                                         networkLogLargeRowsSetting) {
  this._networkLogView = networkLogView;

  /** @type {!WebInspector.Setting} */
  this._persistantSettings =
      WebInspector.settings.createSetting('networkLogColumns', {});
...
WebInspector.NetworkLogViewColumns._defaultColumns = [
  ...
  },
  {
    ...
    sortConfig: {
      sortingFunction:
          WebInspector.NetworkDataGridNode.InitialPriorityComparator
    }
  },
  {
    id: 'connectionid',
  ...
  },
  {
    ...
    sortConfig: {
      sortingFunction:
          WebInspector.NetworkDataGridNode.ResponseHeaderStringComparator.bind(
              null, 'cache-control')
    }
  },
  {
  ...
  },
  {
    ...
    sortConfig: {
      sortingFunction:
          WebInspector.NetworkDataGridNode.ResponseHeaderNumberComparator.bind(
              null, 'content-length')
    }
  },
  {
  ...
  },
  {
    ...
    sortConfig: {
      sortingFunction:
          WebInspector.NetworkDataGridNode.ResponseHeaderDateComparator.bind(
              null, 'last-modified')
    }
  },
  {
    ...
    sortConfig: {
      entries: [
        ...
        },
        {
          ...
          sort: WebInspector.DataGrid.Order.Ascending,
          sortingFunction: WebInspector.NetworkDataGridNode
                               .RequestPropertyComparator.bind(null, 'endTime'),
          calculator: WebInspector.NetworkLogViewColumns._calculatorTypes.Time
        },
        {
          id: 'duration',
          ...
          sortingFunction:
              WebInspector.NetworkDataGridNode.RequestPropertyComparator.bind(
                  null, 'duration'),
          calculator:
              WebInspector.NetworkLogViewColumns._calculatorTypes.Duration
        },
        {
          id: 'latency',
          ...
          sort: WebInspector.DataGrid.Order.Descending,
          sortingFunction: WebInspector.NetworkDataGridNode
                               .RequestPropertyComparator.bind(null, 'latency'),
          calculator:
              WebInspector.NetworkLogViewColumns._calculatorTypes.Duration
        }
      ]
 ...
 */
WebInspector.NetworkLogViewColumns._convertToDataGridDescriptor = function(
    columnConfig) {
...
WebInspector.NetworkLogViewColumns.prototype = {
  ...
  createGrid: function(timeCalculator, durationCalculator) {
    ...
    for (var currentConfigColumn of defaultColumns) {
      var columnConfig =
          /** @type {!WebInspector.NetworkLogViewColumns.Descriptor} */ (
              Object.assign(
                  /** @type {!Object} */ ({}), defaultColumnConfig,
                  currentConfigColumn));
    ...
    this._calculatorsMap.set(
        WebInspector.NetworkLogViewColumns._calculatorTypes.Time,
        timeCalculator);
    this._calculatorsMap.set(
        WebInspector.NetworkLogViewColumns._calculatorTypes.Duration,
        durationCalculator);
    ...
    this._dataGrid = new WebInspector.SortableDataGrid(this._columns.map(
        WebInspector.NetworkLogViewColumns._convertToDataGridDescriptor));
    ...
    this._dataGrid.addEventListener(
        ...
        this.updateDividersIfNeeded, this);
  ...
  _setupDropdownColumns: function() {
    for (var columnConfig of this._columns) {
      ...
      header.createChild('div', 'sort-order-icon-container')
          .createChild('div', 'sort-order-icon');
      columnConfig.selectBox = select;
      select.addEventListener(
          'change', this._sortByDropdownItem.bind(this, columnConfig), false);
  ...
  _sortHandler: function() {
    ...
    var columnConfig = this._columns.find(
        columnConfig => columnConfig.id === columnIdentifier);
    if (!columnConfig)
      return;
    if (columnConfig.sortConfig && columnConfig.sortConfig.entries) {
    ...
    this._dataGrid.sortNodes(
        columnConfig.sortConfig.sortingFunction,
  ...
  _sortByDropdownItem: function(columnConfig) {
    ...
    var selectedItemConfig =
        columnConfig.sortConfig
            .entries[selectedIndex - 1];  // -1 because of placeholder.
    ...
    this._dataGrid.markColumnAsSortedBy(
        columnConfig.id,
        /** @type {!WebInspector.DataGrid.Order} */ (selectedItemConfig.sort));
    if (selectedItemConfig.calculator)
      this._networkLogView.setCalculator(
          this._calculatorsMap.get(selectedItemConfig.calculator));
  ...
  _updateColumns: function() {
    ...
    if (this._gridMode) {
      for (var columnConfig of this._columns)
        visibleColumns[columnConfig.id] =
            (columnConfig.visible || !columnConfig.hideable);
    } else {
  ...
  _loadColumns: function() {
    ...
    for (var columnId of columnIds) {
      ...
      var columnConfig =
          this._columns.find(columnConfig => columnConfig.id === columnId);
      if (!columnConfig)
  ...
  contextMenu: function(event) {
    ...
    var columnConfigs =
        this._columns.filter(columnConfig => columnConfig.hideable);
    var contextMenu = new WebInspector.ContextMenu(event);
    var nonResponseHeaders =
    ...
    var responseSubMenu = contextMenu.appendSubMenuItem(
  ...
  _manageCustomHeaderDialog: function() {
    ...
    for (var columnConfig of this._columns) {
      if (columnConfig.isResponseHeader)
        customHeaders.push(
            {title: columnConfig.title, editable: columnConfig.isCustomHeader});
    }
  ...
  _removeCustomHeader: function(headerId) {
    ...
    var index =
        this._columns.findIndex(columnConfig => columnConfig.id === headerId);
    if (index === -1)
      return false;
  ...
  _addCustomHeader: function(headerTitle, headerId, index) {
    ...
    var currentColumnConfig =
        this._columns.find(columnConfig => columnConfig.id === headerId);
                  ...
                  sortConfig: {
                    sortingFunction:
                        WebInspector.NetworkDataGridNode
                            .ResponseHeaderStringComparator.bind(null, headerId)
                  }
                }));
  ...
  _changeCustomHeader: function(oldHeaderId, newHeaderTitle, newHeaderId) {
    ...
    var oldIndex = this._columns.findIndex(
        columnConfig => columnConfig.id === oldHeaderId);
    var oldColumnConfig = this._columns[oldIndex];
    var currentColumnConfig =
        this._columns.find(columnConfig => columnConfig.id === newHeaderId);
    if (!oldColumnConfig ||
        (currentColumnConfig && oldHeaderId !== newHeaderId))
      return false;
  ...
  updateDividersIfNeeded: function() {
    ...
    var calculator = this._calculatorsMap.get(
        WebInspector.NetworkLogViewColumns._calculatorTypes.Time);
  ...
  _getPopoverAnchor: function(element, event) {
    ...
    if (anchor && anchor.parentElement.request &&
    ...
    if (anchor && anchor.request) {
      var initiator =
          /** @type {!WebInspector.NetworkRequest} */ (anchor.request)
              .initiator();
  ...
  _showPopover: function(anchor, popover) {
    ...
    if (anchor.classList.contains('network-script-initiated')) {
      var request =
          /** @type {!WebInspector.NetworkRequest} */ (anchor.request);
      var initiator =
          /** @type {!NetworkAgent.Initiator} */ (request.initiator());
      content =
      ...
      content = WebInspector.RequestTimingView.createTimingTable(
          ...
          this._networkLogView.timeCalculator().minimumBoundary());
  ...
  addEventDividers: function(times, className) {
    for (var i = 0; i < times.length; ++i) {
      var element =
          createElementWithClass('div', 'network-event-divider ' + className);
  ...
  _updateRowsSize: function() {
    this._timelineGrid.element.classList.toggle(
        'small', !this._networkLogLargeRowsSetting.get());
  }
}