...
WebInspector.BlockedURLsPane =
  ...
  var addButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Add pattern'), 'add-toolbar-item');
  ...
  var clearButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Remove all'), 'clear-toolbar-item');
  clearButton.addEventListener('click', this._removeAll.bind(this));
  this._toolbar.appendToolbarItem(clearButton);

  this._emptyElement =
      this.contentElement.createChild('div', 'no-blocked-urls');
  this._emptyElement.createChild('span').textContent =
      WebInspector.UIString('Requests are not blocked. ');
  ...
  this._emptyElement.addEventListener(
      'contextmenu', this._emptyElementContextMenu.bind(this), true);

  this._listElement =
      this.contentElement.createChild('div', 'blocked-urls-list');
  ...
  _emptyElementContextMenu: function(event) {
    ...
    contextMenu.appendItem(
        WebInspector.UIString.capitalize('Add ^pattern'),
  ...
  _addButtonClicked: function() {
    ...
    var element =
        this._createElement('', this._blockedURLsSetting.get().length);
  ...
  _edit: function(content, element, onAccept) {
    ...
    input.value = content;
    input.placeholder = WebInspector.UIString(
  ...
  _removeAll: function() { this._blockedURLsSetting.set([]); },
  ...
  _contextMenu: function(index, event) {
    ...
    contextMenu.appendItem(
        ...
        this._addButtonClicked.bind(this));
    contextMenu.appendItem(
        WebInspector.UIString.capitalize('Remove ^pattern'),
        this._removeBlockedURL.bind(this, index));
    contextMenu.appendItem(
        WebInspector.UIString.capitalize('Remove ^all'),
  ...
  _createElement: function(url, index) {
    ...
    countElement.title = WebInspector.UIString(
        count === 1 ? '%d request blocked by this pattern' :
                      '%d requests blocked by this pattern',
        count);

    var removeButton = element.createChild('div', 'remove-button');
    removeButton.title = WebInspector.UIString('Remove');
    removeButton.addEventListener(
        'click', this._removeBlockedURL.bind(this, index), false);

    element.addEventListener(
        'contextmenu', this._contextMenu.bind(this, index), true);
    element.addEventListener(
        'dblclick',
        this._edit.bind(
            this, url, element, this._changeBlockedURL.bind(this, index)),
        false);
    return element;
  },
  ...
  reset: function() { this._blockedCountForUrl.clear(); },
...
WebInspector.BlockedURLsPane.reset =
  ...
  if (WebInspector.BlockedURLsPane._instance)
     ...
     */
    WebInspector.BlockedURLsPane.ActionDelegate =
        function() {}
