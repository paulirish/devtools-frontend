...
WebInspector.ResourceWebSocketFrameView =
  ...
  this._dataGrid.markColumnAsSortedBy(
      'time', WebInspector.DataGrid.Order.Ascending);
  ...
  this._dataGrid.addEventListener(
      WebInspector.DataGrid.Events.DeselectedNode, this._onFrameDeselected,
      this);
  this._splitWidget.setMainWidget(this._dataGrid.asWidget());

  var view =
      new WebInspector.EmptyWidget('Select frame to browse its content.');
 ...
 * @return {string}
 */
WebInspector.ResourceWebSocketFrameView.opCodeDescription =
    function(opCode, mask) {
  var rawDescription =
      WebInspector.ResourceWebSocketFrameView.opCodeDescriptions[opCode] || '';
  ...
  _frameAdded: function(event) {
    var frame =
        /** @type {!WebInspector.NetworkRequest.WebSocketFrame} */ (event.data);
  ...
  _onFrameSelected: function(event) {
    var selectedNode = /** @type {!WebInspector.ResourceWebSocketFrameNode} */ (
    ...
    function handleJSONData(parsedJSON) {
      ...
      if (parsedJSON)
        this._splitWidget.setSidebarWidget(
            WebInspector.JSONView.createSearchableView(parsedJSON));
      else
        this._splitWidget.setSidebarWidget(
            new WebInspector.ResourceSourceFrame(contentProvider));
    }
  },
  ...
  refresh: function() {
    ...
    for (var i = 0; i < frames.length; ++i)
      this._dataGrid.insertChild(new WebInspector.ResourceWebSocketFrameNode(
          this._request.url, frames[i]));
  },
  ...
  _sortItems: function() {
    this._dataGrid.sortNodes(
        this._timeComparator, !this._dataGrid.isSortOrderAscending());
  },
  ...
  __proto__: WebInspector.VBox.prototype
                      ...
                      * @param {string} url
                      * @param {!WebInspector.NetworkRequest.WebSocketFrame}
                      * frame
                      */
                     WebInspector.ResourceWebSocketFrameNode =
        function(url, frame) {
  this._frame = frame;
  ...
  var timeText = ('0' + time.getHours()).substr(-2) + ':' +
  ...
  this._isTextFrame = frame.opCode ===
      WebInspector.ResourceWebSocketFrameView.OpCodes.TextFrame;
  if (!this._isTextFrame)
    this._dataText = WebInspector.ResourceWebSocketFrameView.opCodeDescription(
        frame.opCode, frame.mask);
  ...
  createCells: function() {
    ...
    element.classList.toggle(
        'websocket-frame-view-row-error', this._frame.type ===
            WebInspector.NetworkRequest.WebSocketFrameType.Error);
    element.classList.toggle(
        'websocket-frame-view-row-outcoming', this._frame.type ===
            WebInspector.NetworkRequest.WebSocketFrameType.Send);
    element.classList.toggle(
        'websocket-frame-view-row-opcode', !this._isTextFrame);
