 ...
 */
WebInspector.NetworkConfigView =
    function() {
  ...
  _createSection: function(title, className) {
    var section =
        this.contentElement.createChild('section', 'network-config-group');
    if (className)
  ...
  _createCacheSection: function() {
    var section = this._createSection(
        WebInspector.UIString('Caching'), 'network-config-disable-cache');
    section.appendChild(WebInspector.SettingsUI.createSettingCheckbox(
        WebInspector.UIString('Disable cache'),
        WebInspector.moduleSetting('cacheDisabled'), true));
  },
  ...
  _createNetworkThrottlingSection: function() {
    ...
    WebInspector.NetworkConditionsSelector.decorateSelect(
        /** @type {!HTMLSelectElement} */ (
            section.createChild('select', 'chrome-select')));
  },

  _createUserAgentSection: function() {
    var section = this._createSection(
        WebInspector.UIString('User agent'), 'network-config-ua');
    var checkboxLabel = createCheckboxLabel(
        WebInspector.UIString('Select automatically'), true);
    section.appendChild(checkboxLabel);
    this._autoCheckbox = checkboxLabel.checkboxElement;
    this._autoCheckbox.addEventListener(
        'change', this._userAgentTypeChanged.bind(this));

    this._customUserAgentSetting =
        WebInspector.settings.createSetting('customUserAgent', '');
    this._customUserAgentSetting.addChangeListener(
        this._customUserAgentChanged, this);

    this._customUserAgent =
        section.createChild('div', 'network-config-ua-custom');
    this._customSelectAndInput =
        WebInspector.NetworkConfigView.createUserAgentSelectAndInput();
  ...
  _customUserAgentChanged: function() {
    if (this._autoCheckbox.checked)
      return;
    WebInspector.multitargetNetworkManager.setCustomUserAgentOverride(
        this._customUserAgentSetting.get());
  },
 ...
 */
  ...
  userAgentSelectElement.appendChild(
      new Option(customOverride.title, customOverride.value));
    ...
    for (var userAgentVersion of userAgentDescriptor.values) {
      var userAgentValue =
          WebInspector.MultitargetNetworkManager
              .patchUserAgentWithChromeVersion(userAgentVersion.value);
      groupElement.appendChild(
          new Option(userAgentVersion.title, userAgentValue));
    }
  }
  ...
  otherUserAgentElement.placeholder =
      WebInspector.UIString('Enter a custom user agent');
  ...
  function userAgentSelected() {
    var value =
        userAgentSelectElement.options[userAgentSelectElement.selectedIndex]
            .value;
  ...
  return {select: userAgentSelectElement, input: otherUserAgentElement};
        ...
        /** @type {!Array.<{title: string, values: !Array.<{title: string,
           value: string}>}>} */
