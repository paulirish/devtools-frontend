...
WebInspector.NetworkOverview.prototype = {
  ...
  onResize: function() {
    ...
    this.resetCanvas();
    var numBands =
        (((height - 1) / WebInspector.NetworkOverview._bandHeight) - 1) | 0;
  ...
  update: function() {
    ...
    if (!this._lastBoundary || !newBoundary.equals(this._lastBoundary)) {
      ...
      this._calculator.setBounds(
          ...
          this._calculator.minimumBoundary() + this._span);
    ...
    function drawLines(type, strokeStyle) {
      ...
      for (var i = 0; i < n;) {
        var y =
            lines[i++] * WebInspector.NetworkOverview._bandHeight + paddingTop;
    ...
    for (var i = 0; i < n; ++i) {
      ...
      for (var j = 0; j < timeRanges.length; ++j) {
        ...
        if (band !== -1 || type === WebInspector.RequestTimeRangeNames.Total)
          addLine(
              type, y, timeRanges[j].start * 1000, timeRanges[j].end * 1000);
      }
    }
    ...
    drawLines(
        WebInspector.RequestTimeRangeNames.ServiceWorkerPreparation, '#FF9800');
    ...
    context.beginPath();
    context.strokeStyle =
        '#8080FF';  // Keep in sync with .network-blue-divider CSS rule.
    for (var i = this._domContentLoadedEvents.length - 1; i >= 0; --i) {
      var x = Math.round(
                  calculator.computePosition(this._domContentLoadedEvents[i])) +
          0.5;
      context.moveTo(x, 0);
    ...
    context.beginPath();
    context.strokeStyle =
        '#FF8080';  // Keep in sync with .network-red-divider CSS rule.
    ...
    if (this._selectedFilmStripTime !== -1) {
      ...
      context.beginPath();
      context.strokeStyle =
          '#FCCC49';  // Keep in sync with .network-frame-divider CSS rule.
      var x =
          Math.round(calculator.computePosition(this._selectedFilmStripTime));
      context.moveTo(x, 0);
      context.lineTo(x, height);
      context.stroke();
    }
