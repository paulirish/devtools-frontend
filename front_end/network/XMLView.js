 ...
 */
    WebInspector.XMLView.createSearchableView =
        function(parsedXML) {
 ...
 * @return {?Document}
 */
        WebInspector.XMLView.parseXML =
            function(text, mimeType) {
  var parsedXML;
  try {
  ...
  _jumpToMatch: function(index, shouldJump) {
    ...
    var previousFocusElement =
        this._currentSearchTreeElements[this._currentSearchFocusIndex];
    ...
    if (newFocusElement) {
      ...
      newFocusElement.setSearchRegex(
          regex, WebInspector.highlightedCurrentSearchResultClassName);
    } else {
  ...
  _innerPerformSearch: function(shouldJump, jumpBackwards) {
    ...
    for (var element = this._treeOutline.rootElement(); element;
  ...
  jumpToNextSearchResult: function() {
    ...
    var newIndex =
        mod(this._currentSearchFocusIndex + 1,
  ...
  __proto__: WebInspector.Widget.prototype
                              ...
                              */
                             WebInspector.XMLView.Node =
                function(node, closeTag, xmlView) {
  ...
  this._updateTitle();
                 ...
                 * @param {!WebInspector.XMLView} xmlView
                 */
                WebInspector.XMLView.Node.populate =
                    function(root, xmlNode, xmlView) {
  ...
  while (node) {
    ...
    if (nodeType === 3 && currentNode.nodeValue.match(/\s+/))
      continue;
    // ignore ATTRIBUTE, ENTITY_REFERENCE, ENTITY, DOCUMENT, DOCUMENT_TYPE,
    // DOCUMENT_FRAGMENT, NOTATION
    if ((nodeType !== 1) && (nodeType !== 3) && (nodeType !== 4) &&
        (nodeType !== 7) && (nodeType !== 8))
      continue;
    root.appendChild(
        new WebInspector.XMLView.Node(currentNode, false, xmlView));
  }
}
  ...
  setSearchRegex: function(regex, additionalCssClassName) {
    ...
    if (ranges.length)
      WebInspector.highlightRangesWithStyleClass(
          this.listItemElement, ranges, cssClasses, this._highlightChanges);
    ...
    switch (node.nodeType) {
      case 1:  // ELEMENT
        ...
        for (var i = 0; i < attributes.length; ++i) {
          ...
          titleItems.push(
              '\u00a0', 'shadow-xml-view-tag', attributeNode.name,
              'shadow-xml-view-attribute-name', '="', 'shadow-xml-view-tag',
        ...
        if (!this.expanded) {
          if (node.childElementCount) {
            titleItems.push(
                '>', 'shadow-xml-view-tag', '\u2026', 'shadow-xml-view-comment',
                '</' + tag, 'shadow-xml-view-tag');
          } else if (this._node.textContent) {
            titleItems.push(
                '>', 'shadow-xml-view-tag', node.textContent,
                'shadow-xml-view-text', '</' + tag, 'shadow-xml-view-tag');
          } else {
      ...
      case 4:  // CDATA
        this._setTitle([
          '<![CDATA[', 'shadow-xml-view-cdata', node.nodeValue,
          'shadow-xml-view-text', ']]>', 'shadow-xml-view-cdata'
        ]);
        return;
      ...
      case 8:  // COMMENT
        this._setTitle(
            ['<!--' + node.nodeValue + '-->', 'shadow-xml-view-comment']);
        return;
    }
  },
  ...
  onattach: function() {
    this.listItemElement.classList.toggle(
        'shadow-xml-view-close-tag', this._closeTag);
  },

  onexpand: function() { this._updateTitle(); },

  oncollapse: function() { this._updateTitle(); },

  onpopulate: function() {
    WebInspector.XMLView.Node.populate(this, this._node, this._xmlView);
    this.appendChild(
        new WebInspector.XMLView.Node(this._node, true, this._xmlView));
  },
