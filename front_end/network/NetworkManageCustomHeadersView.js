 ...
 */
WebInspector.NetworkManageCustomHeadersView =
    function(
        columnData, addHeaderColumnCallback, changeHeaderColumnCallback,
  ...
  var placeholder =
      createElementWithClass('div', 'custom-headers-list-list-empty');
  ...
  this.contentElement.appendChild(createTextButton(
      WebInspector.UIString('Add custom header\u2026'),
  ...
  columnData.forEach(
      columnData =>
          this._columnConfigs.set(columnData.title.toLowerCase(), columnData));
  ...
  _headersUpdated: function() {
    ...
    this._columnConfigs.forEach(
        headerData => this._list.appendItem(
            {header: headerData.title}, headerData.editable));
  },
  ...
  commitEdit: function(item, editor, isNew) {
    ...
    if (success)
      this._columnConfigs.set(
          headerId.toLowerCase(), {title: headerId, editable: true});
  ...
  _createEditor: function() {
    ...
    fields.createChild('div', 'custom-headers-header')
        .appendChild(editor.createInput(
            'header', 'text', 'x-custom-header', validateHeader.bind(this)));
