 ...
 */
WebInspector.EventSourceMessagesView =
    function(request) {
  ...
  wasShown: function() {
    ...
    for (var i = 0; i < messages.length; ++i)
      this._dataGrid.insertChild(
          new WebInspector.EventSourceMessageNode(messages[i]));

    this._request.addEventListener(
        WebInspector.NetworkRequest.Events.EventSourceMessageAdded,
        this._messageAdded, this);
  },
  ...
  _messageAdded: function(event) {
    var message =
        /** @type {!WebInspector.NetworkRequest.EventSourceMessage} */ (
            event.data);
    this._dataGrid.insertChild(
        new WebInspector.EventSourceMessageNode(message));
  },
  ...
  _sortItems: function() {
    ...
    if (!sortColumnIdentifier)
      return;
    var comparator =
        WebInspector.EventSourceMessageNode.Comparators[sortColumnIdentifier];
    if (!comparator)
      return;
    this._dataGrid.sortNodes(
        comparator, !this._dataGrid.isSortOrderAscending());
  },
  ...
  __proto__: WebInspector.VBox.prototype
                      ...
                      * @param {!WebInspector.NetworkRequest.EventSourceMessage}
                      * message
                      */
                     WebInspector.EventSourceMessageNode =
        function(message) {
 ...
 * @return {number}
 */
                         WebInspector.EventSourceMessageNodeComparator =
            function(field, a, b) {
