...
(function(mod) {
  ...
  CodeMirror.defineMode('stylus', function(config) {
    var indentUnit = config.indentUnit, tagKeywords = keySet(tagKeywords_),
        ...
        commonAtoms = keySet(commonAtoms_), firstWordMatch = '', states = {},
        ch, style, type, override;
    ...
    function tokenBase(stream, state) {
      ...
      state.context.line.firstWord =
          firstWordMatch ? firstWordMatch[0].replace(/^\s*/, '') : '';
    ...
    function pushContext(state, stream, type, indent) {
      ...
      state.context =
          new Context(type, stream.indentation() + indent, state.context);
      return type;
    }
    ...
    function typeIsBlock(type, stream) {
      return (
          (endOfLine(stream) && (type == '{' || type == ']' || type == 'hash' ||
                                 type == 'qualifier')) ||
          type == 'block-mixin');
    }
    ...
    states.block = function(type, stream, state) {
      if ((type == 'comment' && startOfLine(stream)) ||
          (type == ',' && endOfLine(stream)) || type == 'mixin') {
      ...
      if (/@(font-face|media|supports|(-moz-)?document)/.test(type)) {
        return pushContext(
            state, stream, endOfLine(stream) ? 'block' : 'atBlock');
      }
    ...
    return {
      ...
      indent: function(state, textAfter, line) {
        ...
        var cx = state.context, ch = textAfter && textAfter.charAt(0),
            indent = cx.indent, lineFirstWord = firstWordOfLine(textAfter),
            lineIndent = line.length - line.replace(/^\s*/, '').length,
            prevLineFirstWord =
                state.context.prev ? state.context.prev.line.firstWord : '',
        ...
        if (cx.prev &&
          ...
          if (/@|\$|\d/.test(ch) || /^\{/.test(textAfter) ||
              /^\s*\/(\/|\*)/.test(textAfter) ||
              /^\s*\/\*/.test(prevLineFirstWord) ||
              /^\s*[\w-\.\[\]\'\"]+\s*(\?|:|\+)?=/i.test(textAfter) ||
              /^(\+|-)?[a-z][\w-]*\(/i.test(textAfter) ||
              /^return/.test(textAfter) || wordIsBlock(lineFirstWord)) {
            indent = lineIndent;
          } else if (
              /(\.|#|:|\[|\*|&|>|~|\+|\/)/.test(ch) ||
            ...
            if (/\,\s*$/.test(prevLineFirstWord)) {
              ...
              !/,\s*$/.test(line) && (wordIsVendorPrefix(lineFirstWord) ||
  ...
  var tagKeywords_ = [
    ...
    'meta',       'meter',  'nav',      'nobr',     'noframes',   'noscript',
  ...
  var nonStandardPropertyKeywords_ = [
    ...
    'searchfield-results-button', 'searchfield-results-decoration', 'zoom'
  ];
  var fontProperties_ = [
  ...
  var wordOperatorKeywords_ =
          [
            'in', 'and', 'or', 'not', 'is not', 'is a', 'is', 'isnt', 'defined',
            'if unless'
          ],
      ...
      commonDef_ = [
        '@font-face', '@keyframes', '@media', '@viewport', '@page', '@host',
        '@supports', '@block', '@css'
      ];

  var hintWords = tagKeywords_.concat(
      documentTypes_, mediaTypes_, mediaFeatures_, propertyKeywords_,
      nonStandardPropertyKeywords_, colorKeywords_, valueKeywords_,
      fontProperties_, wordOperatorKeywords_, blockKeywords_, commonAtoms_,
      commonDef_);
