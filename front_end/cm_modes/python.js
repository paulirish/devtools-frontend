...
(function(mod) {
  ...
  var commonKeywords = [
    ...
    'if',    'import', 'lambda', 'pass',    'raise',    'return', 'try',
    'while', 'with',   'yield',  'in'
  ];
  var commonBuiltins = [
    ...
    'enumerate',    'eval',       'filter',  'float',       'format',
  ...
  CodeMirror.registerHelper(
      'hintWords', 'python', commonKeywords.concat(commonBuiltins));

  function top(state) { return state.scopes[state.scopes.length - 1]; }
  ...
  CodeMirror.defineMode('python', function(conf, parserConf) {
    ...
    if (py3) {
      ...
      myBuiltins = myBuiltins.concat([
        ...
        'intern', 'long', 'raw_input', 'reduce', 'reload', 'unichr', 'unicode',
        'xrange', 'False', 'True', 'None'
      ]);
      var stringPrefixes =
    ...
    function tokenLexer(stream, state) {
      ...
      if (state.beginningOfLine && current == '@')
        return stream.match(identifiers, false) ? 'meta' :
      ...
      if (delimiter_index != -1)
        pushBracketScope(
            stream, state, '])}'.slice(delimiter_index, delimiter_index + 1));
    ...
    var external = {
      ...
      indent: function(state, textAfter) {
        ...
        var scope = top(state), closing = scope.type ==
            textAfter.charAt(0) if (scope.align != null) return scope.align -
                (closing ? 1 : 0) else return scope.offset -
                (closing ? hangingIndent : 0)
      },
