...
/**
 * Author: Hans Engel
 * Branched from CodeMirror's Scheme mode (by Koh Zi Han, based on
 * implementation by Koh Zi Chun)
 */
...
CodeMirror.defineMode('clojure', function (options) {
    var BUILTIN = 'builtin', COMMENT = 'comment', STRING = 'string',
        CHARACTER = 'string-2', ATOM = 'atom', NUMBER = 'number',
    ...
    function stateStack(
        indent, type, prev) {  // represents a state stack object
      this.indent = indent;
      this.type = type;
      this.prev = prev;
    }
          ...
          */
          ...
          if (keyWord.length > 0 &&
            ...
            if (stream.eol() || stream.peek() == ';') {
              ...
              pushStack(
                  state, indentTemp + stream.current().length,
                  ch);  // else we match
            }
          }
