...
(function(mod) {
  ...
  function pushContext(state, col, type, info) {
    ...
    return state.context =
               new Context(indent, col, type, info, null, state.context);
  }
    ...
    return {
      startState: function(basecolumn) {
        return {
          tokenize: null,
          context: new Context(
              (basecolumn || 0) - indentUnit, 0, 'top', null, false),
          indented: 0,
          startOfLine: true,
          prevToken: null
        };
  ...
  function cppHook(stream, state) {
    if (!state.startOfLine)
      return false for (var ch, next = null; ch = stream.peek();) {
        if (ch == '\\' && stream.match(/^.$/)) {
          next = cppHook
          break
      ...
      '=': function(stream, state) {
        ...
        if (cx.type == '}' && cx.align && stream.eat('>')) {
          state.context = new Context(
              cx.indented, cx.column, cx.type, cx.info, null,
              cx.prev) return 'operator'
        }
        else {
          return false
        }
  ...
  def('text/x-kotlin', {
    ...
    multiLineStrings: true,
    blockKeywords:
        words('catch class do else finally for if where try while enum'),
  ...
  def('text/x-squirrel', {
    ...
    types: words(cTypes),
    blockKeywords:
        words('case catch class else for foreach if switch try while'),
  ...
  def('text/x-ceylon', {
    ...
    defKeywords:
        words('class dynamic function interface module object package value'),
    builtin: words(
    ...
    hooks: {
      ...
      '"': function(stream, state) {
        state.tokenize =
            tokenCeylonString(stream.match('""') ? 'triple' : 'single');
        return state.tokenize(stream, state);
      },
      '`': function(stream, state) {
