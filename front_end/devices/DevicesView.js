...
WebInspector.DevicesView =
  ...
  sidebar.createChild('div', 'devices-view-title')
      .createTextChild(WebInspector.UIString('Devices'));
  ...
  this._sidebarListSpacer =
      this._sidebarList.createChild('div', 'devices-sidebar-spacer');
  this._discoveryListItem =
      this._sidebarList.createChild('div', 'devices-sidebar-item');
  this._discoveryListItem.textContent = WebInspector.UIString('Settings');
  this._discoveryListItem.addEventListener(
      'click', this._selectSidebarListItem.bind(
                   this, this._discoveryListItem, this._discoveryView));
  ...
  var discoveryFooter =
      this.contentElement.createChild('div', 'devices-footer');
  this._deviceCountSpan = discoveryFooter.createChild('span');
  discoveryFooter.createChild('span').textContent =
      WebInspector.UIString(' Read ');
  discoveryFooter.appendChild(WebInspector.linkifyURLAsNode(
      'https://developers.google.com/chrome-developer-tools/docs/remote-debugging',
      WebInspector.UIString('remote debugging documentation'), undefined,
      true));
  discoveryFooter.createChild('span').textContent =
      WebInspector.UIString(' for more information.');
  this._updateFooter();
  this._selectSidebarListItem(this._discoveryListItem, this._discoveryView);

  InspectorFrontendHost.events.addEventListener(
      InspectorFrontendHostAPI.Events.DevicesUpdated, this._devicesUpdated,
      this);
  InspectorFrontendHost.events.addEventListener(
      InspectorFrontendHostAPI.Events.DevicesDiscoveryConfigChanged,
      this._devicesDiscoveryConfigChanged, this);
  InspectorFrontendHost.events.addEventListener(
  ...
  _devicesUpdated: function(event) {
    this._devices = /** @type {!Array.<!Adb.Device>} */ (event.data)
                        .slice()
    ...
    for (var deviceId of this._viewById.keys()) {
      if (!ids.has(deviceId)) {
        var listItem =
            /** @type {!Element} */ (this._listItemById.get(deviceId));
    ...
    for (var device of this._devices) {
      ...
      listItem._status.textContent = device.adbConnected ?
          WebInspector.UIString('Connected') :
          WebInspector.UIString('Pending Authorization');
  ...
  _createSidebarListItem: function(view) {
    ...
    listItem.addEventListener(
        'click', this._selectSidebarListItem.bind(this, listItem, view));
    listItem._title = listItem.createChild('div', 'devices-sidebar-item-title');
    listItem._status =
        listItem.createChild('div', 'devices-sidebar-item-status');
    return listItem;
  },
  ...
  _devicesDiscoveryConfigChanged: function(event) {
    var discoverUsbDevices =
        /** @type {boolean} */ (event.data['discoverUsbDevices']);
    var portForwardingEnabled =
        /** @type {boolean} */ (event.data['portForwardingEnabled']);
    var portForwardingConfig = /** @type {!Adb.PortForwardingConfig} */ (
        event.data['portForwardingConfig']);
  ...
  _updateFooter: function() {
    this._deviceCountSpan.textContent = !this._devices.length ?
        ...
        WebInspector.UIString('1 device detected.') :
 ...
 */
                                         WebInspector.DevicesView._instance =
        function() {
  ...
  return WebInspector.DevicesView._instanceObject;
         ...
         */
        WebInspector.DevicesView.DiscoveryView =
            function() {
  WebInspector.VBox.call(this);
  this.setMinimumSize(100, 100);
  this.element.classList.add('discovery-view');

  this.contentElement.createChild('div', 'hbox device-text-row')
      .createChild('div', 'view-title')
      .textContent = WebInspector.UIString('Settings');

  var discoverUsbDevicesCheckbox =
      createCheckboxLabel(WebInspector.UIString('Discover USB devices'));
  ...
  this._discoverUsbDevicesCheckbox.addEventListener(
      'click', this._updateDiscoveryConfig.bind(this), false);

  var help = this.element.createChild('div', 'discovery-help');
  help.createChild('span').textContent =
      WebInspector.UIString('Need help? Read Chrome ');
  help.appendChild(WebInspector.linkifyURLAsNode(
      'https://developers.google.com/chrome-developer-tools/docs/remote-debugging',
      WebInspector.UIString('remote debugging documentation.'), undefined,
      true));

  var portForwardingHeader =
      this.element.createChild('div', 'port-forwarding-header');
  var portForwardingEnabledCheckbox =
      createCheckboxLabel(WebInspector.UIString('Port forwarding'));
  ...
  this._portForwardingEnabledCheckbox =
      portForwardingEnabledCheckbox.checkboxElement;
  this._portForwardingEnabledCheckbox.addEventListener(
      'click', this._updateDiscoveryConfig.bind(this), false);

  var portForwardingFooter =
      this.element.createChild('div', 'port-forwarding-footer');
  ...
  this.element.appendChild(createTextButton(
      WebInspector.UIString('Add rule'), this._addRuleButtonClicked.bind(this),
      'add-rule-button'));
  ...
  _addRuleButtonClicked: function() {
    this._list.addNewItem(
        this._portForwardingConfig.length, {port: '', address: ''});
  },
   ...
   */
  discoveryConfigChanged: function(
      discoverUsbDevices, portForwardingEnabled, portForwardingConfig) {
    ...
    for (var key of Object.keys(portForwardingConfig)) {
      var rule = /** @type {!Adb.PortForwardingRule} */ (
          {port: key, address: portForwardingConfig[key]});
  ...
  renderItem: function(item, editable) {
    ...
    var port = element.createChild(
        'div', 'port-forwarding-value port-forwarding-port');
    port.createChild('span', 'port-localhost').textContent =
        WebInspector.UIString('localhost:');
    port.createTextChild(rule.port);
    element.createChild('div', 'port-forwarding-separator');
    element.createChild('div', 'port-forwarding-value').textContent =
        rule.address;
    return element;
  },
  ...
  _createEditor: function() {
    ...
    fields.createChild('div', 'port-forwarding-value port-forwarding-port')
        .appendChild(editor.createInput(
            'port', 'text', 'Device port (3333)', portValidator.bind(this)));
    fields.createChild(
        'div', 'port-forwarding-separator port-forwarding-separator-invisible');
    fields.createChild('div', 'port-forwarding-value')
        .appendChild(editor.createInput(
            'address', 'text', 'Local address (dev.example.corp:3333)',
            addressValidator));
    return editor;
  ...
  _updateDiscoveryConfig: function() {
    ...
    InspectorFrontendHost.setDevicesDiscoveryConfig(
        this._discoverUsbDevicesCheckbox.checked,
        this._portForwardingEnabledCheckbox.checked, configMap);
  },
  ...
  __proto__: WebInspector.VBox.prototype
                              ...
                              * @extends {WebInspector.VBox}
                              */
                             WebInspector.DevicesView.DeviceView =
                function() {
  WebInspector.VBox.call(this);
  this.setMinimumSize(100, 100);
  ...
  this._portStatus =
      this.contentElement.createChild('div', 'device-port-status hidden');

  this._deviceOffline = this.contentElement.createChild('div');
  this._deviceOffline.textContent = WebInspector.UIString(
  ...
  this._browsers =
      this.contentElement.createChild('div', 'device-browser-list vbox');
  ...
  this._device = null;
                ...
                /** @typedef {!{browser: ?Adb.Browser, element: !Element, title:
                   !Element, pages: !Element, viewMore: !Element, newTab:
                   !Element, pageSections: !Map<string,
                   !WebInspector.DevicesView.PageSection>}} */
                WebInspector.DevicesView.BrowserSection;

/** @typedef {!{page: ?Adb.Page, element: !Element, title: !Element, url:
...
WebInspector.DevicesView.DeviceView.prototype = {
  ...
  update: function(device) {
    ...
    this._noBrowsers.classList.toggle(
        'hidden', !device.adbConnected || !!device.browsers.length);
    this._browsers.classList.toggle(
        'hidden', !device.adbConnected || !device.browsers.length);
  ...
  _createBrowserSection: function() {
    ...
    newTabRow.createChild('div', '').textContent =
        WebInspector.UIString('New tab:');
    ...
    var newTabButton =
        createTextButton(WebInspector.UIString('Open'), openNewTab);
    ...
    function updateViewMoreTitle() {
      viewMore.textContent =
          pages.classList.contains('device-view-more-toggled') ?
    ...
    function openNewTab() {
      if (section.browser) {
        InspectorFrontendHost.openRemotePage(
            section.browser.id, newTabInput.value.trim() || 'about:blank');
        newTabInput.value = '';
      }
    }
  },
  ...
  _updateBrowserSection: function(section, browser) {
    if (!section.browser ||
        section.browser.adbBrowserName !== browser.adbBrowserName ||
        section.browser.adbBrowserVersion !== browser.adbBrowserVersion) {
      if (browser.adbBrowserVersion)
        section.title.textContent = String.sprintf(
            '%s (%s)', browser.adbBrowserName, browser.adbBrowserVersion);
      else
    ...
    for (var index = 0; index < browser.pages.length; ++index) {
      ...
      if (!index && section.pages.firstChild !== pageSection.element)
        section.pages.insertBefore(
            pageSection.element, section.pages.firstChild);
    }

    var kViewMoreCount = 3;
    for (var index = 0, element = section.pages.firstChild; element;
         element = element.nextSibling, ++index)
      element.classList.toggle(
          'device-view-more-page', index >= kViewMoreCount);
    section.viewMore.classList.toggle(
        'device-needs-view-more', browser.pages.length > kViewMoreCount);
  ...
  _createPageSection: function() {
    ...
    var inspect = createTextButton(
        WebInspector.UIString('Inspect'), doAction.bind(null, 'inspect'),
        'device-inspect-button');
    titleRow.appendChild(inspect);

    var toolbar = new WebInspector.Toolbar('');
    toolbar.appendToolbarItem(
        new WebInspector.ToolbarMenuButton(appendActions));
    ...
    function appendActions(contextMenu) {
      contextMenu.appendItem(
          WebInspector.UIString('Reload'), doAction.bind(null, 'reload'));
      contextMenu.appendItem(
          WebInspector.UIString('Focus'), doAction.bind(null, 'activate'));
      contextMenu.appendItem(
          WebInspector.UIString('Close'), doAction.bind(null, 'close'));
    }
    ...
    function doAction(action) {
      if (section.page)
        InspectorFrontendHost.performActionOnRemotePage(
            section.page.id, action);
    }
  },
  ...
  _updatePageSection: function(section, page) {
    ...
    if (!section.page || section.page.url !== page.url) {
      ...
      section.url.appendChild(
          WebInspector.linkifyURLAsNode(page.url, undefined, undefined, true));
    }
  ...
  portForwardingStatusChanged: function(status) {
    ...
    for (var port in status.ports) {
      ...
      var portNumber =
          createElementWithClass('div', 'device-view-port-number monospace');
