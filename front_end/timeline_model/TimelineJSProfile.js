 ...
 */
WebInspector.TimelineJSProfileProcessor.generateTracingEventsFromCpuProfile =
    function(jsProfileModel, thread) {
 ...
 * @return {!Array<!WebInspector.TracingModel.Event>}
 */
    WebInspector.TimelineJSProfileProcessor.generateJSFrameEvents =
        function(events) {
  /**
  ...
  function onStartEvent(e) {
    e.ordinal = ++ordinal;
    extractStackTrace(e);
    // For the duration of the event we cannot go beyond the stack associated
    // with it.
  ...
  function truncateJSStack(depth, time) {
    if (lockedJsStackDepth.length) {
      ...
      if (depth < lockedDepth) {
        console.error(
            'Child stack is shallower (' + depth + ') than the parent stack (' +
            lockedDepth + ') at ' + time);
        depth = lockedDepth;
      }
    }
  ...
  function extractStackTrace(e) {
    ...
    if (e.name === recordTypes.JSSample) {
      ...
      callFrames = /** @type {!Array<!RuntimeAgent.CallFrame>} */ (
          eventData && eventData['stackTrace']);
    } else {
      callFrames = /** @type {!Array<!RuntimeAgent.CallFrame>} */ (
    ...
    for (; i < numFrames; ++i) {
      ...
      var jsFrameEvent = new WebInspector.TracingModel.Event(
          WebInspector.TracingModel.DevToolsTimelineEventCategory,
          recordTypes.JSFrame, WebInspector.TracingModel.Phase.Complete,
  ...
  if (firstTopLevelEvent)
    WebInspector.TimelineModel.forEachEvent(
        events, onStartEvent, onEndEvent, onInstantEvent,
  ...
  return jsFrameEvents;
         ...
         * @constructor
         */
        WebInspector.TimelineJSProfileProcessor.CodeMap =
            function() {
  /** @type {!Map<string,
   * !WebInspector.TimelineJSProfileProcessor.CodeMap.Bank>} */
  this._banks = new Map();
}
             ...
             */
            WebInspector.TimelineJSProfileProcessor.CodeMap.Entry =
                function(address, size, callFrame) {
 ...
 * @return {number}
 */
                WebInspector.TimelineJSProfileProcessor.CodeMap.comparator =
  ...
  moveEntry: function(oldAddressHex, newAddressHex, size) {
    var entry = this._getBank(oldAddressHex)
                    .removeEntry(this._getAddress(oldAddressHex));
    if (!entry) {
  ...
  lookupEntry: function(address) {
    var index =
        this._entries.upperBound(
            address,
            WebInspector.TimelineJSProfileProcessor.CodeMap.comparator) -
        1;
    var entry = this._entries[index];
    return entry && address < entry.address + entry.size ? entry.callFrame :
                                                           null;
  },
 ...
 */
  ...
  return createFrame(
      functionName, url, String(scriptId), line, column, isNative);
}
 ...
 */
                            WebInspector.TimelineJSProfileProcessor
                                .processRawV8Samples = function(events) {
  ...
  for (var i = 0; i < events.length; ++i) {
    ...
    switch (e.name) {
      ...
      case recordTypes.JitCodeMoved:
        codeMap.moveEntry(
            data['code_start'], data['new_code_start'], data['code_len']);
        break;
      case recordTypes.V8Sample:
        var rawStack = data['stack'];
        // Sometimes backend fails to collect a stack and returns an empty
        // stack.
