 ...
 * @constructor
 */
WebInspector.TimelineProfileTree.Node =
    function() {
  /** @type {number} */
  this.totalTime;
  /** @type {number} */
  ...
  /**
     * @return {boolean}
     */
  isGroupNode: function() { return this._isGroupNode; }
}
 ...
 */
                     WebInspector.TimelineProfileTree.buildTopDown =
        function(events, filters, startTime, endTime, eventIdCallback) {
  ...
  function onStartEvent(e) {
    if (!WebInspector.TimelineModel.isVisible(filters, e))
      return;
    var time = e.endTime ?
        Math.min(endTime, e.endTime) - Math.max(startTime, e.startTime) :
        0;
  ...
  var instantEventCallback = eventIdCallback ?
      undefined :
      onStartEvent;  // Ignore instant events when aggregating.
  WebInspector.TimelineModel.forEachEvent(
      events, onStartEvent, onEndEvent, instantEventCallback, startTime,
      endTime);
 ...
 * @return {!WebInspector.TimelineProfileTree.Node}
 */
        WebInspector.TimelineProfileTree.buildBottomUp =
            function(topDownTree, groupingCallback) {
 ...
 * @return {?string}
 */
            WebInspector.TimelineProfileTree.eventURL =
                function(event) {
 ...
 */
                WebInspector.TimelineProfileTree.eventStackFrame =
                    function(event) {
  ...
  return /** @type {?RuntimeAgent.CallFrame} */ (
      initiator && initiator.stackTrace && initiator.stackTrace[0] || null);
}

                    /**
                     * @constructor
                     * @param
                     * {function(!WebInspector.TracingModel.Event):string}
                     * titleMapper
                     * @param
                     * {function(!WebInspector.TracingModel.Event):string}
                     * categoryMapper
                     */
                    WebInspector.TimelineAggregator =
                        function(titleMapper, categoryMapper) {
 ...
 */
  ...
  if (event.name === WebInspector.TimelineModel.RecordType.JSFrame) {
    ...
    return 'f:' + data['functionName'] + '@' +
        (data['scriptId'] || data['url'] || '');
  }
  return event.name + ':@' + WebInspector.TimelineProfileTree.eventURL(event);
}

                            WebInspector.TimelineAggregator
                                ._extensionInternalPrefix = 'extensions::';
 ...
 * @return {boolean}
 */
WebInspector.TimelineAggregator.isExtensionInternalURL =
    function(url) {
  return url.startsWith(
      WebInspector.TimelineAggregator._extensionInternalPrefix);
}
