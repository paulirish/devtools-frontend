 ...
 */
WebInspector.TimelineIRModel.phaseForEvent =
    function(event) {
  ...
  populate: function(inputLatencies, animations) {
    ...
    range.appendRange(this._drags);  // Drags take lower precedence than
                                     // animation, as we can't detect them
                                     // reliably.
  ...
  _processInputLatencies: function(events) {
    ...
    for (var i = 0; i < events.length; ++i) {
      ...
      switch (type) {
        ...
        case eventTypes.ScrollEnd:
          if (scrollStart)
            this._scrolls.append(
                this._segmentForEventRange(scrollStart, event, phases.Scroll));
          else
            this._scrolls.append(this._segmentForEvent(event, phases.Scroll));
          scrollStart = null;
          break;
        ...
        case eventTypes.ScrollUpdate:
          touchStart = null;  // Since we're scrolling now, disregard other
                              // touch gestures.
          this._scrolls.append(this._segmentForEvent(event, phases.Scroll));
          break;
        ...
        case eventTypes.FlingStart:
          if (flingStart) {
            WebInspector.console.error(WebInspector.UIString(
                'Two flings at the same time? %s vs %s', flingStart.startTime,
                event.startTime));
            break;
          }
          flingStart = event;
        ...
        case eventTypes.FlingCancel:
          ...
          if (!flingStart)
            break;
          this._scrolls.append(
              this._segmentForEventRange(flingStart, event, phases.Fling));
          flingStart = null;
          break;
        ...
        case eventTypes.TouchStart:
          // We do not produce any response segment for TouchStart -- there's
          // either going to be one upon
          // TouchMove for drag, or one for GestureTap.
          if (touchStart) {
            WebInspector.console.error(WebInspector.UIString(
                'Two touches at the same time? %s vs %s', touchStart.startTime,
                event.startTime));
            break;
          }
          touchStart = event;
          event.steps[0][WebInspector.TimelineIRModel._eventIRPhase] =
              phases.Response;
          firstTouchMove = null;
          break;
        ...
        case eventTypes.TouchMove:
          if (firstTouchMove) {
            ...
            this._responses.append(
                this._segmentForEventRange(touchStart, event, phases.Response));
          }
          break;
        ...
        case eventTypes.MouseMove:
          if (mouseDown && !mouseMove &&
            ...
            this._responses.append(
                this._segmentForEvent(mouseDown, phases.Response));
            this._responses.append(
                this._segmentForEvent(event, phases.Response));
          } else if (mouseDown) {
            this._drags.append(this._segmentForEvent(event, phases.Drag));
          }
          mouseMove = event;
        ...
        case eventTypes.MouseWheel:
          // Do not consider first MouseWheel as trace viewer's implementation
          // does -- in case of MouseWheel it's not really special.
          if (mouseWheel && canMerge(thresholdsMs.mouse, mouseWheel, event))
            this._scrolls.append(
                this._segmentForEventRange(mouseWheel, event, phases.Scroll));
          else
            this._scrolls.append(this._segmentForEvent(event, phases.Scroll));
          mouseWheel = event;
          break;
    ...
    function canMerge(threshold, first, second) {
      return first.endTime < second.startTime &&
          second.startTime < first.endTime + threshold;
    }
  },
  ...
  _processAnimations: function(events) {
    for (var i = 0; i < events.length; ++i)
      this._cssAnimations.append(this._segmentForEvent(
          events[i], WebInspector.TimelineIRModel.Phases.Animation));
  },
  ...
  _segmentForEventRange: function(startEvent, endEvent, phase) {
    ...
    return new WebInspector.Segment(
        startEvent.startTime, endEvent.endTime, phase);
  },
  ...
  reset: function() {
    ...
    this._segments = [];
    this._drags =
        new WebInspector.SegmentedRange(merge.bind(null, thresholdsMs.mouse));
    this._cssAnimations = new WebInspector.SegmentedRange(
        merge.bind(null, thresholdsMs.animation));
    this._responses = new WebInspector.SegmentedRange(merge.bind(null, 0));
    this._scrolls = new WebInspector.SegmentedRange(
        merge.bind(null, thresholdsMs.animation));
    ...
    function merge(threshold, first, second) {
      return first.end + threshold >= second.begin &&
