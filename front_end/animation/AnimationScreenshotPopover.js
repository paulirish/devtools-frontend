 ...
 */
WebInspector.AnimationScreenshotPopover =
    function(images) {
  ...
  this._progressBar =
      this.contentElement.createChild('div', 'animation-progress');
}

    WebInspector.AnimationScreenshotPopover.prototype = {
  wasShown: function() {
    this._rafId = this.contentElement.window().requestAnimationFrame(
        this._changeFrame.bind(this));
  },
  ...
  _changeFrame: function() {
    ...
    this._progressBar.style.width =
        (this._currentFrame % numFrames + 1) / numFrames * 100 + '%';
  },
