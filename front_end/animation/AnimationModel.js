 ...
 */
WebInspector.AnimationModel =
    function(target) {
  WebInspector.SDKModel.call(this, WebInspector.AnimationModel, target);
  this._agent = target.animationAgent();
  target.registerAnimationDispatcher(
      new WebInspector.AnimationDispatcher(this));
  ...
  resourceTreeModel.addEventListener(
      WebInspector.ResourceTreeModel.Events.MainFrameNavigated, this._reset,
      this);
  this._screenshotCapture = new WebInspector.AnimationModel.ScreenshotCapture(
  ...
  _reset: function() {
    ...
    this.dispatchEventToListeners(
        WebInspector.AnimationModel.Events.ModelReset);
  },
  ...
  _createGroupFromPendingAnimations: function() {
    ...
    var groupedAnimations =
        [this._animationsById.get(this._pendingAnimations.shift())];
  ...
  __proto__: WebInspector.SDKModel.prototype
                                   ...
                                   WebInspector.AnimationModel._symbol =
 ...
 */
WebInspector.AnimationModel.fromTarget =
    function(target) {
  if (!target.hasDOMCapability())
    return null;
  if (!target[WebInspector.AnimationModel._symbol])
    target[WebInspector.AnimationModel._symbol] =
        new WebInspector.AnimationModel(target);
  ...
  return target[WebInspector.AnimationModel._symbol];
     ...
     * @param {!AnimationAgent.Animation} payload
     */
    WebInspector.AnimationModel.Animation =
        function(target, payload) {
  WebInspector.SDKObject.call(this, target);
  this._payload = payload;
  this._source = new WebInspector.AnimationModel.AnimationEffect(
      this.target(), this._payload.source);
}
 ...
 */
        WebInspector.AnimationModel.Animation.parsePayload =
            function(target, payload) {
  ...
  /**
     ...
     */
  type: function() {
    return /** @type {!WebInspector.AnimationModel.Animation.Type} */ (
        this._payload.type);
  },
  ...
  overlaps: function(animation) {
    ...
    var firstAnimation =
        this.startTime() < animation.startTime() ? this : animation;
  ...
  _updateNodeStyle: function(duration, delay, node) {
    var animationPrefix;
    if (this.type() ===
        WebInspector.AnimationModel.Animation.Type.CSSTransition)
      animationPrefix = 'transition-';
    else if (
        this.type() === WebInspector.AnimationModel.Animation.Type.CSSAnimation)
    ...
    cssModel.setEffectivePropertyValueForNode(
        node.id, animationPrefix + 'delay', delay + 'ms');
  },
  ...
  remoteObjectPromise: function() {
    ...
    function callback(error, payload) {
      return !error ? this.target().runtimeModel.createRemoteObject(payload) :
                      null;
    }

    return this.target().animationAgent().resolveAnimation(
        this.id(), callback.bind(this));
  },
  ...
  if (payload.keyframesRule)
    this._keyframesRule = new WebInspector.AnimationModel.KeyframesRule(
        target, payload.keyframesRule);
  this._delay = this._payload.delay;
  this._duration = this._payload.duration;
}

                WebInspector.AnimationModel.AnimationEffect
                    .prototype = {
  /**
     * @return {number}
     */
  ...
  /**
     ...
     */
  iterations: function() {
    // Animations with zero duration, zero delays and infinite iterations can't
    // be shown.
  ...
  node: function() {
    if (!this._deferredNode)
      this._deferredNode =
          new WebInspector.DeferredDOMNode(this.target(), this.backendNodeId());
  ...
  deferredNode: function() {
    return new WebInspector.DeferredDOMNode(
        this.target(), this.backendNodeId());
  },
  ...
  __proto__: WebInspector.SDKObject.prototype
                                  ...
                                  * @extends {WebInspector.SDKObject}
                                  * @param {!WebInspector.Target} target
                                  * @param {!AnimationAgent.KeyframesRule}
                                  * payload
                                  */
                                 WebInspector.AnimationModel.KeyframesRule =
                    function(target, payload) {
  WebInspector.SDKObject.call(this, target);
  this._payload = payload;
  ...
  __proto__: WebInspector.SDKObject.prototype
                                      ...
                                      * @extends {WebInspector.SDKObject}
                                      * @param {!WebInspector.Target} target
                                      * @param {!AnimationAgent.KeyframeStyle}
                                      * payload
                                      */
                                     WebInspector.AnimationModel.KeyframeStyle =
                        function(target, payload) {
  WebInspector.SDKObject.call(this, target);
  this._payload = payload;
  this._offset = this._payload.offset;
     ...
     */
                             ...
                             * @param {string} id
                             * @param
                             * {!Array.<!WebInspector.AnimationModel.Animation>}
                             * animations
                             */
                            WebInspector.AnimationModel.AnimationGroup =
                                function(model, id, animations) {
     ...
     */
                              ...
                              release: function() {
                                ...
                                this._model._releaseAnimations(
                                    this._animationIds());
                              },
     ...
     */
                              finiteDuration: function() {
                                var maxDuration = 0;
                                for (var i = 0; i < this._animations.length;
                                     ++i)
                                  maxDuration = Math.max(
                                      maxDuration,
                                      this._animations[i]._finiteDuration());
                                return maxDuration;
                              },
                              ...
                              seekTo: function(currentTime) {
                                this.target().animationAgent().seekAnimations(
                                    this._animationIds(), currentTime);
                              },
     ...
     */
                              ...
                              togglePause: function(paused) {
                                if (paused === this._paused)
                                  return;
                                this._paused = paused;
                                this.target().animationAgent().setPaused(
                                    this._animationIds(), paused);
                              },
     ...
     */
         ...
         */
                                ...
                                for (var anim of this._animations) {
                                  if (!longestAnim ||
                                      anim.endTime() > longestAnim.endTime())
                                    longestAnim = anim;
                                }
                                return this.target()
                                    .animationAgent()
     ...
     */
         ...
         */
                                ...
                                if (this._animations.length !==
                                    group._animations.length)
                                  return false;
                                var left =
                                    this._animations.map(extractId).sort();
                                var right =
                                    group._animations.map(extractId).sort();
     ...
     */
                              screenshots: function() {
                                for (var i = 0; i < this._screenshots.length;
                                     ++i) {
                                  var image = new Image();
                                    ...
                                    WebInspector.AnimationDispatcher.prototype =
                                        {
                                          ...
                                          /**
                                           * @override
                                           * @param {!AnimationAgent.Animation}
                                           * payload
                                           */
                                          animationStarted: function(payload) {
                                            this._animationModel
                                         ...
                                         * @constructor
                                         * @param {!WebInspector.Target} target
                                         * @param {!WebInspector.AnimationModel}
                                         * model
                                         * @param
                                        ...
                                        WebInspector.AnimationModel
                                          ...
                                          this._target = target;
                                          /** @type
                                           * {!Array<!WebInspector.AnimationModel.ScreenshotCapture.Request>}
                                           */
                                          this._requests = [];
                                          resourceTreeModel.addEventListener(
                                              WebInspector.ResourceTreeModel
                                                  .Events.ScreencastFrame,
                                              this._screencastFrame, this);
                                          this._model = model;
...
WebInspector.AnimationModel.ScreenshotCapture.prototype = {
  ...
  captureScreenshots: function(duration, screenshots) {
    var screencastDuration =
        Math.min(duration / this._model._playbackRate, 3000);
    ...
    if (!this._endTime || endTime > this._endTime) {
      ...
      this._stopTimer =
          setTimeout(this._stopScreencast.bind(this), screencastDuration);
