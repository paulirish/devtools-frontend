...
WebInspector.AnimationGroupPreviewUI =
  ...
  this._removeButton =
      this.element.createChild('div', 'animation-remove-button');
  ...
  _render: function() {
    ...
    var maxToShow = 10;
    var numberOfAnimations =
        Math.min(this._model.animations().length, maxToShow);
    ...
    for (var i = 0; i < numberOfAnimations; i++) {
      ...
      line.setAttribute(
          'x2', (effect.delay() + effect.duration()) * timeToPixelRatio);
      var y = Math.floor(
          this._viewBoxHeight / Math.max(6, numberOfAnimations) * i + 1);
      line.setAttribute('y1', y);
      line.setAttribute('y2', y);
      line.style.stroke =
          WebInspector.AnimationUI.Color(this._model.animations()[i]);
    }
  }
}
