 ...
 */
WebInspector.AnimationTimeline =
    function() {
  ...
  this._grid =
      this.contentElement.createSVGChild('svg', 'animation-timeline-grid');

  this._playbackRate = 1;
  this._allPaused = false;
  this._createHeader();
  this._animationsContainer =
      this.contentElement.createChild('div', 'animation-timeline-rows');
  var timelineHint =
      this.contentElement.createChild('div', 'animation-timeline-rows-hint');
  timelineHint.textContent =
      WebInspector.UIString('Select an effect above to inspect and modify.');

  /** @const */ this._defaultDuration = 100;
  this._duration = this._defaultDuration;
  /** @const */ this._timelineControlsWidth = 150;
  /** @type {!Map.<!DOMAgent.BackendNodeId,
   * !WebInspector.AnimationTimeline.NodeUI>} */
  this._nodesMap = new Map();
  this._uiAnimations = [];
  this._groupBuffer = [];
  /** @type {!Map.<!WebInspector.AnimationModel.AnimationGroup,
   * !WebInspector.AnimationGroupPreviewUI>} */
  ...
  WebInspector.targetManager.addModelListener(
      WebInspector.DOMModel, WebInspector.DOMModel.Events.NodeRemoved,
      this._nodeRemoved, this);
  WebInspector.targetManager.observeTargets(
      this, WebInspector.Target.Capability.DOM);
  WebInspector.context.addFlavorChangeListener(
      WebInspector.DOMNode, this._nodeChanged, this);
}
...
WebInspector.AnimationTimeline._ControlState = {
  ...
  Pause: 'pause-outline'
                                                   ...
                                                   .prototype =
    {
       ...
       * @param {!WebInspector.Target} target
       */
      targetRemoved: function(target) { this._removeEventListeners(target); },
      ...
      _addEventListeners: function(target) {
        ...
        animationModel.addEventListener(
            WebInspector.AnimationModel.Events.AnimationGroupStarted,
            this._animationGroupStarted, this);
        animationModel.addEventListener(
            WebInspector.AnimationModel.Events.ModelReset, this._reset, this);
      },
     ...
     */
      _createScrubber: function() {
        this._timelineScrubber =
            createElementWithClass('div', 'animation-scrubber hidden');
        this._timelineScrubberLine = this._timelineScrubber.createChild(
            'div', 'animation-scrubber-line');
        this._timelineScrubberLine.createChild(
            'div', 'animation-scrubber-head');
        this._timelineScrubber.createChild('div', 'animation-time-overlay');
        return this._timelineScrubber;
      },

      _createHeader: function() {
        var toolbarContainer = this.contentElement.createChild(
            'div', 'animation-timeline-toolbar-container');
        var topToolbar = new WebInspector.Toolbar(
            'animation-timeline-toolbar', toolbarContainer);
        var clearButton = new WebInspector.ToolbarButton(
            WebInspector.UIString('Clear all'), 'clear-toolbar-item');
        clearButton.addEventListener('click', this._reset.bind(this));
        topToolbar.appendToolbarItem(clearButton);
        topToolbar.appendSeparator();

        this._pauseButton = new WebInspector.ToolbarToggle(
            WebInspector.UIString('Pause all'), 'pause-toolbar-item');
        this._pauseButton.addEventListener(
            'click', this._togglePauseAll.bind(this));
        topToolbar.appendToolbarItem(this._pauseButton);

        var playbackRateControl = toolbarContainer.createChild(
            'div', 'animation-playback-rate-control');
        this._playbackRateButtons = [];
        for (var playbackRate of
                 WebInspector.AnimationTimeline.GlobalPlaybackRates) {
          var button = playbackRateControl.createChild(
              'div', 'animation-playback-rate-button');
          button.textContent = playbackRate ?
              WebInspector.UIString(playbackRate * 100 + '%') :
              WebInspector.UIString('Pause');
          button.playbackRate = playbackRate;
          button.addEventListener(
              'click', this._setPlaybackRate.bind(this, playbackRate));
          button.title =
              WebInspector.UIString('Set speed to ') + button.textContent;
          this._playbackRateButtons.push(button);
        }
        this._updatePlaybackControls();

        this._previewContainer =
            this.contentElement.createChild('div', 'animation-timeline-buffer');
        this._popoverHelper = new WebInspector.PopoverHelper(
            this._previewContainer, this._getPopoverAnchor.bind(this),
            this._showPopover.bind(this), this._onHidePopover.bind(this), true);
        this._popoverHelper.setTimeout(0);
        var emptyBufferHint = this.contentElement.createChild(
            'div', 'animation-timeline-buffer-hint');
        emptyBufferHint.textContent =
            WebInspector.UIString('Listening for animations...');
        var container =
            this.contentElement.createChild('div', 'animation-timeline-header');
        var controls = container.createChild('div', 'animation-controls');
        this._currentTime = controls.createChild(
            'div', 'animation-timeline-current-time monospace');

        var toolbar =
            new WebInspector.Toolbar('animation-controls-toolbar', controls);
        this._controlButton = new WebInspector.ToolbarButton(
            WebInspector.UIString('Replay timeline'),
            'animation-control-toolbar-item');
        this._controlButton.setState(
            WebInspector.AnimationTimeline._ControlState.Replay);
        this._controlButton.addEventListener(
            'click', this._controlButtonToggle.bind(this));
        ...
        WebInspector.installDragHandle(
            gridHeader, this._repositionScrubber.bind(this),
            this._scrubberDragMove.bind(this), this._scrubberDragEnd.bind(this),
            'text');
        container.appendChild(this._createScrubber());
        WebInspector.installDragHandle(
            this._timelineScrubberLine, this._scrubberDragStart.bind(this),
            this._scrubberDragMove.bind(this), this._scrubberDragEnd.bind(this),
            'col-resize');
     ...
     */
      ...
      _showPopover: function(anchor, popover) {
        ...
        if (!screenshots[0].complete)
          screenshots[0].onload =
              onFirstScreenshotLoaded.bind(null, screenshots);
        else
        ...
        function onFirstScreenshotLoaded(screenshots) {
          var content =
              new WebInspector.AnimationScreenshotPopover(screenshots);
      ...
      _togglePauseAll: function() {
        ...
        this._pauseButton.setTitle(
            this._allPaused ? WebInspector.UIString('Resume all') :
                              WebInspector.UIString('Pause all'));
      },
      ...
      _setPlaybackRate: function(playbackRate) {
        ...
        if (target)
          WebInspector.AnimationModel.fromTarget(target).setPlaybackRate(
              this._allPaused ? 0 : this._playbackRate);
        WebInspector.userMetrics.actionTaken(
            WebInspector.UserMetrics.Action.AnimationsPlaybackRateChanged);
      ...
      _controlButtonToggle: function() {
        if (this._controlButton.state() ===
            WebInspector.AnimationTimeline._ControlState.Play)
          this._togglePause(false);
        else if (
            this._controlButton.state() ===
            WebInspector.AnimationTimeline._ControlState.Replay)
          this._replay();
        else
      ...
      _updateControlButton: function() {
        ...
        if (this._selectedGroup && this._selectedGroup.paused()) {
          ...
          this._controlButton.setTitle(WebInspector.UIString('Play timeline'));
            ...
            !this._scrubberPlayer ||
            this._scrubberPlayer.currentTime >= this.duration()) {
          this._controlButton.setState(
              WebInspector.AnimationTimeline._ControlState.Replay);
          this._controlButton.setTitle(
              WebInspector.UIString('Replay timeline'));
        } else {
          this._controlButton.setState(
              WebInspector.AnimationTimeline._ControlState.Pause);
     ...
     */
      _effectivePlaybackRate: function() {
        return (this._allPaused ||
                (this._selectedGroup && this._selectedGroup.paused())) ?
            0 :
            this._playbackRate;
      },
      ...
      _togglePause: function(pause) {
        ...
        if (this._scrubberPlayer)
          this._scrubberPlayer.playbackRate = this._effectivePlaybackRate();
        this._previewMap.get(this._selectedGroup)
            .element.classList.toggle('paused', pause);
     ...
     */
      ...
      _animationGroupStarted: function(event) {
        this._addAnimationGroup(
            /** @type {!WebInspector.AnimationModel.AnimationGroup} */ (
                event.data));
      },
      ...
      _addAnimationGroup: function(group) {
        ...
        while (this._groupBuffer.length > bufferSize) {
          var toDiscard = this._groupBuffer.splice(
              this._groupBuffer[0] === this._selectedGroup ? 1 : 0, 1);
        ...
        preview.removeButton().addEventListener(
            'click', this._removeAnimationGroup.bind(this, group));
        preview.element.addEventListener(
            'click', this._selectAnimationGroup.bind(this, group));
      },
      ...
      _selectAnimationGroup: function(group) {
        ...
        function applySelectionClass(ui, group) {
          ui.element.classList.toggle(
              'selected', this._selectedGroup === group);
        }
      ...
      _addAnimation: function(animation) {
        ...
        if (!nodeUI) {
          nodeUI =
              new WebInspector.AnimationTimeline.NodeUI(animation.source());
        ...
        var uiAnimation =
            new WebInspector.AnimationUI(animation, this, nodeRow);
      ...
      _renderGrid: function() {
        ...
        for (var time = 0; time < this.duration(); time += gridSize) {
          var line =
              this._grid.createSVGChild('rect', 'animation-timeline-grid-line');
        ...
        for (var time = 0; time < this.duration(); time += gridSize) {
          ...
          if (lastDraw === undefined || gridWidth - lastDraw > 50) {
            lastDraw = gridWidth;
            var label = this._grid.createSVGChild(
                'text', 'animation-timeline-grid-label');
            label.textContent =
                WebInspector.UIString(Number.millisToString(time));
      ...
      scheduleRedraw: function() {
        ...
        this._renderGrid();
        this._animationsContainer.window().requestAnimationFrame(
      ...
      _render: function(timestamp) {
        while (this._renderQueue.length &&
               (!timestamp || window.performance.now() - timestamp < 50))
      ...
      onResize: function() {
        this._cachedTimelineWidth =
            Math.max(
                0, this._animationsContainer.offsetWidth -
                    this._timelineControlsWidth) ||
            0;
     ...
     * @return {number}
     */
      width: function() { return this._cachedTimelineWidth || 0; },
     ...
     * @return {number}
     */
      pixelMsRatio: function() { return this.width() / this.duration() || 0; },
      ...
      _updateScrubber: function(timestamp) {
        if (!this._scrubberPlayer)
          return;
        this._currentTime.textContent = WebInspector.UIString(
            Number.millisToString(this._scrubberPlayer.currentTime));
        if (this._scrubberPlayer.playState === 'pending' ||
            this._scrubberPlayer.playState === 'running') {
          this.element.window().requestAnimationFrame(
              this._updateScrubber.bind(this));
     ...
     */
      _repositionScrubber: function(event) {
        ...
        var seekTime =
            Math.max(0, event.x - this._gridOffsetLeft) / this.pixelMsRatio();
     ...
     */
      ...
      _scrubberDragMove: function(event) {
        ...
        var currentTime = Math.max(
            0, Math.min(
                   this._originalScrubberTime + delta / this.pixelMsRatio(),
                   this.duration()));
        this._scrubberPlayer.currentTime = currentTime;
        this._currentTime.textContent = WebInspector.UIString(
            Number.millisToString(Math.round(currentTime)));
      ...
      _scrubberDragEnd: function(event) {
        ...
        this._scrubberPlayer.currentTime = currentTime;
        this._currentTime.window().requestAnimationFrame(
            this._updateScrubber.bind(this));
      },
    ...
    WebInspector.AnimationTimeline.NodeUI =
      ...
      this._description =
          this.element.createChild('div', 'animation-node-description');
      this._timelineElement =
          this.element.createChild('div', 'animation-node-timeline');
    }
      ...
      nodeResolved: function(node) {
        ...
        this._node = node;
        this._nodeChanged();
        this._description.appendChild(
            WebInspector.DOMPresentationUtils.linkifyNodeReference(node));
     ...
     */
      createNewRow: function() {
        return this._timelineElement.createChild(
            'div', 'animation-timeline-row');
      },
      ...
      _nodeChanged: function() {
        this.element.classList.toggle(
            'animation-node-selected', this._node &&
                this._node ===
                    WebInspector.context.flavor(WebInspector.DOMNode));
      }
    }
                          ...
                          */
                         WebInspector.AnimationTimeline.StepTimingFunction =
            function(steps, stepAtPosition) {
      this.steps = steps;
 ...
 */
            WebInspector.AnimationTimeline.StepTimingFunction.parse = function(
                text) {
      var match = text.match(/^steps\((\d+), (start|middle)\)$/);
      if (match)
        return new WebInspector.AnimationTimeline.StepTimingFunction(
            parseInt(match[1], 10), match[2]);
      match = text.match(/^steps\((\d+)\)$/);
      if (match)
        return new WebInspector.AnimationTimeline.StepTimingFunction(
            parseInt(match[1], 10), 'end');
      return null;
    }
