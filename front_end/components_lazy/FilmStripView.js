  ...
  FrameExit: Symbol('FrameExit'),
        ...
        WebInspector.FilmStripView.prototype = {
          ...
          setMode: function(mode) {
            ...
            this.contentElement.classList.toggle(
                'time-based',
                mode === WebInspector.FilmStripView.Modes.TimeBased);
            this.update();
          },
     ...
     */
          createFrameElement: function(frame) {
            ...
            element.title = WebInspector.UIString(
                'Doubleclick to zoom image. Click to view preceding requests.');
            element.createChild('div', 'time').textContent =
                Number.millisToString(time - this._zeroTime);
            var imageElement =
                element.createChild('div', 'thumbnail').createChild('img');
            element.addEventListener(
                'mousedown',
                this._onMouseEvent.bind(
                    this, WebInspector.FilmStripView.Events.FrameSelected,
                    time),
                false);
            element.addEventListener(
                'mouseenter',
                this._onMouseEvent.bind(
                    this, WebInspector.FilmStripView.Events.FrameEnter, time),
                false);
            element.addEventListener(
                'mouseout',
                this._onMouseEvent.bind(
                    this, WebInspector.FilmStripView.Events.FrameExit, time),
                false);
            element.addEventListener(
                'dblclick', this._onDoubleClick.bind(this, frame), false);

            return frame.imageDataPromise()
                .then(WebInspector.FilmStripView._setImageData.bind(
                    null, imageElement))
                .then(returnElement);
            /**
         * @return {!Element}
     ...
     */
         ...
         */
            ...
            // Using the first frame to fill the interval between recording
            // start
          ...
          update: function() {
            ...
            if (this._mode === WebInspector.FilmStripView.Modes.FrameBased) {
              Promise.all(frames.map(this.createFrameElement.bind(this)))
                  .then(appendElements.bind(this));
              return;
            }
            ...
            this.createFrameElement(frames[0]).then(
                continueWhenFrameImageLoaded.bind(
            ...
            function continueWhenFrameImageLoaded(element0) {
              var frameWidth = Math.ceil(
                  WebInspector
                      .measurePreferredSize(element0, this.contentElement)
                      .width);
              if (!frameWidth)
                return;
              ...
              for (var pos = frameWidth; pos < width; pos += frameWidth) {
                ...
                promises.push(this.createFrameElement(this.frameByTime(time))
                                  .then(fixWidth));
              }
          ...
          _onDoubleClick: function(filmStripFrame) {
            new WebInspector.FilmStripView.Dialog(
                filmStripFrame, this._zeroTime);
          },
          ...
          if (data)
             ...
             * @param {number=} zeroTime
             */
            WebInspector.FilmStripView.Dialog =
                function(filmStripFrame, zeroTime) {
          ...
          var footerElement =
              this.contentElement.createChild('div', 'filmstrip-dialog-footer');
          footerElement.createChild('div', 'flex-auto');
          var prevButton = createTextButton(
              '\u25C0', this._onPrevFrame.bind(this), undefined,
              WebInspector.UIString('Previous frame'));
          footerElement.appendChild(prevButton);
          this._timeLabel =
              footerElement.createChild('div', 'filmstrip-dialog-label');
          var nextButton = createTextButton(
              '\u25B6', this._onNextFrame.bind(this), undefined,
              WebInspector.UIString('Next frame'));
          footerElement.appendChild(nextButton);
          footerElement.createChild('div', 'flex-auto');

          this.contentElement.addEventListener(
              'keydown', this._keyDown.bind(this), false);
     ...
     */
          _render: function() {
            ...
            this._timeLabel.textContent =
                Number.millisToString(frame.timestamp - this._zeroTime);
            return frame.imageDataPromise()
                .then(WebInspector.FilmStripView._setImageData.bind(
                    null, this._imageElement))
