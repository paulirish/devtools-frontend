 ...
 */
    WebInspector.LineLevelProfile.instance =
        function() {
  if (!WebInspector.LineLevelProfile._instance)
    WebInspector.LineLevelProfile._instance =
        new WebInspector.LineLevelProfile();
  ...
  appendCPUProfile: function(profile) {
    ...
    var sampleDuration = (profile.profileEndTime - profile.profileStartTime) /
  ...
  _doUpdate: function() {
    ...
    WebInspector.workspace.uiSourceCodes().forEach(
        uiSourceCode => uiSourceCode.removeAllLineDecorations(
            WebInspector.LineLevelProfile.LineDecorator.type));
    var debuggerModel = WebInspector.DebuggerModel.fromTarget(
    ...
    for (var fileInfo of this._files) {
      ...
      for (var lineInfo of fileInfo[1]) {
        ...
        if (rawLocation)
          new WebInspector.LineLevelProfile.Presentation(
              rawLocation, time, this._locationPool);
        else if (uiSourceCode)
          uiSourceCode.addLineDecoration(
              line, WebInspector.LineLevelProfile.LineDecorator.type, time);
      }
    }
  }
}
                          ...
                          */
                         WebInspector.LineLevelProfile.Presentation =
            function(rawLocation, time, locationPool) {
  this._time = time;
  WebInspector.debuggerWorkspaceBinding.createLiveLocation(
      rawLocation, this.updateLocation.bind(this), locationPool);
            ...
            WebInspector.LineLevelProfile.Presentation.prototype =
                 ...
                 */
                WebInspector.LineLevelProfile.LineDecorator =
                    function() {}
...
WebInspector.LineLevelProfile.LineDecorator.prototype = {
  ...
  decorate: function(uiSourceCode, textEditor) {
    ...
    var decorations = uiSourceCode.lineDecorations(
        WebInspector.LineLevelProfile.LineDecorator.type);
    ...
    for (var decoration of decorations.values()) {
      ...
      var element =
          createElementWithClass('div', 'text-editor-line-marker-performance');
      element.textContent = text;
      element.style.backgroundColor =
          `hsla(44, 100%, 50%, ${intensity.toFixed(3)})`;
