...
WebInspector.InspectedPagePlaceholder = function() {
  ...
  WebInspector.zoomManager.addEventListener(
      WebInspector.ZoomManager.Events.ZoomChanged, this._scheduleUpdate, this);
...
WebInspector.InspectedPagePlaceholder.prototype = {
  _findMargins: function() {
    ...
    if (this._useMargins) {
      ...
      while (widget.parentWidget()) {
        ...
        // This view assumes it's always inside the main split widget element,
        // not a sidebar.
        // Every parent which is not a split widget, must be of the same size as
        // this widget.
    ...
    if (this._margins.top !== margins.top ||
  ...
  _dipPageRect: function() {
    ...
    var left = Math.max(
        rect.left * zoomFactor + this._margins.left,
        bodyRect.left * zoomFactor);
    var top = Math.max(
        rect.top * zoomFactor + this._margins.top, bodyRect.top * zoomFactor);
    var bottom = Math.min(
        rect.bottom * zoomFactor - this._margins.bottom,
        bodyRect.bottom * zoomFactor);
    var right = Math.min(
        rect.right * zoomFactor - this._margins.right,
        bodyRect.right * zoomFactor);
  ...
  update: function() {
    ...
    this.dispatchEventToListeners(
        WebInspector.InspectedPagePlaceholder.Events.Update, bounds);
  },
