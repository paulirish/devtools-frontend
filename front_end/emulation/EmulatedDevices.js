...
WebInspector.EmulatedDevice =
  ...
  this.vertical =
      {width: 0, height: 0, outlineInsets: null, outlineImage: null};
  /** @type {!WebInspector.EmulatedDevice.Orientation} */
  this.horizontal =
      {width: 0, height: 0, outlineInsets: null, outlineImage: null};
  /** @type {number} */
  ...
  /** @type {!Array.<string>} */
  this.capabilities = [
    WebInspector.EmulatedDevice.Capability.Touch,
    WebInspector.EmulatedDevice.Capability.Mobile
  ];
  /** @type {string} */
  this.userAgent = '';
  ...
  this._extension = null;
    ...
    /** @typedef {!{title: string, orientation: string, insets: !Insets, image:
       ?string}} */
    WebInspector.EmulatedDevice.Mode;

/** @typedef {!{width: number, height: number, outlineInsets: ?Insets,
 * outlineImage: ?string}} */
  ...
  try {
    ...
    function parseIntValue(object, key) {
      ...
      if (value !== Math.abs(value))
        throw new Error(
            'Emulated device value \'' + key + '\' must be integer');
      return value;
    }
    ...
    function parseInsets(json) {
      return new Insets(
          parseIntValue(json, 'left'), parseIntValue(json, 'top'),
          parseIntValue(json, 'right'), parseIntValue(json, 'bottom'));
    }
    ...
    function parseOrientation(json) {
      ...
      if (result.width < 0 ||
          result.width > WebInspector.DeviceModeModel.MaxDeviceSize ||
      ...
      if (result.height < 0 ||
          result.height > WebInspector.DeviceModeModel.MaxDeviceSize ||
      ...
      if (outlineInsets) {
        ...
        result.outlineImage = /** @type {string} */ (
            parseValue(json['outline'], 'image', 'string'));
      }
    ...
    var rawUserAgent =
        /** @type {string} */ (parseValue(json, 'user-agent', 'string'));
    result.userAgent =
    ...
    result.vertical =
        parseOrientation(parseValue(json['screen'], 'vertical', 'object'));
    result.horizontal =
        parseOrientation(parseValue(json['screen'], 'horizontal', 'object'));
    ...
    for (var i = 0; i < modes.length; ++i) {
      var mode = {};
      mode.title =
          /** @type {string} */ (parseValue(modes[i], 'title', 'string'));
      mode.orientation =
          /** @type {string} */ (parseValue(modes[i], 'orientation', 'string'));
      ...
      if (mode.insets.top < 0 || mode.insets.left < 0 ||
          mode.insets.right < 0 || mode.insets.bottom < 0 ||
      ...
      mode.image =
          /** @type {string} */ (parseValue(modes[i], 'image', 'string', null));
      result.modes.push(mode);
    }

    result._showByDefault = /** @type {boolean} */ (
        parseValue(json, 'show-by-default', 'boolean', undefined));
    result._show = /** @type {string} */ (parseValue(
        json, 'show', 'string', WebInspector.EmulatedDevice._Show.Default));
 ...
 * @return {number}
 */
    WebInspector.EmulatedDevice.deviceComparator =
        function(device1, device2) {
  var order1 =
      (device1._extension && device1._extension.descriptor()['order']) || -1;
  var order2 =
      (device2._extension && device2._extension.descriptor()['order']) || -1;
  if (order1 > order2)
    return 1;
  if (order2 > order1)
    return -1;
  return device1.title < device2.title ?
      -1 :
      (device1.title > device2.title ? 1 : 0);
}
  ...
  orientationByName: function(name) {
    return name === WebInspector.EmulatedDevice.Vertical ? this.vertical :
                                                           this.horizontal;
  },
  ...
  setShow: function(show) {
    this._show = show ? WebInspector.EmulatedDevice._Show.Always :
                        WebInspector.EmulatedDevice._Show.Never;
  },
  ...
  touch: function() {
    return this.capabilities.indexOf(
               WebInspector.EmulatedDevice.Capability.Touch) !== -1;
  },
  ...
  mobile: function() {
    return this.capabilities.indexOf(
               WebInspector.EmulatedDevice.Capability.Mobile) !== -1;
  }
}
  ...
  this._standardSetting =
      WebInspector.settings.createSetting('standardEmulatedDeviceList', []);
  ...
  StandardDevicesUpdated: Symbol('StandardDevicesUpdated')
                          ...
                          WebInspector.EmulatedDevicesList.prototype =
                {
                  _updateStandardDevices: function() {
                    ...
                    for (var i = 0; i < extensions.length; ++i) {
                      var device = WebInspector.EmulatedDevice.fromJSONV1(
                          extensions[i].descriptor()['device']);
     ...
     */
                  _listFromJSONV1: function(jsonArray, result) {
                    ...
                    for (var i = 0; i < jsonArray.length; ++i) {
                      var device =
                          WebInspector.EmulatedDevice.fromJSONV1(jsonArray[i]);
                      if (device) {
                        result.push(device);
     ...
     */
                  ...
                  saveCustomDevices: function() {
                    var json = this._custom.map(
                        /** @param {!WebInspector.EmulatedDevice} device */
                        function(device) { return device._toJSON(); });
...
WebInspector.EmulatedDevicesList.instance = function() {
  if (!WebInspector.EmulatedDevicesList._instance)
    WebInspector.EmulatedDevicesList._instance =
        new WebInspector.EmulatedDevicesList();
