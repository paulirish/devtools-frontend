...
WebInspector.SensorsView =
  ...
  this._geolocation =
      WebInspector.Geolocation.parseSetting(this._geolocationSetting.get());
  this._geolocationOverrideEnabled = false;
  this._createGeolocationSection(this._geolocation);

  this.contentElement.createChild('div').classList.add(
      'panel-section-separator');

  this._deviceOrientationSetting = WebInspector.settings.createSetting(
      'emulation.deviceOrientationOverride', '');
  this._deviceOrientation = WebInspector.DeviceOrientation.parseSetting(
  ...
  _createGeolocationSection: function(geolocation) {
    ...
    this._locationSelectElement =
        this.contentElement.createChild('select', 'chrome-select');
    ...
    var latitudeGroup =
        this._fieldsetElement.createChild('div', 'latlong-group');
    var longitudeGroup =
        this._fieldsetElement.createChild('div', 'latlong-group');
  ...
  _geolocationSelectChanged: function() {
    ...
    var value = this._locationSelectElement
                    .options[this._locationSelectElement.selectedIndex]
                    .value;
    if (value === WebInspector.SensorsView.NonPresetOptions.NoOverride) {
      ...
      this._geolocationOverrideEnabled = true;
        ...
        value === WebInspector.SensorsView.NonPresetOptions.Unavailable) {
      this._geolocationOverrideEnabled = true;
      ...
      this._geolocation =
          new WebInspector.Geolocation(coordinates[0], coordinates[1], false);
  ...
  _applyGeolocationUserInput: function() {
    ...
    this._setSelectElementLabel(
        this._locationSelectElement,
        WebInspector.SensorsView.NonPresetOptions.Custom);
  ...
  _createDeviceOrientationSection: function() {
    ...
    this._orientationSelectElement =
        this.contentElement.createChild('select', 'chrome-select');
    this._orientationSelectElement.appendChild(new Option(
        orientationOffOption.title, orientationOffOption.orientation));
    this._orientationSelectElement.appendChild(new Option(
        customOrientationOption.title, customOrientationOption.orientation));
    ...
    for (var i = 0; i < orientationGroups.length; ++i) {
      ...
      for (var j = 0; j < group.length; ++j)
        groupElement.appendChild(
            new Option(group[j].title, group[j].orientation));
    }
    ...
    this._stageElement =
        orientationContent.createChild('div', 'orientation-stage');
    this._stageElement.title = WebInspector.UIString(
        'Shift+drag horizontally to rotate around the y-axis');
    this._orientationLayer =
        this._stageElement.createChild('div', 'orientation-layer');
    this._boxElement = this._orientationLayer.createChild(
        'section', 'orientation-box orientation-element');

    this._boxElement.createChild(
        'section', 'orientation-front orientation-element');
    this._boxElement.createChild(
        'section', 'orientation-top orientation-element');
    this._boxElement.createChild(
        'section', 'orientation-back orientation-element');
    this._boxElement.createChild(
        'section', 'orientation-left orientation-element');
    this._boxElement.createChild(
        'section', 'orientation-right orientation-element');
    this._boxElement.createChild(
        'section', 'orientation-bottom orientation-element');

    WebInspector.installDragHandle(
        this._stageElement, this._onBoxDragStart.bind(this),
        this._onBoxDrag.bind(this), null, '-webkit-grabbing', '-webkit-grab');
  ...
  _orientationSelectChanged: function() {
    var value = this._orientationSelectElement
                    .options[this._orientationSelectElement.selectedIndex]
                    .value;
    ...
    if (value === WebInspector.SensorsView.NonPresetOptions.NoOverride) {
      ...
      this._deviceOrientationOverrideEnabled = true;
      this._deviceOrientation = new WebInspector.DeviceOrientation(
  ...
  _setSelectElementLabel: function(selectElement, labelValue) {
    var optionValues =
        Array.prototype.map.call(selectElement.options, x => x.value);
   ...
   * @param {!WebInspector.SensorsView.DeviceOrientationModificationSource}
   * modificationSource
   */
  ...
  _createAxisInput: function(parentElement, input, label) {
    var div =
        parentElement.createChild('div', 'orientation-axis-input-container');
  ...
  _createDeviceOrientationOverrideElement: function(deviceOrientation) {
    ...
    var cellElement =
        fieldsetElement.createChild('td', 'orientation-inputs-cell');

    this._alphaElement = createElement('input');
    this._alphaSetter = this._createAxisInput(
        cellElement, this._alphaElement,
        WebInspector.UIString('\u03B1 (alpha)'));
    ...
    this._gammaSetter = this._createAxisInput(
        cellElement, this._gammaElement,
        WebInspector.UIString('\u03B3 (gamma)'));
  ...
  _setBoxOrientation: function(deviceOrientation, animate) {
    ...
    // The CSS transform should not depend on matrix3d, which does not
    // interpolate well.
    var matrix = new WebKitCSSMatrix();
    this._boxMatrix = matrix.rotate(
        -deviceOrientation.beta, deviceOrientation.gamma,
        -deviceOrientation.alpha);
    var eulerAngles = new WebInspector.Geometry.EulerAngles(
        deviceOrientation.alpha, deviceOrientation.beta,
  ...
  _onBoxDrag: function(event) {
    ...
    if (event.shiftKey) {
      ...
      axis = WebInspector.Geometry.crossProduct(
          this._mouseDownVector, mouseMoveVector);
      angle = WebInspector.Geometry.calculateAngle(
          this._mouseDownVector, mouseMoveVector);
    }

    // The mouse movement vectors occur in the screen space, which is offset by
    // 90 degrees from
    ...
    var eulerAngles =
        WebInspector.Geometry.EulerAngles.fromRotationMatrix(currentMatrix);
  ...
  _calculateRadiusVector: function(x, y) {
    ...
    if (sqrSum > 0.5)
      return new WebInspector.Geometry.Vector(
          sphereX, sphereY, 0.5 / Math.sqrt(sqrSum));

    return new WebInspector.Geometry.Vector(
        sphereX, sphereY, Math.sqrt(1 - sqrSum));
  },
  ...
  _appendTouchControl: function() {
    ...
    select.appendChild(
        new Option(WebInspector.UIString('Device-based'), 'auto'));
    select.appendChild(
        new Option(WebInspector.UIString('Force enabled'), 'enabled'));
  ...
  __proto__: WebInspector.VBox.prototype
        ...
        WebInspector.SensorsView.NonPresetOptions = {
                                                        ...
                                                        .PresetLocations = [
          {
            ...
            value: [
              {
                title: WebInspector.UIString('Berlin'),
                location: '[52.520007, 13.404954]'
              },
              {
                title: WebInspector.UIString('London'),
                location: '[51.507351, -0.127758]'
              },
              {
                title: WebInspector.UIString('Moscow'),
                location: '[55.755826, 37.617300]'
              },
              {
                title: WebInspector.UIString('Mountain View'),
                location: '[37.386052, -122.083851]'
              },
              {
                title: WebInspector.UIString('Mumbai'),
                location: '[19.075984, 72.877656]'
              },
              {
                title: WebInspector.UIString('San Francisco'),
                location: '[37.774929, -122.419416]'
              },
              {
                title: WebInspector.UIString('Shanghai'),
                location: '[31.230416, 121.473701]'
              },
              {
                title: WebInspector.UIString('São Paulo'),
                location: '[-23.550520, -46.633309]'
              },
              {
                title: WebInspector.UIString('Tokyo'),
...
WebInspector.SensorsView.PresetOrientations = [{
  ...
  value: [
    {title: WebInspector.UIString('Portrait'), orientation: '[0, 90, 0]'}, {
      title: WebInspector.UIString('Portrait upside down'),
      orientation: '[180, -90, 0]'
    },
    {
      title: WebInspector.UIString('Landscape left'),
      orientation: '[0, 90, -90]'
    },
    {
      title: WebInspector.UIString('Landscape right'),
      orientation: '[0, 90, 90]'
    },
    {title: WebInspector.UIString('Display up'), orientation: '[0, 0, 0]'}, {
      title: WebInspector.UIString('Display down'),
      orientation: '[0, 180, 0]'
    }
  ]
}]
 ...
 */
WebInspector.SensorsView.instance =
    function() {
  ...
  return WebInspector.SensorsView._instanceObject;
     ...
     */
    WebInspector.SensorsView.ShowActionDelegate =
        function() {}
