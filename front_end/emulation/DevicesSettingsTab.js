 ...
 */
WebInspector.DevicesSettingsTab =
    function() {
  ...
  header.createChild('h3').createTextChild(
      WebInspector.UIString('Emulated Devices'));
  this.containerElement =
      this.element.createChild('div', 'help-container-wrapper')
          .createChild('div', 'settings-tab help-content help-container');

  var buttonsRow =
      this.containerElement.createChild('div', 'devices-button-row');
  this._addCustomButton = createTextButton(
      WebInspector.UIString('Add custom device...'),
      this._addCustomDevice.bind(this));
  ...
  /**
     ...
     * @return {string}
     */
  _toNumericInputValue: function(value) { return value ? String(value) : ''; },
  ...
  removeItemRequested: function(item, index) {
    this._emulatedDevicesList.removeCustomDevice(
        /** @type {!WebInspector.EmulatedDevice} */ (item));
  },
  ...
  commitEdit: function(item, editor, isNew) {
    ...
    device.vertical.width = editor.control('width').value ?
        parseInt(editor.control('width').value, 10) :
        0;
    device.vertical.height = editor.control('height').value ?
        parseInt(editor.control('height').value, 10) :
        0;
    device.horizontal.width = device.vertical.height;
    device.horizontal.height = device.vertical.width;
    device.deviceScaleFactor = editor.control('scale').value ?
        parseFloat(editor.control('scale').value) :
        0;
    ...
    if (uaType === WebInspector.DeviceModeModel.UA.Mobile ||
        uaType === WebInspector.DeviceModeModel.UA.MobileNoTouch)
      device.capabilities.push(WebInspector.EmulatedDevice.Capability.Mobile);
    if (uaType === WebInspector.DeviceModeModel.UA.Mobile ||
        uaType === WebInspector.DeviceModeModel.UA.DesktopTouch)
  ...
  beginEdit: function(item) {
    ...
    editor.control('width').value =
        this._toNumericInputValue(device.vertical.width);
    editor.control('height').value =
        this._toNumericInputValue(device.vertical.height);
    editor.control('scale').value =
        this._toNumericInputValue(device.deviceScaleFactor);
    editor.control('user-agent').value = device.userAgent;
    var uaType;
    if (device.mobile())
      uaType = device.touch() ? WebInspector.DeviceModeModel.UA.Mobile :
                                WebInspector.DeviceModeModel.UA.MobileNoTouch;
    else
      uaType = device.touch() ? WebInspector.DeviceModeModel.UA.DesktopTouch :
  ...
  _createEditor: function() {
    ...
    fields.createChild('div', 'hbox')
        .appendChild(editor.createInput(
            'title', 'text', WebInspector.UIString('Device name'),
            titleValidator));
    var screen = fields.createChild('div', 'hbox');
    screen.appendChild(editor.createInput(
        'width', 'text', WebInspector.UIString('Width'), sizeValidator));
    screen.appendChild(editor.createInput(
        'height', 'text', WebInspector.UIString('height'), sizeValidator));
    var dpr = editor.createInput(
        'scale', 'text', WebInspector.UIString('Device pixel ratio'),
        scaleValidator);
    dpr.classList.add('device-edit-fixed');
    screen.appendChild(dpr);
    var ua = fields.createChild('div', 'hbox');
    ua.appendChild(editor.createInput(
        'user-agent', 'text', WebInspector.UIString('User agent string'),
        () => true));
    ...
    function scaleValidator(item, index, input) {
      return WebInspector.DeviceModeModel.deviceScaleFactorValidator(
          input.value);
    }
  },
