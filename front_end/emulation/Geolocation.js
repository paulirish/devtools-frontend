 ...
 * @param {boolean} error
 */
WebInspector.Geolocation =
    function(latitude, longitude, error) {
    ...
    for (var target of WebInspector.targetManager.targets(
      ...
      if (this.error)
        ...
        target.emulationAgent().setGeolocationOverride(
            ...
            WebInspector.Geolocation.DefaultMockAccuracy);
    }
  },
 ...
 */
                                         WebInspector.Geolocation.parseSetting =
        function(value) {
  if (value) {
    ...
    if (splitError.length === 2) {
      ...
      if (splitPosition.length === 2)
        return new WebInspector.Geolocation(
            parseFloat(splitPosition[0]), parseFloat(splitPosition[1]),
            splitError[1]);
    }
  }
 ...
 * @return {?WebInspector.Geolocation}
 */
        WebInspector.Geolocation.parseUserInput =
            function(latitudeString, longitudeString, errorStatus) {
  if (!latitudeString && !longitudeString)
    return null;

  var isLatitudeValid =
      WebInspector.Geolocation.latitudeValidator(latitudeString);
  var isLongitudeValid =
      WebInspector.Geolocation.longitudeValidator(longitudeString);
 ...
 * @return {boolean}
 */
            WebInspector.Geolocation.latitudeValidator =
                function(value) {
  var numValue = parseFloat(value);
  return /^([+-]?[\d]+(\.\d+)?|[+-]?\.\d+)$/.test(value) && numValue >= -90 &&
      numValue <= 90;
}
 ...
 * @return {boolean}
 */
                WebInspector.Geolocation.longitudeValidator =
                    function(value) {
  var numValue = parseFloat(value);
  return /^([+-]?[\d]+(\.\d+)?|[+-]?\.\d+)$/.test(value) && numValue >= -180 &&
      numValue <= 180;
}
