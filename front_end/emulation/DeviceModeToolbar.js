 ...
 * @constructor
 */
WebInspector.DeviceModeToolbar =
    function(model, showMediaInspectorSetting, showRulersSetting) {
  this._model = model;
  ...
  this._showDeviceScaleFactorSetting = WebInspector.settings.createSetting(
      'emulation.showDeviceScaleFactor', false);
  this._showDeviceScaleFactorSetting.addChangeListener(
      this._updateDeviceScaleFactorVisibility, this);

  this._showUserAgentTypeSetting =
      WebInspector.settings.createSetting('emulation.showUserAgentType', false);
  this._showUserAgentTypeSetting.addChangeListener(
      this._updateUserAgentTypeVisibility, this);

  this._showNetworkConditionsSetting = WebInspector.settings.createSetting(
      'emulation.showNetworkConditions', false);
  this._showNetworkConditionsSetting.addChangeListener(
      this._updateNetworkConditionsVisibility, this);

  /** @type {!Map<!WebInspector.EmulatedDevice,
  ...
  var leftContainer =
      this._element.createChild('div', 'device-mode-toolbar-spacer');
  ...
  var rightContainer =
      this._element.createChild('div', 'device-mode-toolbar-spacer');
  var rightToolbar = new WebInspector.Toolbar(
      'device-mode-toolbar-fixed-size', rightContainer);
  rightToolbar.makeWrappable();
  this._fillRightToolbar(rightToolbar);
  var modeToolbar = new WebInspector.Toolbar(
      'device-mode-toolbar-fixed-size', rightContainer);
  ...
  this._emulatedDevicesList.addEventListener(
      WebInspector.EmulatedDevicesList.Events.CustomDevicesUpdated,
      this._deviceListChanged, this);
  this._emulatedDevicesList.addEventListener(
      WebInspector.EmulatedDevicesList.Events.StandardDevicesUpdated,
      this._deviceListChanged, this);

  this._persistenceSetting = WebInspector.settings.createSetting(
  ...
  _fillLeftToolbar: function(toolbar) {
    toolbar.appendToolbarItem(this._wrapToolbarItem(
        createElementWithClass('div', 'device-mode-empty-toolbar-element')));
    this._deviceSelectItem = new WebInspector.ToolbarMenuButton(
        this._appendDeviceMenuItems.bind(this));
  ...
  _fillMainToolbar: function(toolbar) {
    ...
    this._updateWidthInput = WebInspector.bindInput(
        widthInput, applyWidth.bind(this),
        WebInspector.DeviceModeModel.deviceSizeValidator, true);
    ...
    this._updateHeightInput = WebInspector.bindInput(
        heightInput, applyHeight.bind(this), validateHeight, true);
  ...
  _fillRightToolbar: function(toolbar) {
    toolbar.appendToolbarItem(this._wrapToolbarItem(
        createElementWithClass('div', 'device-mode-empty-toolbar-element')));
    this._scaleItem = new WebInspector.ToolbarMenuButton(
        this._appendScaleMenuItems.bind(this));
    ...
    toolbar.appendToolbarItem(this._wrapToolbarItem(
        createElementWithClass('div', 'device-mode-empty-toolbar-element')));
    this._deviceScaleItem = new WebInspector.ToolbarMenuButton(
        this._appendDeviceScaleMenuItems.bind(this));
    ...
    toolbar.appendToolbarItem(this._wrapToolbarItem(
        createElementWithClass('div', 'device-mode-empty-toolbar-element')));
    this._uaItem = new WebInspector.ToolbarMenuButton(
        this._appendUserAgentMenuItems.bind(this));
  ...
  _fillModeToolbar: function(toolbar) {
    toolbar.appendToolbarItem(this._wrapToolbarItem(
        createElementWithClass('div', 'device-mode-empty-toolbar-element')));
    this._modeButton =
        new WebInspector.ToolbarButton('', 'rotate-screen-toolbar-item');
  ...
  _fillOptionsToolbar: function(toolbar) {
    this._networkConditionsItem =
        WebInspector.NetworkConditionsSelector.createToolbarMenuButton();
    this._networkConditionsItem.setVisible(
        this._showNetworkConditionsSetting.get());
    this._networkConditionsItem.setTitle(
        WebInspector.UIString('Network throttling'));
    ...
    var moreOptionsButton = new WebInspector.ToolbarMenuButton(
        this._appendOptionsMenuItems.bind(this));
  ...
  _appendScaleMenuItems: function(contextMenu) {
    ...
    if (this._model.type() === WebInspector.DeviceModeModel.Type.Device) {
      contextMenu.appendItem(
          WebInspector.UIString(
              'Fit to window (%.0f%%)', this._model.fitScale() * 100),
          scaleSetting.set.bind(scaleSetting, this._model.fitScale()), false);
    ...
    function appendScaleItem(title, value) {
      contextMenu.appendCheckboxItem(
          title, scaleSetting.set.bind(scaleSetting, value),
          scaleSetting.get() === value, false);
    }
  },
  ...
  _appendDeviceScaleMenuItems: function(contextMenu) {
    ...
    var defaultValue = this._model.uaSetting().get() ===
                WebInspector.DeviceModeModel.UA.Mobile ||
            this._model.uaSetting().get() ===
    ...
    appendDeviceScaleFactorItem(
        WebInspector.UIString('Default: %.1f', defaultValue), 0);
    contextMenu.appendSeparator();
  ...
  _appendOptionsMenuItems: function(contextMenu) {
    ...
    appendToggleItem(
        ...
        WebInspector.UIString('Show device frame'),
        model.type() !== WebInspector.DeviceModeModel.Type.Device);
    appendToggleItem(
        this._showMediaInspectorSetting,
        WebInspector.UIString('Hide media queries'),
    ...
    appendToggleItem(
        ...
        WebInspector.UIString('Remove device pixel ratio'),
        WebInspector.UIString('Add device pixel ratio'));
    appendToggleItem(
        this._showUserAgentTypeSetting,
        WebInspector.UIString('Remove device type'),
        WebInspector.UIString('Add device type'));
    appendToggleItem(
        this._showNetworkConditionsSetting,
        WebInspector.UIString('Remove network throttling'),
        WebInspector.UIString('Add network throttling'));
    contextMenu.appendSeparator();
    contextMenu.appendItemsAtLocation('deviceModeMenu');
    contextMenu.appendSeparator();
    contextMenu.appendItem(
        WebInspector.UIString('Reset to defaults'), this._reset.bind(this));
    ...
    function appendToggleItem(setting, title1, title2, disabled) {
      ...
      contextMenu.appendItem(
          setting.get() ? title1 : title2,
          setting.set.bind(setting, !setting.get()), disabled);
    }
  },
  ...
  _wrapToolbarItem: function(element) {
    ...
    var shadowRoot = WebInspector.createShadowRootWithCoreStyles(
        container, 'emulation/deviceModeToolbar.css');
  ...
  _emulateDevice: function(device) {
    this._model.emulate(
        WebInspector.DeviceModeModel.Type.Device, device,
        this._lastMode.get(device) || device.modes[0]);
  },

  _switchToResponsive: function() {
    this._model.emulate(
        WebInspector.DeviceModeModel.Type.Responsive, null, null);
  },
  ...
  _appendDeviceMenuItems: function(contextMenu) {
    contextMenu.appendCheckboxItem(
        WebInspector.UIString('Responsive'),
        this._switchToResponsive.bind(this),
    ...
    contextMenu.appendItem(
        ...
        this._emulatedDevicesList.revealCustomSetting.bind(
            this._emulatedDevicesList),
        false);
    ...
    function appendGroup(devices) {
      ...
      for (var device of devices)
        contextMenu.appendCheckboxItem(
            device.title, this._emulateDevice.bind(this, device),
            this._model.device() === device, false);
    }
  },
  ...
  _modeMenuClicked: function(event) {
    ...
    if (device.modes.length === 2 &&
        device.modes[0].orientation !== device.modes[1].orientation) {
      model.emulate(
          model.type(), model.device(),
          model.mode() === device.modes[0] ? device.modes[1] : device.modes[0]);
      return;
    }

    var contextMenu = new WebInspector.ContextMenu(
        /** @type {!Event} */ (event.data), false,
        event.target.element.totalOffsetLeft(),
        event.target.element.totalOffsetTop() +
            event.target.element.offsetHeight);
    addOrientation(
        WebInspector.EmulatedDevice.Vertical,
        WebInspector.UIString('Portrait'));
    addOrientation(
    ...
    function addMode(mode, title) {
      contextMenu.appendCheckboxItem(
          title, applyMode.bind(null, mode), model.mode() === mode, false);
    }
  ...
  update: function() {
    if (this._model.type() !== this._cachedModelType) {
      ...
      this._widthInput.disabled =
          this._model.type() !== WebInspector.DeviceModeModel.Type.Responsive;
      this._heightInput.disabled =
          this._model.type() !== WebInspector.DeviceModeModel.Type.Responsive;
      this._deviceScaleItem.setEnabled(
          this._model.type() === WebInspector.DeviceModeModel.Type.Responsive);
      this._uaItem.setEnabled(
          this._model.type() === WebInspector.DeviceModeModel.Type.Responsive);
    }
    ...
    if (this._model.scale() !== this._cachedScale) {
      this._scaleItem.setText(
          WebInspector.UIString('%.0f%%', this._model.scale() * 100));
    ...
    if (deviceScale !== this._cachedDeviceScale) {
      this._deviceScaleItem.setText(
          WebInspector.UIString('DPR: %.1f', deviceScale));
    ...
    if (this._model.device() !== this._cachedModelDevice) {
      ...
      if (device) {
        ...
        this._modeButton.setTitle(
            modeCount === 2 ? WebInspector.UIString('Rotate') :
                              WebInspector.UIString('Screen options'));
      }
    ...
    if (this._model.mode() !== this._cachedModelMode &&
      ...
      if (this._model.device()) {
        ...
        value.orientation =
            this._model.mode() ? this._model.mode().orientation : '';
