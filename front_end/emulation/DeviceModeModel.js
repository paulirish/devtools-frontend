...
WebInspector.DeviceModeModel =
  ...
  this._scaleSetting =
      WebInspector.settings.createSetting('emulation.deviceScale', 1);
  ...
  this._widthSetting =
      WebInspector.settings.createSetting('emulation.deviceWidth', 400);
  ...
  this._heightSetting =
      WebInspector.settings.createSetting('emulation.deviceHeight', 0);
  ...
  this._deviceScaleFactorSetting.addChangeListener(
      this._deviceScaleFactorSettingChanged, this);

  this._deviceOutlineSetting =
      WebInspector.settings.moduleSetting('emulation.showDeviceOutline');
  this._deviceOutlineSetting.addChangeListener(
      this._deviceOutlineSettingChanged, this);
 ...
 * @return {boolean}
 */
WebInspector.DeviceModeModel.deviceSizeValidator =
    function(value) {
  if (/^[\d]+$/.test(value) &&
      value >= WebInspector.DeviceModeModel.MinDeviceSize &&
 ...
 * @return {boolean}
 */
    WebInspector.DeviceModeModel.deviceScaleFactorValidator =
        function(value) {
  if (!value ||
      (/^[\d]+(\.\d+)?|\.\d+$/.test(value) && value >= 0 && value <= 10))
    return true;
  return false;
}
...
WebInspector.DeviceModeModel.prototype = {
  ...
  emulate: function(type, device, mode) {
    ...
    if (type === WebInspector.DeviceModeModel.Type.Device) {
      console.assert(
          device && mode, 'Must pass device and mode for device emulation');
      this._device = device;
      this._mode = mode;
      if (this._initialized) {
        var orientation = device.orientationByName(mode.orientation);
        this._scaleSetting.set(this._calculateFitScale(
            orientation.width, orientation.height, this._currentOutline(),
            this._currentInsets()));
      }
    } else {
      this._device = null;
    ...
    if (type !== WebInspector.DeviceModeModel.Type.None)
      WebInspector.userMetrics.actionTaken(
          WebInspector.UserMetrics.Action.DeviceModeEnabled);
  ...
  setWidthAndScaleToFit: function(width) {
    width = Math.max(
        Math.min(width, WebInspector.DeviceModeModel.MaxDeviceSize), 1);
    this._scaleSetting.set(
        this._calculateFitScale(width, this._heightSetting.get()));
    this._widthSetting.set(width);
  },
  ...
  setHeightAndScaleToFit: function(height) {
    height = Math.max(
        Math.min(height, WebInspector.DeviceModeModel.MaxDeviceSize), 0);
    this._scaleSetting.set(
        this._calculateFitScale(this._widthSetting.get(), height));
    this._heightSetting.set(height);
  },
  ...
  _preferredScaledWidth: function() {
    return Math.floor(
        this._preferredSize.width / (this._scaleSetting.get() || 1));
  },
  ...
  _preferredScaledHeight: function() {
    return Math.floor(
        this._preferredSize.height / (this._scaleSetting.get() || 1));
  },
  ...
  _calculateAndEmulate: function(resetPageScaleFactor) {
    if (!this._target)
      this._onTargetAvailable =
          this._calculateAndEmulate.bind(this, resetPageScaleFactor);

    if (this._type === WebInspector.DeviceModeModel.Type.Device) {
      var orientation = this._device.orientationByName(this._mode.orientation);
      var outline = this._currentOutline();
      var insets = this._currentInsets();
      this._fitScale = this._calculateFitScale(
          orientation.width, orientation.height, outline, insets);
      ...
      this._applyDeviceMetrics(
          ...
          this._scaleSetting.get(), this._device.deviceScaleFactor,
          this._device.mobile(),
          this._mode.orientation === WebInspector.EmulatedDevice.Horizontal ?
              'landscapePrimary' :
              'portraitPrimary',
      ...
      this._fitScale = this._calculateFitScale(
          this._availableSize.width, this._availableSize.height);
      this._appliedUserAgentType = WebInspector.DeviceModeModel.UA.Desktop;
      this._applyDeviceMetrics(
          this._availableSize, new Insets(0, 0, 0, 0), new Insets(0, 0, 0, 0),
      ...
      this._fitScale = this._calculateFitScale(
          this._widthSetting.get(), this._heightSetting.get());
      this._appliedUserAgentType = this._uaSetting.get();
      this._applyDeviceMetrics(
          new Size(screenWidth, screenHeight), new Insets(0, 0, 0, 0),
          new Insets(0, 0, 0, 0), this._scaleSetting.get(),
          this._deviceScaleFactorSetting.get() || defaultDeviceScaleFactor,
          mobile,
          screenHeight >= screenWidth ? 'portraitPrimary' : 'landscapePrimary',
          resetPageScaleFactor);
      this._applyUserAgent(
          mobile ? WebInspector.DeviceModeModel._defaultMobileUserAgent : '');
      this._applyTouch(
          this._uaSetting.get() ===
                  WebInspector.DeviceModeModel.UA.DesktopTouch ||
              this._uaSetting.get() === WebInspector.DeviceModeModel.UA.Mobile,
  ...
  _calculateFitScale: function(screenWidth, screenHeight, outline, insets) {
    ...
    var scale = Math.min(
        screenWidth ? this._preferredSize.width / (screenWidth + outlineWidth) :
                      1,
        screenHeight ?
            this._preferredSize.height / (screenHeight + outlineHeight) :
            1);
    ...
    while (sharpScale > scale * 0.7) {
      ...
      if (screenWidth)
        sharp = sharp &&
            Number.isInteger((screenWidth - insetsWidth) * sharpScale / 100);
      if (screenHeight)
        sharp = sharp &&
            Number.isInteger((screenHeight - insetsHeight) * sharpScale / 100);
      if (sharp)
  ...
  _applyDeviceMetrics: function(
      ...
      screenOrientation, resetPageScaleFactor) {
    ...
    this._visiblePageRect = new WebInspector.Rect(
        ...
        Math.min(
            pageHeight * scale, this._availableSize.height -
                this._screenRect.top - positionY * scale));
    this._scale = scale;

    if (scale === 1 && this._availableSize.width >= screenSize.width &&
        this._availableSize.height >= screenSize.height) {
      // When we have enough space, no page size override is required. This will
    ...
    if (this._visiblePageRect.width === pageWidth * scale &&
        ...
        Number.isInteger(pageWidth * scale) &&
        Number.isInteger(pageHeight * scale)) {
      // When we only have to apply scale, do not resize the page. This will
      // speed things up and remove lag.
      pageWidth = 0;
      pageHeight = 0;
    }
    ...
    function setDeviceMetricsOverride() {
      ...
      var clear = !pageWidth && !pageHeight && !mobile && !deviceScaleFactor &&
      ...
      if (clear) {
        ...
        setDevicePromise =
            this._target.emulationAgent().invoke_setDeviceMetricsOverride(
  ...
  _applyTouch: function(touchEnabled, mobile) {
    WebInspector.MultitargetTouchModel.instance().setTouchEnabled(
        touchEnabled, mobile);
  }
}
