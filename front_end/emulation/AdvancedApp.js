...
WebInspector.AdvancedApp.prototype = {
  ...
  presentUI: function(document) {
    ...
    this._rootSplitWidget = new WebInspector.SplitWidget(
        false, true, 'InspectorView.splitViewState', 555, 300, true);
    ...
    this._inspectedPagePlaceholder =
        new WebInspector.InspectedPagePlaceholder();
    ...
    this._deviceModeView =
        new WebInspector.DeviceModeWrapper(this._inspectedPagePlaceholder);

    WebInspector.dockController.addEventListener(
        WebInspector.DockController.Events.BeforeDockSideChanged,
        this._onBeforeDockSideChange, this);
    WebInspector.dockController.addEventListener(
        WebInspector.DockController.Events.DockSideChanged,
        this._onDockSideChange, this);
    WebInspector.dockController.addEventListener(
  ...
  toolboxLoaded: function(toolboxDocument) {
    WebInspector.initializeUIUtils(
        toolboxDocument,
        WebInspector.settings.createSetting('uiTheme', 'default'));
    WebInspector.installComponentRootStyles(
        /** @type {!Element} */ (toolboxDocument.body));
  ...
  _onDockSideChange: function(event) {
    ...
    var toDockSide = event ? /** @type {string} */ (event.data.to) :
  ...
  _onAfterDockSideChange: function(event) {
    // We may get here on the first dock side change while loading without
    // BeforeDockSideChange.
    if (!this._changingDockSide)
      return;
    if (/** @type {string} */ (event.data.from) ===
        WebInspector.DockController.State.Undocked) {
  ...
  _updateForDocked: function(dockSide) {
    ...
    this._rootSplitWidget.toggleResizer(
        this._rootSplitWidget.resizerElement(), true);
    this._rootSplitWidget.toggleResizer(
  ...
  _updateForUndocked: function() {
    this._rootSplitWidget.toggleResizer(
        this._rootSplitWidget.resizerElement(), false);
    this._rootSplitWidget.toggleResizer(
        WebInspector.inspectorView.topResizerElement(), false);
    this._rootSplitWidget.hideMain();
  },

  _isDocked: function() {
    return WebInspector.dockController.dockSide() !==
        WebInspector.DockController.State.Undocked;
  },
