 ...
 */
WebInspector.Infobar =
    function(type, text, disableSetting) {
  this.element = createElementWithClass('div', 'flex-none');
  this._shadowRoot = WebInspector.createShadowRootWithCoreStyles(
      this.element, 'ui/infobar.css');
  this._contentElement =
      this._shadowRoot.createChild('div', 'infobar infobar-' + type);
  ...
  this._detailsRows =
      this._contentElement.createChild('div', 'infobar-details-rows hidden');

  this._toggleElement =
      this._mainRow.createChild('div', 'infobar-toggle hidden');
  this._toggleElement.addEventListener(
      'click', this._onToggleDetails.bind(this), false);
  ...
  this._closeButton = this._contentElement.createChild(
      'div', 'close-button', 'dt-close-button');
 ...
 */
   ...
   */
  setCloseCallback: function(callback) { this._closeCallback = callback; },

  /**
   * @param {!WebInspector.Widget} parentView
   */
  setParentView: function(parentView) { this._parentView = parentView; },
  ...
  createDetailsRowMessage: function(message) {
    ...
    var infobarDetailsRow =
        this._detailsRows.createChild('div', 'infobar-details-row');
    var detailsRowMessage =
        infobarDetailsRow.createChild('span', 'infobar-row-message');
