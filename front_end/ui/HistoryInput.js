 ...
 */
                            WebInspector.HistoryInput.create =
    function() {
  if (!WebInspector.HistoryInput._constructor)
    WebInspector.HistoryInput._constructor = registerCustomElement(
        'input', 'history-input', WebInspector.HistoryInput.prototype);
  ...
  _onKeyDown: function(event) {
    if (event.keyCode === WebInspector.KeyboardShortcut.Keys.Up.code) {
      ...
      this._historyPosition =
          Math.min(this._historyPosition + 1, this._history.length - 1);
      this.value = this._history[this._historyPosition];
      this.dispatchEvent(createEvent('input', true, true));
      event.consume(true);
    } else if (
        event.keyCode === WebInspector.KeyboardShortcut.Keys.Enter.code) {
      this._saveToHistory();
    }
  },
