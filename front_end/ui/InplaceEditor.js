...
WebInspector.InplaceEditor = function() {}
                             ...
                             /**
                              * @typedef {{cancel: function(), commit:
                              * function(), setWidth: function(number)}}
                              */
...
WebInspector.InplaceEditor.startEditing =
  ...
  if (!WebInspector.InplaceEditor._defaultInstance)
    WebInspector.InplaceEditor._defaultInstance =
        new WebInspector.InplaceEditor();
  return WebInspector.InplaceEditor._defaultInstance.startEditing(
      element, config);
}
 ...
 */
    WebInspector.InplaceEditor.startMultilineEditing =
        function(element, config) {
  return self.runtime.extension(WebInspector.InplaceEditor)
      .instance()
      .then(startEditing);
  ...
  function startEditing(inplaceEditor) {
    var controller = /** @type {!WebInspector.InplaceEditor} */ (inplaceEditor)
  ...
  startEditing: function(element, config) {
    ...
    config = config ||
        new WebInspector.InplaceEditor.Config(function() {}, function() {});
     ...
     * @param {!Event} e
     */
    function consumeCopy(e) { e.consume(); }

    this.setUpEditor(editingContext);

    editingContext.oldText =
        isMultiline ? config.initialValue : this.editorContent(editingContext);
    ...
    function blurEventListener(e) {
      if (config.blurHandler && !config.blurHandler(element, e))
        return;
      if (!isMultiline || !e || !e.relatedTarget ||
    ...
    function editingCommitted() {
      ...
      committedCallback(
          this, self.editorContent(editingContext), editingContext.oldText,
    ...
    return handle;
                         ...
                         /**
                          * @constructor
                          * @param {function(!Element,string,string,T,string)}
                          * commitHandler
                          * @param {function(!Element,T)} cancelHandler
                          * @param {T=} context
                          * @param {function(!Element,!Event):boolean=}
                          * blurHandler
                          * @template T
                          */
                         WebInspector.InplaceEditor.Config =
            function(commitHandler, cancelHandler, context, blurHandler) {
  ...
  setPasteHandler: function(pasteHandler) { this.pasteHandler = pasteHandler; },
  ...
  setMultilineOptions: function(
      initialValue, mode, theme, lineWrapping, smartIndent) {
    this.multiline = true;
