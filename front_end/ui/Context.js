...
WebInspector.Context =
  ...
  _dispatchFlavorChange: function(flavorType, flavorValue) {
    for (var extension of self.runtime.extensions(
             WebInspector.ContextFlavorListener)) {
      if (extension.hasContextType(flavorType))
        extension.instance().then(
            instance =>
                /** @type {!WebInspector.ContextFlavorListener} */ (instance)
                    .flavorChanged(flavorValue));
    }
    var dispatcher = this._eventDispatchers.get(flavorType);
    if (!dispatcher)
      return;
    dispatcher.dispatchEventToListeners(
        WebInspector.Context.Events.FlavorChanged, flavorValue);
  },
  ...
  addFlavorChangeListener: function(flavorType, listener, thisObject) {
    ...
    dispatcher.addEventListener(
        WebInspector.Context.Events.FlavorChanged, listener, thisObject);
  },
  ...
  removeFlavorChangeListener: function(flavorType, listener, thisObject) {
    ...
    if (!dispatcher.hasEventListeners(
            WebInspector.Context.Events.FlavorChanged))
  ...
  /**
     ...
     */
  flavors: function() { return new Set(this._flavors.keys()); },
  ...
  applicableExtensions: function(extensions) {
    ...
    extensions.forEach(function(extension) {
      if (self.runtime.isExtensionApplicableToContextTypes(
