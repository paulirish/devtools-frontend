...
WebInspector.CSSShadowEditor =
  ...
  this._typeField =
      this.contentElement.createChild('div', 'shadow-editor-field');
  this._typeField.createChild('label', 'shadow-editor-label').textContent =
      WebInspector.UIString('Type');
  this._outsetButton =
      this._typeField.createChild('button', 'shadow-editor-button-left');
  this._outsetButton.textContent = WebInspector.UIString('Outset');
  this._outsetButton.addEventListener(
      'click', this._onButtonClick.bind(this), false);
  this._insetButton =
      this._typeField.createChild('button', 'shadow-editor-button-right');
  this._insetButton.textContent = WebInspector.UIString('Inset');
  this._insetButton.addEventListener(
      'click', this._onButtonClick.bind(this), false);

  var xField = this.contentElement.createChild('div', 'shadow-editor-field');
  this._xInput =
      this._createTextInput(xField, WebInspector.UIString('X offset'));
  var yField = this.contentElement.createChild('div', 'shadow-editor-field');
  this._yInput =
      this._createTextInput(yField, WebInspector.UIString('Y offset'));
  ...
  this._innerCanvasSize =
      this._halfCanvasSize - WebInspector.CSSShadowEditor.sliderThumbRadius;
  WebInspector.installDragHandle(
      this._xySlider, this._dragStart.bind(this), this._dragMove.bind(this),
      null, 'default');
  this._xySlider.addEventListener(
      'keydown', this._onCanvasArrowKey.bind(this), false);
  this._xySlider.addEventListener('blur', this._onCanvasBlur.bind(this), false);

  var blurField =
      this.contentElement.createChild('div', 'shadow-editor-blur-field');
  this._blurInput =
      this._createTextInput(blurField, WebInspector.UIString('Blur'));
  this._blurSlider = this._createSlider(blurField);

  this._spreadField =
      this.contentElement.createChild('div', 'shadow-editor-field');
  this._spreadInput =
      this._createTextInput(this._spreadField, WebInspector.UIString('Spread'));
...
WebInspector.CSSShadowEditor.prototype = {
  ...
  _createTextInput: function(field, propertyName) {
    ...
    textInput.addEventListener(
        'keydown', this._handleValueModification.bind(this), false);
    textInput.addEventListener(
        'mousewheel', this._handleValueModification.bind(this), false);
  ...
  _createSlider: function(field) {
    var slider =
        createSliderLabel(0, WebInspector.CSSShadowEditor.maxRange, -1);
   ...
   * @override
   */
  wasShown: function() { this._updateUI(); },
  ...
  _updateCanvas: function(drawFocus) {
    ...
    context.lineTo(
        this._halfCanvasSize, WebInspector.CSSShadowEditor.canvasSize);
    context.moveTo(0, this._halfCanvasSize);
    context.lineTo(
        WebInspector.CSSShadowEditor.canvasSize, this._halfCanvasSize);
    context.stroke();
    context.restore();
    ...
    if (drawFocus) {
      ...
      context.arc(
          thumbPoint.x, thumbPoint.y,
          WebInspector.CSSShadowEditor.sliderThumbRadius + 2, 0, 2 * Math.PI);
      context.fill();
    }
    context.beginPath()
    ...
    context.arc(
        thumbPoint.x, thumbPoint.y,
        WebInspector.CSSShadowEditor.sliderThumbRadius, 0, 2 * Math.PI);
    context.fill();
    context.restore();
  },
  ...
  _onButtonClick: function(event) {
    ...
    if (insetClicked && this._model.inset() ||
        !insetClicked && !this._model.inset())
      return;
    this._model.setInset(insetClicked);
    this._updateButtons();
    this.dispatchEventToListeners(
        WebInspector.CSSShadowEditor.Events.ShadowChanged, this._model);
  },
  ...
  _handleValueModification: function(event) {
    var modifiedValue = WebInspector.createReplacementString(
        event.currentTarget.value, event, customNumberHandler);
    if (!modifiedValue)
      return;
  ...
  _onTextBlur: function() {
    if (!this._changedElement)
      return;
    var length = !this._changedElement.value.trim() ?
        WebInspector.CSSLength.zero() :
        WebInspector.CSSLength.parse(this._changedElement.value);
    if (!length)
      length = WebInspector.CSSLength.parse(
          this._changedElement.value +
          WebInspector.CSSShadowEditor.defaultUnit);
    if (!length) {
  ...
  _onSliderInput: function(event) {
    if (event.currentTarget === this._blurSlider) {
      this._model.setBlurRadius(new WebInspector.CSSLength(
          this._blurSlider.value, this._model.blurRadius().unit ||
              WebInspector.CSSShadowEditor.defaultUnit));
      this._blurInput.value = this._model.blurRadius().asCSSText();
      ...
      this._model.setSpreadRadius(new WebInspector.CSSLength(
          this._spreadSlider.value, this._model.spreadRadius().unit ||
              WebInspector.CSSShadowEditor.defaultUnit));
      this._spreadInput.value = this._model.spreadRadius().asCSSText();
  ...
  _dragStart: function(event) {
    ...
    var clickedPoint = new WebInspector.Geometry.Point(
        event.x - this._canvasOrigin.x, event.y - this._canvasOrigin.y);
    var thumbPoint = this._sliderThumbPosition();
    if (clickedPoint.distanceTo(thumbPoint) >=
        WebInspector.CSSShadowEditor.sliderThumbRadius)
  ...
  _dragMove: function(event) {
    var point = new WebInspector.Geometry.Point(
        event.x - this._canvasOrigin.x, event.y - this._canvasOrigin.y);
    if (event.shiftKey)
      point = this._snapToClosestDirection(point);
    var constrainedPoint = this._constrainPoint(point, this._innerCanvasSize);
    var newX = Math.round(
        (constrainedPoint.x / this._innerCanvasSize) *
        WebInspector.CSSShadowEditor.maxRange);
    var newY = Math.round(
        (constrainedPoint.y / this._innerCanvasSize) *
        WebInspector.CSSShadowEditor.maxRange);

    if (event.shiftKey) {
      this._model.setOffsetX(new WebInspector.CSSLength(
          newX, this._model.offsetX().unit ||
              WebInspector.CSSShadowEditor.defaultUnit));
      this._model.setOffsetY(new WebInspector.CSSLength(
          newY, this._model.offsetY().unit ||
              WebInspector.CSSShadowEditor.defaultUnit));
    } else {
      if (!event.altKey)
        this._model.setOffsetX(new WebInspector.CSSLength(
  ...
  _onCanvasBlur: function() { this._updateCanvas(false); },
  ...
  _onCanvasArrowKey: function(event) {
    ...
    if (shiftX) {
      ...
      var newAmount = Number.constrain(
          offsetX.amount + shiftX, -WebInspector.CSSShadowEditor.maxRange,
          WebInspector.CSSShadowEditor.maxRange);
      if (newAmount === offsetX.amount)
        return;
      this._model.setOffsetX(new WebInspector.CSSLength(
          newAmount, offsetX.unit || WebInspector.CSSShadowEditor.defaultUnit));
      this._xInput.value = this._model.offsetX().asCSSText();
      this._xInput.classList.remove('invalid');
    }
    if (shiftY) {
      ...
      var newAmount = Number.constrain(
          offsetY.amount + shiftY, -WebInspector.CSSShadowEditor.maxRange,
          WebInspector.CSSShadowEditor.maxRange);
      if (newAmount === offsetY.amount)
        return;
      this._model.setOffsetY(new WebInspector.CSSLength(
          newAmount, offsetY.unit || WebInspector.CSSShadowEditor.defaultUnit));
      this._yInput.value = this._model.offsetY().asCSSText();
      this._yInput.classList.remove('invalid');
    }
    this._updateCanvas(true);
  ...
  _sliderThumbPosition: function() {
    var x =
        (this._model.offsetX().amount / WebInspector.CSSShadowEditor.maxRange) *
        this._innerCanvasSize;
    var y =
        (this._model.offsetY().amount / WebInspector.CSSShadowEditor.maxRange) *
        this._innerCanvasSize;
    return this._constrainPoint(
        new WebInspector.Geometry.Point(x, y), this._innerCanvasSize);
  },
