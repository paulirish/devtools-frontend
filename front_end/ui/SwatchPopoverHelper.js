 ...
 */
WebInspector.SwatchPopoverHelper =
    function() {
  ...
  _onFocusOut: function(event) {
    if (!event.relatedTarget ||
        event.relatedTarget.isSelfOrDescendant(this._view.contentElement))
      return;
    this._hideProxy();
  },
  ...
  /**
     * @return {boolean}
     */
  isShowing: function() { return this._popover.isShowing(); },
  ...
  show: function(view, anchorElement, hiddenCallback) {
    ...
    this._view.contentElement.addEventListener(
        'keydown', this._boundOnKeyDown, false);
  },

  reposition: function() {
    if (!this._previousFocusElement)
      this._previousFocusElement = WebInspector.currentFocusElement();
    // Unbind "blur" listener to avoid reenterability: |popover.showView| will
    // hide the popover and trigger it synchronously.
    this._view.contentElement.removeEventListener(
        'focusout', this._boundFocusOut, false);
