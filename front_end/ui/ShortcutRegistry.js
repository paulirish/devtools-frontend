...
WebInspector.ShortcutRegistry =
    function(actionRegistry, document) {
  ...
  _applicableActions: function(key) {
    return this._actionRegistry.applicableActions(
        this._defaultActionsForKey(key).valuesArray(), WebInspector.context);
  },
  ...
  handleShortcut: function(event) {
    this.handleKey(
        WebInspector.KeyboardShortcut.makeKeyFromEvent(event), event.key,
        event);
  },
  ...
  handleKey: function(key, domKey, event) {
    ...
    if (!isPossiblyInputKey()) {
      ...
      this._pendingActionTimer =
          setTimeout(processNextAction.bind(this, false), 0);
    }
    ...
    function isPossiblyInputKey() {
      if (!event || !WebInspector.isEditing() ||
          /^F\d+|Control|Shift|Alt|Meta|Escape|Win|U\+001B$/.test(domKey))
        return false;
      ...
      if ((keyModifiers & (modifiers.Ctrl | modifiers.Alt)) ===
  ...
  _registerBindings: function(document) {
    document.addEventListener(
        'input', this.dismissPendingShortcutAction.bind(this), true);
    ...
    function registerExtension(extension) {
      ...
      for (var i = 0; bindings && i < bindings.length; ++i) {
        ...
        shortcuts.forEach(
            this.registerShortcut.bind(this, descriptor['actionId']));
      }
    }
    ...
    function platformMatches(platformsString) {
      ...
      return isMatch;
        ...
        WebInspector.ShortcutRegistry.ForwardedShortcut.instance =
            new WebInspector.ShortcutRegistry.ForwardedShortcut();
