...
WebInspector.Tooltip =
  ...
  WebInspector.zoomManager.addEventListener(
      WebInspector.ZoomManager.Events.ZoomChanged, this._reset, this);
  ...
  _show: function(anchorElement, event) {
    ...
    for (var element of WebInspector.Tooltip._nativeOverrideContainer) {
      if (this._anchorElement.isSelfOrDescendant(element)) {
        Object.defineProperty(
            this._anchorElement, 'title', WebInspector.Tooltip._nativeTitle);
    ...
    if (tooltip.actionId) {
      ...
      for (var shortcut of shortcuts) {
        var shortcutElement =
            this._tooltipElement.createChild('div', 'tooltip-shortcut');
    ...
    var instant =
        (this._tooltipLastClosed &&
         now - this._tooltipLastClosed <
             WebInspector.Tooltip.Timing.InstantThreshold);
    this._tooltipElement.classList.toggle('instant', instant);
    this._tooltipLastOpened =
        instant ? now : now + WebInspector.Tooltip.Timing.OpeningDelay;
    ...
    var containerOffset =
        container.offsetRelativeToWindow(this.element.window());
    ...
    const pageMargin = 2;
    var cursorOffset = 10;
    this._tooltipElement.classList.toggle(
        'tooltip-breakword', !this._tooltipElement.textContent.match('\\s'));
    this._tooltipElement.style.maxWidth =
        (containerOffsetWidth - pageMargin * 2) + 'px';
    ...
    var anchorTooltipAtElement = this._anchorElement.nodeName === 'BUTTON' ||
        this._anchorElement.nodeName === 'LABEL';
    var tooltipX =
        anchorTooltipAtElement ? anchorBox.x : event.x + cursorOffset;
    tooltipX = Number.constrain(
        tooltipX, containerOffset.x + pageMargin,
        containerOffset.x + containerOffsetWidth - tooltipWidth - pageMargin);
    var tooltipY;
    if (!anchorTooltipAtElement) {
      tooltipY = event.y + cursorOffset + tooltipHeight <
              containerOffset.y + containerOffsetHeight ?
          event.y + cursorOffset :
          event.y - tooltipHeight;
    } else {
      var onBottom =
          anchorBox.y + anchorOffset + anchorBox.height + tooltipHeight <
          containerOffset.y + containerOffsetHeight;
      tooltipY = onBottom ? anchorBox.y + anchorBox.height + anchorOffset :
 ...
 * @param {!Document} doc
 */
WebInspector.Tooltip.installHandler =
    function(doc) {
  new WebInspector.Tooltip(doc);
     ...
     */
    WebInspector.Tooltip.install =
        function(element, tooltipContent, actionId, options) {
  if (typeof tooltipContent === 'string' && tooltipContent === '') {
    delete element[WebInspector.Tooltip._symbol];
    return;
  }
  element[WebInspector.Tooltip._symbol] = {
    ...
    options: options || {}
         ...
         * @param {!Element} element
         */
        WebInspector.Tooltip.addNativeOverrideContainer =
            function(element) {
...
WebInspector.Tooltip._nativeTitle = /** @type {!ObjectPropertyDescriptor} */ (
...
Object.defineProperty(HTMLElement.prototype, 'title', {
   ...
   * @this {!Element}
   */
  set: function(x) { WebInspector.Tooltip.install(this, x); }
});
