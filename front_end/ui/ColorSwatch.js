 ...
 */
                           WebInspector.ColorSwatch.create =
    function() {
  if (!WebInspector.ColorSwatch._constructor)
    WebInspector.ColorSwatch._constructor = registerCustomElement(
        'span', 'color-swatch', WebInspector.ColorSwatch.prototype);
   ...
   * @param {boolean} hide
   */
  hideText: function(hide) { this._colorValueElement.hidden = hide; },
  ...
  toggleNextFormat: function() {
    do {
      this._format =
          WebInspector.ColorSwatch._nextColorFormat(this._color, this._format);
  ...
  /**
     * @return {!Element}
     */
  iconElement: function() { return this._iconElement; },

  createdCallback: function() {
    var root =
        WebInspector.createShadowRootWithCoreStyles(this, 'ui/colorSwatch.css');

    this._iconElement = root.createChild('span', 'color-swatch');
    this._iconElement.title =
        WebInspector.UIString('Shift-click to change color format');
    this._swatchInner =
        this._iconElement.createChild('span', 'color-swatch-inner');
    ...
    this._swatchInner.addEventListener(
        'click', this._handleClick.bind(this), true);
 ...
 */
                                    WebInspector.BezierSwatch.create =
            function() {
  if (!WebInspector.BezierSwatch._constructor)
    WebInspector.BezierSwatch._constructor = registerCustomElement(
        'span', 'bezier-swatch', WebInspector.BezierSwatch.prototype);

  return /** @type {!WebInspector.BezierSwatch} */ (
      new WebInspector.BezierSwatch._constructor());
}
  ...
  /**
     * @return {string}
     */
  bezierText: function() { return this._textElement.textContent; },

  /**
   * @param {string} text
   */
  setBezierText: function(text) { this._textElement.textContent = text; },

  /**
   * @param {boolean} hide
   */
  hideText: function(hide) { this._textElement.hidden = hide; },
  ...
  __proto__: HTMLSpanElement.prototype
                                                   ...
                                                   * @extends {HTMLSpanElement}
                                                   */
                                                  WebInspector.CSSShadowSwatch =
                function() {}

                /**
 * @return {!WebInspector.CSSShadowSwatch}
 */
                WebInspector.CSSShadowSwatch.create =
                    function() {
  if (!WebInspector.CSSShadowSwatch._constructor)
    WebInspector.CSSShadowSwatch._constructor = registerCustomElement(
        'span', 'css-shadow-swatch', WebInspector.CSSShadowSwatch.prototype);

  return /** @type {!WebInspector.CSSShadowSwatch} */ (
      new WebInspector.CSSShadowSwatch._constructor());
}
  ...
  setCSSShadow: function(model) {
    ...
    var results = WebInspector.TextUtils.splitStringByRegexes(
        model.asCSSText(), [/inset/g, WebInspector.Color.Regex]);
   ...
   * @param {boolean} hide
   */
  hideText: function(hide) { this._contentElement.hidden = hide; },
  ...
  /**
     * @return {?WebInspector.ColorSwatch}
     */
  colorSwatch: function() { return this._colorSwatch; },

  createdCallback: function() {
    var root = WebInspector.createShadowRootWithCoreStyles(
        this, 'ui/cssShadowSwatch.css');
