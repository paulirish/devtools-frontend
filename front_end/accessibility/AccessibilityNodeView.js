...
WebInspector.AXNodeSubPane = function() {
  WebInspector.AccessibilitySubPane.call(
      this, WebInspector.UIString('Computed Properties'));

  this._noNodeInfo =
      this.createInfo(WebInspector.UIString('No accessibility node'));
  this._ignoredInfo = this.createInfo(
      WebInspector.UIString('Accessibility node not exposed'),
  ...
  setAXNode: function(axNode) {
    ...
    if (!axNode) {
      ...
      function addIgnoredReason(property) {
        ignoredReasons.appendChild(
            new WebInspector.AXNodeIgnoredReasonTreeElement(
                property, axNode, target));
      }
      var ignoredReasonsArray =
          /** @type {!Array<!AccessibilityAgent.AXProperty>} */ (
    ...
    function addProperty(property) {
      treeOutline.appendChild(
          new WebInspector.AXNodePropertyTreePropertyElement(property, target));
    }

    for (var propertyName of ['name', 'description', 'help', 'value']) {
      if (propertyName in axNode) {
        var defaultProperty = /** @type {!AccessibilityAgent.AXProperty} */ (
            {name: propertyName, value: axNode[propertyName]});
        addProperty(defaultProperty);
      }
    }

    var roleProperty = /** @type {!AccessibilityAgent.AXProperty} */ (
        {name: 'role', value: axNode.role});
    ...
    for (var propertySet
             of [AccessibilityAgent.AXWidgetAttributes,
 ...
 * @return {!Element}
 */
WebInspector.AXNodePropertyTreeElement.createSimpleValueElement = function(
    type, value) {
  var valueElement;
  var AXValueType = AccessibilityAgent.AXValueType;
  if (!type || type === AXValueType.ValueUndefined ||
      type === AXValueType.ComputedString)
    valueElement = createElement('span');
  else
    valueElement = createElementWithClass('span', 'monospace');
  var valueText;
  var isStringProperty =
      type && WebInspector.AXNodePropertyTreeElement.StringProperties.has(type);
  if (isStringProperty) {
  ...
  if (type && type in WebInspector.AXNodePropertyTreeElement.TypeStyles)
    valueElement.classList.add(
        WebInspector.AXNodePropertyTreeElement.TypeStyles[type]);
 ...
 * @return {!Element}
 */
WebInspector.AXNodePropertyTreeElement.createExclamationMark = function(
    tooltip) {
...
WebInspector.AXNodePropertyTreeElement.prototype = {
  ...
  appendValueElement: function(value) {
    ...
    if (value.type === AXValueType.Idref || value.type === AXValueType.Node ||
        value.type === AXValueType.IdrefList ||
      ...
      for (var i = 0; i < sources.length; i++) {
        ...
        var child =
            new WebInspector.AXValueSourceTreeElement(source, this._target);
        this.appendChild(child);
      }
      this.expand();
    }
    var element =
        WebInspector.AXNodePropertyTreeElement.createSimpleValueElement(
  ...
  appendRelatedNode: function(relatedNode, index) {
    var deferredNode = new WebInspector.DeferredDOMNode(
        this._target, relatedNode.backendNodeId);
 ...
 * @param {!WebInspector.Target} target
 */
WebInspector.AXNodePropertyTreePropertyElement =
   ...
   * @override
   */
  onattach: function() { this._update(); },
 ...
 * @param {!WebInspector.Target} target
 */
WebInspector.AXValueSourceTreeElement =
  ...
  appendIDRefValueElement: function(value) {
    ...
    if (idrefs.length === 1) {
      ...
      if (matchingNode) {
        ...
        this.listItemElement.appendChild(
            new WebInspector.AXRelatedNodeElement({idref: idref}).render());
      }
    } else {
      ...
      for (var i = 0; i < idrefs.length; ++i) {
        ...
        if (matchingNode) {
          ...
          this.appendChild(
              new WebInspector.AXRelatedNodeSourceTreeElement({idref: idref}));
        }
      }
    }
  },
  ...
  appendRelatedNodeListValueElement: function(value) {
    ...
    if (value.type === AccessibilityAgent.AXValueType.IdrefList ||
      ...
      WebInspector.AXNodePropertyTreeElement.prototype
          .appendRelatedNodeListValueElement.call(this, value);
    }
    ...
    switch (type) {
      ...
      case AXValueSourceType.RelatedElement:
        if (source.nativeSource) {
          ...
          nameElement.textContent =
              WebInspector.UIString(AXNativeSourceTypes[nativeSource].name);
          nameElement.title = WebInspector.UIString(
              AXNativeSourceTypes[nativeSource].description);
          nameElement.classList.add('ax-readable-name');
          break;
        }
      ...
      default:
        ...
        if (type in AXSourceTypes) {
          nameElement.textContent =
              WebInspector.UIString(AXSourceTypes[type].name);
          nameElement.title =
              WebInspector.UIString(AXSourceTypes[type].description);
          nameElement.classList.add('ax-readable-name');
        } else {
  ...
  _update: function() {
    ...
    if (this._source.invalid) {
      var exclamationMark =
          WebInspector.AXNodePropertyTreeElement.createExclamationMark(
              WebInspector.UIString('Invalid source.'));
    ...
    if (this._source.attributeValue) {
      ...
      var valueElement =
          WebInspector.AXNodePropertyTreeElement.createSimpleValueElement(
              AccessibilityAgent.AXValueType.ValueUndefined,
              WebInspector.UIString('Not specified'));
  ...
  /**
     ...
     */
  appendValueElement: function(value) {
    var element = WebInspector.AXNodePropertyTreeElement.prototype
                      .appendValueElement.call(this, value);
    if (!element) {
      element = WebInspector.AXNodePropertyTreeElement.createSimpleValueElement(
          value.type, String(value.value));
      this.listItemElement.appendChild(element);
    }
    return element;
...
/**
 * @constructor
 * @param {{deferredNode: (!WebInspector.DeferredDOMNode|undefined), idref:
...
WebInspector.AXRelatedNodeSourceTreeElement = function(node, value) {
  this._value = value;
  this._axRelatedNodeElement =
      new WebInspector.AXRelatedNodeElement(node, value);
...
WebInspector.AXRelatedNodeElement.prototype = {
  ...
  render: function() {
    ...
    if (this._deferredNode) {
      ...
      valueElement =
          WebInspector.AXNodePropertyTreeElement.createExclamationMark(
              WebInspector.UIString('No node with this ID.'));
 ...
 * @param {!WebInspector.Target} target
 */
WebInspector.AXNodeIgnoredReasonTreeElement =
    function(property, axNode, target) {
  this._property = property;
  this._axNode = axNode;
  ...
  onattach: function() {
    ...
    this._reasonElement =
        WebInspector.AXNodeIgnoredReasonTreeElement.createReasonElement(
            this._property.name, this._axNode);
 ...
 * @return {?Element}
 */
WebInspector.AXNodeIgnoredReasonTreeElement.createReasonElement = function(
    reason, axNode) {
  ...
  switch (reason) {
    ...
    case 'ancestorDisallowsChild':
      reasonElement = WebInspector.formatLocalized(
          'Element is not permitted as child of ', []);
      break;
    ...
    case 'ancestorIsLeafNode':
      ...
      break;
    case 'ariaHidden':
      var ariaHiddenSpan = createElement('span', 'source-code').textContent =
          'aria-hidden';
      reasonElement =
          WebInspector.formatLocalized('Element is %s.', [ariaHiddenSpan]);
      break;
    case 'ariaHiddenRoot':
      var ariaHiddenSpan = createElement('span', 'source-code').textContent =
          'aria-hidden';
      ...
      reasonElement = WebInspector.formatLocalized(
          '%s is %s on ancestor:\u00a0', [ariaHiddenSpan, trueSpan]);
      break;
    case 'emptyAlt':
      reasonElement =
          WebInspector.formatLocalized('Element has empty alt text.', []);
      break;
    case 'emptyText':
    ...
    case 'labelContainer':
      reasonElement =
          WebInspector.formatLocalized('Part of label element:\u00a0', []);
      break;
    case 'labelFor':
    ...
    case 'notRendered':
      reasonElement =
          WebInspector.formatLocalized('Element is not rendered.', []);
      break;
    case 'notVisible':
      reasonElement =
          WebInspector.formatLocalized('Element is not visible.', []);
      break;
    case 'presentationalRole':
      var rolePresentationSpan =
          createElement('span', 'source-code').textContent =
              'role=' + axNode.role.value;
      reasonElement = WebInspector.formatLocalized(
          'Element has %s.', [rolePresentationSpan]);
      break;
    case 'probablyPresentational':
      reasonElement =
          WebInspector.formatLocalized('Element is presentational.', []);
      break;
