...
WebInspector.AXTreePane = function() {
  WebInspector.AccessibilitySubPane.call(
      this, WebInspector.UIString('Accessibility Tree'));
...
WebInspector.AXTreePane.prototype = {
  ...
  setAXNodeAndAncestors: function(nodes) {
    ...
    while (nodes.length) {
      ...
      var ancestorTreeElement =
          new WebInspector.AXNodeTreeElement(ancestor, target);
...
WebInspector.AXNodeTreeElement.prototype = {
    ...
    if (this._axNode.ignored) {
      ...
      if ('name' in this._axNode && this._axNode.name.value) {
        this.listItemElement.createChild('span', 'separator').textContent =
            '\u00A0';
        this._appendNameElement(
            /** @type {string} */ (this._axNode.name.value));
      }
    }
  },
  ...
  _appendRoleElement: function(role) {
    ...
    roleElement.classList.add(
        WebInspector.AXNodeTreeElement.RoleStyles[role.type]);
