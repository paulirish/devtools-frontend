...
WebInspector.ARIAAttributesPane = function() {
  WebInspector.AccessibilitySubPane.call(
      this, WebInspector.UIString('ARIA Attributes'));

  this._noPropertiesInfo =
      this.createInfo(WebInspector.UIString('No ARIA attributes'));
...
WebInspector.ARIAAttributesPane.prototype = {
  ...
  setNode: function(node) {
    ...
    for (var i = 0; i < attributes.length; ++i) {
      ...
      if (WebInspector.ARIAAttributesPane._attributes.indexOf(attribute.name) <
          0)
        continue;
 ...
 */
WebInspector.ARIAAttributesTreeElement = function(
    parentPane, attribute, target) {
...
WebInspector.ARIAAttributesTreeElement.prototype = {
  ...
  appendAttributeValueElement: function(value) {
    this._valueElement =
        WebInspector.ARIAAttributesTreeElement.createARIAValueElement(value);
  ...
  _startEditing: function() {
    ...
    this._prompt = new WebInspector.ARIAAttributesPane.ARIAAttributePrompt(
        WebInspector.ariaMetadata().valuesForProperty(
            this._nameElement.textContent),
        this);
    this._prompt.setAutocompletionTimeout(0);
    var proxyElement = this._prompt.attachAndStartEditing(
        valueElement, blurListener.bind(this, previousContent));

    proxyElement.addEventListener(
        'keydown', this._editingValueKeyDown.bind(this, previousContent),
        false);

    valueElement.getComponentSelection().setBaseAndExtent(
        valueElement, 0, valueElement, 1);
  },
  ...
  _editingCommitted: function(userInput, previousContent) {
    ...
    if (userInput !== previousContent)
      this._parentPane.node().setAttributeValue(
          this._attribute.name, userInput);
  },
 ...
 * @return {!Element}
 */
WebInspector.ARIAAttributesTreeElement.createARIAValueElement = function(
    value) {
 ...
 */
WebInspector.ARIAAttributesPane.ARIAAttributePrompt = function(
    ariaCompletions, treeElement) {
...
WebInspector.ARIAAttributesPane.ARIAAttributePrompt.prototype =
    {
      ...
      _buildPropertyCompletions: function(
        ...
        var results =
            this._ariaCompletions.filter((value) => value.startsWith(prefix));
