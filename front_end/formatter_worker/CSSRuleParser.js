...
WebInspector.parseCSS =
  ...
  function processToken(tokenValue, tokenTypes, column, newColumn) {
    var tokenType =
        tokenTypes ? new Set(tokenTypes.split(' ')) : UndefTokenType;
    switch (state) {
      case WebInspector.CSSParserStates.Initial:
        if (tokenType.has('qualifier') || tokenType.has('builtin') ||
            tokenType.has('tag')) {
          rule = {
      ...
      case WebInspector.CSSParserStates.Style:
        if (tokenType.has('meta') || tokenType.has('property')) {
          ...
          // The |processToken| is called per-line, so no token spans more than
          // one line.
          // Support only a one-line comments.
          if (tokenValue.substring(0, 2) !== '/*' ||
          ...
          if (disabledRules.length === 1 &&
              disabledRules[0].properties.length === 1) {
