 ...
 */
WebInspector.FormattedContentBuilder =
  ...
  addToken: function(token, offset) {
    ...
    if (this._enforceSpaceBetweenWords && last &&
        /\w/.test(last[last.length - 1]) && /\w/.test(token))
      this.addSoftSpace();
  ...
  /**
     ...
     */
  _indent: function() {
    var cachedValue = this._cachedIndents.get(
        this._nestingLevel) if (cachedValue) return cachedValue;
