...
WebInspector.javaScriptOutline = function(content) {
  ...
  while (tokenizer.peekToken()) {
    ...
    if (AT.identifier(token) && previousToken &&
        (AT.identifier(previousToken, 'get') ||
         AT.identifier(previousToken, 'set'))) {
      currentFunction = {
      ...
      if (tokenValue && previousToken &&
          AT.keyword(previousToken, 'function')) {
      ...
      previousIdentifier = null;
        ...
        AT.punctuator(token, '.') && previousToken &&
        AT.identifier(previousToken))
      previousIdentifier += '.';
