 ...
 */
WebInspector.AcornTokenizer =
    function(content) {
  this._content = content;
  this._comments = [];
  this._tokenizer = acorn.tokenizer(
      this._content, {ecmaVersion: 6, onComment: this._comments});
 ...
 * @return {boolean}
 */
    WebInspector.AcornTokenizer.punctuator =
  ...
  return token.type !== acorn.tokTypes.num &&
      ...
      (!values || (token.type.label.length === 1 &&
                   values.indexOf(token.type.label) !== -1));
}
 ...
 * @return {boolean}
 */
        WebInspector.AcornTokenizer.keyword =
 ...
 * @return {boolean}
 */
            WebInspector.AcornTokenizer.identifier =
 ...
 * @return {boolean}
 */
                WebInspector.AcornTokenizer.lineComment =
                    function(token) {
 ...
 * @return {boolean}
 */
                    WebInspector.AcornTokenizer.blockComment =
                        function(token) {
  ...
  peekToken: function() {
    ...
    return this._bufferedToken.type !== acorn.tokTypes.eof ?
        this._bufferedToken :
        null;
  },
