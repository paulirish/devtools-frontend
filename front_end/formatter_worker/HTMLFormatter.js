...
WebInspector.HTMLFormatter.prototype = {
  ...
  _formatTokensTill: function(element, offset) {
    while (this._model.peekToken() &&
           this._model.peekToken().startOffset < offset) {
  ...
  _formatToken: function(element, token) {
    ...
    if (isBodyToken && element.name === 'style') {
      this._builder.addNewLine();
      this._builder.increaseNestingLevel();
      this._cssFormatter.format(
          this._text, this._lineEndings, token.startOffset, token.endOffset);
      this._builder.decreaseNestingLevel();
      return;
    }
    if (isBodyToken && element.name === 'script') {
      ...
      var mimeType = element.openTag.attributes.has('type') ?
          ...
          null;
      if (!mimeType ||
          WebInspector.HTMLFormatter.SupportedJavaScriptMimeTypes.has(
              mimeType)) {
        this._jsFormatter.format(
            this._text, this._lineEndings, token.startOffset, token.endOffset);
      } else {
    ...
    this._builder.addToken(token.value, token.startOffset);
                                        ...
                                        * @param {string} text
                                        */
                                       WebInspector.HTMLModel =
    function(text) {
  this._state = WebInspector.HTMLModel.ParseState.Initial;
  this._document = new WebInspector.HTMLModel.Element('document');
  this._document.openTag =
      new WebInspector.HTMLModel.Tag('document', 0, 0, new Map(), true, false);
  this._document.closeTag = new WebInspector.HTMLModel.Tag(
      'document', text.length, text.length, new Map(), false, false);
...
WebInspector.HTMLModel
  ...
  _build: function(text) {
    ...
    while (true) {
      tokenizer(
          text.substring(lastOffset), processToken.bind(this, lastOffset));
      ...
      this._tokens.push(new WebInspector.HTMLModel.Token(
          tokenValue, new Set(), tokenStart, tokenEnd));
    }
    ...
    function processToken(baseOffset, tokenValue, type, tokenStart, tokenEnd) {
      ...
      var token = new WebInspector.HTMLModel.Token(
          tokenValue, tokenType, tokenStart, tokenEnd);
  ...
  _onEndTag: function(token) {
    ...
    var selfClosingTag = token.value === '/>' ||
        WebInspector.HTMLModel.SelfClosingTags.has(this._tagName);
    var tag = new WebInspector.HTMLModel.Tag(
        this._tagName, this._tagStartOffset, this._tagEndOffset,
        this._attributes, this._isOpenTag, selfClosingTag);
  ...
  _onTagComplete: function(tag) {
    if (tag.isOpenTag) {
      ...
      if (topElement !== this._document && topElement.openTag.selfClosingTag)
        this._popElement(
            autocloseTag(topElement, topElement.openTag.endOffset));
      else if (
    ...
    function autocloseTag(element, offset) {
      return new WebInspector.HTMLModel.Tag(
          element.name, offset, offset, new Map(), false, false);
    }
  },
  ...
  document: function() { return this._document; }
                                    ...
                                    */
                                   WebInspector.HTMLModel.Token =
    function(value, type, startOffset, endOffset) {
  this.value = value;
  this.type = type;
  ...
  this.endOffset = endOffset;
     ...
     */
    WebInspector.HTMLModel.Tag =
        function(
            name, startOffset, endOffset, attributes, isOpenTag,
            selfClosingTag) {
  this.name = name;
  this.startOffset = startOffset;
  this.endOffset = endOffset;
