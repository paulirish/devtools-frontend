 ...
 */
WebInspector.SourcesTextEditor =
    function(delegate) {
  WebInspector.CodeMirrorTextEditor.call(this, {
    lineNumbers: true,
    lineWrapping: false,
    bracketMatchingSetting:
        WebInspector.moduleSetting('textEditorBracketMatching'),
  });

  this.codeMirror().addKeyMap(
      {'Enter': 'smartNewlineAndIndent', 'Esc': 'sourcesDismiss'});
  ...
  this.codeMirror().on(
      ...
      this._beforeSelectionChangeForDelegate.bind(this));
  this.element.addEventListener(
      'contextmenu', this._contextMenu.bind(this), false);

  this._blockIndentController =
      new WebInspector.SourcesTextEditor.BlockIndentController(
          this.codeMirror());
  this._tokenHighlighter = new WebInspector.SourcesTextEditor.TokenHighlighter(
  ...
  this.element.addEventListener(
      'mousedown', updateAnticipateJumpFlag.bind(this, true), true);
  this.element.addEventListener(
      'mousedown', updateAnticipateJumpFlag.bind(this, false), false);
    ...
    function innerHighlightRegex() {
      if (range) {
        ...
        if (range.endColumn >
          ...
          this.setSelection(WebInspector.TextRange.createFromLocation(
  ...
  setGutterDecoration: function(lineNumber, type, element) {
    console
        .assert(
            this._gutters.indexOf(type) !== -1,
            'Cannot decorate unexisting gutter.') this.codeMirror()
        .setGutterMarker(lineNumber, type, element);
  },
  ...
  setExecutionLocation: function(lineNumber, columnNumber) {
    ...
    this.codeMirror().addLineClass(
        this._executionLine, 'wrap', 'cm-execution-line');
  ...
  _contextMenu: function(event) {
    ...
    event.consume(true);  // Consume event now to prevent document from handling
                          // the async menu
    var target =
        event.target.enclosingNodeOrSelfWithClass('CodeMirror-gutter-elt');
    var promise;
    if (target) {
  ...
  editRange: function(range, text, origin) {
    var newRange = WebInspector.CodeMirrorTextEditor.prototype.editRange.call(
        this, range, text, origin);
  ...
  _onUpdateEditorIndentation: function() {
    this._setEditorIndentation(WebInspector.CodeMirrorUtils.pullLines(
        this.codeMirror(),
        WebInspector.SourcesTextEditor.LinesToScanForIndentationGuessing));
  },
    ...
    if (indent === WebInspector.TextUtils.Indent.TabCharacter) {
      ...
      extraKeys.Tab = function(codeMirror) {
        ...
        codeMirror.replaceRange(
            indent.substring(pos.ch % indent.length), codeMirror.getCursor());
      }
    }
  ...
  _onAutoAppendedSpaces: function() {
    ...
    for (var i = 0; i < selections.length; ++i) {
      ...
      this._autoAppendedSpaces.push(this.textEditorPositionHandle(
  ...
  _changesForDelegate: function(codeMirror, changes) {
    ...
    for (var changeIndex = 0; changeIndex < changes.length; ++changeIndex) {
      ...
      var edit = WebInspector.CodeMirrorUtils.changeObjectToEditOperation(
          changeObject);
  ...
  _scroll: function() {
    var topmostLineNumber = this.codeMirror().lineAtHeight(
        this.codeMirror().getScrollInfo().top, 'local');
  ...
  _beforeSelectionChangeForDelegate: function(codeMirror, selection) {
    ...
    this._reportJump(
        this.selection(), WebInspector.CodeMirrorUtils.toRange(
  ...
  _allWhitespaceOverlayMode: function(mimeType) {
    var modeName = CodeMirror.mimeModes[mimeType] ?
        (CodeMirror.mimeModes[mimeType].name ||
         CodeMirror.mimeModes[mimeType]) :
    ...
    function modeConstructor(config, parserConfig) {
      function nextToken(stream) {
        if (stream.peek() === ' ') {
          var spaces = 0;
          while (spaces < WebInspector.SourcesTextEditor
      ...
      return CodeMirror.overlayMode(
          CodeMirror.getMode(config, mimeType), whitespaceMode, false);
    }
  ...
  _setupWhitespaceHighlight: function() {
    ...
    var rules = '';
    for (var i = 1; i <=
         WebInspector.SourcesTextEditor.MaximumNumberOfWhitespacesPerSingleSpan;
         ++i) {
      spaceChars += spaceChar;
      var rule =
          classBase + i + '::before { content: \'' + spaceChars + '\';}\n';
      rules += rule;
    }
...
WebInspector.SourcesTextEditor.Events = {
  GutterClick: Symbol('GutterClick')
                                         ...
                                         * @interface
                                         */
                                        WebInspector.SourcesTextEditorDelegate =
    function() {} WebInspector.SourcesTextEditorDelegate
        .prototype = {
  ...
  function innerSmartNewlineAndIndent(codeMirror) {
    ...
    for (var i = 0; i < selections.length; ++i) {
      var selection = selections[i];
      var cur = CodeMirror.cmpPos(selection.head, selection.anchor) < 0 ?
          selection.head :
          selection.anchor;
 ...
 */
        CodeMirror.commands.sourcesDismiss =
            function(codemirror) {
  ...
  return CodeMirror.commands.dismiss(codemirror);
             ...
             * @param {!CodeMirror} codeMirror
             */
            WebInspector.SourcesTextEditor.BlockIndentController =
                function(codeMirror) {
  ...
  Enter: function(codeMirror) {
    ...
    for (var i = 0; i < selections.length; ++i) {
      var selection = selections[i];
      var start = CodeMirror.cmpPos(selection.head, selection.anchor) < 0 ?
          selection.head :
          selection.anchor;
 ...
 */
  ...
  return ' '.repeat(minimumIndent);
                     ...
                     * @param {!CodeMirror} codeMirror
                     */
                    WebInspector.SourcesTextEditor.TokenHighlighter =
  ...
  highlightSearchResults: function(regex, range) {
    ...
    if (this._highlightDescriptor && this._highlightDescriptor.selectionStart)
      this._codeMirror.removeLineClass(
          this._highlightDescriptor.selectionStart.line, 'wrap',
          'cm-line-with-selection');
    var selectionStart = this._highlightRange ?
        new CodeMirror.Pos(
            this._highlightRange.startLine, this._highlightRange.startColumn) :
        null;
    if (selectionStart)
      this._codeMirror.addLineClass(
          selectionStart.line, 'wrap', 'cm-line-with-selection');
    if (this._highlightRegex === oldRegex) {
      // Do not re-add overlay mode if regex did not change for better
      // performance.
    ...
    if (this._highlightRange) {
      ...
      this._searchResultMarker = this._codeMirror.markText(
          pos.start, pos.end, {className: 'cm-column-with-selection'});
    }
  },
  ...
  highlightSelectedTokens: function() {
    ...
    if (this._isWord(
            selectedText, selectionStart.line, selectionStart.ch,
            selectionEnd.ch)) {
      if (selectionStart)
        this._codeMirror.addLineClass(
            selectionStart.line, 'wrap', 'cm-line-with-selection');
  ...
  _tokenHighlighter: function(token, selectionStart, stream) {
    ...
    } while (eatenChar && (WebInspector.TextUtils.isWordChar(eatenChar) ||
