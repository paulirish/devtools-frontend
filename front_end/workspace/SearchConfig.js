 ...
 */
WebInspector.SearchConfig =
    function(query, ignoreCase, isRegex) {
  this._query = query;
 ...
 */
WebInspector.SearchConfig.fromPlainObject =
    function(object) {
  return new WebInspector.SearchConfig(
      object.query, object.ignoreCase, object.isRegex);
}
  ...
  /**
     ...
     * @return {boolean}
     */
  ignoreCase: function() { return this._ignoreCase; },

  /**
     * @override
     * @return {boolean}
     */
  isRegex: function() { return this._isRegex; },
  ...
  _parse: function() {
    var filePattern =
        '-?f(ile)?:(([^\\\\ ]|\\\\.)+)';  // After file: prefix: any symbol
                                          // except space and backslash or any
                                          // symbol escaped with a backslash.
    var quotedPattern = '"(([^\\\\"]|\\\\.)+)"';  // Inside double quotes: any
                                                  // symbol except double quote
                                                  // and backslash or any symbol
                                                  // escaped with a backslash.

    // A word is a sequence of any symbols except space and backslash or any
  ...
  filePathMatchesFileQuery: function(filePath) {
    ...
    for (var i = 0; i < this._fileRegexQueries.length; ++i) {
      if (!!filePath.match(this._fileRegexQueries[i].regex) ===
  ...
  /**
     ...
     * @return {!Array.<string>}
     */
  queries: function() { return this._queries; },
  ...
  _parseFileQuery: function(query) {
    ...
    return new WebInspector.SearchConfig.QueryTerm(result, isNegative);
                                           ...
                                           */
                                          WebInspector.SearchConfig.QueryTerm =
        function(text, isNegative) {
  this.text = text;
