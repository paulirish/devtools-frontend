...
WebInspector.ServiceManager =
  ...
  createService: function(serviceName) {
    return this._sendCommand(serviceName + '.create').then(result => {
      if (!result)
        return null;
      var service =
          new WebInspector.ServiceManager.Service(this, serviceName, result.id);
  ...
  _onMessage: function(message) {
    ...
    if (!service) {
      console.error(
          'Unable to lookup stub for ' + serviceName + ':' + object.params.id);
      return;
    }
  ...
  _connectionClosed: function() {
    ...
    delete this._connectionPromise;
                      ...
                      */
                     WebInspector.ServiceManager.Service =
        function(manager, serviceName, objectId) {
  ...
  send: function(methodName, params) {
    ...
    return this._manager._sendCommand(
        this._serviceName + '.' + methodName, params);
  },
  ...
  _dispatchNotification: function(methodName, params) {
    ...
    if (!handler) {
      console.error(
          'Could not report notification \'' + methodName + '\' on \'' +
          this._objectId + '\'');
      return;
    }
    handler(params);
                                                        ...
                                                        WebInspector
                                                            .serviceManager =
            new WebInspector.ServiceManager();
