 ...
 */
WebInspector.RemoteLocationManager =
    function(target) {
  ...
  _requestTargets: function(error) {
    ...
    return this.target().browserAgent().getTargets(
        this._processTargets.bind(this));
  },
  ...
  _processTargets: function(error, targetInfos) {
    ...
    for (var targetId of newTargetInfos.keys()) {
      ...
      promises.push(this.target().browserAgent().attach(
          targetId, this._createConnection.bind(
                        this, targetId, newTargetInfos.get(targetId).title)));
    }
  ...
  _createConnection: function(targetId, title, error, success) {
    if (!success || !this._connectedTargets.has(targetId)) {
      // Could not attach or target was deleted while we were connecting, do not
      // proceed.
      return;
    }

    var nodeConnection = new WebInspector.RemoteLocationConnection(
        this.target().browserAgent(), targetId);
  ...
  __proto__: WebInspector.SDKObject.prototype
                      ...
                      * @param {string} targetId
                      */
                     WebInspector.RemoteLocationConnection =
