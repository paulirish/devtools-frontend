...
/*
bunch of stuff from old TimelineAsyncEventTracker that i probably dont need anymore...
 ...
 const frameSequenceJoiner = e => {
   ...
   // weirdly this kinda works cuz a lot of these are parents of eachother. if that didnt conveniently happen.. we'd need to refactor the root/subsequent thing over
here
  // we can't match from main to compositor cuz we process Main thread first. ideally wed like.. run compositor stuff before mainthread or something.
   ...
   events.set('ProxyImpl::ScheduledActionSendBeginMainFrame', {subsequents: ['ProxyMain::BeginMainFrame'], joinBy: frameSequenceJoiner}); // hack to see if crossthre
ad works
