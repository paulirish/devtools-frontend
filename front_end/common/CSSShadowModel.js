...
WebInspector.CSSShadowModel =
  ...
  this._color =
      /** @type {!WebInspector.Color} */ (WebInspector.Color.parse('black'));
  this._format = [
    WebInspector.CSSShadowModel._Part.OffsetX,
    WebInspector.CSSShadowModel._Part.OffsetY
  ];
}
 ...
 * @return {!Array<!WebInspector.CSSShadowModel>}
 */
        WebInspector.CSSShadowModel.parseBoxShadow =
            function(text) {
   ...
   */
  setOffsetX: function(offsetX) { this._offsetX = offsetX; },

  /**
   * @param {!WebInspector.CSSLength} offsetY
   */
  setOffsetY: function(offsetY) { this._offsetY = offsetY; },
  ...
  setBlurRadius: function(blurRadius) {
    ...
    if (this._format.indexOf(WebInspector.CSSShadowModel._Part.BlurRadius) ===
        -1) {
      var yIndex =
          this._format.indexOf(WebInspector.CSSShadowModel._Part.OffsetY);
      this._format.splice(
          yIndex + 1, 0, WebInspector.CSSShadowModel._Part.BlurRadius);
    }
  },
  ...
  setSpreadRadius: function(spreadRadius) {
    ...
    if (this._format.indexOf(WebInspector.CSSShadowModel._Part.SpreadRadius) ===
        -1) {
      this.setBlurRadius(this._blurRadius);
      var blurIndex =
          this._format.indexOf(WebInspector.CSSShadowModel._Part.BlurRadius);
      this._format.splice(
          blurIndex + 1, 0, WebInspector.CSSShadowModel._Part.SpreadRadius);
    }
  },
  ...
  /**
     * @return {boolean}
     */
  isBoxShadow: function() { return this._isBoxShadow; },

  /**
     * @return {boolean}
     */
  inset: function() { return this._inset; },
  ...
  /**
     * @return {!WebInspector.CSSLength}
     */
  blurRadius: function() { return this._blurRadius; },

  /**
     * @return {!WebInspector.CSSLength}
     */
  spreadRadius: function() { return this._spreadRadius; },

  /**
     * @return {!WebInspector.Color}
     */
  color: function() { return this._color; },
 ...
 */
                                                    WebInspector.CSSShadowModel
                                                        ._parseShadow =
                function(text, isBoxShadow) {
  var shadowTexts = [];
  // Split by commas that aren't inside of color values to get the individual
  // shadow values.
  var splits = WebInspector.TextUtils.splitStringByRegexes(
      text, [WebInspector.Color.Regex, /,/g]);
  var currentIndex = 0;
  ...
  for (var i = 0; i < shadowTexts.length; i++) {
    ...
    var regexes =
        [/inset/gi, WebInspector.Color.Regex, WebInspector.CSSLength.Regex];
    var results =
        WebInspector.TextUtils.splitStringByRegexes(shadowTexts[i], regexes);
    for (var j = 0; j < results.length; j++) {
      var result = results[j];
      if (result.regexIndex === -1) {
        // Don't allow anything other than inset, color, length values, and
        // whitespace.
        ...
        if (result.regexIndex === 0) {
          ...
          if (!length)
            return [];
          var previousPart = shadow._format.length > 0 ?
              shadow._format[shadow._format.length - 1] :
              '';
          if (previousPart === WebInspector.CSSShadowModel._Part.OffsetX) {
            ...
            shadow._format.push(WebInspector.CSSShadowModel._Part.OffsetY);
              ...
              previousPart === WebInspector.CSSShadowModel._Part.OffsetY) {
            shadow._blurRadius = length;
            shadow._format.push(WebInspector.CSSShadowModel._Part.BlurRadius);
          } else if (
              previousPart === WebInspector.CSSShadowModel._Part.BlurRadius) {
    ...
    if (invalidCount(WebInspector.CSSShadowModel._Part.OffsetX, 1, 1) ||
        ...
        invalidCount(
            WebInspector.CSSShadowModel._Part.Inset, 0, isBoxShadow ? 1 : 0) ||
        invalidCount(
            WebInspector.CSSShadowModel._Part.SpreadRadius, 0,
            isBoxShadow ? 1 : 0))
      return [];
    shadows.push(shadow);
  }
 ...
 * @return {?WebInspector.CSSLength}
 */
WebInspector.CSSLength.parse =
    function(text) {
  var lengthRegex =
      new RegExp('^(?:' + WebInspector.CSSLength.Regex.source + ')$', 'i');
 ...
 */
  ...
  /**
     * @return {string}
     */
  asCSSText: function() { return this.amount + this.unit; }
}
