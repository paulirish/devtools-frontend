...
WebInspector.FormatterWorkerPool
  ...
  _processNextTask: function() {
    ...
    var freeWorker = this._workerTasks.keysArray().find(
        worker => !this._workerTasks.get(worker));
    if (!freeWorker &&
        this._workerTasks.size < WebInspector.FormatterWorkerPool.MaxWorkers)
  ...
  runChunkedTask: function(methodName, params, callback) {
    var task = new WebInspector.FormatterWorkerPool.Task(
        methodName, params, callback, true);
  ...
  runTask: function(methodName, params) {
    ...
    var task = new WebInspector.FormatterWorkerPool.Task(
        methodName, params, callback, false);
    this._taskQueue.push(task);
    this._processNextTask();
    return promise;
  },
                  ...
                  */
                 WebInspector.FormatterWorkerPool.Task =
    function(method, params, callback, isChunked) {
  this.method = method;
  this.params = params;
