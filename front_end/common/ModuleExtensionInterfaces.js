 ...
 * @return {!Promise.<!Element>}
 */
                                      WebInspector.Renderer.renderPromise =
        function(object) {
  if (!object)
    return Promise.reject(new Error('Can\'t render ' + object));

  return self.runtime.extension(WebInspector.Renderer, object)
      .instance()
      .then(render);
   ...
   */
  function render(renderer) { return renderer.render(object); }
}
 ...
 */
            WebInspector.Revealer.revealPromise =
                function(revealable, omitFocus) {
  if (!revealable)
    return Promise.reject(new Error('Can\'t reveal ' + revealable));
  return self.runtime.allInstances(WebInspector.Revealer, revealable)
      .then(reveal);
  ...
  function reveal(revealers) {
    ...
    for (var i = 0; i < revealers.length; ++i)
      promises.push(
          revealers[i].reveal(/** @type {!Object} */ (revealable), omitFocus));
  ...
  presentUI: function(document) {}
     ...
     */
                             ...
                             * @interface
                             */
                            WebInspector.QueryParamHandler =
                                function() {}
