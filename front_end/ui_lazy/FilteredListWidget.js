...
WebInspector.FilteredListWidget =
  ...
  this._promptElement =
      this.contentElement.createChild('div', 'filtered-list-widget-input');
 ...
 * @return {!RegExp}
 */
    WebInspector.FilteredListWidget.filterRegex =
        function(query) {
  ...
  /**
     * @return {string}
     */
  _value: function() { return this._prompt.userEnteredText().trim(); },
  ...
  _createItemElement: function(index) {
    ...
    itemElement.className = 'filtered-list-widget-item ' +
        (this._renderAsTwoRows ? 'two-rows' : 'one-row');
    itemElement._titleElement =
        itemElement.createChild('div', 'filtered-list-widget-title');
    itemElement._subtitleElement =
        itemElement.createChild('div', 'filtered-list-widget-subtitle');
    itemElement._subtitleElement.textContent = '\u200B';
    itemElement._index = index;
    this._delegate.renderItem(
        index, this._value(), itemElement._titleElement,
        itemElement._subtitleElement);
    return itemElement;
  },
  ...
  _autocomplete: function(
    ...
    var completions = wordRange.startOffset === 0 ?
        [this._delegate.autocomplete(wordRange.toString())] :
        [];
  ...
  _filterItems: function() {
    ...
    this._query = query;
    var filterRegex =
        query ? WebInspector.FilteredListWidget.filterRegex(query) : null;
    ...
    /**
         ...
         * @return {number}
         */
    function compareIntegers(a, b) { return b - a; }
    ...
    function scoreItems(fromIndex) {
      ...
      var workDone = 0;
      for (var i = fromIndex;
           i < this._delegate.itemCount() && workDone < maxWorkItems; ++i) {
        ...
        // Find its index in the scores array (earlier elements have bigger
        // scores).
      ...
      this._filteredItems =
          bestItems.concat(overflowItems).concat(filteredItems);
  ...
  _updateShowMatchingItems: function() {
    ...
    this._itemElementsContainer.classList.toggle(
        'hidden', !shouldShowMatchingItems);
  },
    ...
    switch (event.keyCode) {
      ...
      case WebInspector.KeyboardShortcut.Keys.PageDown.code:
        newSelectedIndex = Math.min(
            newSelectedIndex + this._rowsPerViewport(),
      ...
      case WebInspector.KeyboardShortcut.Keys.PageUp.code:
        newSelectedIndex =
            Math.max(newSelectedIndex - this._rowsPerViewport(), 0);
        this._updateSelection(newSelectedIndex, false);
        event.consume(true);
        break;
  ...
  _onClick: function(event) {
    var itemElement =
        event.target.enclosingNodeOrSelfWithClass('filtered-list-widget-item');
    if (!itemElement)
      return;
  ...
  /**
     ...
     * @return {number}
     */
  itemCount: function() { return this._filteredItems.length; },
  ...
  fastHeight: function(index) {
    if (!this._rowHeight) {
      ...
      this._rowHeight = WebInspector
                            .measurePreferredSize(
                                element, this._viewportControl.contentElement())
                            .height;
    }
  ...
  /**
     ...
     * @return {number}
     */
  minimumRowHeight: function() { return this.fastHeight(0); },
  ...
  __proto__: WebInspector.VBox.prototype
                          ...
                          */
                         WebInspector.FilteredListWidget.Delegate =
  ...
  /**
     ...
     * @return {boolean}
     */
  shouldShowMatchingItems: function(query) { return true; },
  ...
  /**
     ...
     * @return {string}
     */
  itemKeyAt: function(itemIndex) { return ''; },
  ...
  /**
     ...
     * @return {number}
     */
  itemScoreAt: function(itemIndex, query) { return 1; },
  ...
  /**
     * @return {boolean}
     */
  caseSensitive: function() { return true; },

  /**
     * @return {boolean}
     */
  renderMonospace: function() { return true; },

  /**
     * @return {boolean}
     */
  renderAsTwoRows: function() { return false; },
  ...
  refresh: function() { this._refreshCallback(); },

  /**
     * @param {string} query
     * @return {string}
     */
  rewriteQuery: function(query) { return query; },
  ...
  autocomplete: function(query) {
    for (var i = this._promptHistory.length - 1; i >= 0; i--) {
      if (this._promptHistory[i] !== query &&
          this._promptHistory[i].startsWith(query))
