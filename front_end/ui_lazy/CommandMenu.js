...
WebInspector.CommandMenu =
  ...
  _loadCommands: function() {
    ...
    for (var extension of panelExtensions)
      this._commands.push(
          WebInspector.CommandMenu.createRevealPanelCommand(extension));
    ...
    for (var extension of drawerExtensions) {
      if (extension.descriptor()['location'] !== 'drawer-view')
        continue;
      this._commands.push(
          WebInspector.CommandMenu.createRevealDrawerCommand(extension));
    }
    ...
    for (var extension of settingExtensions) {
      ...
      for (var pair of options)
        this._commands.push(WebInspector.CommandMenu.createSettingCommand(
            extension, pair['title'], pair['value']));
    }
  },
  ...
  commands: function() { return this._commands; }
                      ...
                      */
                     WebInspector.CommandMenuDelegate =
        function() {
...
WebInspector.CommandMenuDelegate
  ...
  _appendAvailableCommands: function() {
    ...
    for (var action of actions) {
      if (action.category())
        this._commands.push(
            WebInspector.CommandMenu.createActionCommand(action));
    }
  ...
  /**
     ...
     * @return {number}
     */
  itemCount: function() { return this._commands.length; },
  ...
  /**
     ...
     * @return {string}
     */
  itemKeyAt: function(itemIndex) { return this._commands[itemIndex].key(); },
  ...
  itemScoreAt: function(itemIndex, query) {
    ...
    var opcodes = WebInspector.Diff.charDiff(
        query.toLowerCase(), command.title().toLowerCase());
    var score = 0;
  ...
  renderItem: function(itemIndex, query, titleElement, subtitleElement) {
    ...
    var index = String.hashCode(command.category()) %
        WebInspector.CommandMenuDelegate.MaterialPaletteColors.length;
    tagElement.style.backgroundColor =
        WebInspector.CommandMenuDelegate.MaterialPaletteColors[index];
  ...
  /**
     ...
     * @return {boolean}
     */
  caseSensitive: function() { return false; },

  /**
     * @override
     * @return {boolean}
     */
  renderMonospace: function() { return false; },
  ...
  __proto__: WebInspector.FilteredListWidget.Delegate.prototype
                  ...
                  */
                 WebInspector.CommandMenu.Command =
    function(category, title, key, shortcut, executeHandler, availableHandler) {
  ...
  /**
     * @return {string}
     */
  shortcut: function() { return this._shortcut; },
  ...
  execute: function() { this._executeHandler(); }
}
 ...
 */
                                                 WebInspector.CommandMenu
                                                     .createCommand =
        function(
            category, keys, title, shortcut, executeHandler, availableHandler) {
  // Separate keys by null character, to prevent fuzzy matching from matching
  // across them.
  var key = keys.replace(/,/g, '\0');
  return new WebInspector.CommandMenu.Command(
      category, title, key, shortcut, executeHandler, availableHandler);
}
 ...
 */
        WebInspector.CommandMenu.createSettingCommand =
            function(extension, title, value) {
  var category = extension.descriptor()['category'] || '';
  var tags = extension.descriptor()['tags'] || '';
  var setting = WebInspector.settings.moduleSetting(
      extension.descriptor()['settingName']);
  return WebInspector.CommandMenu.createCommand(
      category, tags, title, '', setting.set.bind(setting, value),
      availableHandler);

  /**
     * @return {boolean}
     */
  function availableHandler() { return setting.get() !== value; }
}
 ...
 * @return {!WebInspector.CommandMenu.Command}
 */
            WebInspector.CommandMenu.createActionCommand =
                function(action) {
  var shortcut =
      WebInspector.shortcutRegistry.shortcutTitleForAction(action.id()) || '';
  return WebInspector.CommandMenu.createCommand(
      action.category(), action.tags(), action.title(), shortcut,
 ...
 * @return {!WebInspector.CommandMenu.Command}
 */
                WebInspector.CommandMenu.createRevealPanelCommand =
                    function(extension) {
  var panelName = extension.descriptor()['name'];
  var tags = extension.descriptor()['tags'] || '';
  return WebInspector.CommandMenu.createCommand(
      WebInspector.UIString('Panel'), tags,
      WebInspector.UIString('Show %s', extension.title()), '', executeHandler,
      availableHandler);
  ...
  function executeHandler() {
    WebInspector.inspectorView.panel(panelName).then(
        WebInspector.inspectorView.setCurrentPanel.bind(
 ...
 * @return {!WebInspector.CommandMenu.Command}
 */
                    WebInspector.CommandMenu.createRevealDrawerCommand =
                        function(extension) {
  var drawerId = extension.descriptor()['id'];
  var executeHandler = WebInspector.viewManager.showView.bind(
      WebInspector.viewManager, drawerId);
  var tags = extension.descriptor()['tags'] || '';
  return WebInspector.CommandMenu.createCommand(
      WebInspector.UIString('Drawer'), tags,
      WebInspector.UIString('Show %s', extension.title()), '', executeHandler);
}

                        /** @type {!WebInspector.CommandMenu} */
                        WebInspector.commandMenu =
                            new WebInspector.CommandMenu();

/**
 * @constructor
 * @implements {WebInspector.ActionDelegate}
 */
WebInspector.CommandMenu.ShowActionDelegate =
    function() {}
  ...
  handleAction: function(context, actionId) {
    new WebInspector.FilteredListWidget(new WebInspector.CommandMenuDelegate())
        .showAsDialog();
