 ...
 * contextMenuCallback
 */
WebInspector.SortableDataGrid =
    function(
        columnsArray, editCallback, deleteCallback, refreshCallback,
        contextMenuCallback) {
  WebInspector.ViewportDataGrid.call(
      this, columnsArray, editCallback, deleteCallback, refreshCallback,
      contextMenuCallback);
  ...
  this.setRootNode(new WebInspector.SortableDataGridNode());
    ...
    /** @typedef {function(!WebInspector.DataGridNode,
       !WebInspector.DataGridNode):number} */
 ...
 * @return {number}
 */
WebInspector.SortableDataGrid.TrivialComparator =
    function(a, b) {
  return 0;
}
 ...
 * @return {number}
 */
    WebInspector.SortableDataGrid.NumericComparator =
        function(columnIdentifier, a, b) {
 ...
 * @return {number}
 */
            WebInspector.SortableDataGrid.Comparator =
                function(comparator, reverseMode, a, b) {
 ...
 */
                WebInspector.SortableDataGrid.create =
                    function(columnNames, values) {
  ...
  for (var i = 0; i < columnNames.length; ++i)
    columns.push(
        {title: columnNames[i], width: columnNames[i].length, sortable: true});
  ...
  dataGrid.addEventListener(
      WebInspector.DataGrid.Events.SortingChanged, sortDataGrid);
  ...
  function sortDataGrid() {
    ...
    var comparator = columnIsNumeric ?
        ...
        WebInspector.SortableDataGrid.StringComparator;
    dataGrid.sortNodes(
        comparator.bind(null, sortColumnIdentifier),
  ...
  insertChild: function(node) {
    var root =
        /** @type {!WebInspector.SortableDataGridNode} */ (this.rootNode());
  ...
  sortNodes: function(comparator, reverseMode) {
    this._sortingFunction = WebInspector.SortableDataGrid.Comparator.bind(
        null, comparator, reverseMode);
  ...
  __proto__: WebInspector.ViewportDataGrid.prototype
                                      ...
                                      * @constructor
                                      * @extends
                                      * {WebInspector.ViewportDataGridNode}
