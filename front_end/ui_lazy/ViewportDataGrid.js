...
WebInspector.ViewportDataGrid =
  ...
  this._scrollContainer.addEventListener(
      'mousewheel', this._onWheel.bind(this), true);
  ...
  // Wheel target shouldn't be removed from DOM to preserve native kinetic
  // scrolling.
  /** @type {?Node} */
   ...
   * @param {boolean} stick
   */
  setStickToBottom: function(stick) { this._stickToBottom = stick; },

  /**
   * @param {?Event} event
   */
  _onWheel: function(event) {
    this._wheelTarget = event.target ?
        event.target.enclosingNodeOrSelfWithNodeName('tr') :
        null;
  },
  ...
  _update: function() {
    ...
    if (this._hiddenWheelTarget &&
        this._hiddenWheelTarget !== this._wheelTarget) {
    ...
    var contentFits = viewportState.contentHeight <= clientHeight &&
        viewportState.topPadding + viewportState.bottomPadding === 0;
    if (contentFits !==
        this.element.classList.contains('data-grid-fits-viewport')) {
   ...
   * @param {number} depth
   */
  setDepth: function(depth) { this._depth = depth; },
  ...
  reveal: function() { this.dataGrid._revealViewportNode(this); },
