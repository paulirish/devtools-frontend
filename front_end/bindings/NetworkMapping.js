...
WebInspector.NetworkMapping =
    function(
        targetManager, workspace, fileSystemWorkspaceBinding,
        fileSystemMapping) {
  ...
  InspectorFrontendHost.events.addEventListener(
      InspectorFrontendHostAPI.Events.RevealSourceLine, this._revealSourceLine,
      this);
  ...
  fileSystemManager.waitForFileSystems().then(
      this._fileSystemsLoaded.bind(this));
}
  ...
  _addMappingsForFilesystem: function(fileSystem) {
    ...
    for (var i = 0; Array.isArray(mappings) && i < mappings.length; ++i) {
      ...
      if (typeof folder !== 'string' || typeof url !== 'string')
        continue;
      this._fileSystemMapping.addNonConfigurableFileMapping(
          fileSystem.path(), url, folder);
    }
  ...
  addMapping: function(networkUISourceCode, uiSourceCode) {
    var fileSystemPath = WebInspector.FileSystemWorkspaceBinding.fileSystemPath(
