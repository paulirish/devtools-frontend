...
WebInspector.DebuggerWorkspaceBinding =
   ...
   * !WebInspector.DebuggerWorkspaceBinding.TargetData>} */
  ...
  targetAdded: function(target) {
    ...
    if (debuggerModel)
      this._targetToData.set(
          target, new WebInspector.DebuggerWorkspaceBinding.TargetData(
                      debuggerModel, this));
  },
  ...
  /**
     ...
     */
  createStackTraceTopFrameLiveLocation: function(
      rawLocations, updateDelegate, locationPool) {
    console.assert(rawLocations.length);
    var location =
        new WebInspector.DebuggerWorkspaceBinding.StackTraceTopFrameLocation(
  ...
  /**
     ...
     * @return {!WebInspector.DebuggerWorkspaceBinding.Location}
     */
  createCallFrameLiveLocation: function(
      location, updateDelegate, locationPool) {
    var target = location.target();
    this._ensureInfoForScript(
        /** @type {!WebInspector.Script} */ (location.script()));
    var liveLocation =
        this.createLiveLocation(location, updateDelegate, locationPool);
  ...
  uiLocationToRawLocation: function(
      target, uiSourceCode, lineNumber, columnNumber) {
    var targetData = this._targetToData.get(target);
  ...
  uiLocationToRawLocations: function(uiSourceCode, lineNumber, columnNumber) {
    ...
    var targetDatas = this._targetToData.valuesArray();
    for (var i = 0; i < targetDatas.length; ++i) {
      var rawLocation = targetDatas[i]._uiLocationToRawLocation(
          uiSourceCode, lineNumber, columnNumber);
      if (rawLocation)
  ...
  normalizeUILocation: function(uiLocation) {
    var target = WebInspector.NetworkProject.targetForUISourceCode(
        uiLocation.uiSourceCode);
    if (target) {
      var rawLocation = this.uiLocationToRawLocation(
          target, uiLocation.uiSourceCode, uiLocation.lineNumber,
  ...
  scriptFile: function(uiSourceCode, target) {
    var targetData = this._targetToData.get(target);
    return targetData ? targetData._resourceMapping.scriptFile(uiSourceCode) :
                        null;
  },
  ...
  _removeLiveLocation: function(location) {
    var info = this._infoForScript(
        location._script.target(), location._script.scriptId);
    if (info)
  ...
  _beforeDebuggerPaused: function(event) {
    ...
    if (!targetData._compilerMapping.mapsToSourceCode(rawLocation)) {
      ...
      event.preventDefault();
                      ...
                      */
                     WebInspector.DebuggerWorkspaceBinding.TargetData =
        function(debuggerModel, debuggerWorkspaceBinding) {
  ...
  this._defaultMapping = new WebInspector.DefaultScriptMapping(
      debuggerModel, workspace, debuggerWorkspaceBinding);
  ...
  this._compilerMapping = new WebInspector.CompilerScriptMapping(
      debuggerModel, workspace, networkMapping,
      WebInspector.NetworkProject.forTarget(this._target),
      debuggerWorkspaceBinding);
  ...
  _uiLocationToRawLocation: function(uiSourceCode, lineNumber, columnNumber) {
    ...
    return sourceMapping ?
        sourceMapping.uiLocationToRawLocation(
            uiSourceCode, lineNumber, columnNumber) :
        null;
  },
  ...
  _uiLineHasMapping: function(uiSourceCode, lineNumber) {
    var sourceMapping = this._uiSourceCodeToSourceMapping.get(uiSourceCode);
    return sourceMapping ?
        sourceMapping.uiLineHasMapping(uiSourceCode, lineNumber) :
        true;
  },
  ...
  _dispose: function() {
    ...
    this._uiSourceCodeToSourceMapping.clear();
                          ...
                          * @param {!WebInspector.Script} script
                          */
                         WebInspector.DebuggerWorkspaceBinding.ScriptInfo =
            function(script) {
  this._script = script;
  ...
  _rawLocationToUILocation: function(rawLocation) {
    ...
    console.assert(
        uiLocation, 'Script raw location cannot be mapped to any UI location.');
    return /** @type {!WebInspector.UILocation} */ (uiLocation);
                              ...
                              * @param {!WebInspector.DebuggerWorkspaceBinding}
                              * binding
                              * @param {function(!WebInspector.LiveLocation)}
                              * updateDelegate
                              * @param {!WebInspector.LiveLocationPool}
                              * locationPool
                              */
                             WebInspector.DebuggerWorkspaceBinding.Location =
                function(
                    script, rawLocation, binding, updateDelegate,
                    locationPool) {
  ...
  this._binding = binding;
     ...
     */
                      uiLocation: function() {
                        ...
                        return this._binding.rawLocationToUILocation(
                       ...
                       */
                      dispose: function() {
                        WebInspector.LiveLocationWithPool.prototype.dispose
                            .call(this);
     ...
     */
                      isBlackboxed: function() {
                        return WebInspector.blackboxManager
                            .isBlackboxedRawLocation(this._rawLocation);
                      },
                     ...
                     * @constructor
                     * @extends {WebInspector.LiveLocationWithPool}
                     * @param {!Array<!WebInspector.DebuggerModel.Location>}
                     * rawLocations
                    ...
                    WebInspector.DebuggerWorkspaceBinding
                        ...
                        function(
                            rawLocations, binding, updateDelegate,
                            locationPool) {
                      WebInspector.LiveLocationWithPool.call(
                          this, updateDelegate, locationPool);

                      this._updateScheduled = true;
                      /** @type {!Set<!WebInspector.LiveLocation>} */
                      this._locations = new Set();
                      for (var location of rawLocations)
                        this._locations.add(binding.createCallFrameLiveLocation(
                            location, this._scheduleUpdate.bind(this),
                            locationPool));
     ...
     * @return {?WebInspector.UILocation}
     */
                                  rawLocationToUILocation: function(
                                      rawLocation) {},
     ...
     * @return {boolean}
     */
                                  uiLineHasMapping: function(
                                      uiSourceCode, lineNumber) {}
                                }
