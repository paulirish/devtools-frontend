...
WebInspector.CSSWorkspaceBinding =
  ...
  /** @type {!Map.<!WebInspector.CSSModel,
   * !WebInspector.CSSWorkspaceBinding.TargetInfo>} */
  ...
  targetAdded: function(target) {
    ...
    if (cssModel)
      this._modelToTargetInfo.set(
          cssModel, new WebInspector.CSSWorkspaceBinding.TargetInfo(
  ...
  _ensureTargetInfo: function(header) {
    ...
    if (!targetInfo) {
      targetInfo = new WebInspector.CSSWorkspaceBinding.TargetInfo(
          header.cssModel(), this._workspace, this._networkMapping);
  ...
  createLiveLocation: function(rawLocation, updateDelegate, locationPool) {
    var header = rawLocation.styleSheetId ?
        rawLocation.cssModel().styleSheetHeaderForId(rawLocation.styleSheetId) :
        null;
    return new WebInspector.CSSWorkspaceBinding.LiveLocation(
        rawLocation.cssModel(), header, rawLocation, this, updateDelegate,
        locationPool);
  },
  ...
  propertyUILocation: function(cssProperty, forName) {
    ...
    if (!style ||
        style.type !== WebInspector.CSSStyleDeclaration.Type.Regular ||
        !style.styleSheetId)
      return null;
    ...
    var rawLocation = new WebInspector.CSSLocation(
        header, header.lineNumberInSource(lineNumber),
  ...
  rawLocationToUILocation: function(rawLocation) {
    ...
    var targetInfo = this._targetInfo(header);
    return targetInfo ?
        targetInfo._rawLocationToUILocation(
            header, rawLocation.lineNumber, rawLocation.columnNumber) :
        null;
  }
}
                      ...
                      */
                     WebInspector.CSSWorkspaceBinding.TargetInfo =
        function(cssModel, workspace, networkMapping) {
  this._cssModel = cssModel;
  this._stylesSourceMapping =
      new WebInspector.StylesSourceMapping(cssModel, workspace, networkMapping);
  this._sassSourceMapping = new WebInspector.SASSSourceMapping(
      cssModel, networkMapping,
      WebInspector.NetworkProject.forTarget(cssModel.target()));

  /** @type {!Multimap<!WebInspector.CSSStyleSheetHeader,
  ...
  _rawLocationToUILocation: function(header, lineNumber, columnNumber) {
    var rawLocation =
        new WebInspector.CSSLocation(header, lineNumber, columnNumber);
    var uiLocation = null;
    uiLocation = uiLocation ||
        this._sassSourceMapping.rawLocationToUILocation(rawLocation);
    uiLocation = uiLocation ||
        this._stylesSourceMapping.rawLocationToUILocation(rawLocation);
    return uiLocation;
  },
  ...
  _dispose: function() {
    ...
    this._sassSourceMapping.dispose();
                          ...
                          * @param {!WebInspector.LiveLocationPool} locationPool
                          */
                         WebInspector.CSSWorkspaceBinding.LiveLocation =
            function(
                cssModel, header, rawLocation, binding, updateDelegate,
                locationPool) {
  ...
  _setStyleSheet: function(header) {
    ...
    this._cssModel.removeEventListener(
        WebInspector.CSSModel.Events.StyleSheetAdded, this._styleSheetAdded,
        this);
    this._cssModel.addEventListener(
        WebInspector.CSSModel.Events.StyleSheetRemoved, this._styleSheetRemoved,
        this);
  },
  ...
  uiLocation: function() {
    ...
    if (this._header) {
      ...
      return targetInfo._rawLocationToUILocation(
          this._header, cssLocation.lineNumber, cssLocation.columnNumber);
    }
    var uiSourceCode = this._binding._networkMapping.uiSourceCodeForStyleURL(
        cssLocation.url, cssLocation.header());
    if (!uiSourceCode)
      return null;
    return uiSourceCode.uiLocation(
        cssLocation.lineNumber, cssLocation.columnNumber);
  },
  ...
  /**
     ...
     * @return {boolean}
     */
  isBlackboxed: function() { return false; },
