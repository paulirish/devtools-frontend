...
WebInspector.BlackboxManager =
    function(debuggerWorkspaceBinding) {
  ...
  /** @type {!Map<!WebInspector.DebuggerModel, !Map<string,
   * !Array<!DebuggerAgent.ScriptPosition>>>} */
  ...
  */
  ...
  _setBlackboxPatterns: function(debuggerModel) {
    var regexPatterns =
        WebInspector.moduleSetting('skipStackFramesPattern').getAsArray();
  ...
  isBlackboxedUISourceCode: function(uiSourceCode) {
    ...
    var isContentScript =
        projectType === WebInspector.projectTypes.ContentScripts;
    if (isContentScript &&
        WebInspector.moduleSetting('skipContentScripts').get())
      return true;
  ...
  sourceMapLoaded: function(script, sourceMap) {
    ...
    var positions = [];
    // If content in script file begin is not mapped and one or more ranges are
    ...
    for (var mapping of mappings) {
      if (mapping.sourceURL &&
          currentBlackboxed !== this.isBlackboxedURL(mapping.sourceURL)) {
        positions.push({
  ...
  _patternChanged: function() {
    ...
    for (var debuggerModel of WebInspector.DebuggerModel.instances()) {
      ...
      for (var scriptId in debuggerModel.scripts) {
        ...
        promises.push(
            this._addScript(script).then(loadSourceMap.bind(this, script)));
      }
    }
  ...
  _addScript: function(script) {
    ...
    return this._setScriptState(
        script, blackboxed ? [{lineNumber: 0, columnNumber: 0}] : []);
  },
  ...
  _scriptPositions: function(script) {
    if (this._debuggerModelData.has(script.debuggerModel))
      return this._debuggerModelData.get(script.debuggerModel)
                 .get(script.scriptId) ||
          null;
    return null;
  },
  ...
  _setScriptState: function(script, positions) {
    ...
    if (previousScriptState) {
      ...
      for (var i = 0; !hasChanged && i < positions.length; ++i)
        hasChanged =
            positions[i].lineNumber !== previousScriptState[i].lineNumber ||
