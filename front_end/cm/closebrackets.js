...
(function(mod) {
  ...
  function handleBackspace(cm) {
    ...
    for (var i = ranges.length - 1; i >= 0; i--) {
      ...
      cm.replaceRange(
          '', Pos(cur.line, cur.ch - 1), Pos(cur.line, cur.ch + 1), '+delete');
    }
  }
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (opening && !range.empty()) {
        ...
        if (!CodeMirror.isWordChar(next) && enteringString(cm, cur, ch))
          ...
          opening && (cm.getLine(cur.line).length == cur.ch ||
                      isClosingBracket(next, pairs) || /\s/.test(next))) {
        curType = 'both';
      } else {
        return CodeMirror.Pass;
      }
  ...
  function enteringString(cm, pos, ch) {
    ...
    var stream = new CodeMirror.StringStream(
        line.slice(0, pos.ch) + ch + line.slice(pos.ch), 4);
