...
(function(mod) {
  ...
  function expressionAllowed(stream, state, backUp) {
    return /^(?:operator|sof|keyword c|case|new|[\[{}\(,;:]|=>)$/.test(
               state.lastType) ||
  ...
  CodeMirror.defineMode('javascript', function(config, parserConfig) {
    ...
    function parseJS(state, style, type, content, stream) {
      ...
      while (true) {
        var combinator =
            cc.length ? cc.pop() : jsonMode ? expression : statement;
      ...
      var result = function() {
        ...
        if (state.lexical.type == 'stat')
          ...
          for (var outer = state.lexical;
               outer && outer.type == ')' && outer.align; outer = outer.prev)
            indent = outer.indented;
        state.lexical = new JSLexical(
            indent, cx.stream.column(), type, null, state.lexical, info);
      };
      result.lex = true;
      return result;
    }
    ...
    function statement(type, value) {
      if (type == 'var')
        return cont(
            pushlex('vardef', value.length), vardef, expect(';'), poplex);
      if (type == 'keyword a')
      ...
      if (type == 'switch')
        return cont(
            pushlex('form'), expression, pushlex('}', 'switch'), expect('{'),
            block, poplex, poplex);
      if (type == 'case')
    ...
    function expressionInner(type, noComma) {
      if (cx.state.fatArrowAt == cx.stream.start) {
        ...
        if (type == '(')
          return cont(
              pushcontext, pushlex(')'), commasep(pattern, ')'), poplex,
      ...
      if (type == '(')
        return cont(
            pushlex(')'), maybeexpression, expect(')'), poplex, maybeop);
    ...
    function maybeoperatorNoComma(type, value, noComma) {
      ...
      if (type == '=>')
        return cont(
            pushcontext, noComma ? arrowBodyNoComma : arrowBody, popcontext);
      if (type == 'operator') {
        if (/\+\+|--/.test(value))
          return cont(me);
        if (value == '?')
    ...
    function afterType(type, value) {
      if (value == '<')
        return cont(
            commasep(typeexpr, '>'),
            afterType) if (type == '[') return cont(expect(']'), afterType)
    }
    function vardef() {
    ...
    function functiondef(type, value) {
      ...
      if (type == '(')
        return cont(
            pushcontext, pushlex(')'), commasep(funarg, ')'), poplex, maybetype,
    ...
    return {
      startState: function(basecolumn) {
        var state = {
          ...
          cc: [],
          lexical:
              new JSLexical((basecolumn || 0) - indentUnit, 0, 'block', false),
      ...
      skipExpression: function(state) {
        var top = state
                      .cc[state.cc.length -
                          1] if (top == expression || top == expressionNoComma)
                          state.cc.pop()
      }
    };
  });
  ...
  CodeMirror.defineMIME(
      'application/ld+json', {name: 'javascript', jsonld: true});
  CodeMirror.defineMIME(
