...
(function(mod) {
  ...
  function Display(place, doc, input) {
    ...
    d.mover =
        elt('div', [elt('div', [d.lineSpace], 'CodeMirror-lines')], null,
            'position: relative');
    ...
    d.scroller =
        elt('div', [d.sizer, d.heightForcer, d.gutters], 'CodeMirror-scroll');
    d.scroller.setAttribute('tabIndex', '-1');
    // The element in which the editor lives.
    d.wrapper = elt(
        'div', [d.scrollbarFiller, d.gutterFiller, d.scroller], 'CodeMirror');
  ...
  function estimateHeight(cm) {
    ...
    var perLine = wrapping &&
        Math.max(
            5, cm.display.scroller.clientWidth / charWidth(cm.display) - 3);
  ...
  function NativeScrollbars(place, scroll, cm) {
    this.cm = cm;
    var vert = this.vert =
        elt('div', [elt('div', null, null, 'min-width: 1px')],
            'CodeMirror-vscrollbar');
    var horiz = this.horiz =
        elt('div', [elt('div', null, null, 'height: 100%; min-height: 1px')],
  ...
  function initScrollbars(cm) {
    ...
    cm.display.scrollbars =
        new CodeMirror.scrollbarModel[cm.options.scrollbarStyle](
            function(node) {
  ...
  function visibleLines(display, doc, viewport) {
    ...
    if (viewport && viewport.ensure) {
      ...
      if (ensureFrom < from) {
        from = ensureFrom;
        to = lineAtHeight(
            doc, heightAtLine(getLine(doc, ensureFrom)) +
                display.wrapper.clientHeight);
      } else if (Math.min(ensureTo, doc.lastLine()) >= to) {
        from = lineAtHeight(
            doc, heightAtLine(getLine(doc, ensureTo)) -
                display.wrapper.clientHeight);
        to = ensureTo;
      }
    }
  ...
  function updateDisplayIfNeeded(cm, update) {
    ...
    if (!update.force && update.visible.from >= display.viewFrom &&
    ...
    var from =
        Math.max(update.visible.from - cm.options.viewportMargin, doc.first);
  ...
  function postUpdateDisplay(cm, update) {
    ...
    for (var first = true;; first = false) {
      if (!first || !cm.options.lineWrapping ||
    ...
    if (cm.display.viewFrom != cm.display.reportedViewFrom ||
      ...
      update.signal(
          cm, 'viewportChange', cm, cm.display.viewFrom, cm.display.viewTo);
  ...
  function updateLineBackground(lineView) {
    ...
    if (lineView.background) {
      ...
      lineView.background =
          wrap.insertBefore(elt('div', null, cls), wrap.firstChild);
    }
  }
        ...
        for (var k = 0; k < cm.options.gutters.length; ++k) {
          ...
          if (found)
            gutterWrap.appendChild(
                elt('div', [found], 'CodeMirror-gutter-elt', 'left: ' +
  ...
  function handlePaste(e, cm) {
    ...
    if (pasted) {
      ...
      if (!cm.isReadOnly() && !cm.options.disableInput)
        runInOp(
            cm, function() { applyTextInput(cm, pasted, 0, null, 'paste'); });
      return true;
    }
  }
  ...
  TextareaInput.prototype = copyObj(
      {
        init: function(display) {
          ...
          // Needed to hide big blue blinking cursor on Mobile Safari (doesn't
          // seem to work in iOS 8 anymore)
          if (ios)
            te.style.width = '0px';
        ...
        reset: function(typing) {
          ...
          if (cm.somethingSelected()) {
            ...
            minimal =
                hasCopyEvent && (range.to().line - range.from().line > 100 ||
        ...
        poll: function() {
          ...
          runInOp(cm, function() {
            applyTextInput(
                cm, text.slice(same), prevInput.length - same, null,
            ...
            if (self.composing) {
              ...
              self.composing.range = cm.markText(
                  self.composing.start, cm.getCursor('to'),
                  {className: 'CodeMirror-composing'});
            }
          });
          return true;
        ...
        onContextMenu: function(e) {
          ...
          if (webkit)
            var oldScrollY =
                window.scrollY;  // Work around Chrome issue (#2712)
          display.input.focus();
          if (webkit)
            ...
            if (te.selectionStart != null) {
              ...
              var i = 0, poll = function() {
                if (display.selForContextMenu == cm.doc.sel &&
                    te.selectionStart == 0 && te.selectionEnd > 0 &&
                    input.prevInput == '\u200b')
  ...
  ContentEditableInput.prototype = copyObj(
      {
        init: function(display) {
          ...
          on(div, 'compositionstart', function(e) {
            ...
            var found =
                line.indexOf(data, Math.max(0, prim.head.ch - data.length));
          ...
          on(div, 'compositionupdate',
             function(e) { input.composing.data = e.data; });
          ...
          function onCopyCut(e) {
            ...
            if (e.clipboardData && !ios) {
              ...
              cm.display.lineSpace.insertBefore(
                  kludge, cm.display.lineSpace.firstChild);
              te.value = lastCopied.text.join('\n');
        ...
        showPrimarySelection: function() {
          ...
          }  // Our model of the DOM might be outdated, in which case the range
        ...
        pollSelection: function() {
          if (!this.composing && !this.gracePeriod && this.selectionChanged()) {
            ...
            if (anchor && head)
              runInOp(cm, function() {
                setSelection(
                    cm.doc, simpleSelection(anchor, head), sel_dontScroll);
        ...
        pollContent: function() {
          ...
          var chTo =
              Pos(toLine, oldText.length ? lst(oldText).length - cutEnd : 0);
        ...
        applyComposition: function(composing) {
          if (this.cm.isReadOnly())
            operation(this.cm, regChange)(this.cm) else if (
                composing.data && composing.data != composing.startData)
            operation(this.cm, applyTextInput)(
                this.cm, composing.data, 0, composing.sel);
        },

        setUneditable: function(node) { node.contentEditable = 'false' },

        onKeyPress: function(e) {
          e.preventDefault();
          if (!this.cm.isReadOnly())
            operation(this.cm, applyTextInput)(
                this.cm, String.fromCharCode(
  ...
  function locateNodeInLineView(lineView, node, offset) {
    ...
    for (var before = topNode.previousSibling, dist = offset; before;
  ...
  function domTextBetween(cm, from, to, fromLine, toLine) {
    ...
    function recognizeMarker(id) {
      return function(marker) { return marker.id == id; };
    }
    function walk(node) {
      if (node.nodeType == 1) {
        ...
        if (markerID) {
          var found = cm.findMarks(
              Pos(fromLine, 0), Pos(toLine + 1, 0), recognizeMarker(+markerID));
  ...
  Selection.prototype = {
    ...
    equals: function(other) {
      if (other == this)
        return true;
      if (other.primIndex != this.primIndex ||
    ...
    deepCopy: function() {
      for (var out = [], i = 0; i < this.ranges.length; i++)
        out[i] = new Range(
            copyPos(this.ranges[i].anchor), copyPos(this.ranges[i].head));
  ...
  function normalizeSelection(ranges, primIndex) {
    ...
    for (var i = 1; i < ranges.length; i++) {
      ...
      if (cmp(prev.to(), cur.from()) >= 0) {
        ...
        var inv =
            prev.empty() ? cur.from() == cur.head : prev.from() == prev.head;
        if (i <= primIndex)
          --primIndex;
      ...
      update: function(ranges) {
        ...
        for (var i = 0; i < ranges.length; i++)
          this.ranges[i] = new Range(
              clipPos(doc, ranges[i].anchor), clipPos(doc, ranges[i].head));
      },
  ...
  function setSelection(doc, sel, options) {
    ...
    addSelectionToHistory(
        doc, doc.sel, doc.cm ? doc.cm.curOp.id : NaN, options);
  }

  function setSelectionNoUndo(doc, sel, options) {
    if (hasHandler(doc, 'beforeSelectionChange') ||
  ...
  function reCheckSelection(doc) {
    setSelectionInner(
        doc, skipAtomicInSelection(doc, doc.sel, null, false), sel_dontScroll);
  }
  ...
  function skipAtomicInSelection(doc, sel, bias, mayClear) {
    ...
    for (var i = 0; i < sel.ranges.length; i++) {
      ...
      var newAnchor =
          skipAtomic(doc, range.anchor, old && old.anchor, bias, mayClear);
      var newHead =
        ...
        if ((sp.from == null ||
          ...
          if (oldPos) {
            ...
            if (dir < 0 ? m.inclusiveRight : m.inclusiveLeft)
              near = movePos(
                  doc, near, -dir, near && near.line == pos.line ? line : null);
  ...
  function prepareSelection(cm, primary) {
    ...
    for (var i = 0; i < doc.sel.ranges.length; i++) {
      ...
      if (range.from().line >= cm.display.viewTo ||
  ...
  function drawSelectionCursor(cm, head, output) {
    var pos = cursorCoords(
        cm, head, 'div', null, null, !cm.options.singleCursorHeightPerLine);
    ...
    if (pos.other) {
      ...
      otherCursor.style.height =
          (pos.other.bottom - pos.other.top) * .85 + 'px';
    }
  }
  ...
  function drawSelectionRange(cm, range, output) {
    ...
    var rightSide =
        Math.max(
            display.sizerWidth, displayWidth(cm) - display.sizer.offsetLeft) -
        padding.right;
    ...
    function add(left, top, width, bottom) {
      ...
      fragment.appendChild(
          elt('div', null, 'CodeMirror-selected', 'position: absolute; left: ' +
                  left + 'px; top: ' + top + 'px; width: ' +
                  (width == null ? rightSide - left : width) + 'px; height: ' +
                  (bottom - top) + 'px'));
    }
    ...
    function drawForLine(line, fromArg, toArg) {
      ...
      iterateBidiSections(
          getOrder(lineObj), fromArg || 0, toArg == null ? lineLen : toArg,
          function(from, to, dir) {
            ...
            if (toArg == null && to == lineLen)
              right = rightSide;
            if (!start || leftPos.top < start.top ||
    ...
    if (sFrom.line == sTo.line) {
      ...
      var leftEnd = drawForLine(
                        ...
                        .end;
      var rightStart =
          drawForLine(sTo.line, singleVLine ? 0 : null, sTo.ch).start;
      if (singleVLine) {
  ...
  function highlightWorker(cm) {
    ...
    doc.iter(
        doc.frontier, Math.min(doc.first + doc.size, cm.display.viewTo + 500),
        function(line) {
          if (doc.frontier >= cm.display.viewFrom) {  // Visible
            var oldStyles = line.styles,
                tooLong = line.text.length > cm.options.maxHighlightLength;
            var highlighted = highlightLine(
                cm, line, tooLong ? copyState(doc.mode, state) : state, true);
  ...
  function paddingH(display) {
    ...
    var style =
        window.getComputedStyle ? window.getComputedStyle(e) : e.currentStyle;
    var data = {
  ...
  function measureCharInner(cm, prepared, ch, bias) {
    ...
    if (node.nodeType ==
      ...
      for (var i = 0; i < 4; i++) {  // Retry a maximum of 4 times when nonsense
        ...
        if (ie && ie_version < 9 && start == 0 &&
          ...
          rect = getUsefulRect(
              ...
              bias) if (rect.left || rect.right || start == 0) break;
        end = start;
        start = start - 1;
        collapse = 'right';
      }
  ...
  function charCoords(cm, pos, context, lineObj, bias) {
    ...
    return intoCoordSystem(
        cm, lineObj, measureChar(cm, lineObj, pos.ch, bias), context);
  }
  ...
  function cursorCoords(cm, pos, context, lineObj, preparedMeasure, varHeight) {
    ...
    function get(ch, right) {
      var m = measureCharPrepared(
          cm, preparedMeasure, ch, right ? 'right' : 'left', varHeight);
      if (right)
        m.left = m.right;
      else
        m.right = m.left;
  ...
  function coordsChar(cm, x, y) {
    ...
    if (lineN > last)
      return PosWithInfo(
          doc.first + doc.size - 1, getLine(doc, last).text.length, true, 1);
    if (x < 0)
      x = 0;
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    function getX(ch) {
      var sp =
          cursorCoords(cm, Pos(lineNo, ch), 'line', lineObj, preparedMeasure);
      wrongLine = true;
    ...
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1) :
                 to - from <= 1) {
        ...
        // breaks. (Issue #4078)
        if (toOutside && !bidi && !/\s/.test(lineObj.text.charAt(ch)) &&
            xDiff > 0 && ch < lineObj.text.length &&
            preparedMeasure.view.measure.heights.length > 1) {
          var charSize = measureCharPrepared(cm, preparedMeasure, ch, 'right');
          if (innerOff <= charSize.bottom && innerOff >= charSize.top &&
        ...
        while (isExtendingChar(lineObj.text.charAt(ch)))
          ++ch;
        var pos = PosWithInfo(
            lineNo, ch, outside, xDiff < -1 ? -1 : xDiff > 1 ? 1 : 0);
        return pos;
      }
  ...
  function startOperation(cm) {
    cm.curOp = {
      ...
      typing: false,  // Whether this reset should be careful to leave existing
                      // text (for compositing)
    ...
    if (operationGroup) {
      ...
      cm.curOp.ownsGroup =
          operationGroup = {ops: [cm.curOp], delayedCallbacks: []};
    }
  }
  ...
  function endOperation_R1(op) {
    ...
    op.update =
        op.mustUpdate &&
        new DisplayUpdate(
            cm, op.mustUpdate && {top: op.scrollTop, ensure: op.scrollToPos},
            op.forceUpdate);
  }
  ...
  function endOperation_R2(op) {
    ...
    if (display.maxLineChanged && !cm.options.lineWrapping) {
      ...
      op.barMeasure.scrollWidth = Math.max(
          display.scroller.clientWidth, display.sizer.offsetLeft +
              op.adjustWidthTo + scrollGap(cm) + cm.display.barWidth);
      op.maxScrollLeft = Math.max(
          0, display.sizer.offsetLeft + op.adjustWidthTo - displayWidth(cm));
    }
  ...
  function endOperation_W2(op) {
    ...
    if (op.adjustWidthTo != null) {
      ...
      if (op.maxScrollLeft < cm.doc.scrollLeft)
        setScrollLeft(
            cm, Math.min(cm.display.scroller.scrollLeft, op.maxScrollLeft),
            true);
      cm.display.maxLineChanged = false;
    }

    var takeFocus = op.focus && op.focus == activeElt() &&
        (!document.hasFocus || document.hasFocus()) if (op.preparedSelection)
  ...
  function adjustView(cm, from, to) {
    ...
    if (view.length == 0 || from >= display.viewTo || to <= display.viewFrom) {
      ...
      if (display.viewFrom > from)
        display.view =
            buildViewArray(cm, from, display.viewFrom).concat(display.view);
      else if (display.viewFrom < from)
        display.view = display.view.slice(findViewIndex(cm, from));
      display.viewFrom = from;
      if (display.viewTo < to)
        display.view =
            display.view.concat(buildViewArray(cm, display.viewTo, to));
  ...
  function registerEventHandlers(cm) {
    ...
    on(d.scroller, 'touchend', function(e) {
      ...
      if (touch && !eventInWidget(d, e) && touch.left != null && !touch.moved &&
        ...
        if (!touch.prev || farAway(touch, touch.prev))  // Single tap
          ...
          range = new Range(
              Pos(pos.line, 0), clipPos(cm.doc, Pos(pos.line + 1, 0)));
    ...
    on(d.wrapper, 'scroll',
       function() { d.wrapper.scrollTop = d.wrapper.scrollLeft = 0; });
  ...
  function leftButtonDown(cm, e, start) {
    ...
    if (lastDoubleClick && lastDoubleClick.time > now - 400 &&
      ...
      type = 'triple';
        ...
        lastClick && lastClick.time > now - 400 &&
  ...
  function leftButtonSelect(cm, e, start, type, addNew) {
    ...
    if (chromeOS ? e.shiftKey && e.metaKey : e.altKey) {
      ...
      var line =
          new Range(Pos(start.line, 0), clipPos(doc, Pos(start.line + 1, 0)));
      if (cm.display.shift || doc.extend)
    ...
    if (!addNew) {
      ...
      setSelection(
          doc, normalizeSelection(ranges.concat([ourRange]), ourIndex),
          {scroll: false, origin: '*mouse'});
    } else if (
        ranges.length > 1 && ranges[ourIndex].empty() && type == 'single' &&
        !e.shiftKey) {
      setSelection(
          doc,
          normalizeSelection(
              ranges.slice(0, ourIndex).concat(ranges.slice(ourIndex + 1)), 0),
          {scroll: false, origin: '*mouse'});
      startSel = doc.sel;
    } else {
    ...
    function extendTo(pos) {
      ...
      if (type == 'rect') {
        ...
        var startCol =
            countColumn(getLine(doc, start.line).text, start.ch, tabSize);
        ...
        if (type != 'single') {
          if (type == 'double')
            ...
            var range =
                new Range(Pos(pos.line, 0), clipPos(doc, Pos(pos.line + 1, 0)));
        ...
        reader.onload = operation(cm, function() {
          ...
          if (++read == n) {
            ...
            setSelectionReplaceHistory(
                cm.doc, simpleSelection(pos, changeEnd(change)));
          }
        });
  ...
  function onDragOver(cm, e) {
    ...
    if (!cm.display.dragCursor) {
      cm.display.dragCursor =
          elt('div', null, 'CodeMirror-cursors CodeMirror-dragcursors');
      cm.display.lineSpace.insertBefore(
          cm.display.dragCursor, cm.display.cursorDiv);
    }
  ...
  function setScrollLeft(cm, val, isScroller) {
    if (isScroller ? val == cm.doc.scrollLeft :
                     Math.abs(cm.doc.scrollLeft - val) < 2)
      return;
    val = Math.min(
        val, cm.display.scroller.scrollWidth - cm.display.scroller.clientWidth);
  ...
  function handleKeyBinding(cm, e) {
    ...
    if (e.shiftKey && !cm.state.keySeq) {
      ...
      return dispatchKey(
          cm, name, e, function(b) { return doHandleBinding(cm, b); });
    }
  }
  ...
  var changeEnd = CodeMirror.changeEnd = function(change) {
    if (!change.text)
      return change.to;
    return Pos(
        change.from.line + change.text.length - 1, lst(change.text).length +
  ...
  function makeChange(doc, change, ignoreReadOnly) {
    ...
    if (hasHandler(doc, 'beforeChange') ||
  ...
  function makeChangeInner(doc, change) {
    ...
    makeChangeSingleDoc(
        doc, change, selAfter, stretchSpansOverChange(doc, change));
    var rebased = [];
    ...
    linkedDocs(doc, function(doc, sharedHist) {
      ...
      makeChangeSingleDoc(
          doc, change, null, stretchSpansOverChange(doc, change));
    });
  }
  ...
  function shiftDoc(doc, distance) {
    ...
    doc.first += distance;
    doc.sel =
        new Selection(map(doc.sel.ranges, function(range) {
                        return new Range(
  ...
  function makeChangeSingleDoc(doc, change, selAfter, spans) {
    ...
    if (change.to.line < doc.first) {
      shiftDoc(
          doc, change.text.length - 1 - (change.to.line - change.from.line));
      return;
    }
  ...
  function addToScrollPos(cm, left, top) {
    ...
    if (left != null)
      cm.curOp.scrollLeft =
          (cm.curOp.scrollLeft == null ? cm.doc.scrollLeft :
                                         cm.curOp.scrollLeft) +
          left;
    if (top != null)
      cm.curOp.scrollTop =
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    if (!aggressive && !/\S/.test(line.text)) {
      ...
      indentation = doc.mode.indent(
          state, line.text.slice(curSpaceString.length), line.text);
    ...
    if (indentString != curSpaceString) {
      replaceRange(
          doc, indentString, Pos(n, 0), Pos(n, curSpaceString.length),
          '+input');
      line.stateAfter = null;
      return true;
    } else {
  ...
  function findPosH(doc, pos, dir, unit, visually) {
    ...
    function moveOnce(boundToLine) {
      var next =
          (visually ? moveVisually : moveLogically)(lineObj, ch, dir, true);
      if (next == null) {
      ...
      for (var i = 0; i < ranges.length; i++) {
        ...
        if (!range.empty()) {
          ...
          if (from.ch == 0 && ranges.length == newRanges.length &&
            ...
            replaceOneSelection(
                this.doc, i, new Range(from, newRanges[i].to()),
                sel_dontScroll);
    ...
    addWidget: function(pos, node, scroll, vert, horiz) {
      ...
      if (vert == 'over') {
        ...
        var vspace = Math.max(display.wrapper.clientHeight, this.doc.height),
            hspace = Math.max(
                display.sizer.clientWidth, display.lineSpace.clientWidth);
      ...
      if (scroll)
        scrollIntoView(
            this, left, top, left + node.offsetWidth, top + node.offsetHeight);
    },
    ...
    moveH: methodOp(function(dir, unit) {
      ...
      cm.extendSelectionsBy(function(range) {
        if (cm.display.shift || cm.doc.extend || range.empty())
          return findPosH(
              cm.doc, range.head, dir, unit, cm.options.rtlMoveVisually);
        else
    ...
    deleteH: methodOp(function(dir, unit) {
      ...
      if (sel.somethingSelected())
        ...
        deleteNearSelection(this, function(range) {
          ...
          return dir < 0 ? {from: other, to: range.head} :
    ...
    moveV: methodOp(function(dir, unit) {
      ...
      doc.extendSelectionsBy(function(range) {
        ...
        if (unit == 'page' && range == doc.sel.primary())
          addToScrollPos(
              cm, null, charCoords(cm, pos, 'div').top - headPos.top);
        return pos;
      }, sel_move);
      if (goals.length)
        for (var i = 0; i < doc.sel.ranges.length; i++)
  ...
  option(
      ...
      /[\u0000-\u001f\u007f\u00ad\u200b-\u200f\u2028\u2029\ufeff]/g,
      function(cm, val, old) {
        cm.state.specialChars =
            new RegExp(val.source + (val.test('\t') ? '' : '|\t'), 'g');
  ...
  option('fixedGutter', true, function(cm, val) {
    cm.display.gutters.style.left =
        val ? compensateForHScroll(cm.display) + 'px' : '0';
    cm.refresh();
  }, true);
  ...
  option(
      'undoDepth', 200, function(cm, val) { cm.doc.history.undoDepth = val; });
  ...
  var commands = CodeMirror.commands = {
    selectAll: function(cm) {
      cm.setSelection(
          Pos(cm.firstLine(), 0), Pos(cm.lastLine()), sel_dontScroll);
    },
    singleSelection: function(cm) {
      cm.setSelection(
          cm.getCursor('anchor'), cm.getCursor('head'), sel_dontScroll);
    },
    ...
    delWrappedLineRight: function(cm) {
      deleteNearSelection(cm, function(range) {
        ...
        var rightPos = cm.coordsChar(
            {left: cm.display.lineDiv.offsetWidth + 100, top: top}, 'div');
    ...
    newlineAndIndent: function(cm) {
      runInOp(cm, function() {
        ...
        for (var i = 0; i < len; i++) {
          ...
          cm.replaceRange(
              cm.doc.lineSeparator(), range.anchor, range.head, '+input');
  ...
  function markText(doc, from, to, options, type) {
    ...
    if (marker.replacedWith) {
      ...
      marker.widgetNode =
          elt('span', [marker.replacedWith], 'CodeMirror-widget');
    ...
    if (marker.addToHistory)
      addChangeToHistory(
          doc, {from: from, to: to, origin: 'markText'}, doc.sel, NaN);
    ...
    doc.iter(curLine, to.line + 1, function(line) {
      ...
      addMarkedSpan(
          line, new MarkedSpan(
                    marker, curLine == from.line ? from.ch : null,
  ...
  var SharedTextMarker =
      CodeMirror.SharedTextMarker = function(markers, primary) {
  ...
  function copySharedMarkers(doc, markers) {
    for (var i = 0; i < markers.length; i++) {
      ...
      if (cmp(mFrom, mTo)) {
        var subMark =
            markText(doc, mFrom, mTo, marker.primary, marker.primary.type);
  ...
  function addMarkedSpan(line, span) {
    line.markedSpans =
        line.markedSpans ? line.markedSpans.concat([span]) : [span];
    ...
    if (old)
      for (var i = 0, nw; i < old.length; ++i) {
        ...
        if (startsBefore ||
          ...
          (nw || (nw = []))
              .push(new MarkedSpan(
                  marker, span.from, endsAfter ? null : span.to));
        }
      }
    return nw;
  }
  ...
  function lineIsHiddenInner(doc, line, span) {
    if (span.to == null) {
      ...
      return lineIsHiddenInner(
          doc, end.line, getMarkedSpanFor(end.line.markedSpans, span.marker));
    }
  ...
  function widgetHeight(widget) {
    ...
    if (!contains(document.body, widget.node)) {
      ...
      removeChildrenAndAdd(
          cm.display.measure, elt('div', [widget.node], null, parentStyle));
    }
  ...
  function addLineWidget(doc, handle, node, options) {
    ...
    changeLine(doc, handle, 'widget', function(line) {
      ...
      if (widget.insertAt == null)
        ...
        widgets.splice(
            Math.min(widgets.length - 1, Math.max(0, widget.insertAt)), 0,
            widget);
      widget.line = line;
    ...
    while (!stream.eol()) {
      if (stream.pos > cm.options.maxHighlightLength) {
        ...
        style = extractLineClasses(
            readToken(mode, stream, state, inner), lineClasses);
      }
      if (inner) {
  ...
  function getLineStyles(cm, line, updateFrontier) {
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
      ...
      var result = highlightLine(
          cm, line, line.text.length > cm.options.maxHighlightLength ?
  ...
  function interpretTokenStyle(style, options) {
    if (!style || /^\s*$/.test(style))
      return null;
    var cache =
        options.addModeClass ? styleToClassCacheWithMode : styleToClassCache;
  ...
  function buildLineContent(cm, lineView) {
    ...
    var content =
        elt('span', null, null, webkit ? 'padding-right: .1px' : null);
    var builder = {
    ...
    for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      ...
      insertLineContent(
          line, builder, getLineStyles(cm, line, allowFrontierUpdate));
      if (line.styleClasses) {
        if (line.styleClasses.bgClass)
          builder.bgClass =
              joinClasses(line.styleClasses.bgClass, builder.bgClass || '');
        if (line.styleClasses.textClass)
          builder.textClass =
              joinClasses(line.styleClasses.textClass, builder.textClass || '');
      }
    ...
    if (webkit) {
      ...
      if (/\bcm-tab\b/.test(last.className) ||
    ...
    if (builder.pre.className)
      builder.textClass =
          joinClasses(builder.pre.className, builder.textClass || '');
  ...
  function buildToken(builder, text, style, startStyle, endStyle, title, css) {
    if (!text)
      return;
    var displayText =
        builder.splitSpaces ? splitSpaces(text, builder.trailingSpace) : text
    ...
    if (!special.test(text)) {
      ...
      while (true) {
        ...
        if (skipped) {
          var txt =
              document.createTextNode(displayText.slice(pos, pos + skipped));
          if (ie && ie_version < 9)
        ...
        if (m[0] == '\t') {
          ...
          var txt = content.appendChild(elt(
              'span', m[0] == '\r' ? '\u240d' : '\u2424', 'cm-invalidchar'));
  ...
  function buildTokenBadBidi(inner, order) {
    return function(builder, text, style, startStyle, endStyle, title, css) {
      ...
      for (;;) {
        ...
        inner(
            builder, text.slice(0, part.to - start), style, startStyle, null,
            title, css);
        startStyle = null;
  ...
  function insertLineContent(line, builder, styles) {
    ...
    if (!spans) {
      for (var i = 1; i < styles.length; i += 2)
        builder.addToken(
            builder, allText.slice(at, at = styles[i]),
        ...
        for (var j = 0; j < spans.length; ++j) {
          ...
          if (m.type == 'bookmark' && sp.from == pos && m.widgetNode) {
            ...
            if (m.endStyle && sp.to == nextChange)
              (endStyles || (endStyles = []))
                  .push(m.endStyle, sp.to) if (m.title && !title)
                      title = m.title;
            if (m.collapsed &&
                (!collapsed ||
          ...
          for (var j = 0; j < endStyles.length; j += 2)
            if (endStyles[j + 1] == nextChange)
              spanEndStyle += ' ' + endStyles[j]
                  ...
                  if (!collapsed ||
                      collapsed.from ==
                          pos) for (var j = 0; j < foundBookmarks.length; ++j)
        ...
        if (text) {
          ...
          if (!collapsed) {
            ...
            builder.addToken(
                ...
                spanStartStyle,
                pos + tokenText.length == nextChange ? spanEndStyle : '', title,
                css);
          }
          if (end >= upto) {
  ...
  function updateDoc(doc, change, markedSpans, estimateHeight) {
    ...
    if (change.full) {
      ...
      if (text.length == 1) {
        update(
            firstLine, firstLine.text.slice(0, from.ch) + lastText +
                firstLine.text.slice(to.ch),
            lastSpans);
      } else {
        var added = linesFor(1, text.length - 1);
        added.push(new Line(
            lastText + firstLine.text.slice(to.ch), lastSpans, estimateHeight));
        update(
            firstLine, firstLine.text.slice(0, from.ch) + text[0], spansFor(0));
        doc.insert(from.line + 1, added);
      }
    } else if (text.length == 1) {
      update(
          firstLine, firstLine.text.slice(0, from.ch) + text[0] +
  ...
  LeafChunk.prototype = {
    ...
    insertInner: function(at, lines, height) {
      this.height += height;
      this.lines =
          this.lines.slice(0, at).concat(lines).concat(this.lines.slice(at));
  ...
  BranchChunk.prototype = {
    ...
    removeInner: function(at, n) {
      ...
      // single leaf node.
      if (this.size - n < 25 && (this.children.length > 1 ||
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    setSelection: docMethodOp(function(anchor, head, options) {
      setSimpleSelection(
          this, clipPos(this, anchor), clipPos(this, head || anchor), options);
    }),
    extendSelection: docMethodOp(function(head, other, options) {
      extendSelection(
          this, clipPos(this, head), other && clipPos(this, other), options);
    }),
    ...
    setSelections: docMethodOp(function(ranges, primary, options) {
      ...
      for (var i = 0, out = []; i < ranges.length; i++)
        out[i] = new Range(
            clipPos(this, ranges[i].anchor), clipPos(this, ranges[i].head));
      if (primary == null)
    ...
    addSelection: docMethodOp(function(anchor, head, options) {
      ...
      setSelection(
          this, normalizeSelection(ranges, ranges.length - 1), options);
    }),
    ...
    addLineClass: docMethodOp(function(handle, where, cls) {
      return changeLine(
          this, handle, where == 'gutter' ? 'gutter' : 'class', function(line) {
            var prop = where == 'text' ?
                'textClass' :
                where == 'background' ?
                'bgClass' :
    ...
    markText: function(from, to, options) {
      return markText(
          this, clipPos(this, from), clipPos(this, to), options,
    ...
    copy: function(copyHistory) {
      var doc = new Doc(
          getLines(this, this.first, this.first + this.size), this.modeOption,
          this.first, this.lineSep);
    ...
    linkedDoc: function(options) {
      ...
      if (options.to != null && options.to < to)
        to = options.to;
      var copy = new Doc(
          getLines(this, from, to), options.mode || this.modeOption, from,
          this.lineSep);
  ...
  function attachLocalSpans(doc, change, from, to) {
    ...
    doc.iter(
        Math.max(doc.first, from), Math.min(doc.first + doc.size, to),
        function(line) {
          if (line.markedSpans)
  ...
  function e_defaultPrevented(e) {
    return e.defaultPrevented != null ? e.defaultPrevented :
                                        e.returnValue == false;
  }
  ...
  var noHandlers = [] function getHandlers(emitter, type, copy) {
    var arr = emitter._handlers &&
            emitter._handlers[type] if (copy) return arr && arr.length > 0 ?
        arr.slice() :
        noHandlers
  ...
  var off = CodeMirror.off = function(emitter, type, f) {
    if (emitter.removeEventListener)
      ...
      var handlers =
          getHandlers(emitter, type, false) for (var i = 0; i < handlers.length;
                                                 ++i) if (handlers[i] == f) {
        handlers.splice(i, 1);
        break;
      }
  ...
  var signal = CodeMirror.signal = function(emitter, type /*, values...*/) {
    var handlers =
        getHandlers(emitter, type, true) if (!handlers.length) return;
  ...
  function signalLater(emitter, type /*, values...*/) {
    var arr = getHandlers(emitter, type, false) if (!arr.length) return;
  ...
  // handling this'.
  var Pass =
      CodeMirror.Pass = {toString: function() { return 'CodeMirror.Pass'; }};
  ...
  // a particular column.
  var findColumn = CodeMirror.findColumn =
      function(string, goal, tabSize) {
    for (var pos = 0, col = 0;;) {
  ...
  var isWordCharBasic = CodeMirror.isWordChar = function(ch) {
    return /\w/.test(ch) ||
        ch > '\x80' && (ch.toUpperCase() != ch.toLowerCase() ||
  ...
  function activeElt() {
    ...
    while (activeElement && activeElement.root &&
  ...
  function zeroWidthElement(measure) {
    if (zwspSupported == null) {
      var test = elt('span', '\u200b');
      removeChildrenAndAdd(
          measure, elt('span', [test, document.createTextNode('x')]));
  ...
  function hasBadBidiRects(measure) {
    if (badBidiRects != null)
      return badBidiRects;
    var txt =
        removeChildrenAndAdd(measure, document.createTextNode('A\u062eA'));
  ...
  function lineStart(cm, lineN) {
    ...
    var ch =
        !order ? 0 : order[0].level % 2 ? lineRight(visual) : lineLeft(visual);
    return Pos(lineN, ch);
  }
      ...
      for (var i = 1, prev = types[0]; i < len - 1; ++i) {
        ...
        if (type == '+' && prev == '1' && types[i + 1] == '1')
          types[i] = '1';
            ...
            type == ',' && prev == types[i + 1] && (prev == '1' || prev == 'n'))
          types[i] = prev;
        prev = type;
      }
