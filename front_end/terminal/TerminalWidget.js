...
WebInspector.TerminalWidget =
  ...
  _init: function() {
    WebInspector.serviceManager.createService('Terminal')
        .then(this._initialized.bind(this));
  },
  ...
  _initialized: function(backend) {
    if (!backend) {
      if (!this._unavailableLabel) {
        this._unavailableLabel =
            this.element.createChild('div', 'terminal-error-message fill');
    ...
    if (!this._term) {
      ...
      this._term.on(
          'data', data => { this._backend.send('write', {data: data}); });
      this._term.fit();
