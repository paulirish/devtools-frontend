...
WebInspector.SecurityPanel =
  ...
  this._sidebarMainViewElement =
      new WebInspector.SecurityPanelSidebarTreeElement(
          WebInspector.UIString('Overview'),
          this._setVisibleView.bind(this, this._mainView),
  ...
  /** @type {!Map<!WebInspector.NetworkLogView.MixedContentFilterValues,
   * number>} */
  this._filterRequestCounts = new Map();

  WebInspector.targetManager.observeTargets(
      this, WebInspector.Target.Capability.Network);
}
...
/**
 * @typedef {Object}
 * @property {!SecurityAgent.SecurityState} securityState - Current security
 * state of the origin.
 * @property {?NetworkAgent.SecurityDetails} securityDetails - Security details
 * of the origin, if available.
 * @property {?Promise<>} certificateDetailsPromise - Certificate details of the
 * origin.
 * @property {?WebInspector.SecurityOriginView} originView - Current
 * SecurityOriginView corresponding to origin.
 */
WebInspector.SecurityPanel.OriginState;
...
WebInspector.SecurityPanel.prototype = {
  ...
  _updateSecurityState: function(
      newSecurityState, explanations, insecureContentStatus,
  ...
  _onSecurityStateChanged: function(event) {
    ...
    var securityState =
        /** @type {!SecurityAgent.SecurityState} */ (data.securityState);
    var explanations =
        /** @type {!Array<!SecurityAgent.SecurityStateExplanation>} */ (
            data.explanations);
    var insecureContentStatus =
        /** @type {?SecurityAgent.InsecureContentStatus} */ (
            data.insecureContentStatus);
    var schemeIsCryptographic =
        /** @type {boolean} */ (data.schemeIsCryptographic);
    this._updateSecurityState(
        securityState, explanations, insecureContentStatus,
        schemeIsCryptographic);
  },

  selectAndSwitchToMainView: function() {
    // The sidebar element will trigger displaying the main view. Rather than
    // making a redundant call to display the main view, we rely on this.
  ...
  showOrigin: function(origin) {
    ...
    if (!originState.originView)
      originState.originView =
          new WebInspector.SecurityOriginView(this, origin, originState);
  ...
  _processRequest: function(request) {
    ...
    if (!origin) {
      // We don't handle resources like data: URIs. Most of them don't affect
      // the lock icon.
      return;
    }

    var securityState =
        /** @type {!SecurityAgent.SecurityState} */ (request.securityState());

    if (request.mixedContentType ===
            NetworkAgent.RequestMixedContentType.Blockable &&
        this._ranInsecureContentStyle)
      securityState = this._ranInsecureContentStyle;
    else if (
        request.mixedContentType ===
            NetworkAgent.RequestMixedContentType.OptionallyBlockable &&
    ...
    if (this._origins.has(origin)) {
      ...
      originState.securityState =
          this._securityStateMin(oldSecurityState, securityState);
      ...
      // TODO(lgarron): Store a (deduplicated) list of different security
  ...
  _updateFilterRequestCounts: function(request) {
    ...
    if (!this._filterRequestCounts.has(filterKey))
      ...
      this._filterRequestCounts.set(
          filterKey, this._filterRequestCounts.get(filterKey) + 1);
  ...
  _securityStateMin: function(stateA, stateB) {
    return WebInspector.SecurityModel.SecurityStateComparator(stateA, stateB) <
            0 ?
        stateA :
        stateB;
  },
  ...
  targetAdded: function(target) {
    ...
    var resourceTreeModel =
        WebInspector.ResourceTreeModel.fromTarget(this._target);
    if (resourceTreeModel) {
      ...
      resourceTreeModel.addEventListener(
          ...
          this._onInterstitialShown, this);
      resourceTreeModel.addEventListener(
    ...
    securityModel.addEventListener(
        WebInspector.SecurityModel.Events.SecurityStateChanged,
 ...
 */
                                       WebInspector.SecurityPanel._instance =
    function() {
 ...
 * @return {!Element}
 */
    WebInspector.SecurityPanel.createCertificateViewerButton =
        function(text, panel) {
  /**
   * @param {!Event} e
   */
  ...
  return createTextButton(
      text, showCertificateViewer, 'security-certificate-button');
}
 ...
 */
  ...
  function showCertificateViewer(e) {
    ...
    e.consume();
    WebInspector.multitargetNetworkManager.getCertificate(
        origin, certificateCallback);
  }
  ...
  return createTextButton(
      text, showCertificateViewer, 'security-certificate-button');
             ...
             * @param {!WebInspector.SecurityPanelSidebarTreeElement}
             * mainViewElement
             * @param {function(!WebInspector.SecurityPanel.Origin)}
             * showOriginInPanel
             */
            WebInspector.SecurityPanelSidebarTree =
                function(mainViewElement, showOriginInPanel) {
  ...
  /** @type {!Map<!WebInspector.SecurityPanelSidebarTree.OriginGroupName,
   * !TreeElement>} */
  this._originGroups = new Map();

  for (var key in WebInspector.SecurityPanelSidebarTree.OriginGroupName) {
    var originGroupName =
        WebInspector.SecurityPanelSidebarTree.OriginGroupName[key];
  ...
  // This message will be removed by clearOrigins() during the first new page
  // load after the panel was opened.
  var mainViewReloadMessage =
      new TreeElement(WebInspector.UIString('Reload to view details'));
  mainViewReloadMessage.selectable = false;
  mainViewReloadMessage.listItemElement.classList.add(
      'security-main-view-reload-message');
  this._originGroups
      .get(WebInspector.SecurityPanelSidebarTree.OriginGroupName.MainOrigin)
  ...
  addOrigin: function(origin, securityState) {
    var originElement = new WebInspector.SecurityPanelSidebarTreeElement(
        origin, this._showOriginInPanel.bind(this, origin),
  ...
  updateOrigin: function(origin, securityState) {
    var originElement =
        /** @type {!WebInspector.SecurityPanelSidebarTreeElement} */ (
    ...
    if (origin === this._mainOrigin) {
      newParent = this._originGroups.get(
          WebInspector.SecurityPanelSidebarTree.OriginGroupName.MainOrigin);
    } else {
      switch (securityState) {
        case SecurityAgent.SecurityState.Secure:
          newParent = this._originGroups.get(
              WebInspector.SecurityPanelSidebarTree.OriginGroupName.Secure);
          break;
        case SecurityAgent.SecurityState.Unknown:
          newParent = this._originGroups.get(
              WebInspector.SecurityPanelSidebarTree.OriginGroupName.Unknown);
          break;
        default:
          newParent = this._originGroups.get(
  ...
  __proto__: TreeOutlineInShadow.prototype
                                 ...
                                 /**
                                  * A mapping from Javascript key IDs to names
                                  * (sidebar section titles).
                                  * Note: The names are used as keys into a map,
                                  * so they must be distinct from each other.
                                  * @enum {string}
                                  */
                                 WebInspector.SecurityPanelSidebarTree
                                     .OriginGroupName = {
  ...
  Unknown: WebInspector.UIString('Unknown / Canceled')
                    ...
                    function(text, selectCallback, className, cssPrefix) {
  ...
  setSecurityState: function(newSecurityState) {
    if (this._securityState)
      this._iconElement.classList.remove(
          this._cssPrefix + '-' + this._securityState);
 ...
 */
                                     WebInspector
                                         .SecurityPanelSidebarTreeElement
                                         .SecurityStateComparator =
                        function(a, b) {
  return WebInspector.SecurityModel.SecurityStateComparator(
      a.securityState(), b.securityState());
}
  ...
  this._summarySection =
      this.contentElement.createChild('div', 'security-summary');
  ...
  this._summarySection.createChild('div', 'security-summary-section-title')
      .textContent = WebInspector.UIString('Security Overview');
  ...
  this._summaryText =
      this._summarySection.createChild('div', 'security-summary-text');
}
  ...
  _addExplanation: function(parent, explanation) {
    ...
    explanationSection.classList.add(
        'security-explanation-' + explanation.securityState);

    explanationSection.createChild('div', 'security-property')
        .classList.add('security-property-' + explanation.securityState);
    var text =
        explanationSection.createChild('div', 'security-explanation-text');
    text.createChild('div', 'security-explanation-title').textContent =
        explanation.summary;
  ...
  updateSecurityState: function(
    ...
    this._summarySection.classList.remove(
        'security-summary-' + this._securityState);

    // Add new state.
    this._securityState = newSecurityState;
    this._summarySection.classList.add(
        'security-summary-' + this._securityState);
    var summaryExplanationStrings = {
      'unknown': WebInspector.UIString('The security of this page is unknown.'),
      'insecure':
          WebInspector.UIString('This page is insecure (broken HTTPS).'),
    ...
    this._panel.setRanInsecureContentStyle(
        insecureContentStatus.ranInsecureContentStyle);
    ...
    if (this._insecureContentStatus &&
      ...
      if (this._insecureContentStatus.ranMixedContent)
        this._addMixedContentExplanation(
            this._securityExplanationsMain,
            this._insecureContentStatus.ranInsecureContentStyle,
  ...
  _addMixedContentExplanation: function(
    ...
    if (filterRequestCount > 0) {
      ...
      // Network instrumentation might not have been enabled for the page load,
      // so the security panel does not necessarily know a count of individual
      // mixed requests at this point. Point the user at the Network Panel which
      // prompts them to refresh.
      requestsAnchor.textContent =
          WebInspector.UIString('View requests in Network Panel');
    }
    requestsAnchor.href = '';
      ...
      this._addExplanation(
          ...
          /** @type {!SecurityAgent.SecurityStateExplanation} */ ({
            'securityState':
                this._insecureContentStatus.ranInsecureContentStyle,
            'summary':
                WebInspector.UIString('Active content with certificate errors'),
            'description': WebInspector.UIString(
  ...
  __proto__: WebInspector.VBox.prototype
                                                                      ...
                                                                      WebInspector
                                                                          .SecurityOriginView =
                                function(panel, origin, originState) {
  this._panel = panel;
  ...
  this._originLockIcon.classList.add(
      'security-property-' + originState.securityState);
  ...
  originNetworkLink.addEventListener(
      'click', showOriginRequestsInNetworkPanel, false);


  if (originState.securityDetails) {
    var connectionSection =
        this.element.createChild('div', 'origin-view-section');
    connectionSection.createChild('div', 'origin-view-section-title')
        .textContent = WebInspector.UIString('Connection');
    ...
    if (originState.securityDetails.keyExchangeGroup)
      table.addRow(
          'Key Exchange Group', originState.securityDetails.keyExchangeGroup);
    table.addRow(
        'Cipher Suite', originState.securityDetails.cipher +
            (originState.securityDetails.mac ?
                 ' with ' + originState.securityDetails.mac :
                 ''));

    // Create the certificate section outside the callback, so that it appears
    // in the right place.
    var certificateSection =
        this.element.createChild('div', 'origin-view-section');
    certificateSection.createChild('div', 'origin-view-section-title')
        .textContent = WebInspector.UIString('Certificate');

    if (originState.securityDetails.signedCertificateTimestampList.length) {
      // Create the Certificate Transparency section outside the callback, so
    ...
    var validFromString =
        new Date(1000 * originState.securityDetails.validFrom).toUTCString();
    var validUntilString =
        new Date(1000 * originState.securityDetails.validTo).toUTCString();

    table = new WebInspector.SecurityDetailsTable();
    certificateSection.appendChild(table.element());
    table.addRow(
        WebInspector.UIString('Subject'),
        originState.securityDetails.subjectName);
    ...
    table.addRow(
        WebInspector.UIString('Issuer'), originState.securityDetails.issuer);
    table.addRow(
        '',
        WebInspector.SecurityPanel.createCertificateViewerButton2(
    ...
    for (var i = 0;
         i < originState.securityDetails.signedCertificateTimestampList.length;
         i++) {
    ...
    for (var i = 0;
      ...
      sctTable.addRow(
          WebInspector.UIString('Log ID'), sct.logId.replace(/(.{2})/g, '$1 '));
      ...
      sctTable.addRow(
          WebInspector.UIString('Issued At'),
          new Date(sct.timestamp).toUTCString());
      sctTable.addRow(
          WebInspector.UIString('Hash Algorithm'), sct.hashAlgorithm);
      sctTable.addRow(
          WebInspector.UIString('Signature Algorithm'), sct.signatureAlgorithm);
      sctTable.addRow(
          WebInspector.UIString('Signature Data'),
          sct.signatureData.replace(/(.{2})/g, '$1 '));
    }

    // Add link to toggle between displaying of the summary of the SCT(s) and
    // the detailed SCT(s).
    var toggleSctsDetailsLink = sctSection.createChild('div', 'link');
    toggleSctsDetailsLink.classList.add('sct-toggle');
    toggleSctsDetailsLink.textContent =
        WebInspector.UIString('Show full details');
    function toggleSctDetailsDisplay() {
      ...
      if (isDetailsShown)
        ...
        toggleSctsDetailsLink.textContent =
            WebInspector.UIString('Hide full details');
      sctSummaryTable.element().classList.toggle('hidden');
      sctTableWrapper.classList.toggle('hidden');
    }
    toggleSctsDetailsLink.addEventListener(
        'click', toggleSctDetailsDisplay, false);
    ...
    noteSection.createChild('div').textContent = WebInspector.UIString(
        'The security details above are from the first inspected response.');
  } else if (
      originState.securityState !== SecurityAgent.SecurityState.Unknown) {
    var notSecureSection =
        this.element.createChild('div', 'origin-view-section');
    notSecureSection.createChild('div', 'origin-view-section-title')
        .textContent = WebInspector.UIString('Not Secure');
    ...
    noInfoSection.createChild('div').textContent = WebInspector.UIString(
                                ...
                                WebInspector.SecurityOriginView.prototype =
                                    {
                                      ...
                                      _createSanDiv: function(sanList) {
                                        ...
                                        if (sanList.length === 0) {
                                          sanDiv.textContent =
                                              WebInspector.UIString('(N/A)');
                                          sanDiv.classList.add('empty-san');
                                        } else {
                                          var truncatedNumToShow = 2;
                                          var listIsTruncated = sanList.length >
                                              truncatedNumToShow;
                                          for (var i = 0; i < sanList.length;
                                               i++) {
                                            var span = sanDiv.createChild(
                                                'span', 'san-entry');
                                          ...
                                          if (listIsTruncated) {
                                            var truncatedSANToggle =
                                                sanDiv.createChild(
                                                    'div', 'link');
                                            truncatedSANToggle.href = '';

                                            function toggleSANTruncation() {
                                              if (sanDiv.classList.contains(
                                                      'truncated-san')) {
                                                sanDiv.classList.remove(
                                                    'truncated-san')
                                                truncatedSANToggle.textContent =
                                                    WebInspector.UIString(
                                                        'Show less');
                                              } else {
                                                sanDiv.classList.add(
                                                    'truncated-san');
                                                truncatedSANToggle.textContent =
                                                    WebInspector.UIString(
                                                        'Show more (%d total)',
                                            ...
                                            truncatedSANToggle.addEventListener(
                                                'click', toggleSANTruncation,
                                                false);
                                            toggleSANTruncation();
                                          }
                                        }
                                      ...
                                      /**
                                       * @param {!SecurityAgent.SecurityState}
                                       * newSecurityState
                                       */
                                      setSecurityState: function(
                                          newSecurityState) {
                                        for (var className of
                                                 Array.prototype.slice.call(
                                                     this._originLockIcon
                                                         .classList)) {
                                          if (className.startsWith(
                                                  'security-property-'))
                                            this._originLockIcon.classList
                                                .remove(className);
                                        }
                                     ...
                                     * @constructor
                                     */
                                    WebInspector.SecurityDetailsTable =
                                        function() {
                                      ...
                                      addRow: function(key, value) {
                                        var row = this._element.createChild(
                                            'div', 'details-table-row');
                                        row.createChild('div').textContent =
                                            key;
