 ...
 */
                                   WebInspector.SecurityModel.fromTarget =
        function(target) {
  var model = /** @type {?WebInspector.SecurityModel} */ (
      target.model(WebInspector.SecurityModel));
  if (!model)
 ...
 * @return {number}
 */
        WebInspector.SecurityModel.SecurityStateComparator =
            function(a, b) {
  var securityStateMap;
  if (WebInspector.SecurityModel._symbolicToNumericSecurityState) {
    securityStateMap =
        WebInspector.SecurityModel._symbolicToNumericSecurityState;
  } else {
    ...
    var ordering = [
      ...
      // Unknown is max so that failed/cancelled requests don't overwrite the
      // origin security state for successful requests,
      // and so that failed/cancelled requests appear at the bottom of the
      // origins list.
  ...
  return aScore - bScore;
             ...
             * @param {!Array<!SecurityAgent.SecurityStateExplanation>}
             * explanations
             * @param {?SecurityAgent.InsecureContentStatus}
             * insecureContentStatus
             * @param {boolean} schemeIsCryptographic
             */
            WebInspector.PageSecurityState =
                function(
  ...
  this.schemeIsCryptographic = schemeIsCryptographic;
                 ...
                 */
                WebInspector.SecurityDispatcher =
                    function(model) {
  this._model = model;
}
