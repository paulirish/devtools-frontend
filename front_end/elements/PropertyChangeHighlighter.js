 ...
 */
WebInspector.PropertyChangeHighlighter =
    function(ssp, cssModel, styleSheetId, range) {
  ...
  perform: function() {
    ...
    for (var section of this._styleSidebarPane.allSections()) {
      ...
      var isMatchingRule = parentRule && parentRule.selectorRange() &&
          this._range.compareTo(parentRule.selectorRange()) === 0;
    ...
    while (!highlightElement && treeElement) {
      if (treeElement.property.range &&
          this._checkRanges(treeElement.property.range, this._range)) {
    ...
    if (highlightElement) {
      highlightElement.animate(
          [
            {
              offset: 0,
              backgroundColor: 'rgba(158, 54, 153, 1)',
              color: 'white'
            },
            {
              offset: 0.5,
            ...
            {
              offset: 0.9,
              backgroundColor: 'rgba(158, 54, 153, 0)',
              color: 'initial'
            },
  ...
  _checkRanges: function(outterRange, innerRange) {
    var startsBefore = outterRange.startLine < innerRange.startLine ||
        (outterRange.startLine === innerRange.startLine &&
         outterRange.startColumn <= innerRange.startColumn);
    // SSP might be outdated, so inner range will a bit bigger than outter.
    // E.g.; "padding-left: 9px" -> "padding-left: 10px"
    var eps = 5;
    var endsAfter = outterRange.endLine > innerRange.endLine ||
        (outterRange.endLine === innerRange.endLine &&
         outterRange.endColumn + eps >= innerRange.endColumn);
    return startsBefore && endsAfter;
                      ...
                      */
                     WebInspector.PropertyRevealHighlighter =
        function(ssp, cssProperty) {
  this._styleSidebarPane = ssp;
  ...
  perform: function() {
    ...
    for (var section of this._styleSidebarPane.allSections()) {
      for (var treeElement = section.propertiesTreeOutline.firstChild();
