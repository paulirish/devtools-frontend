 ...
 */
WebInspector.ElementsBreadcrumbs =
    function() {
  ...
  this.crumbsElement.addEventListener(
      'mousemove', this._mouseMovedInCrumbs.bind(this), false);
  this.crumbsElement.addEventListener(
      'mouseleave', this._mouseMovedOutOfCrumbs.bind(this), false);
  ...
  _mouseMovedInCrumbs: function(event) {
    ...
    var node = /** @type {?WebInspector.DOMNode} */ (
        crumbElement ? crumbElement[this._nodeSymbol] : null);
    if (node)
      node.highlight();
  },
    ...
    for (var current = currentDOMNode; current; current = current.parentNode) {
      ...
      switch (current.nodeType()) {
        ...
        case Node.DOCUMENT_FRAGMENT_NODE:
          crumbTitle = current.shadowRootType() ?
              '#shadow-root' :
              current.nodeNameInCorrectCase();
          break;
  ...
  updateSizes: function(focusedCrumb) {
    ...
    function crumbsAreSmallerThanContainer() {
      ...
      for (var i = 0; i < crumbs.childNodes.length; ++i) {
        ...
        totalSize += crumb.classList.contains('compact') ? compactSizes[i] :
                                                           normalSizes[i];
      }
    ...
    function makeCrumbsSmaller(shrinkingFunction, direction) {
      ...
      var significantIndex =
          significantCrumb === selectedCrumb ? selectedIndex : focusedIndex;
      ...
      // Shrink crumbs one at a time by applying the shrinkingFunction until the
      // crumbs
      // fit in the container or we run out of crumbs to shrink.
      if (direction) {
        // Crumbs are shrunk on only one side (based on direction) of the
        // signifcant crumb.
        ...
        // Crumbs are shrunk in order of descending distance from the signifcant
        // crumb,
        // with a tie going to child crumbs.
        var startIndex = 0;
        var endIndex = crumbs.childNodes.length - 1;
        while (startIndex !== significantIndex ||
               endIndex !== significantIndex) {
    ...
    if (!focusedCrumb) {
      // When not focused on a crumb we can be biased and collapse less
      // important
    ...
    // Collapse the selected crumb as a last resort. Pass true to prevent
    // coalescing.
