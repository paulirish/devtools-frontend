 ...
 */
WebInspector.TextEditorAutocompleteController =
    function(textEditor, codeMirror, config) {
...
WebInspector.TextEditorAutocompleteController.prototype = {
  ...
  _substituteRange: function(lineNumber, columnNumber) {
    ...
    if (!range && this._config.isWordChar)
      range = this._textEditor.wordRangeForCursorPosition(
          lineNumber, columnNumber, this._config.isWordChar);
    return range;
  },
  ...
  _wordsWithPrefix: function(prefixRange, substituteRange) {
    var external = this._config.suggestionsCallback ?
        this._config.suggestionsCallback(prefixRange, substituteRange) :
        null;
    if (external)
      return external;
    ...
    var completions =
        this._dictionary.wordsWithPrefix(this._textEditor.text(prefixRange));
  ...
  _changes: function(codeMirror, changes) {
    ...
    if (this._dictionary && this._updatedLines) {
      ...
      for (var changeIndex = 0; changeIndex < changes.length; ++changeIndex) {
        ...
        var editInfo = WebInspector.CodeMirrorUtils.changeObjectToEditOperation(
            changeObject);
        for (var i = editInfo.newRange.startLine;
             i <= editInfo.newRange.endLine; ++i)
    ...
    for (var changeIndex = 0; changeIndex < changes.length; ++changeIndex) {
      var changeObject = changes[changeIndex];
      if (changeObject.origin === '+input' && changeObject.text.length === 1 &&
      ...
      if (this._suggestBox && changeObject.origin === '+delete' &&
    ...
    if (singleCharInput && this._hintMarker)
      this._hintElement.textContent =
          this._hintElement.textContent.substring(1);

    if (singleCharDelete && this._hintMarker && this._lastPrefix) {
      this._hintElement.textContent =
          this._lastPrefix.charAt(this._lastPrefix.length - 1) +
          this._hintElement.textContent;
      this._lastPrefix =
          this._lastPrefix.substring(0, this._lastPrefix.length - 1);
    }
  ...
  _blur: function() { this.clearAutocomplete(); },
  ...
  _validateSelectionsContexts: function(mainSelection) {
    ...
    for (var i = 0; i < selections.length; ++i) {
      var wordRange =
          this._substituteRange(selections[i].head.line, selections[i].head.ch);
      if (!wordRange)
        return false;
  ...
  autocomplete: function() {
    ...
    this._wordsWithPrefix(prefixRange, substituteRange)
        .then(wordsAcquired.bind(this));
    ...
    function wordsAcquired(wordsWithPrefix) {
      if (!wordsWithPrefix.length || (wordsWithPrefix.length === 1 &&
                                      prefix === wordsWithPrefix[0].title) ||
      ...
      if (!this._suggestBox)
        this._suggestBox =
            new WebInspector.SuggestBox(this, 20, this._config.captureEnter);
      ...
      this._suggestBox.updateSuggestions(
          this._anchorBox, wordsWithPrefix, 0, !this._isCursorAtEndOfLine(),
          prefix);
  ...
  _addHintMarker: function(hint) {
    ...
    this._hintElement.textContent =
        hint.substring(prefix.length).split('\n')[0];
    var cursor = this._codeMirror.getCursor('to');
    this._hintMarker = this._textEditor.addBookmark(
        cursor.line, cursor.ch, this._hintElement,
        WebInspector.TextEditorAutocompleteController.HintBookmark, true);
  },
  ...
  acceptSuggestion: function() {
    if (this._prefixRange.endColumn - this._prefixRange.startColumn ===
    ...
    for (var i = selections.length - 1; i >= 0; --i) {
      ...
      this._codeMirror.replaceRange(
          this._currentSuggestion, start, end, '+autocomplete');
    }
  },
  ...
  _onScroll: function() {
    ...
    var topmostLineNumber =
        this._codeMirror.lineAtHeight(scrollInfo.top, 'local');
  ...
  _onCursorActivity: function() {
    ...
    if (cursor.line !== this._prefixRange.startLine ||
        cursor.ch > this._prefixRange.endColumn + 1 ||
