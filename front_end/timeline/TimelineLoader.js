...
WebInspector.TimelineLoader =
  ...
  /** @type {number} */
  this._totalSize;
  this._jsonTokenizer = new WebInspector.TextUtils.BalancedJSONTokenizer(
      this._writeBalancedJSON.bind(this), true);
}
 ...
 */
    WebInspector.TimelineLoader.loadFromFile =
        function(model, file, delegate) {
 ...
 * @return {!WebInspector.TimelineLoader}
 */
        WebInspector.TimelineLoader.loadFromURL =
            function(model, url, delegate) {
 ...
 */
WebInspector.TimelineLoader._createFileReader =
    function(file, delegate) {
  ...
  write: function(chunk) {
    ...
    if (this._state === WebInspector.TimelineLoader.State.LookingForEvents) {
      ...
      if (pos === -1)
        return;
      chunk = this._buffer.slice(pos + objectName.length) this._state =
          WebInspector.TimelineLoader.State.ReadingEvents;
    }
  ...
  onError: function(reader, event) {
    switch (event.target.error.name) {
      ...
      case 'NotReadableError':
        this._reportErrorAndCancelLoading(WebInspector.UIString(
            'File "%s" is not readable', reader.fileName()));
        break;
      case 'AbortError':
        break;
      default:
        this._reportErrorAndCancelLoading(WebInspector.UIString(
            ...
            reader.fileName()));
                                   ...
                                   * @constructor
                                   * @implements
                                   * {WebInspector.OutputStreamDelegate}
                                   */
                                  WebInspector.TracingTimelineSaver =
        function() {}
  ...
  onError: function(reader, event) {
    ...
    WebInspector.console.error(WebInspector.UIString(
        'Failed to save timeline: %s (%s, %s)', error.message, error.name,
        error.code));
  }
}
