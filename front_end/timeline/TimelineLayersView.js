 ...
 */
WebInspector.TimelineLayersView =
    function(model, showEventDetailsCallback) {
  ...
  this._rightSplitWidget =
      new WebInspector.SplitWidget(true, true, 'timelineLayersViewDetails');
  this._rightSplitWidget.element.classList.add(
  ...
  _jumpToPaintEvent: function(event) {
    var traceEvent =
        /** @type {!WebInspector.TracingModel.Event} */ (event.data);
  ...
  _update: function() {
    ...
    this._deferredLayerTree.resolve(
        tilesReadyBarrier.createCallback(onLayersReady));
    for (var i = 0; this._paints && i < this._paints.length; ++i)
      this._paints[i].loadSnapshot(tilesReadyBarrier.createCallback(
          onSnapshotLoaded.bind(this, this._paints[i])));
    tilesReadyBarrier.callWhenDone(onLayersAndTilesReady.bind(this));

    /**
     * @param {!WebInspector.LayerTreeBase} resolvedLayerTree
     */
    function onLayersReady(resolvedLayerTree) { layerTree = resolvedLayerTree; }
    ...
    function onSnapshotLoaded(paintEvent, rect, snapshot) {
      ...
      this._paintTiles.push({
        layerId: paintEvent.layerId(),
        rect: rect,
        snapshot: snapshot,
        traceEvent: paintEvent.event()
      });
    }
