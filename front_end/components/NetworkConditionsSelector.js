 ...
 * @constructor
 * @param
 * {function(!Array<!WebInspector.NetworkConditionsGroup>):!Array<?WebInspector.NetworkManager.Conditions>}
 * populateCallback
 * @param {function(number)} selectCallback
 */
WebInspector.NetworkConditionsSelector =
    function(populateCallback, selectCallback) {
  ...
  this._manager.addEventListener(
      WebInspector.MultitargetNetworkManager.Events.ConditionsChanged,
      this._conditionsChanged, this);
  this._populateOptions();
}

    /** @typedef {!{title: string, items:
       !Array<!WebInspector.NetworkManager.Conditions>}} */
 ...
 * @return {string}
 */
WebInspector.NetworkConditionsSelector._throughputText =
    function(throughput, plainText) {
  if (throughput < 0)
    return '';
  ...
  if (throughputInKbps < 1024 * 10)
    return WebInspector.UIString(
        '%.1f%sMb/s', throughputInKbps / 1024, delimiter);
  return WebInspector.UIString(
      '%d%sMb/s', (throughputInKbps / 1024) | 0, delimiter);
}
 ...
 */
WebInspector.NetworkConditionsSelector._conditionsTitle =
    function(conditions, plainText) {
  var downloadInKbps = conditions.download / (1024 / 8);
  var uploadInKbps = conditions.upload / (1024 / 8);
  var isThrottling =
      (downloadInKbps >= 0) || (uploadInKbps >= 0) || (conditions.latency > 0);
  var conditionTitle = WebInspector.UIString(conditions.title);
  if (!isThrottling)
    return {text: conditionTitle, title: conditionTitle};

  var downloadText = WebInspector.NetworkConditionsSelector._throughputText(
      conditions.download, plainText);
  var uploadText = WebInspector.NetworkConditionsSelector._throughputText(
      conditions.upload, plainText);
  var pattern =
      plainText ? '%s (%dms, %s, %s)' : '%s (%dms RTT, %s\u2b07, %s\u2b06)';
  var title = WebInspector.UIString(
      pattern, conditionTitle, conditions.latency, downloadText, uploadText);
  return {
    text: title,
  ...
  _populateOptions: function() {
    ...
    this._options =
        this._populateCallback([customGroup, presetsGroup, disabledGroup]);
  ...
  _conditionsChanged: function() {
    ...
    for (var index = 0; index < this._options.length; ++index) {
      ...
      if (option && option.download === value.download &&
          option.upload === value.upload && option.latency === value.latency &&
  ...
  function populate(groups) {
    ...
    for (var i = 0; i < groups.length; ++i) {
      ...
      if (!i) {
        groupElement.appendChild(new Option(
            WebInspector.UIString('Add\u2026'),
            WebInspector.UIString('Add\u2026')));
        options.push(null);
      }
      for (var conditions of group.items) {
        var title = WebInspector.NetworkConditionsSelector._conditionsTitle(
            conditions, true);
 ...
 */
  ...
  function appendItems(contextMenu) {
    for (var index = 0; index < options.length; ++index) {
      ...
      if (!conditions)
        ...
        contextMenu.appendCheckboxItem(
            WebInspector.NetworkConditionsSelector
                ._conditionsTitle(conditions, true)
                .text,
    ...
    contextMenu.appendItem(
        ...
        selector.revealAndUpdate.bind(selector));
  }
 ...
 */
  ...
  var checkbox = new WebInspector.ToolbarCheckbox(
      ...
      WebInspector.UIString('Force disconnected from network'), undefined,
      forceOffline);
  WebInspector.multitargetNetworkManager.addEventListener(
      WebInspector.MultitargetNetworkManager.Events.ConditionsChanged,
      networkConditionsChanged);
  checkbox.setChecked(
      WebInspector.multitargetNetworkManager.networkConditions() ===
      WebInspector.NetworkManager.OfflineConditions);
  ...
  function forceOffline() {
    if (checkbox.checked()) {
      lastNetworkConditions =
          WebInspector.multitargetNetworkManager.networkConditions();
      WebInspector.multitargetNetworkManager.setNetworkConditions(
          WebInspector.NetworkManager.OfflineConditions);
    } else {
      WebInspector.multitargetNetworkManager.setNetworkConditions(
          lastNetworkConditions);
  ...
  function networkConditionsChanged() {
    ...
    checkbox.setChecked(
        conditions === WebInspector.NetworkManager.OfflineConditions);
  }
  return checkbox;
}
                 ...
                 */
                WebInspector.NetworkConditionsSettingsTab =
                    function() {
  ...
  this.contentElement.createChild('div', 'header').textContent =
      WebInspector.UIString('Network Throttling Profiles');

  var addButton = createTextButton(
      WebInspector.UIString('Add custom profile...'),
  ...
  _addButtonClicked: function() {
    this._list.addNewItem(
        this._customSetting.get().length,
        {title: '', download: -1, upload: -1, latency: 0});
  },
  ...
  renderItem: function(item, editable) {
    var conditions =
        /** @type {!WebInspector.NetworkManager.Conditions} */ (item);
    var element = createElementWithClass('div', 'conditions-list-item');
    var title = element.createChild(
        'div', 'conditions-list-text conditions-list-title');
    ...
    element.createChild('div', 'conditions-list-separator');
    element.createChild('div', 'conditions-list-text').textContent =
        WebInspector.UIString('%dms', conditions.latency);
    return element;
  },
  ...
  beginEdit: function(item) {
    ...
    editor.control('download').value = conditions.download <= 0 ?
        '' :
        String(conditions.download / (1024 / 8));
    editor.control('upload').value =
        conditions.upload <= 0 ? '' : String(conditions.upload / (1024 / 8));
    editor.control('latency').value =
        conditions.latency ? String(conditions.latency) : '';
    return editor;
  },
  ...
  _createEditor: function() {
    ...
    titles.createChild('div', 'conditions-list-text conditions-list-title')
        .textContent = WebInspector.UIString('Profile Name');
    titles.createChild(
        'div', 'conditions-list-separator conditions-list-separator-invisible');
    titles.createChild('div', 'conditions-list-text').textContent =
        WebInspector.UIString('Download');
    titles.createChild(
        'div', 'conditions-list-separator conditions-list-separator-invisible');
    titles.createChild('div', 'conditions-list-text').textContent =
        WebInspector.UIString('Upload');
    titles.createChild(
        'div', 'conditions-list-separator conditions-list-separator-invisible');
    titles.createChild('div', 'conditions-list-text').textContent =
    ...
    cell.appendChild(editor.createInput(
        'download', 'text', WebInspector.UIString('kb/s'),
        throughputValidator));
    cell.createChild('div', 'conditions-edit-optional').textContent =
        WebInspector.UIString('optional');
    fields.createChild(
        'div', 'conditions-list-separator conditions-list-separator-invisible');

    cell = fields.createChild('div', 'conditions-list-text');
    cell.appendChild(editor.createInput(
        'upload', 'text', WebInspector.UIString('kb/s'), throughputValidator));
    cell.createChild('div', 'conditions-edit-optional').textContent =
        WebInspector.UIString('optional');
    fields.createChild(
        'div', 'conditions-list-separator conditions-list-separator-invisible');
    ...
    cell = fields.createChild('div', 'conditions-list-text');
    cell.appendChild(editor.createInput(
        'latency', 'text', WebInspector.UIString('ms'), latencyValidator));
    cell.createChild('div', 'conditions-edit-optional').textContent =
        WebInspector.UIString('optional');
    ...
    function throughputValidator(item, index, input) {
      var value = input.value.trim();
      return !value || (/^[\d]+(\.\d+)?|\.\d+$/.test(value) && value >= 0 &&
                        value <= 10000000);
    }
    ...
    function latencyValidator(item, index, input) {
      var value = input.value.trim();
      return !value ||
          (/^[\d]+$/.test(value) && value >= 0 && value <= 1000000);
    }
  },
  ...
  __proto__: WebInspector.VBox.prototype
                                      ...
                                      */
                                     WebInspector
                                         .NetworkConditionsActionDelegate =
                        function() {}

                        WebInspector.NetworkConditionsActionDelegate
                            .prototype = {
  /**
     * @override
