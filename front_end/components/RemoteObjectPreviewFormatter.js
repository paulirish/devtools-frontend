 ...
 * @constructor
 */
WebInspector.RemoteObjectPreviewFormatter =
    function() {}
  ...
  appendObjectPreview: function(parentElement, preview) {
    ...
    if (preview.type !== 'object' || preview.subtype === 'null') {
      parentElement.appendChild(this.renderPropertyPreview(
          preview.type, preview.subtype, description));
      return;
    }
    if (description && preview.subtype !== 'array' &&
        preview.subtype !== 'typedarray') {
      var text = preview.subtype ?
          description :
          this._abbreviateFullQualifiedClassName(description);
  ...
  _appendPropertiesPreview: function(parentElement, preview) {
    var isArray =
        preview.subtype === 'array' || preview.subtype === 'typedarray';
    ...
    for (var i = 0; i < properties.length; ++i) {
      ...
      if (!isArray || name !== String(i) || i >= arrayLength) {
        if (/^\s|\s$|^$|\n/.test(name))
          parentElement.createChild('span', 'name')
              .createTextChildren('"', name.replace(/\n/g, '\u21B5'), '"');
        else
      ...
      parentElement.appendChild(
          this._renderPropertyPreviewOrAccessor([property]));
    }
    if (preview.overflow)
  ...
  renderPropertyPreview: function(type, subtype, description) {
    var span =
        createElementWithClass('span', 'object-value-' + (subtype || type));
