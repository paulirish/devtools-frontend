...
WebInspector.ExecutionContextSelector =
    function(targetManager, context) {
  ...
  context.addFlavorChangeListener(
      WebInspector.Target, this._targetChanged, this);
  ...
  targetManager.addModelListener(
      WebInspector.RuntimeModel,
      WebInspector.RuntimeModel.Events.ExecutionContextOrderChanged,
  ...
  targetRemoved: function(target) {
    var currentExecutionContext =
        this._context.flavor(WebInspector.ExecutionContext);
    if (currentExecutionContext && currentExecutionContext.target() === target)
      this._currentExecutionContextGone();

    var targets =
        this._targetManager.targets(WebInspector.Target.Capability.JS);
  ...
  _targetChanged: function(event) {
    ...
    this._context.setFlavor(
        WebInspector.ExecutionContext, newContext || executionContexts[0]);
  ...
  _shouldSwitchToContext: function(executionContext) {
    if (this._lastSelectedContextId &&
        this._lastSelectedContextId ===
            this._contextPersistentId(executionContext))
      return true;
    if (!this._lastSelectedContextId &&
        this._isDefaultContext(executionContext))
      return true;
    return false;
  },
  ...
  _onExecutionContextCreated: function(event) {
    this._switchContextIfNecessary(
  ...
  _currentExecutionContextGone: function() {
    ...
    this._ignoreContextChanged = false;
                      ...
                      * @param {boolean} force
                      * @param {function(!Array.<string>, number=)}
  ...
  WebInspector.ExecutionContextSelector.completionsForTextInCurrentContext(
      expressionRange.toString(), wordRange.toString(), force,
      completionsReadyCallback);
}
        /**
         * @param {string} text
         ...
         */
        WebInspector.ExecutionContextSelector
            .completionsForTextInCurrentContext = function(
  ...
  for (index = text.length - 1; index >= 0; index--) {
    // Pass less stop characters to rangeOfWord so the range will be a more
    // complete expression.
