...
/**
* @typedef {Array<{object: !WebInspector.RemoteObject, eventListeners:
* ?Array<!WebInspector.EventListener>, frameworkEventListeners:
* ?{eventListeners: ?Array<!WebInspector.EventListener>, internalHandlers:
* ?WebInspector.RemoteArray}, isInternal: ?Array<boolean>}>}
*/
 ...
 */
WebInspector.EventListenersView =
    function(element, changeCallback) {
  ...
  this._treeOutline.setComparator(
      WebInspector.EventListenersTreeElement.comparator);
  this._treeOutline.element.classList.add('monospace');
  this._element.appendChild(this._treeOutline.element) this._emptyHolder =
  ...
  _addObject: function(object) {
    ...
    promises.push(
        WebInspector.EventListener.frameworkEventListeners(object).then(
            storeFrameworkEventListenersObject));
    return Promise.all(promises)
        .then(markInternalEventListeners)
        .then(addEventListeners.bind(this));

    /**
     * @param {?Array<!WebInspector.EventListener>} result
     */
    function storeEventListeners(result) { eventListeners = result; }
    ...
    function markInternalEventListeners() {
      ...
      return frameworkEventListenersObject.internalHandlers.object()
          .callFunctionJSONPromise(
              isInternalEventListener, eventListeners.map(handlerArgument))
          .then(setIsInternal);
    ...
    function addEventListeners() {
      ...
      this._addObjectEventListeners(
          object, frameworkEventListenersObject.eventListeners);
    }
  },
  ...
  _getOrCreateTreeElementForType: function(type) {
    ...
    if (!treeItem) {
      treeItem = new WebInspector.EventListenersTreeElement(
          type, this._linkifier, this._changeCallback);
  ...
  _eventListenersArrivedForTest: function() {}
                      ...
                      * @param {function()} changeCallback
                      */
                     WebInspector.EventListenersTreeElement =
        function(type, linkifier, changeCallback) {
 ...
 */
  ...
  addObjectEventListener: function(eventListener, object) {
    var treeElement = new WebInspector.ObjectEventListenerBar(
        eventListener, object, this._linkifier, this._changeCallback);
  ...
  __proto__: TreeElement.prototype
                              ...
                              * @constructor
                              * @extends {TreeElement}
                              * @param {!WebInspector.EventListener}
                              * eventListener
                              * @param {!WebInspector.RemoteObject} object
                              * @param {!WebInspector.Linkifier} linkifier
                              * @param {function()} changeCallback
                              */
                             WebInspector.ObjectEventListenerBar =
                function(eventListener, object, linkifier, changeCallback) {
  ...
  _setTitle: function(object, linkifier) {
    ...
    var subtitle = this.listItemElement.createChild(
        'span', 'event-listener-tree-subtitle');
    subtitle.appendChild(linkifier.linkifyRawLocation(
        this._eventListener.location(), this._eventListener.sourceURL()));
    ...
    if (this._eventListener.isScrollBlockingType() &&
      ...
      passiveButton.title = WebInspector.UIString(
          'Toggle whether event listener is passive or blocking');
      passiveButton.addEventListener(
          'click', togglePassiveListener.bind(this), false);
  ...
  /**
     ...
     */
  eventListener: function() { return this._eventListener; },
