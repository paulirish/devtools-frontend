...
/** @typedef {{type: string, useCapture: boolean, passive: boolean, handler:
 * function()}} */
...
WebInspector.EventListener.frameworkEventListeners = function(object) {
  ...
  function createEventListeners(result) {
    ...
    for (var property of result.properties) {
      if (property.name === 'eventListeners' && property.value)
        promises.push(
            convertToEventListeners(property.value).then(storeEventListeners));
      if (property.name === 'internalHandlers' && property.value)
        promises.push(convertToInternalHandlers(property.value)
                          .then(storeInternalHandlers));
  ...
  function convertToEventListeners(pageEventListenersObject) {
    ...
    function toEventListener(listenerObject) {
      ...
      promises.push(
          listenerObject
              .callFunctionJSONPromise(truncatePageEventListener, undefined)
      ...
      /**
       * @param {!{type:string, useCapture: boolean, passive: boolean}}
       * truncatedListener
       */
      ...
      function createEventListener() {
        ...
        return new WebInspector.EventListener(
            handler._target, object, type, useCapture, passive, handler,
            originalHandler, location, removeFunctionObject, 'frameworkUser');
      }
    }
  }
  ...
  function toTargetFunction(functionObject) {
    return WebInspector.RemoteFunction.objectAsFunction(functionObject)
        .targetFunction();
  }
  ...
  */
  ...
  function frameworkEventListeners() {
    ...
    try {
      if (self.devtoolsFrameworkEventListeners &&
          isArrayLike(self.devtoolsFrameworkEventListeners))
        fetchers = fetchers.concat(self.devtoolsFrameworkEventListeners);
    } catch (e) {
      errorLines.push(
          'devtoolsFrameworkEventListeners call produced error: ' +
          toString(e));
    }
    ...
    for (var i = 0; i < fetchers.length; ++i) {
      try {
        ...
        if (fetcherResult.eventListeners &&
            isArrayLike(fetcherResult.eventListeners)) {
          eventListeners = eventListeners.concat(
              fetcherResult.eventListeners.map(checkEventListener)
                  .filter(nonEmptyObject));
        }
        if (fetcherResult.internalHandlers &&
            isArrayLike(fetcherResult.internalHandlers))
          internalHandlers = internalHandlers.concat(
              fetcherResult.internalHandlers.map(checkInternalHandler)
                  .filter(nonEmptyObject));
      } catch (e) {
    ...
    function checkEventListener(eventListener) {
      try {
        ...
        if (typeof useCapture !== 'boolean')
          errorString +=
              'event listener\'s useCapture isn\'t boolean or undefined, ';
        var passive = eventListener.passive;
        if (typeof passive !== 'boolean')
          errorString +=
              'event listener\'s passive isn\'t boolean or undefined, ';
      ...
      if (typeof data === 'function') {
        ...
        for (var type in events) {
          for (var key in events[type]) {
            ...
            if (typeof frameworkListener === 'object' ||
                typeof frameworkListener === 'function') {
              var listener = {
              ...
              listener.remove =
                  jQueryRemove.bind(node, frameworkListener.selector);
        ...
        for (var type in entryEvents) {
          ...
          for (var key in events) {
            if (typeof events[key] === 'function') {
              ...
              // We don't support removing for old version < 1.4 of jQuery
              // because it doesn't provide API for getting "selector".
              eventListeners.push(listener);
            }
          }
        }
