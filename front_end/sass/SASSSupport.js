 ...
 */
                           WebInspector.SASSSupport.parseSCSS =
  ...
  return WebInspector.formatterWorkerPool
      .runTask('parseSCSS', {content: content})
      .then(onParsed);
  ...
  function onParsed(event) {
    ...
    for (var i = 0; i < data.length; ++i) {
      ...
      var rule = new WebInspector.SASSSupport.Rule(
          document, selectors, range, properties);
      rules.push(rule);
    }
  ...
  function createTextNode(payload) {
    ...
    return new WebInspector.SASSSupport.TextNode(
        document, text.extract(range), range);
  }
  ...
  function createProperty(payload) {
    ...
    return new WebInspector.SASSSupport.Property(
        document, name, value, WebInspector.TextRange.fromObject(payload.range),
        payload.disabled);
  }
}
     ...
     */
    WebInspector.SASSSupport.ASTDocument =
        function(url, text) {
  this.url = url;
  this.text = text;
  this.edits = [];
  ...
  newText: function() {
    ...
    function sequentialOrder(edit1, edit2) {
      ...
      return range1.follows(range2) ? 1 : -1;
                         ...
                         /**
                          * @constructor
                          * @param {!WebInspector.SASSSupport.ASTDocument}
                          * document
                          */
                         WebInspector.SASSSupport.Node =
            function(document) {
  this.document = document;
             ...
             */
            WebInspector.SASSSupport.TextNode =
                function(document, text, range) {
  ...
  setText: function(newText) {
    ...
    this.document.edits.push(
        new WebInspector.SourceEdit(this.document.url, this.range, newText));
  },
  ...
  clone: function(document) {
    return new WebInspector.SASSSupport.TextNode(
        document, this.text, this.range.clone());
  },
  ...
  __proto__: WebInspector.SASSSupport.Node.prototype
                                  ...
                                  * {!WebInspector.SASSSupport.ASTDocument}
                                  * document
                                  * @param {!WebInspector.SASSSupport.TextNode}
                                  * name
                                  * @param {!WebInspector.SASSSupport.TextNode}
                                  * value
                                  * @param {!WebInspector.TextRange} range
                                  ...
                                  */
                                 WebInspector.SASSSupport.Property =
                    function(document, name, value, range, disabled) {
  WebInspector.SASSSupport.Node.call(this, document);
  this.name = name;
  this.value = value;
  this.range = range;
  ...
  this.disabled = disabled;
     ...
     */
                          clone: function(document) {
                            return new WebInspector.SASSSupport.Property(
                                document, this.name.clone(document),
                                this.value.clone(document), this.range.clone(),
                                this.disabled);
                          },

                          /**
                           * @param {function(!WebInspector.SASSSupport.Node)}
                           * callback
                           */
     ...
     */
                          ...
                          setDisabled: function(disabled) {
                            ...
                            var propertyText =
                                this.document.text.extract(this.range);
                            var endsWithSemicolon =
                                propertyText.slice(0, -2).trim().endsWith(';');
                          ...
                          remove: function() {
                            ...
                            var lineRange = new WebInspector.TextRange(
                                this.range.startLine, 0, this.range.endLine + 1,
                                0);
                            var oldRange;
                         ...
                         * @param {!WebInspector.SASSSupport.ASTDocument}
                         * document
                         * @param {!Array<!WebInspector.SASSSupport.TextNode>}
                         * selectors
                         * @param {!WebInspector.TextRange} styleRange
                         * @param {!Array<!WebInspector.SASSSupport.Property>}
                         * properties
                         */
                        WebInspector.SASSSupport.Rule =
                            function(
                                document, selectors, styleRange, properties) {
                          WebInspector.SASSSupport.Node.call(this, document);
                          ...
                          this.blockStart =
                              new WebInspector.SASSSupport.TextNode(
                                  document,
                                  this.document.text.extract(blockStartRange),
                                  blockStartRange);
                          ...
                          this._hasTrailingSemicolon =
                              ...
                              this.document.text
                                  .extract(this.properties.peekLast().range)
                                  .endsWith(';');
                        }
     ...
     */
                          match: function(other, outNodeMapping) {
                            ...
                            if (outNodeMapping)
                              outNodeMapping.set(this, other);
                            var result = this.blockStart.match(
                                other.blockStart, outNodeMapping);
                            for (var i = 0; result && i < this.selectors.length;
                                 ++i)
                              result = result &&
                                  this.selectors[i].match(
                                      other.selectors[i], outNodeMapping);
                            for (var i = 0;
                                 result && i < this.properties.length; ++i)
                              result = result &&
                                  this.properties[i].match(
                                      other.properties[i], outNodeMapping);
                            return result;
                          },
     ...
     */
                          insertProperties: function(
                              anchorProperty, nameTexts, valueTexts,
                              disabledStates) {
                            console.assert(
                                nameTexts.length === valueTexts.length &&
                                    valueTexts.length === disabledStates.length,
                                'Input array should be of the same size.');

                            this._addTrailingSemicolon();
                            var newProperties = [];
                            var index = anchorProperty ?
                                this.properties.indexOf(anchorProperty) :
                                -1;
                            for (var i = 0; i < nameTexts.length; ++i) {
                              var nameText = nameTexts[i];
                              var valueText = valueTexts[i];
                              var disabled = disabledStates[i];
                              this.document.edits.push(this._insertPropertyEdit(
                                  anchorProperty, nameText, valueText,
                                  disabled));

                              var name = new WebInspector.SASSSupport.TextNode(
                                  this.document, nameText,
                                  WebInspector.TextRange.createFromLocation(
                                      0, 0));
                              var value = new WebInspector.SASSSupport.TextNode(
                                  this.document, valueText,
                                  WebInspector.TextRange.createFromLocation(
                                      0, 0));
                              var newProperty =
                                  new WebInspector.SASSSupport.Property(
     ...
     */
                          _insertPropertyEdit: function(
                              anchorProperty, nameText, valueText, disabled) {
                            var anchorRange = anchorProperty ?
                                anchorProperty.range :
                                this.blockStart.range;
                            var indent = this._computePropertyIndent();
                            var leftComment = disabled ? '/* ' : '';
                            var rightComment = disabled ? ' */' : '';
                            var newText = String.sprintf(
                                '\n%s%s%s: %s;%s', indent, leftComment,
                                nameText, valueText, rightComment);
                            return new WebInspector.SourceEdit(
                                this.document.url, anchorRange.collapseToEnd(),
                                newText);
                          },
     ...
     */
                          _computePropertyIndent: function() {
                            var indentProperty = this.properties.find(
                                property => !property.range.isEmpty());
                            var result = '';
                            if (indentProperty) {
                              result = this.document.text.extract(
                                  new WebInspector.TextRange(
                                      indentProperty.range.startLine, 0,
                                      indentProperty.range.startLine,
                                      indentProperty.range.startColumn));
                            } else {
                              var lineNumber = this.blockStart.range.startLine;
                              var columnNumber =
                                  this.blockStart.range.startColumn;
                              var baseLine = this.document.text.extract(
                                  new WebInspector.TextRange(
                                      lineNumber, 0, lineNumber, columnNumber));
                              result =
                                  WebInspector.TextUtils.lineIndent(baseLine) +
                          ...
                          __proto__: WebInspector.SASSSupport.Node.prototype
                                                                       ...
                                                                       * document
                                                                       * @param
                                                                       * {!Array<!WebInspector.SASSSupport.Rule>}
                                                                       * rules
                                                                       */
                                                                      WebInspector
                                                                          .SASSSupport
     ...
     */
                          clone: function() {
                            ...
                            return new WebInspector.SASSSupport.AST(
                                document, rules);
                          },
     ...
     */
                          match: function(other, outNodeMapping) {
                            ...
                            if (outNodeMapping)
                              outNodeMapping.set(this, other);
                            var result = true;
                            for (var i = 0; result && i < this.rules.length;
                                 ++i)
                              result = result &&
                                  this.rules[i].match(
                                      other.rules[i], outNodeMapping);
                            return result;
                          },
     ...
     */
                          findNodeForPosition: function(
                            ...
                            if (!node)
                              return null;
                            return node.range.containsLocation(
                                       lineNumber, columnNumber) ?
                                node :
                                null;
         ...
         */
                            function nodeComparator(position, textNode) {
                              return textNode.range.compareToPosition(
         ...
         */
                            function nodeComparator(text1, text2) {
                              return WebInspector.TextRange.comparator(
                                  text1.range, text2.range);
                            }
                          },
                          ...
                          __proto__: WebInspector.SASSSupport.Node.prototype
                                     ...
                                     * type
                                     * @param {!WebInspector.SASSSupport.Rule}
                                     * oldRule
                                     * @param {!WebInspector.SASSSupport.Rule}
                                     * newRule
                                     ...
                                     */
                                    WebInspector.SASSSupport.PropertyChange =
                                        function(
                                            type, oldRule, newRule,
                                            oldPropertyIndex,
                                            newPropertyIndex) {
                                      this.type = type;
     ...
     */
                                      oldProperty: function() {
                                        return this.oldRule.properties
                                                   [this.oldPropertyIndex] ||
                                            null;
                                      },
     ...
     */
                                      newProperty: function() {
                                        return this.newRule.properties
                                                   [this.newPropertyIndex] ||
                                            null;
                                      }
                                    }
                                                          ...
                                                          * @param {string} url
                                                          * @param
                                                          * {!WebInspector.SASSSupport.AST}
                                                          * oldAST
                                                          * @param
                                                          * {!WebInspector.SASSSupport.AST}
                                                          * newAST
                                                          * @param
                                                          * {!Map<!WebInspector.SASSSupport.TextNode,
                                                          * !WebInspector.SASSSupport.TextNode>}
                                                          * mapping
                                                          * @param
                                                          * {!Array<!WebInspector.SASSSupport.PropertyChange>}
                                                          * changes
                                                          */
                                                         WebInspector
                                                             .SASSSupport
 ...
 */
                                      ...
                                      console.assert(
                                          oldAST.rules.length ===
                                              newAST.rules.length,
                                          'Not implemented for rule diff.');
                                      console.assert(
                                          oldAST.document.url ===
                                              newAST.document.url,
                                          'Diff makes sense for models with the same url.');
                                      var T = WebInspector.SASSSupport
                                                  .PropertyChangeType;
                                      var changes = [];
                                      /** @type
                                       * {!Map<!WebInspector.SASSSupport.TextNode,
                                       * !WebInspector.SASSSupport.TextNode>} */
                                      var mapping = new Map();
                                      for (var i = 0; i < oldAST.rules.length;
                                           ++i) {
                                        var oldRule = oldAST.rules[i];
                                        var newRule = newAST.rules[i];
                                        computeRuleDiff(
                                            mapping, oldRule, newRule);
                                      }
                                      return new WebInspector.SASSSupport
                                          .ASTDiff(
                                              oldAST.document.url, oldAST,
                                              newAST, mapping, changes);
                                      ...
                                      function addChange(
                                          type, oldRule, newRule,
                                          oldPropertyIndex, newPropertyIndex) {
                                        changes.push(
                                            new WebInspector.SASSSupport
                                                .PropertyChange(
                                                    type, oldRule, newRule,
                                                    oldPropertyIndex,
                                                    newPropertyIndex));
                                      ...
                                      function computeRuleDiff(
                                        ...
                                        var oldLines = [];
                                        for (var i = 0;
                                             i < oldRule.properties.length; ++i)
                                          oldLines.push(
                                              oldRule.properties[i]
                                                  .name.text.trim() +
                                              ':' +
                                              oldRule.properties[i]
                                                  .value.text.trim());
                                        var newLines = [];
                                        for (var i = 0;
                                             i < newRule.properties.length; ++i)
                                          newLines.push(
                                              newRule.properties[i]
                                                  .name.text.trim() +
                                              ':' +
                                              newRule.properties[i]
                                                  .value.text.trim());
                                        var diff = WebInspector.Diff.lineDiff(
                                            oldLines, newLines);
                                        diff =
                                            WebInspector.Diff.convertToEditDiff(
                                                diff);
                                      ...
                                      function computePropertyDiff(
                                        ...
                                        mapping.set(
                                            oldProperty.value,
                                            newProperty.value);
                                        if (oldProperty.name.text.trim() !==
                                            newProperty.name.text.trim())
                                          addChange(
                                              T.NameChanged, oldRule, newRule,
                                              oldPropertyIndex,
                                              newPropertyIndex);
                                        if (oldProperty.value.text.trim() !==
                                            newProperty.value.text.trim())
                                          addChange(
                                              T.ValueChanged, oldRule, newRule,
                                              oldPropertyIndex,
                                              newPropertyIndex);
                                        if (oldProperty.disabled !==
                                            newProperty.disabled)
                                          addChange(
                                              T.PropertyToggled, oldRule,
                                              newRule, oldPropertyIndex,
                                              newPropertyIndex);
                                      }
                                    }
