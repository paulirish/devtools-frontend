 ...
 */
WebInspector.SASSProcessor =
    function(astService, map, editOperations) {
  ...
  _mutate: function() {
    ...
    for (var ast of this._map.models().values()) {
      ...
      if (ast.document.url === this._map.compiledURL())
        promise = this._astService.parseCSS(
            ast.document.url, ast.document.newText().value());
      else
        promise = this._astService.parseSCSS(
            ast.document.url, ast.document.newText().value());
      promises.push(promise);
    }

    return Promise.all(promises).then(
        this._onFinished.bind(this, changedCSSRules));
  },
  ...
  _onFinished: function(changedCSSRules, changedModels) {
    ...
    for (var rule of changedCSSRules) {
      ...
      cssEdits.push(
          new WebInspector.SourceEdit(newRule.document.url, oldRange, newText));
    }
 ...
 */
        WebInspector.SASSProcessor._toCSSProperties =
            function(map, sassProperty) {
 ...
 * @return {!Promise<?WebInspector.SourceMap.EditResult>}
 */
            WebInspector.SASSProcessor.processCSSEdits =
                function(astService, map, ranges, newTexts) {
  console.assert(ranges.length === newTexts.length);
  var cssURL = map.compiledURL();
  var cssText = map.compiledModel().document.text;
  for (var i = 0; i < ranges.length; ++i)
    cssText =
        new WebInspector.Text(cssText.replaceRange(ranges[i], newTexts[i]));
  ...
  function onCSSParsed(newCSSAST) {
    if (newCSSAST.rules.length !== map.compiledModel().rules.length)
      return Promise.resolve(
          /** @type {?WebInspector.SourceMap.EditResult} */ (null));
    // TODO(lushnikov): only diff changed styles.
    var cssDiff =
        WebInspector.SASSSupport.diffModels(map.compiledModel(), newCSSAST);
    ...
    var nodeMapping = new Map();
    var rebasedMap = /** @type {!WebInspector.ASTSourceMap} */ (
        map.rebase(clonedModels, nodeMapping));
    console.assert(rebasedMap);
    var rebasedEdits = edits.map(edit => edit.rebase(rebasedMap, nodeMapping));

    return new WebInspector.SASSProcessor(astService, rebasedMap, rebasedEdits)
        ._mutate();
  }
}
 ...
 */
                WebInspector.SASSProcessor._editsFromCSSDiff =
                    function(cssDiff, map) {
  ...
  for (var i = 0; i < cssDiff.changes.length; ++i) {
    ...
    if (change.type === T.ValueChanged || change.type === T.NameChanged)
      operation = WebInspector.SASSProcessor.SetTextOperation.fromCSSChange(
          change, map);
    else if (change.type === T.PropertyToggled)
      operation =
          WebInspector.SASSProcessor.TogglePropertyOperation.fromCSSChange(
              change, map);
    else if (change.type === T.PropertyRemoved)
      operation =
          WebInspector.SASSProcessor.RemovePropertyOperation.fromCSSChange(
              change, map);
    else if (change.type === T.PropertyAdded)
      operation =
          WebInspector.SASSProcessor.InsertPropertiesOperation.fromCSSChange(
              change, map);
    if (!operation) {
  ...
  return operations;
                     ...
                     */
                    WebInspector.SASSProcessor.EditOperation =
                        function(map, sassURL) {
  this.map = map;
  ...
  rebase: function(newMap, nodeMapping) { return this; },
                                          ...
                                          * @extends
                                          * {WebInspector.SASSProcessor.EditOperation}
                                          * @param {!WebInspector.ASTSourceMap}
                                          * map
                                          * @param
                                         ...
                                         WebInspector.SASSProcessor
                                             .SetTextOperation =
                            function(map, sassNode, newText) {
  WebInspector.SASSProcessor.EditOperation.call(
      this, map, sassNode.document.url);
 ...
 */
                            WebInspector.SASSProcessor.SetTextOperation
                                .fromCSSChange =
                                function(change, map) {
  var oldProperty =
      /** @type {!WebInspector.SASSSupport.Property} */ (change.oldProperty());
  var newProperty =
      /** @type {!WebInspector.SASSSupport.Property} */ (change.newProperty());
  console.assert(
      oldProperty && newProperty,
      'SetTextOperation must have both oldProperty and newProperty');
  var newValue = null;
  var sassNode = null;
  ...
  if (!sassNode)
    return null;
  return new WebInspector.SASSProcessor.SetTextOperation(
      map, sassNode, newValue);
}
  ...
  rebase: function(newMap, nodeMapping) {
    var sassNode = /** @type {?WebInspector.SASSSupport.TextNode} */ (
                       nodeMapping.get(this._sassNode)) ||
        this._sassNode;
    return new WebInspector.SASSProcessor.SetTextOperation(
        newMap, sassNode, this._newText);
  },
  ...
  __proto__: WebInspector.SASSProcessor.EditOperation.prototype
                                                  ...
                                                  */
                                                 WebInspector.SASSProcessor
                                                     .TogglePropertyOperation =
                                    function(map, sassProperty, newDisabled) {
  WebInspector.SASSProcessor.EditOperation.call(
      this, map, sassProperty.document.url);
 ...
 */
                                    WebInspector.SASSProcessor
                                        .TogglePropertyOperation.fromCSSChange =
                                        function(change, map) {
  var oldCSSProperty =
      /** @type {!WebInspector.SASSSupport.Property} */ (change.oldProperty());
  console.assert(
      oldCSSProperty, 'TogglePropertyOperation must have old CSS property');
  var sassProperty =
      WebInspector.SASSProcessor._toSASSProperty(map, oldCSSProperty);
  if (!sassProperty)
    return null;
  var newDisabled = change.newProperty().disabled;
  return new WebInspector.SASSProcessor.TogglePropertyOperation(
      map, sassProperty, newDisabled);
}
  ...
  perform: function() {
    ...
    var cssProperties = WebInspector.SASSProcessor._toCSSProperties(
        this.map, this._sassProperty);
  ...
  /**
     ...
     */
  rebase: function(newMap, nodeMapping) {
    var sassProperty = /** @type {?WebInspector.SASSSupport.Property} */ (
                           nodeMapping.get(this._sassProperty)) ||
        this._sassProperty;
    return new WebInspector.SASSProcessor.TogglePropertyOperation(
 ...
 */
  ...
  console.assert(
      removedProperty,
      'RemovePropertyOperation must have removed CSS property');
  var sassProperty =
      WebInspector.SASSProcessor._toSASSProperty(map, removedProperty);
  if (!sassProperty)
    return null;
  return new WebInspector.SASSProcessor.RemovePropertyOperation(
      map, sassProperty);
}
  ...
  WebInspector.SASSProcessor.EditOperation.call(
      this, map, sassRule.document.url);
  this._sassRule = sassRule;
  this._afterSASSProperty = afterSASSProperty this._nameTexts = propertyNames;
 ...
 */
  ...
  if (!sassRule)
    return null;
  var insertedProperty =
      /** @type {!WebInspector.SASSSupport.Property} */ (change.newProperty());
  console.assert(
      insertedProperty,
      'InsertPropertiesOperation must have inserted CSS property');
  ...
  merge: function(other) {
    if (!(other instanceof
          WebInspector.SASSProcessor.InsertPropertiesOperation))
      return false;
    if (this._sassRule !== other._sassRule ||
        this._afterSASSProperty !== other._afterSASSProperty)
      return false;
  ...
  perform: function() {
    var newSASSProperties = this._sassRule.insertProperties(
        this._afterSASSProperty, this._nameTexts, this._valueTexts,
        this._disabledStates);
    var cssRules = [];
    var afterCSSProperties = [];
    if (this._afterSASSProperty) {
      afterCSSProperties = WebInspector.SASSProcessor._toCSSProperties(
          this.map, this._afterSASSProperty);
      cssRules = afterCSSProperties.map(property => property.parent);
    } else {
      cssRules = this.map.toCompiledNodes(this._sassRule.blockStart)
                     .map(blockStart => blockStart.parent);
    }
    for (var i = 0; i < cssRules.length; ++i) {
      var cssRule = cssRules[i];
      var afterCSSProperty =
          afterCSSProperties.length ? afterCSSProperties[i] : null;
      var newCSSProperties = cssRule.insertProperties(
          afterCSSProperty, this._nameTexts, this._valueTexts,
          this._disabledStates);
      for (var j = 0; j < newCSSProperties.length; ++j) {
        this.map.addMapping(
            newCSSProperties[j].name, newSASSProperties[j].name);
        this.map.addMapping(
            newCSSProperties[j].value, newSASSProperties[j].value);
      }
    }
    return cssRules;
  },
  ...
  /**
     ...
     */
  rebase: function(newMap, nodeMapping) {
    var sassRule = /** @type {?WebInspector.SASSSupport.Rule} */ (
                       nodeMapping.get(this._sassRule)) ||
        this._sassRule;
