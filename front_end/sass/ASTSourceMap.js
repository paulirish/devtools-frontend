...
WebInspector.ASTSourceMap =
  ...
  findEntry: function(lineNumber, columnNumber) {
    ...
    var compiledNode =
        this.compiledModel().findNodeForPosition(lineNumber, columnNumber);
    if (!compiledNode)
      return null;
    ...
    return new WebInspector.SourceMapEntry(
        lineNumber, columnNumber, sourceNode.document.url,
        sourceNode.range.startLine, sourceNode.range.startColumn);
  },
  ...
  compiledModel: function() {
    return /** @type {!WebInspector.SASSSupport.AST} */ (
        this._models.get(this._compiledURL));
  },
  ...
  /**
     ...
     */
  models: function() {
    return /** @type {!Map<string, !WebInspector.SASSSupport.AST>} */ (
        new Map(this._models));
  },
  ...
  rebase: function(updated, outNodeMapping) {
    ...
    var models = /** @type {!Map<string, !WebInspector.SASSSupport.AST>} */ (
        new Map(this._models));
    ...
    for (var i = 0; i < compiledNodes.length; ++i) {
      ...
      var mappedCompiledNode =
          /** @type {!WebInspector.SASSSupport.TextNode} */ (
