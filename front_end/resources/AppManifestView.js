 ...
 */
WebInspector.AppManifestView =
    function() {
  WebInspector.VBox.call(this, true);
  this.registerRequiredCSS('resources/appManifestView.css');

  this._reportView =
      new WebInspector.ReportView(WebInspector.UIString('App Manifest'));
  this._reportView.show(this.contentElement);

  this._errorsSection = this._reportView.appendSection(
      WebInspector.UIString('Errors and warnings'));
  this._identitySection =
      this._reportView.appendSection(WebInspector.UIString('Identity'));
  ...
  this._presentationSection =
      this._reportView.appendSection(WebInspector.UIString('Presentation'));
  this._iconsSection =
      this._reportView.appendSection(WebInspector.UIString('Icons'));

  this._nameField =
      this._identitySection.appendField(WebInspector.UIString('Name'));
  this._shortNameField =
      this._identitySection.appendField(WebInspector.UIString('Short name'));

  this._startURLField =
      this._presentationSection.appendField(WebInspector.UIString('Start URL'));

  var themeColorField = this._presentationSection.appendField(
      WebInspector.UIString('Theme color'));
  this._themeColorSwatch = WebInspector.ColorSwatch.create();
  themeColorField.appendChild(this._themeColorSwatch);

  var backgroundColorField = this._presentationSection.appendField(
      WebInspector.UIString('Background color'));
  this._backgroundColorSwatch = WebInspector.ColorSwatch.create();
  backgroundColorField.appendChild(this._backgroundColorSwatch);

  this._orientationField = this._presentationSection.appendField(
      WebInspector.UIString('Orientation'));
  this._displayField =
      this._presentationSection.appendField(WebInspector.UIString('Display'));
  ...
  _renderManifest: function(url, data, errors) {
    ...
    for (var error of errors)
      this._errorsSection.appendRow().appendChild(createLabel(
          error.message, error.critical ? 'error-icon' : 'warning-icon'));
    ...
    if (startURL)
      this._startURLField.appendChild(WebInspector.linkifyResourceAsNode(
          /** @type {string} */ (
              WebInspector.ParsedURL.completeURL(url, startURL)),
          undefined, undefined, undefined, undefined, startURL));

    this._themeColorSwatch.classList.toggle(
        'hidden', !stringProperty('theme_color'));
    var themeColor =
        WebInspector.Color.parse(stringProperty('theme_color') || 'white') ||
        WebInspector.Color.parse('white');
    this._themeColorSwatch.setColor(
        /** @type {!WebInspector.Color} */ (themeColor));
    this._backgroundColorSwatch.classList.toggle(
        'hidden', !stringProperty('background_color'));
    var backgroundColor = WebInspector.Color.parse(
                              stringProperty('background_color') || 'white') ||
        WebInspector.Color.parse('white');
    this._backgroundColorSwatch.setColor(
        /** @type {!WebInspector.Color} */ (backgroundColor));
