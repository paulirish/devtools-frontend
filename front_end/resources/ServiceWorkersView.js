 ...
 */
WebInspector.ServiceWorkersView =
    function() {
  WebInspector.VBox.call(this, true);

  this._reportView =
      new WebInspector.ReportView(WebInspector.UIString('Service Workers'));
  this._reportView.show(this.contentElement);

  this._toolbar = this._reportView.createToolbar();

  /** @type {!Map<!WebInspector.ServiceWorkerRegistration,
   * !WebInspector.ServiceWorkersView.Section>} */
  ...
  targetAdded: function(target) {
    var securityOriginManager =
        WebInspector.SecurityOriginManager.fromTarget(target);
    ...
    var forceUpdate = new WebInspector.ToolbarCheckbox(
        WebInspector.UIString('Update on reload'),
        WebInspector.UIString('Force update Service Worker on page reload'),
        this._manager.forceUpdateOnReloadSetting());
    this._toolbar.appendToolbarItem(forceUpdate);
    var networkManager =
        target && WebInspector.NetworkManager.fromTarget(target);
    if (networkManager) {
      var fallbackToNetwork = new WebInspector.ToolbarCheckbox(
          WebInspector.UIString('Bypass for network'),
          WebInspector.UIString(
              'Bypass Service Worker and load resources from the network'),
          networkManager.bypassServiceWorkerSetting());
    ...
    this._showAllCheckbox = new WebInspector.ToolbarCheckbox(
        WebInspector.UIString('Show all'),
        WebInspector.UIString(
            'Show all Service Workers regardless of the origin'));
    this._showAllCheckbox.inputElement.addEventListener(
        'change', this._updateSectionVisibility.bind(this), false);
    ...
    this._manager.addEventListener(
        WebInspector.ServiceWorkerManager.Events.RegistrationUpdated,
        this._registrationUpdated, this);
    this._manager.addEventListener(
        WebInspector.ServiceWorkerManager.Events.RegistrationDeleted,
        this._registrationDeleted, this);
    this._manager.addEventListener(
        WebInspector.ServiceWorkerManager.Events.RegistrationErrorAdded,
        this._registrationErrorAdded, this);
    securityOriginManager.addEventListener(
        WebInspector.SecurityOriginManager.Events.SecurityOriginAdded,
  ...
  _updateSectionVisibility: function() {
    var securityOrigins =
        new Set(this._securityOriginManager.securityOrigins());
    for (var section of this._sections.values()) {
      var visible = this._showAllCheckbox.checked() ||
          securityOrigins.has(section._registration.securityOrigin);
  ...
  _registrationUpdated: function(event) {
    var registration =
        /** @type {!WebInspector.ServiceWorkerRegistration} */ (event.data);
    this._updateRegistration(registration);
  },
  ...
  _registrationErrorAdded: function(event) {
    var registration = /** @type {!WebInspector.ServiceWorkerRegistration} */ (
        event.data['registration']);
    var error = /** @type {!ServiceWorkerAgent.ServiceWorkerErrorMessage} */ (
        event.data['error']);
  ...
  _updateRegistration: function(registration) {
    ...
    if (!section) {
      section = new WebInspector.ServiceWorkersView.Section(
          this._manager, this._subTargetsManager,
  ...
  __proto__: WebInspector.VBox.prototype
                      ...
                      * @param {!WebInspector.SubTargetsManager}
                      * subTargetsManager
                      * @param {!WebInspector.ReportView.Section} section
                      * @param {!WebInspector.ServiceWorkerRegistration}
                      * registration
                      */
                     WebInspector.ServiceWorkersView.Section =
        function(manager, subTargetsManager, section, registration) {
  ...
  this._updateButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Update'), undefined,
      WebInspector.UIString('Update'));
  this._updateButton.addEventListener(
      'click', this._updateButtonClicked.bind(this));
  this._toolbar.appendToolbarItem(this._updateButton);
  this._pushButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Emulate push event'), undefined,
      WebInspector.UIString('Push'));
  this._pushButton.addEventListener(
      'click', this._pushButtonClicked.bind(this));
  this._toolbar.appendToolbarItem(this._pushButton);
  this._syncButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Emulate background sync event'), undefined,
      WebInspector.UIString('Sync'));
  this._syncButton.addEventListener(
      'click', this._syncButtonClicked.bind(this));
  this._toolbar.appendToolbarItem(this._syncButton);
  this._deleteButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Unregister service worker'), undefined,
      WebInspector.UIString('Unregister'));
  this._deleteButton.addEventListener(
      'click', this._unregisterButtonClicked.bind(this));
  ...
  this._errorsList.classList.add(
      'service-worker-error-stack', 'monospace', 'hidden');
  ...
  _update: function() {
    ...
    var title = this._registration.isDeleted ?
        WebInspector.UIString('%s - deleted', this._registration.scopeURL) :
    ...
    var installing =
        versions.get(WebInspector.ServiceWorkerVersion.Modes.Installing);

    var statusValue = this._wrapWidget(
        this._section.appendField(WebInspector.UIString('Status')));
    statusValue.removeChildren();
    var versionsStack =
        statusValue.createChild('div', 'service-worker-version-stack');
    ...
    if (active) {
      ...
      scriptElement.appendChild(
          WebInspector.linkifyURLAsNode(active.scriptURL, fileName));
      scriptElement.createChild('div', 'report-field-value-subtitle')
          .textContent = WebInspector.UIString(
          'Received %s',
          new Date(active.scriptResponseTime * 1000).toLocaleString());

      var activeEntry =
          versionsStack.createChild('div', 'service-worker-version');
      activeEntry.createChild('div', 'service-worker-active-circle');
      activeEntry.createChild('span').textContent = WebInspector.UIString(
          '#%s activated and is %s', active.id, active.runningStatus);

      if (active.isRunning() || active.isStarting()) {
        createLink(
            activeEntry, WebInspector.UIString('stop'),
            this._stopButtonClicked.bind(this, active.id));
        if (!this._targetForVersionId(active.id))
          createLink(
              activeEntry, WebInspector.UIString('inspect'),
              this._inspectButtonClicked.bind(this, active.id));
      } else if (active.isStartable()) {
        createLink(
            activeEntry, WebInspector.UIString('start'),
            this._startButtonClicked.bind(this));
      }

      var clientsList = this._wrapWidget(
          this._section.appendField(WebInspector.UIString('Clients')));
      clientsList.removeChildren();
      this._section.setFieldVisible(
          WebInspector.UIString('Clients'), active.controlledClients.length);
      for (var client of active.controlledClients) {
        var clientLabelText =
            clientsList.createChild('div', 'service-worker-client');
        if (this._clientInfoCache.has(client))
          this._updateClientInfo(
              clientLabelText, /** @type {!WebInspector.TargetInfo} */ (
                  this._clientInfoCache.get(client)));
        this._subTargetsManager.getTargetInfo(
            client, this._onClientInfo.bind(this, clientLabelText));
      }
    }
    ...
    if (waiting) {
      ...
      waitingEntry.createChild('span').textContent =
          WebInspector.UIString('#%s waiting to activate', waiting.id);
      createLink(
          waitingEntry, WebInspector.UIString('skipWaiting'),
          this._skipButtonClicked.bind(this));
      waitingEntry.createChild('div', 'service-worker-subtitle').textContent =
          new Date(waiting.scriptResponseTime * 1000).toLocaleString();
      if (!this._targetForVersionId(waiting.id) &&
          (waiting.isRunning() || waiting.isStarting()))
        createLink(
            waitingEntry, WebInspector.UIString('inspect'),
            this._inspectButtonClicked.bind(this, waiting.id));
    }
    if (installing) {
      var installingEntry =
          versionsStack.createChild('div', 'service-worker-version');
      installingEntry.createChild('div', 'service-worker-installing-circle');
      installingEntry.createChild('span').textContent =
          WebInspector.UIString('#%s installing', installing.id);
      installingEntry.createChild('div', 'service-worker-subtitle')
          .textContent =
          new Date(installing.scriptResponseTime * 1000).toLocaleString();
      if (!this._targetForVersionId(installing.id) &&
          (installing.isRunning() || installing.isStarting()))
        createLink(
            installingEntry, WebInspector.UIString('inspect'),
            this._inspectButtonClicked.bind(this, installing.id));
    }

    this._section.setFieldVisible(
        WebInspector.UIString('Errors'), !!this._registration.errors.length);
    var errorsValue = this._wrapWidget(
        this._section.appendField(WebInspector.UIString('Errors')));
    var errorsLabel =
        createLabel(String(this._registration.errors.length), 'error-icon');
    errorsLabel.classList.add('service-worker-errors-label');
    errorsValue.appendChild(errorsLabel);
    this._moreButton = createLink(
        errorsValue, this._errorsList.classList.contains('hidden') ?
            WebInspector.UIString('details') :
            WebInspector.UIString('hide'),
        this._moreErrorsButtonClicked.bind(this));
    createLink(
        errorsValue, WebInspector.UIString('clear'),
  ...
  _addError: function(error) {
    ...
    message.appendChild(this._linkifier.linkifyScriptLocation(
        target, null, error.sourceURL, error.lineNumber));
    message.appendChild(createLabel(
        '#' + error.versionId + ': ' + error.errorMessage, 'error-icon'));
  },
  ...
  _updateClientInfo: function(element, targetInfo) {
    ...
    focusLabel.addEventListener(
        'click', this._activateTarget.bind(this, targetInfo.id), true);
  },
  ...
  _moreErrorsButtonClicked: function() {
    ...
    this._moreButton.textContent = newVisible ?
        WebInspector.UIString('hide') :
        WebInspector.UIString('details');
