...
WebInspector.ClearStorageView =
  ...
  for (var type
           of [StorageAgent.StorageType.Appcache,
    ...
    this._settings.set(
        type,
        WebInspector.settings.createSetting('clear-storage-' + type, true));
  }

  var application =
      this._reportView.appendSection(WebInspector.UIString('Application'));
  this._appendItem(
      application, WebInspector.UIString('Unregister service workers'),
      'service_workers');

  var storage =
      this._reportView.appendSection(WebInspector.UIString('Storage'));
  this._appendItem(
      storage, WebInspector.UIString('Local and session storage'),
      'local_storage');
  ...
  this._appendItem(
      caches, WebInspector.UIString('Cache storage'), 'cache_storage');
  this._appendItem(
      caches, WebInspector.UIString('Application cache'), 'appcache');

  WebInspector.targetManager.observeTargets(
      this, WebInspector.Target.Capability.Browser);
  var footer =
      this._reportView.appendSection('', 'clear-storage-button').appendRow();
  this._clearButton = createTextButton(
      WebInspector.UIString('Clear site data'), this._clear.bind(this),
      WebInspector.UIString('Clear site data'));
  ...
  _appendItem: function(section, title, settingName) {
    ...
    row.appendChild(WebInspector.SettingsUI.createSettingCheckbox(
        title, this._settings.get(settingName), true));
  },
  ...
  targetAdded: function(target) {
    ...
    securityOriginManager.addEventListener(
        WebInspector.SecurityOriginManager.Events.MainSecurityOriginChanged,
        this._originChanged, this);
  },
  ...
  _clear: function() {
    ...
    if (set.has(StorageAgent.StorageType.Appcache) || hasAll) {
      var appcacheModel =
          WebInspector.ApplicationCacheModel.fromTarget(this._target);
      if (appcacheModel)
        appcacheModel.reset();
    }
