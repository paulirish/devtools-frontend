...
WebInspector.SourcesView =
  ...
  this._searchableView =
      new WebInspector.SearchableView(this, 'sourcesViewSearchConfig');
  ...
  var tabbedEditorPlaceholderText = WebInspector.isMac() ?
      WebInspector.UIString('Hit Cmd+P to open a file') :
      WebInspector.UIString('Hit Ctrl+P to open a file');
  this._editorContainer = new WebInspector.TabbedEditorContainer(
      this,
      WebInspector.settings.createLocalSetting('previouslyViewedFiles', []),
  ...
  this._editorContainer.addEventListener(
      ...
      this._editorClosed, this);

  this._historyManager = new WebInspector.EditingLocationHistoryManager(
      this, this.currentSourceFrame.bind(this));

  this._toolbarContainerElement =
      this.element.createChild('div', 'sources-toolbar');
  this._toolbarEditorActions =
      new WebInspector.Toolbar('', this._toolbarContainerElement);

  self.runtime.allInstances(WebInspector.SourcesView.EditorAction)
  ...
  workspace.addEventListener(
      WebInspector.Workspace.Events.UISourceCodeAdded, this._uiSourceCodeAdded,
      this);
  workspace.addEventListener(
  ...
  function handleBeforeUnload(event) {
    ...
    var projects = WebInspector.workspace.projectsForType(
        WebInspector.projectTypes.FileSystem);
    for (var i = 0; i < projects.length; ++i)
      unsavedSourceCodes = unsavedSourceCodes.concat(
          projects[i].uiSourceCodes().filter(isUnsaved));
  ...
  registerShortcuts: function(registerShortcutDelegate) {
    ...
    registerShortcut.call(
        this, WebInspector.ShortcutsScreen.SourcesPanelShortcuts
    ...
    registerShortcut.call(
        this, WebInspector.ShortcutsScreen.SourcesPanelShortcuts.CloseEditorTab,
        this._onCloseEditorTab.bind(this));
    registerShortcut.call(
        this, WebInspector.ShortcutsScreen.SourcesPanelShortcuts.GoToLine,
        this._showGoToLineDialog.bind(this));
    registerShortcut.call(
        this, WebInspector.ShortcutsScreen.SourcesPanelShortcuts.GoToMember,
    ...
    registerShortcut.call(
        this, WebInspector.ShortcutsScreen.SourcesPanelShortcuts.Save,
        this._save.bind(this));
    registerShortcut.call(
        this, WebInspector.ShortcutsScreen.SourcesPanelShortcuts.SaveAll,
  ...
  /**
     ...
     */
  leftToolbar: function() { return this._editorContainer.leftToolbar(); },

  /**
     * @return {!WebInspector.Toolbar}
     */
  rightToolbar: function() { return this._editorContainer.rightToolbar(); },
  ...
  /**
     * @return {!Element}
     */
  toolbarContainerElement: function() { return this._toolbarContainerElement; },
  ...
  /**
     ...
     */
  visibleView: function() { return this._editorContainer.visibleView; },
  ...
  _updateScriptViewToolbarItems: function() {
    ...
    var view = this.visibleView() if (view instanceof WebInspector.SimpleView) {
      for (var item of (/** @type {?WebInspector.SimpleView} */ (view))
               .syncToolbarItems())
   ...
   */
  showSourceLocation: function(
      uiSourceCode, lineNumber, columnNumber, omitFocus, omitHighlight) {
    this._historyManager.updateCurrentState();
    this._editorContainer.showFile(uiSourceCode) var currentSourceFrame =
        this.currentSourceFrame();
    if (currentSourceFrame && typeof lineNumber === 'number')
      currentSourceFrame.revealPosition(
          lineNumber, columnNumber, !omitHighlight);
  ...
  _createSourceView: function(uiSourceCode) {
    ...
    if (contentType.hasScripts())
      ...
      sourceView = new WebInspector.ImageView(
          WebInspector.NetworkProject.uiSourceCodeMimeType(uiSourceCode),
          uiSourceCode);
    else if (contentType === WebInspector.resourceTypes.Font)
      sourceView = new WebInspector.FontView(
          WebInspector.NetworkProject.uiSourceCodeMimeType(uiSourceCode),
          uiSourceCode);
    else
    ...
    var widget =
        /** @type {!WebInspector.Widget} */ (sourceFrame || sourceView);
  ...
  _getOrCreateSourceView: function(uiSourceCode) {
    return this._sourceViewByUISourceCode.get(uiSourceCode) ||
        this._createSourceView(uiSourceCode);
  },
  ...
  _recreateSourceFrameIfNeeded: function(uiSourceCode) {
    ...
    if (!oldSourceView ||
        !(oldSourceView instanceof WebInspector.UISourceCodeFrame))
      return;
    var oldSourceFrame =
        /** @type {!WebInspector.UISourceCodeFrame} */ (oldSourceView);
  ...
  setExecutionLocation: function(uiLocation) {
    ...
    if (sourceView instanceof WebInspector.UISourceCodeFrame) {
      var sourceFrame =
          /** @type {!WebInspector.UISourceCodeFrame} */ (sourceView);
  ...
  _editorClosed: function(event) {
    ...
    this.dispatchEventToListeners(
        WebInspector.SourcesView.Events.EditorClosed, data);
  },
  ...
  _editorSelected: function(event) {
    var previousSourceFrame =
        event.data.previousView instanceof WebInspector.UISourceCodeFrame ?
        event.data.previousView :
        null;
    if (previousSourceFrame)
      previousSourceFrame.setSearchableView(null);
    var currentSourceFrame =
        event.data.currentView instanceof WebInspector.UISourceCodeFrame ?
        event.data.currentView :
        null;
    if (currentSourceFrame)
      currentSourceFrame.setSearchableView(this._searchableView);

    this._searchableView.setReplaceable(
        !!currentSourceFrame && currentSourceFrame.canEditSource());
  ...
  _uiSourceCodeTitleChanged: function(event) {
    this._recreateSourceFrameIfNeeded(
        /** @type {!WebInspector.UISourceCode} */ (event.target));
  },
  ...
  performSearch: function(searchConfig, shouldJump, jumpBackwards) {
    ...
    this._searchView.performSearch(
        this._searchConfig, shouldJump, jumpBackwards);
  },
  ...
  _showOutlineDialog: function(event) {
    ...
    if (uiSourceCode.contentType().hasScripts()) {
      WebInspector.JavaScriptOutlineDialog.show(
          uiSourceCode, this.showSourceLocation.bind(this, uiSourceCode));
      return true;
    }
    ...
    // We don't want default browser shortcut to be executed, so pretend to
    // handle this event.
    return true;
  },
  ...
  showOpenResourceDialog: function(query) {
    ...
    WebInspector.OpenResourceDialog.show(
        this, query || '', defaultScores, this._openResourceDialogHistory);
  },
  ...
  _saveSourceFrame: function(sourceFrame) {
    if (!(sourceFrame instanceof WebInspector.UISourceCodeFrame))
      return;
    var uiSourceCodeFrame =
        /** @type {!WebInspector.UISourceCodeFrame} */ (sourceFrame);
  ...
  toggleBreakpointsActiveState: function(active) {
    this._editorContainer.view.element.classList.toggle(
        'breakpoints-deactivated', !active);
  },
  ...
  __proto__: WebInspector.VBox.prototype
        ...
        WebInspector.SourcesView.EditorAction
            .prototype = {
  /**
  ...
  button: function(sourcesView) {}
                          ...
                          */
                         WebInspector.SourcesView.SwitchFileActionDelegate =
            function() {}
 ...
 * @return {?WebInspector.UISourceCode}
 */
            WebInspector.SourcesView.SwitchFileActionDelegate._nextFile =
                function(currentUISourceCode) {
  /**
     * @param {string} name
     * @return {string}
     */
  function fileNamePrefix(name) {
    ...
    var namePrefix =
        name.substr(0, lastDotIndex !== -1 ? lastDotIndex : name.length);
  ...
  var nextUISourceCode =
      currentUISourceCode.project().uiSourceCodeForURL(fullURL);
  ...
  handleAction: function(context, actionId) {
    ...
    return true;
                                  ...
                                  */
                                 WebInspector.SourcesView
                                     .CloseAllActionDelegate =
                    function() {}

                    WebInspector.SourcesView.CloseAllActionDelegate
                        .prototype = {
  /**
     * @override
     * @param {!WebInspector.Context} context
