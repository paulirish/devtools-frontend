 ...
 */
WebInspector.WorkspaceMappingTip =
    function(sourcesPanel, workspace) {
  ...
  this._workspaceMappingInfobarDisabledSetting =
      WebInspector.settings.createSetting(
          'workspaceMappingInfobarDisabled', false);
  ...
  WebInspector.persistence.addEventListener(
      WebInspector.Persistence.Events.BindingCreated, this._bindingCreated,
      this);
}
...
WebInspector.WorkspaceMappingTip.prototype = {
  ...
  _bindingCreated: function(event) {
    ...
    if (binding.network[WebInspector.WorkspaceMappingTip._infobarSymbol])
      binding.network[WebInspector.WorkspaceMappingTip._infobarSymbol]
          .dispose();
    if (binding.fileSystem[WebInspector.WorkspaceMappingTip._infobarSymbol])
      binding.fileSystem[WebInspector.WorkspaceMappingTip._infobarSymbol]
          .dispose();
  },
  ...
  _editorSelected: function(event) {
    ...
    this._editorSelectedTimer = setTimeout(
        this._updateSuggestedMappingInfobar.bind(this, uiSourceCode), 3000);
  },
  ...
  _updateSuggestedMappingInfobar: function(uiSourceCode) {
    ...
    if (!this._workspaceMappingInfobarDisabledSetting.get() &&
      ...
      networkProjects = networkProjects.concat(this._workspace.projectsForType(
    ...
    if (uiSourceCode.project().type() === WebInspector.projectTypes.Network ||
      ...
      if (!this._isLocalHost(uiSourceCode.url()) ||
          WebInspector.persistence.binding(uiSourceCode))
        return;
  ...
  _showMappingInfobar: function(uiSourceCode, isNetwork) {
    ...
    if (isNetwork)
      title = WebInspector.UIString(
          'Map network resource \'%s\' to workspace?', uiSourceCode.url());
    else
      title = WebInspector.UIString(
          'Map workspace resource \'%s\' to network?', uiSourceCode.url());

    var infobar = WebInspector.Infobar.create(
        WebInspector.Infobar.Type.Info, title,
        this._workspaceMappingInfobarDisabledSetting);
    if (!infobar)
      return;
    infobar.createDetailsRowMessage(WebInspector.UIString(
    ...
    infobar.createDetailsRowMessage(WebInspector.UIString(
        'Use context menu to establish the mapping at any time.'));
    var anchor = createElementWithClass('a', 'link');
    anchor.textContent = WebInspector.UIString('Establish the mapping now...');
    anchor.addEventListener(
        'click', this._establishTheMapping.bind(this, uiSourceCode), false);
  ...
  _appendInfobar: function(uiSourceCode, infobar) {
    ...
    var rowElement = infobar.createDetailsRowMessage(WebInspector.UIString(
    ...
    rowElement.appendChild(WebInspector.linkifyDocumentationURLAsNode(
        ...
        WebInspector.UIString('workspaces documentation')));
    ...
    WebInspector.runCSSAnimationOnce(
        infobar.element, 'source-frame-infobar-animation');
  }
}
