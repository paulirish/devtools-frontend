 ...
 */
WebInspector.SourceCodeDiff =
    function(diffBaseline, textEditor) {
  this._textEditor = textEditor;
  this._decorations = [];
  this._textEditor.installGutter(
      WebInspector.SourceCodeDiff.DiffGutterType, true);
...
WebInspector.SourceCodeDiff.prototype = {
  updateDiffMarkersWhenPossible: function() {
    ...
    this._updateTimeout = setTimeout(
        this.updateDiffMarkersImmediately.bind(this),
        WebInspector.SourceCodeDiff.UpdateTimeout);
  },
  ...
  highlightModifiedLines: function(oldContent, newContent) {
    ...
    for (var i = 0; i < diff.length; ++i) {
      ...
      if (diffEntry.type ===
          WebInspector.SourceCodeDiff.GutterDecorationType.Delete)
        continue;
      for (var lineNumber = diffEntry.from; lineNumber < diffEntry.to;
           ++lineNumber) {
    ...
    this._animationTimeout = setTimeout(
        ...
        400);  // // Keep this timeout in sync with sourcesView.css.
  },
      ...
      for (var i = 0; i < this._animatedLines.length; ++i) {
        ...
        if (location)
          this._textEditor.toggleLineClass(
              location.lineNumber, 'highlight-line-modification', value);
      }
    }
  },
  ...
  _computeDiff: function(baseline, current) {
    var diff =
        WebInspector.Diff.lineDiff(baseline.split('\n'), current.split('\n'));
    var result = [];
    var hasAdded = false;
  ...
  _innerUpdate: function(baseline) {
    ...
    /** @type {!Map<number, !{lineNumber: number, type:
    ...
    for (var i = 0; i < diff.length; ++i) {
      ...
      for (var lineNumber = diffEntry.from; lineNumber < diffEntry.to;
           ++lineNumber)
        newDecorations.set(
            lineNumber, {lineNumber: lineNumber, type: diffEntry.type});
    }

    var decorationDiff =
        oldDecorations.diff(newDecorations, (e1, e2) => e1.type === e2.type);
    var addedDecorations = decorationDiff.added.map(
        entry => new WebInspector.SourceCodeDiff.GutterDecoration(
            this._textEditor, entry.lineNumber, entry.type));
     ...
     */
    WebInspector.SourceCodeDiff.GutterDecoration =
        function(textEditor, lineNumber, type) {
      this._textEditor = textEditor;
     ...
     */
      ...
      install: function() {
        ...
        this._textEditor.setGutterDecoration(
            location.lineNumber, WebInspector.SourceCodeDiff.DiffGutterType,
            element);
        this._textEditor.toggleLineClass(
            location.lineNumber, this._className, true);
      },
      ...
      remove: function() {
        ...
        this._textEditor.setGutterDecoration(
            ...
            null);
        this._textEditor.toggleLineClass(
            location.lineNumber, this._className, false);
      }
    }
