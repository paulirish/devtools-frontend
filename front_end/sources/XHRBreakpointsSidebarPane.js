...
WebInspector.XHRBreakpointsSidebarPane =
  ...
  this._xhrBreakpointsSetting =
      WebInspector.settings.createLocalSetting('xhrBreakpoints', []);

  /** @type {!Map.<string, !Element>} */
  this._breakpointElements = new Map();

  this._addButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Add breakpoint'), 'add-toolbar-item');
  this._addButton.addEventListener('click', this._addButtonClicked.bind(this));

  this.emptyElement.addEventListener(
      'contextmenu', this._emptyElementContextMenu.bind(this), true);
  ...
  /**
     ...
     */
  toolbarItems: function() { return [this._addButton]; },
  ...
  _addButtonClicked: function(event) {
    ...
    var inputElementContainer =
        createElementWithClass('p', 'breakpoint-condition');
    inputElementContainer.textContent =
        WebInspector.UIString('Break when URL contains:');

    var inputElement = inputElementContainer.createChild('span', 'editing');
    inputElement.id = 'breakpoint-condition-input';
    this.addListElement(
        inputElementContainer,
        /** @type {?Element} */ (this.listElement.firstChild));
  ...
  _setBreakpoint: function(url, enabled, target) {
    ...
    element.addEventListener(
        'contextmenu', this._contextMenu.bind(this, url), true);

    var title = url ? WebInspector.UIString('URL contains "%s"', url) :
                      WebInspector.UIString('Any XHR');
    var label = createCheckboxLabel(title, enabled);
    element.appendChild(label);
    label.checkboxElement.addEventListener(
        'click', this._checkboxClicked.bind(this, url), false);
    element._checkboxElement = label.checkboxElement;

    label.textElement.classList.add('cursor-auto');
    label.textElement.addEventListener(
        'dblclick', this._labelClicked.bind(this, url), false);
  ...
  _updateBreakpointOnTarget: function(url, enable, target) {
    var targets = target ? [target] :
                           WebInspector.targetManager.targets(
  ...
  _contextMenu: function(url, event) {
    ...
    var removeAllTitle =
        WebInspector.UIString.capitalize('Remove ^all ^breakpoints');
  ...
  _labelClicked: function(url) {
    ...
    var inputElement =
        createElementWithClass('span', 'breakpoint-condition editing');
    ...
    WebInspector.InplaceEditor.startEditing(
        inputElement,
        new WebInspector.InplaceEditor.Config(
            finishEditing.bind(this, true), finishEditing.bind(this, false)));
  },
  ...
  _update: function() {
    ...
    if (!details ||
        details.reason !== WebInspector.DebuggerModel.BreakReason.XHR) {
  ...
  _saveBreakpoints: function() {
    ...
    for (var url of this._breakpointElements.keys())
      breakpoints.push({
        url: url,
        enabled: this._breakpointElements.get(url)._checkboxElement.checked
      });
