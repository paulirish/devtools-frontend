...
WebInspector.SourceMapNamesResolver._cachedIdentifiersSymbol =
    Symbol('cachedIdentifiers');
 ...
 */
WebInspector.SourceMapNamesResolver.Identifier =
    function(name, lineNumber, columnNumber) {
  this.name = name;
 ...
 */
    WebInspector.SourceMapNamesResolver._scopeIdentifiers =
        function(scope) {
  var startLocation = scope.startLocation();
  var endLocation = scope.endLocation();

  if (scope.type() === DebuggerAgent.ScopeType.Global || !startLocation ||
      !endLocation || !startLocation.script().sourceMapURL ||
  ...
  function onContent(content) {
    ...
    var scopeRange = new WebInspector.TextRange(
        ...
        endLocation.lineNumber, endLocation.columnNumber) var scopeText =
        text.extract(scopeRange);
  ...
  function onIdentifiers(text, scopeStart, prefix, event) {
    var identifiers = event ?
        /** @type {!Array<!{name: string, offset: number}>} */ (event.data) :
        [];
    var result = [];
 ...
 */
        WebInspector.SourceMapNamesResolver._resolveScope =
            function(scope) {
  var identifiersPromise =
      scope[WebInspector.SourceMapNamesResolver._cachedIdentifiersSymbol];
  if (identifiersPromise)
    return identifiersPromise;

  var script = scope.callFrame().script;
  var sourceMap =
      WebInspector.debuggerWorkspaceBinding.sourceMapForScript(script);
  if (!sourceMap)
  ...
  identifiersPromise =
      WebInspector.SourceMapNamesResolver._scopeIdentifiers(scope).then(
          onIdentifiers);
  scope[WebInspector.SourceMapNamesResolver._cachedIdentifiersSymbol] =
      identifiersPromise;
  return identifiersPromise;
  ...
  function onIdentifiers(identifiers) {
    ...
    for (var i = 0; i < identifiers.length; ++i) {
      ...
      if (namesMapping.has(id.name))
        continue;
      var promise = resolveSourceName(id).then(
          onSourceNameResolved.bind(null, namesMapping, id));
  ...
  function resolveSourceName(id) {
    ...
    var endEntry =
        sourceMap.findEntry(id.lineNumber, id.columnNumber + id.name.length);
    if (!startEntry || !endEntry || !startEntry.sourceURL ||
        startEntry.sourceURL !== endEntry.sourceURL ||
    ...
    var sourceTextRange = new WebInspector.TextRange(
        startEntry.sourceLineNumber, startEntry.sourceColumnNumber,
        endEntry.sourceLineNumber, endEntry.sourceColumnNumber);
    var uiSourceCode = WebInspector.networkMapping.uiSourceCodeForScriptURL(
        startEntry.sourceURL, script);
    if (!uiSourceCode)
      return Promise.resolve(/** @type {?string} */ (null));

    return uiSourceCode.requestContent().then(
        onSourceContent.bind(null, sourceTextRange));
  }
  ...
  function onSourceContent(sourceTextRange, content) {
    ...
    return /[a-zA-Z0-9_$]+/.test(originalIdentifier) ? originalIdentifier :
                                                       null;
  }
}
 ...
 * @return {!Promise.<!Map<string, string>>}
 */
                WebInspector.SourceMapNamesResolver._allVariablesInCallFrame =
                    function(callFrame) {
  ...
  for (var i = 0; i < scopeChain.length; ++i)
    promises.push(
        WebInspector.SourceMapNamesResolver._resolveScope(scopeChain[i]));
  ...
  function mergeVariables(nameMappings) {
    ...
    callFrame[WebInspector.SourceMapNamesResolver._cachedMapSymbol] =
        reverseMapping;
 ...
 * @return {!Promise<string>}
 */
                    WebInspector.SourceMapNamesResolver.resolveExpression =
                        function(
                            callFrame, originalText, uiSourceCode, lineNumber,
  ...
  return WebInspector.SourceMapNamesResolver._allVariablesInCallFrame(callFrame)
  ...
  function findCompiledName(reverseMapping) {
    ...
    return WebInspector.SourceMapNamesResolver._resolveExpression(
        callFrame, uiSourceCode, lineNumber, startColumnNumber,
        endColumnNumber);
  }
}
 ...
 * @return {!Promise<string>}
 */
                        WebInspector.SourceMapNamesResolver._resolveExpression =
                            function(
                                callFrame, uiSourceCode, lineNumber,
                                startColumnNumber, endColumnNumber) {
  ...
  function onContent(content) {
    ...
    var textRange = sourceMap.reverseMapTextRange(
        ...
        new WebInspector.TextRange(
            lineNumber, startColumnNumber, lineNumber, endColumnNumber));
 ...
 */
  ...
  return WebInspector.SourceMapNamesResolver
      ._resolveScope(callFrame.scopeChain()[0])
  ...
  function onScopeResolved(namesMapping) {
    ...
    callFrame.evaluate(
        thisMapping, 'backtrace', false, true, false, true,
        onEvaluated.bind(null, callback));
    return promise;
  }
 ...
 */
  ...
  return new WebInspector.SourceMapNamesResolver.RemoteObject(scope);
                                     ...
                                     * @param
                                     * {!WebInspector.DebuggerModel.Scope} scope
                                     */
                                    WebInspector.SourceMapNamesResolver
                                        .RemoteObject = function(scope) {
...
WebInspector.SourceMapNamesResolver.RemoteObject.prototype = {
  ...
  getAllProperties: function(accessorPropertiesOnly, callback) {
      ...
      if (properties) {
        for (var i = 0; i < properties.length; ++i) {
          ...
          newProperties.push(new WebInspector.RemoteObjectProperty(
              name, property.value, property.enumerable, property.writable,
              property.isOwn, property.wasThrown, property.symbol,
              property.synthetic));
        }
      }
    ...
    this._object.getAllProperties(
        accessorPropertiesOnly, wrappedCallback.bind(this));
  },
