 ...
 */
WebInspector.ThreadsSidebarPane =
    function() {
  ...
  WebInspector.targetManager.addModelListener(
      ...
      this._onDebuggerStateChanged, this);
  WebInspector.targetManager.addModelListener(
      WebInspector.DebuggerModel,
  ...
  _targetChanged: function(event) {
    ...
    var debuggerModel = WebInspector.DebuggerModel.fromTarget(
        newTarget) if (!debuggerModel) return;
    var listItem = /** @type {!WebInspector.UIList.Item} */ (
        this._debuggerModelToListItems.get(debuggerModel));
    this._selectListItem(listItem);
  },
  ...
  _onExecutionContextChanged: function(event) {
    ...
    var debuggerModel = /** @type {!WebInspector.DebuggerModel} */ (
        WebInspector.DebuggerModel.fromTarget(executionContext.target()));
  ...
  _updateDebuggerState: function(debuggerModel) {
    ...
    listItem.setSubtitle(
        WebInspector.UIString(debuggerModel.isPaused() ? 'paused' : ''));
  },
  ...
  _onListItemClick: function(listItem) {
    WebInspector.context.setFlavor(
        WebInspector.Target, this._listItemsToTargets.get(listItem));
