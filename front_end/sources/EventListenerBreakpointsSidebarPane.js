...
WebInspector.EventListenerBreakpointsSidebarPane =
  ...
  this._eventListenerBreakpointsSetting =
      WebInspector.settings.createLocalSetting('eventListenerBreakpoints', []);
  ...
  this._categoryItems = [];
  // FIXME: uncomment following once inspector stops being drop targer in major
  // ports.
  // Otherwise, inspector page reacts on drop event and tries to load the event
  // data.
  // this._createCategory(WebInspector.UIString("Drag"), ["drag", "drop",
  ...
  this._createCategory(
      ...
      true);
  this._createCategory(
      WebInspector.UIString('Clipboard'),
      ['copy', 'cut', 'paste', 'beforecopy', 'beforecut', 'beforepaste']);
  this._createCategory(WebInspector.UIString('Control'), [
    'resize', 'scroll', 'zoom', 'focus', 'blur', 'select', 'change', 'submit',
    'reset'
  ]);
  this._createCategory(
      WebInspector.UIString('Device'), ['deviceorientation', 'devicemotion']);
  ...
  this._createCategory(
      WebInspector.UIString('Drag / drop'),
      ['dragenter', 'dragover', 'dragleave', 'drop']);
  this._createCategory(
      WebInspector.UIString('Keyboard'),
  ...
  this._createCategory(
      ...
      [
        'play',           'pause',      'playing',        'canplay',
        'canplaythrough', 'seeking',    'seeked',         'timeupdate',
  ...
  this._createCategory(
      ...
      ['scriptFirstStatement', 'scriptBlockedByCSP'], true);
  this._createCategory(
      WebInspector.UIString('Timer'), ['setTimer', 'clearTimer', 'timerFired'],
      true);
  this._createCategory(
      WebInspector.UIString('Touch'),
      ['touchstart', 'touchmove', 'touchend', 'touchcancel']);
  this._createCategory(
      WebInspector.UIString('WebGL'), ['webglErrorFired', 'webglWarningFired'],
      true);
  this._createCategory(WebInspector.UIString('Window'), ['close'], true);
  this._createCategory(
      WebInspector.UIString('XHR'),
      [
        'readystatechange', 'load', 'loadstart', 'loadend', 'abort', 'error',
        'progress', 'timeout'
      ],
      false, ['XMLHttpRequest', 'XMLHttpRequestUpload']);

  WebInspector.targetManager.observeTargets(
      this, WebInspector.Target.Capability.DOM);
  WebInspector.targetManager.addModelListener(
      WebInspector.DebuggerModel,
      WebInspector.DebuggerModel.Events.DebuggerPaused, this._update, this);
  WebInspector.targetManager.addModelListener(
      WebInspector.DebuggerModel,
      WebInspector.DebuggerModel.Events.DebuggerResumed, this._update, this);
  WebInspector.context.addFlavorChangeListener(
      WebInspector.Target, this._update, this);
}
...
WebInspector.EventListenerBreakpointsSidebarPane.eventNameForUI =
  ...
  if (!WebInspector.EventListenerBreakpointsSidebarPane._eventNamesForUI) {
    WebInspector.EventListenerBreakpointsSidebarPane._eventNamesForUI = {
      ...
      'instrumentation:scriptFirstStatement':
          WebInspector.UIString('Script First Statement'),
      'instrumentation:scriptBlockedByCSP':
          WebInspector.UIString('Script Blocked by Content Security Policy'),
      ...
      'instrumentation:animationFrameFired':
      ...
      'instrumentation:webglWarningFired':
          WebInspector.UIString('WebGL Warning Fired'),
      'instrumentation:setInnerHTML': WebInspector.UIString('Set innerHTML'),
    };
  }
  if (auxData) {
    if (eventName === 'instrumentation:webglErrorFired' &&
    ...
    if (eventName === 'instrumentation:scriptBlockedByCSP' &&
   ...
   */
  targetAdded: function(target) { this._restoreBreakpoints(target); },
  ...
  _createCategory: function(
      name, eventNames, isInstrumentationEvent, targetNames) {
    ...
    categoryItem.checkbox.addEventListener(
        'click', this._categoryCheckboxClicked.bind(this, categoryItem), true);
    ...
    for (var i = 0; i < eventNames.length; ++i) {
      ...
      breakpointItem.element.listItemElement.createChild(
          'div', 'breakpoint-hit-marker');
      ...
      breakpointItem.checkbox.addEventListener(
          'click', this._breakpointCheckboxClicked.bind(
                       this, eventName, categoryItem.targetNames),
          true);
  ...
  _update: function() {
    ...
    if (!details ||
        details.reason !==
            WebInspector.DebuggerModel.BreakReason.EventListener) {
    ...
    if (!breakpointItem || !breakpointItem.checkbox.checked)
      breakpointItem = this._findBreakpointItem(
          eventName,
          WebInspector.EventListenerBreakpointsSidebarPane.eventTargetAny);
    if (!breakpointItem)
      return;
   ...
   * @param {!WebInspector.Target=} target
   */
  _updateBreakpointOnTarget: function(
      eventName, eventTargetName, enable, target) {
    var targets = target ?
        [target] :
        WebInspector.targetManager.targets(WebInspector.Target.Capability.DOM);
    for (target of targets) {
      if (eventName.startsWith(WebInspector.EventListenerBreakpointsSidebarPane
                                   .categoryListener)) {
        var protocolEventName =
            eventName.substring(WebInspector.EventListenerBreakpointsSidebarPane
                                    .categoryListener.length);
        if (enable)
          target.domdebuggerAgent().setEventListenerBreakpoint(
              protocolEventName, eventTargetName);
        else
          target.domdebuggerAgent().removeEventListenerBreakpoint(
              protocolEventName, eventTargetName);
        ...
        if (enable)
          target.domdebuggerAgent().setInstrumentationBreakpoint(
              protocolEventName);
        else
          target.domdebuggerAgent().removeInstrumentationBreakpoint(
              protocolEventName);
      }
    }
  },
  ...
  _saveBreakpoints: function() {
    ...
    for (var i = 0; i < this._categoryItems.length; ++i) {
      ...
      for (var eventName in categoryItem.children) {
        ...
        if (breakpointItem.checkbox.checked)
          breakpoints.push(
              {eventName: eventName, targetNames: categoryItem.targetNames});
      }
    }
  ...
  _restoreBreakpoints: function(target) {
    ...
    for (var i = 0; i < breakpoints.length; ++i) {
      ...
      if (breakpoint && typeof breakpoint.eventName === 'string')
        this._setBreakpoint(
            breakpoint.eventName, breakpoint.targetNames, target);
    }
  },
