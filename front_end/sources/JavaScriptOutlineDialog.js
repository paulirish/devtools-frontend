...
WebInspector.JavaScriptOutlineDialog =
  ...
  WebInspector.JavaScriptOutlineDialog._instanceForTests =
      new WebInspector.JavaScriptOutlineDialog(
          uiSourceCode, selectItemCallback);
  new WebInspector
      .FilteredListWidget(
          WebInspector.JavaScriptOutlineDialog._instanceForTests)
      .showAsDialog();
  ...
  _didBuildOutlineChunk: function(event) {
    ...
    var data =
        /** @type {!WebInspector.JavaScriptOutlineDialog.MessageEventData} */ (
            event.data);
  ...
  /**
     ...
     * @return {number}
     */
  itemCount: function() { return this._functionItems.length; },
  ...
  selectItem: function(itemIndex, promptValue) {
    ...
    if (!isNaN(lineNumber) && lineNumber >= 0)
      this._selectItemCallback(
          lineNumber, this._functionItems[itemIndex].column);
  },
  ...
  __proto__: WebInspector.FilteredListWidget.Delegate.prototype
                         ...
                         /**
                          * @typedef {{isLastChunk: boolean, chunk:
                          * !Array.<!{selectorText: string, lineNumber: number,
                          * columnNumber: number}>}}
                          */
