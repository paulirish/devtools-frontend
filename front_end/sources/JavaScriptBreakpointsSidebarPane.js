...
WebInspector.JavaScriptBreakpointsSidebarPane =
  ...
  for (var i = 0; i < breakpointLocations.length; ++i)
    this._addBreakpoint(
        breakpointLocations[i].breakpoint, breakpointLocations[i].uiLocation);
  ...
  _addBreakpoint: function(breakpoint, uiLocation) {
    ...
    element.addEventListener(
        'contextmenu', this._breakpointContextMenu.bind(this, breakpoint),
        true);
    element.addEventListener(
        'click', this._breakpointClicked.bind(this, uiLocation), false);

    var checkboxLabel =
        createCheckboxLabel(uiLocation.linkText(), breakpoint.enabled());
    ...
    function didRequestContent(content) {
      ...
      if (lineNumber < text.lineCount()) {
        ...
        snippetElement.textContent =
            lineText.substr(snippetStartIndex).trimEnd(maxSnippetLength);
      }
    ...
    var breakpointItem = {
      element: element,
      checkbox: checkboxLabel.checkboxElement
    };
  ...
  _breakpointsActiveStateChanged: function() {
    this._listElement.classList.toggle(
        ...
        !this._breakpointManager.breakpointsActive());
  },
  ...
  _breakpointContextMenu: function(breakpoint, event) {
    ...
    if (breakpoints.length > 1) {
      ...
      contextMenu.appendItem(
          removeAllTitle, this._breakpointManager.removeAllBreakpoints.bind(
    ...
    if (breakpoints.length > 1) {
      ...
      var enableTitle =
          WebInspector.UIString.capitalize('Enable ^all ^breakpoints');
      var disableTitle =
          WebInspector.UIString.capitalize('Disable ^all ^breakpoints');

      contextMenu.appendSeparator();

      contextMenu.appendItem(
          enableTitle, this._breakpointManager.toggleAllBreakpoints.bind(
                           this._breakpointManager, true),
          !(enableBreakpointCount !== breakpoints.length));
      contextMenu.appendItem(
          disableTitle, this._breakpointManager.toggleAllBreakpoints.bind(
                            this._breakpointManager, false),
