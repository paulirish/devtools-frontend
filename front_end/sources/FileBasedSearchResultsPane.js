...
WebInspector.FileBasedSearchResultsPane
  ...
  _addFileTreeElement: function(searchResult) {
    var fileTreeElement =
        new WebInspector.FileBasedSearchResultsPane.FileTreeElement(
  ...
  __proto__: WebInspector.SearchResultsPane.prototype
                  ...
                  * @param {!WebInspector.FileBasedSearchResult} searchResult
                  */
                 WebInspector.FileBasedSearchResultsPane.FileTreeElement =
    function(searchConfig, searchResult) {
  ...
  _updateSearchMatches: function() {
    ...
    fileNameSpan.textContent =
        this._searchResult.uiSourceCode.fullDisplayName();
    ...
    if (searchMatchesCount === 1)
      ...
      matchesCountSpan.textContent =
          WebInspector.UIString('(%d matches)', searchMatchesCount);
  ...
  _appendSearchMatches: function(fromIndex, toIndex) {
    ...
    for (var i = 0; i < queries.length; ++i)
      regexes.push(createSearchRegex(
          queries[i], !this._searchConfig.ignoreCase(),
    ...
    for (var i = fromIndex; i < toIndex; ++i) {
      ...
      for (var j = 0; j < regexes.length; ++j)
        matchRanges =
            matchRanges.concat(this._regexMatchRanges(lineContent, regexes[j]));

      var anchor =
          this._createAnchor(uiSourceCode, lineNumber, matchRanges[0].offset);
  ...
  _appendShowMoreMatchesElement: function(startMatchIndex) {
    var matchesLeftCount =
        this._searchResult.searchMatches.length - startMatchIndex;
  ...
  _createContentSpan: function(lineContent, matchRanges) {
    ...
    WebInspector.highlightRangesWithStyleClass(
        contentSpan, matchRanges, 'highlighted-match');
    return contentSpan;
  },
  ...
  _showMoreMatchesElementSelected: function(startMatchIndex) {
    ...
    this._appendSearchMatches(
        startMatchIndex, this._searchResult.searchMatches.length);
    return false;
  },
