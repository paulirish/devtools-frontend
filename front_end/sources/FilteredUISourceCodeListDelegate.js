 ...
 */
WebInspector.FilteredUISourceCodeListDelegate =
    function(defaultScores, history) {
  ...
  _populate: function(skipProject) {
    ...
    this._uiSourceCodes = [];
    var projects =
        WebInspector.workspace.projects().filter(this.filterProject.bind(this));
    for (var i = 0; i < projects.length; ++i) {
      if (skipProject && projects[i] === skipProject)
        continue;
      var uiSourceCodes = projects[i].uiSourceCodes().filter(
          this._filterUISourceCode.bind(this));
  ...
  /**
     ...
     * @return {number}
     */
  itemCount: function() { return this._uiSourceCodes.length; },
  ...
  itemScoreAt: function(itemIndex, query) {
    ...
    var score =
        this._defaultScores ? (this._defaultScores.get(uiSourceCode) || 0) : 0;
  ...
  renderItem: function(itemIndex, query, titleElement, subtitleElement) {
    ...
    var score = new WebInspector.FilePathScoreFunction(query).score(
        fullDisplayName, indexes);
    var fileNameIndex = fullDisplayName.lastIndexOf('/');

    titleElement.textContent = uiSourceCode.displayName() +
    ...
    if (indexes[0] > fileNameIndex) {
      ...
      WebInspector.highlightRangesWithStyleClass(
          titleElement, ranges, 'highlight');
    } else {
      WebInspector.highlightRangesWithStyleClass(
          subtitleElement, ranges, 'highlight');
    }
  },
  ...
  selectItem: function(itemIndex, promptValue) {
    ...
    var uiSourceCode =
        itemIndex !== null ? this._uiSourceCodes[itemIndex] : null;
  ...
  rewriteQuery: function(query) {
    ...
    this._queryLineNumberAndColumnNumber =
        lineNumberMatch ? lineNumberMatch[2] : '';
  ...
  _uiSourceCodeAdded: function(event) {
    ...
    if (!this._filterUISourceCode(uiSourceCode) ||
        !this.filterProject(uiSourceCode.project()))
      return;
