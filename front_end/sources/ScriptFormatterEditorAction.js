 ...
 */
WebInspector.FormatterScriptMapping =
    function(debuggerModel, editorAction) {
  ...
  rawLocationToUILocation: function(rawLocation) {
    ...
    var formattedLocation =
        mapping.originalToFormatted(lineNumber, columnNumber);
  ...
  uiLocationToRawLocation: function(uiSourceCode, lineNumber, columnNumber) {
    ...
    var originalLocation =
        formatData.mapping.formattedToOriginal(lineNumber, columnNumber);
  ...
  uiLineHasMapping: function(uiSourceCode, lineNumber) { return true; }
                      ...
                      */
                     WebInspector.FormatterScriptMapping.FormatData =
        function(projectId, path, mapping, scripts) {
  ...
  this.scripts = scripts;
         ...
         */
        WebInspector.ScriptFormatterEditorAction =
            function() {
  this._projectId = 'formatter:';
  this._project = new WebInspector.ContentProviderBasedProject(
      WebInspector.workspace, this._projectId,
      WebInspector.projectTypes.Formatter, 'formatter');
  ...
  _editorSelected: function(event) {
    ...
    if (this._isFormatableScript(uiSourceCode) &&
        this._pathsToFormatOnLoad.has(path) && !this._formattedPaths.get(path))
  ...
  _editorClosed: function(event) {
    var uiSourceCode =
        /** @type {!WebInspector.UISourceCode} */ (event.data.uiSourceCode);
  ...
  _updateButton: function(uiSourceCode) {
    this._button.element.classList.toggle(
        'hidden', !this._isFormatableScript(uiSourceCode));
  },
  ...
  button: function(sourcesView) {
    ...
    this._button.addEventListener(
        'click', this._toggleFormatScriptSource, this);
  ...
  _scriptsForUISourceCode: function(uiSourceCode) {
    ...
    if (uiSourceCode.contentType() === WebInspector.resourceTypes.Document) {
      ...
      for (var i = 0; i < debuggerModels.length; ++i)
        scripts.pushAll(
            debuggerModels[i].scriptsForSourceURL(uiSourceCode.url()));
      return scripts.filter(isInlineScript);
    }
    if (uiSourceCode.contentType().isScript()) {
      var rawLocations =
          WebInspector.debuggerWorkspaceBinding.uiLocationToRawLocations(
              uiSourceCode, 0, 0);
  ...
  _formatUISourceCodeScript: function(uiSourceCode) {
    var formattedPath = this._formattedPaths.get(
        uiSourceCode.project().id() + ':' + uiSourceCode.url());
    if (formattedPath) {
      var uiSourceCodePath = formattedPath;
      var formattedUISourceCode =
          this._workspace.uiSourceCode(this._projectId, uiSourceCodePath);
      var formatData = formattedUISourceCode ?
          this._formatData.get(formattedUISourceCode) :
          null;
      if (formatData)
        this._showIfNeeded(
            uiSourceCode,
            /** @type {!WebInspector.UISourceCode} */ (formattedUISourceCode),
            formatData.mapping);
      return;
    }
    ...
    function contentLoaded(content) {
      ...
      WebInspector.Formatter.format(
          uiSourceCode.contentType(), highlighterType, content || '',
    ...
    function innerCallback(formattedContent, formatterMapping) {
      ...
      var formattedUISourceCode =
          this._project.addContentProvider(formattedURL, contentProvider);
      var formattedPath = formattedUISourceCode.url();
      var formatData = new WebInspector.FormatterScriptMapping.FormatData(
          uiSourceCode.project().id(), uiSourceCode.url(), formatterMapping,
          scripts);
