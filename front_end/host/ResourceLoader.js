 ...
 * @return {number}
 */
WebInspector.ResourceLoader._bindOutputStream =
    function(stream) {
  WebInspector.ResourceLoader
      ._boundStreams[++WebInspector.ResourceLoader._lastStreamId] = stream;
  return WebInspector.ResourceLoader._lastStreamId;
     ...
     * @param {number} id
     */
    WebInspector.ResourceLoader._discardOutputStream =
        function(id) {
  ...
  delete WebInspector.ResourceLoader._boundStreams[id];
         ...
         * @param {string} chunk
         */
        WebInspector.ResourceLoader.streamWrite =
            function(id, chunk) {
  WebInspector.ResourceLoader._boundStreams[id].write(chunk);
             ...
             * @param {function(number, !Object.<string, string>, string)}
             * callback
             */
            WebInspector.ResourceLoader.load =
  ...
  function mycallback(statusCode, headers) {
    callback(statusCode, headers, stream.data());
                 ...
                 */
                WebInspector.ResourceLoader.loadAsStream = function(
                    url, headers, stream, callback) {
  ...
  function dataURLDecodeSuccessful(text) {
    ...
    finishedCallback(
        /** @type {!InspectorFrontendHostAPI.LoadNetworkResourceResult} */ (
            {statusCode: 200}));
  }
