 ...
 */
WebInspector.CPUProfileNode =
    function(node, sampleTime) {
  var callFrame =
      node.callFrame || /** @type {!RuntimeAgent.CallFrame} */ ({
  ...
  // Compatibility: legacy backends could provide "no reason" for optimized
  // functions.
  this.deoptReason = node.deoptReason && node.deoptReason !== 'no reason' ?
      node.deoptReason :
      null;
}
  ...
  __proto__: WebInspector.ProfileNode.prototype
                      ...
                      */
                     WebInspector.CPUProfileDataModel =
        function(profile) {
  var isLegacyFormat = !!profile['head'];
  if (isLegacyFormat) {
    // Legacy format contains raw timestamps and start/stop times are in
    // seconds.
    ...
    // Current format encodes timestamps as deltas. Start/stop times are in
    // microseconds.
    this.profileStartTime = profile.startTime / 1000;
    this.profileEndTime = profile.endTime / 1000;
  ...
  _compatibilityConversionHeadToNodes: function(profile) {
    ...
    function convertNodesTree(node) {
      nodes.push(node);
      node.children =
          (/** @type {!Array<!ProfilerAgent.ProfileNode>} */ (node.children))
              .map(convertNodesTree);
      return node.id;
    }
  },
  ...
  _translateProfileTree: function(nodes) {
    ...
    var sampleTime =
        (this.profileEndTime - this.profileStartTime) / this.totalHitCount;
    var keepNatives =
        !!WebInspector.moduleSetting('showNativeFunctionsInJSProfile').get();
    var root = nodes[0];
    ...
    while (sourceNodeStack.length) {
      ...
      parentNodeStack.push.apply(
          parentNodeStack, sourceNode.children.map(() => parentNode));
  ...
  _normalizeTimestamps: function() {
    ...
    if (!timestamps) {
      ...
      var interval =
          (this.profileEndTime - profileStartTime) / this.samples.length;
    ...
    for (var i = 0; i < timestamps.length; ++i)
      timestamps[i] /= 1000;
    var averageSample =
        (timestamps.peekLast() - timestamps[0]) / (timestamps.length - 1);
  ...
  /**
   * @param {function(number, !WebInspector.CPUProfileNode, number)}
   * openFrameCallback
   * @param {function(number, !WebInspector.CPUProfileNode, number, number,
   * number)} closeFrameCallback
   * @param {number=} startTime
   * @param {number=} stopTime
   */
  forEachFrame: function(
      openFrameCallback, closeFrameCallback, startTime, stopTime) {
    ...
    for (var sampleIndex = startIndex; sampleIndex < samplesCount;
         sampleIndex++) {
      ...
      if (node === gcNode) {
        // GC samples have no stack, so we just put GC node on top of the last
        // recorded sample.
      ...
      while (prevNode !== node) {
        ...
        closeFrameCallback(
            prevNode.depth,
            /** @type {!WebInspector.CPUProfileNode} */ (prevNode), start,
            duration, duration - stackChildrenDuration[stackTop]);
        --stackTop;
    ...
    for (var node = idToNode.get(prevId); node.parent; node = node.parent) {
      ...
      stackChildrenDuration[stackTop - 1] += duration;
      closeFrameCallback(
          node.depth, /** @type {!WebInspector.CPUProfileNode} */ (node), start,
          duration, duration - stackChildrenDuration[stackTop]);
      --stackTop;
    }
  },
