 ...
 */
WebInspector.SubTargetsManager =
    function(target) {
...
WebInspector.SubTargetsManager
  ...
  _capabilitiesForType: function(type) {
    if (type === 'worker')
      return WebInspector.Target.Capability.JS |
          WebInspector.Target.Capability.Log;
    if (type === 'service_worker')
      return WebInspector.Target.Capability.Log |
  ...
  _targetCreated: function(targetId, type, url, waitingForDebugger) {
    var connection =
        new WebInspector.SubTargetConnection(this._agent, targetId);
    this._connections.set(targetId, connection);

    var parsedURL = url.asParsedURL();
    var targetName = parsedURL ? parsedURL.lastPathComponentWithFragment() :
                                 '#' + (++this._lastAnonymousTargetId);
    var target = WebInspector.targetManager.createTarget(
        targetName, this._capabilitiesForType(type), connection, this.target());
    ...
    // Only pause new worker if debugging SW - we are going through the pause on
    // start checkbox.
    var mainIsServiceWorker = !this.target().parentTarget() &&
        this.target().hasWorkerCapability() &&
    ...
    this.dispatchEventToListeners(
        WebInspector.SubTargetsManager.Events.SubTargetAdded, target);
  },
  ...
  _targetRemoved: function(targetId) {
    ...
    this.dispatchEventToListeners(
        WebInspector.SubTargetsManager.Events.SubTargetRemoved, target);
  },
  ...
  receivedMessageFromTarget: function(targetId, message) {
    this._manager._receivedMessageFromTarget(targetId, message);
                      ...
                      * @param {string} targetId
                      */
                     WebInspector.SubTargetConnection =
        function(agent, targetId) {
  ...
  sendMessage: function(messageObject) {
    this._agent.sendMessageToTarget(
        this._targetId, JSON.stringify(messageObject));
  },
