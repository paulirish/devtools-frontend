...
WebInspector.CSSStyleDeclaration =
    function(cssModel, parentRule, payload, type) {
  ...
  _reinitialize: function(payload) {
    ...
    this.range =
        payload.range ? WebInspector.TextRange.fromObject(payload.range) : null;
    ...
    for (var i = 0; i < shorthandEntries.length; ++i) {
      this._shorthandValues.set(
          shorthandEntries[i].name, shorthandEntries[i].value);
    ...
    for (var i = 0; i < payload.cssProperties.length; ++i) {
      var property = WebInspector.CSSProperty.parsePayload(
          this, i, payload.cssProperties[i]);
  ...
  _generateSyntheticPropertiesIfNeeded: function() {
    ...
    // For style-based properties, generate shorthands with values when
    // possible.
    for (var property of this._allProperties) {
      // For style-based properties, try generating shorthands.
      var shorthands =
          WebInspector.cssMetadata().shorthands(property.name) || [];
      for (var shorthand of shorthands) {
        ...
        if (!shorthandValue)
          continue;  // Never generate synthetic shorthands when no value is
                     // available.
        ...
        var shorthandProperty = new WebInspector.CSSProperty(
            this, this.allProperties.length, shorthand, shorthandValue,
            shorthandImportance, false, true, false);
  ...
  _computeLeadingProperties: function() {
    /**
         ...
         * @return {boolean}
         */
    function propertyHasRange(property) { return !!property.range; }
  ...
  propertyAt: function(index) {
    return (index < this.allProperties.length) ? this.allProperties[index] :
                                                 null;
  },
  ...
  _insertionRange: function(index) {
    ...
    return property && property.range ? property.range.collapseToStart() :
  ...
  newBlankProperty: function(index) {
    index = (typeof index === 'undefined') ?
        this.pastLastSourcePropertyIndex() :
        index;
    var property = new WebInspector.CSSProperty(
        this, index, '', '', false, false, true, false, '',
        this._insertionRange(index));
    return property;
  },
  ...
  setText: function(text, majorChange) {
    return this._cssModel.setStyleText(
        this.styleSheetId, this.range, text, majorChange)
  },
  ...
  insertPropertyAt: function(index, name, value, userCallback) {
    this.newBlankProperty(index)
        .setText(name + ': ' + value + ';', false, true)
        .then(userCallback);
  },
