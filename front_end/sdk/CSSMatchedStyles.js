...
WebInspector.CSSMatchedStyles =
    function(
        cssModel, node, inlinePayload, attributesPayload, matchedPayload,
  ...
  function addAttributesStyle() {
    ...
    var style = new WebInspector.CSSStyleDeclaration(
        cssModel, null, attributesPayload,
  ...
  for (var i = matchedPayload.length - 1; i >= 0; --i) {
    ...
    addMatchingSelectors.call(
        this, this._node, rule, matchedPayload[i].matchingSelectors);
  }
  ...
  for (var i = 0; parentNode && inheritedPayload && i < inheritedPayload.length;
       ++i) {
    ...
    for (var j = inheritedMatchedCSSRules.length - 1; j >= 0; --j) {
      var inheritedRule = new WebInspector.CSSStyleRule(
          cssModel, inheritedMatchedCSSRules[j].rule);
      addMatchingSelectors.call(
          this, parentNode, inheritedRule,
          inheritedMatchedCSSRules[j].matchingSelectors);
  ...
  if (pseudoPayload) {
    for (var i = 0; i < pseudoPayload.length; ++i) {
      ...
      // PseudoElement nodes are not created unless "content" css property is
      // set.
      var pseudoElement =
          this._node.pseudoElements().get(entryPayload.pseudoType) || null;
      var pseudoStyles = [];
      ...
      for (var j = rules.length - 1; j >= 0; --j) {
        ...
        if (pseudoElement)
          addMatchingSelectors.call(
              this, pseudoElement, pseudoRule, rules[j].matchingSelectors);
      }
  ...
  if (animationsPayload)
    this._keyframes = animationsPayload.map(
        rule => new WebInspector.CSSKeyframesRule(cssModel, rule));
  ...
  recomputeMatchingSelectors: function(rule) {
    ...
    function querySelector(node, selectorText) {
      ...
      this._node.domModel().querySelectorAll(
          ownerDocument.id, selectorText,
          onQueryComplete.bind(this, node, selectorText, resolve));
      return promise;
    }
    ...
    function onQueryComplete(node, selectorText, callback, matchingNodeIds) {
      if (matchingNodeIds)
        this._setSelectorMatches(
            node, selectorText, matchingNodeIds.indexOf(node.id) !== -1);
      callback();
    }
  },
   ...
   * @param {!Map<!WebInspector.CSSProperty,
   * !WebInspector.CSSMatchedStyles.PropertyState>} result
   */
  _computeActiveProperties: function(styles, result) {
    ...
    for (var i = 0; i < styles.length; ++i) {
      ...
      for (var j = 0; j < allProperties.length; ++j) {
        ...
        if (!property.activeInStyle()) {
          result.set(
              property, WebInspector.CSSMatchedStyles.PropertyState.Overloaded);
          continue;
        }
        ...
        if (!isKnownProperty && inheritedFromNode &&
            !inheritedPropertyToNode.has(canonicalName))
          inheritedPropertyToNode.set(canonicalName, inheritedFromNode);

        if (property.important) {
          if (inherited && isKnownProperty &&
              inheritedFromNode !==
                  inheritedPropertyToNode.get(canonicalName)) {
            result.set(
                property,
        ...
        result.set(
            property, WebInspector.CSSMatchedStyles.PropertyState.Active);
      }

      // If every longhand of the shorthand is not active, then the shorthand is
      // not active too.
      for (var property of style.leadingProperties()) {
        ...
        for (var longhand of longhands) {
          ...
          notUsed =
              notUsed && !styleActiveProperties.has(longhandCanonicalName);
        }
        if (!notUsed)
          continue;
