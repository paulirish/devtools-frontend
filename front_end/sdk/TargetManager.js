...
WebInspector.TargetManager =
  ...
  /** @type {!Map<symbol, !Array<{modelClass: !Function, thisObject:
   * (!Object|undefined), listener: function(!WebInspector.Event)}>>} */
  ...
  SuspendStateChanged: Symbol('SuspendStateChanged')
                                        ...
                                        WebInspector.TargetManager
                                            ._listenersSymbol =
        Symbol('WebInspector.TargetManager.Listeners');
...
WebInspector.TargetManager.prototype = {
  ...
  reloadPage: function(bypassCache, injectedScript) {
    ...
    var resourceTreeModel =
        WebInspector.ResourceTreeModel.fromTarget(this._targets[0]);
  ...
  removeModelListener: function(modelClass, eventType, listener, thisObject) {
    ...
    for (var i = 0; i < listeners.length; ++i) {
      if (listeners[i].modelClass === modelClass &&
  ...
  observeTargets: function(targetObserver, capabilitiesMask) {
    ...
    this.targets(capabilitiesMask)
        .forEach(targetObserver.targetAdded.bind(targetObserver));
  ...
  createTarget: function(name, capabilitiesMask, connection, parentTarget) {
    var target = new WebInspector.Target(
        this, name, capabilitiesMask, connection, parentTarget);
    ...
    target.workerManager = target.hasWorkerCapability() ?
        new WebInspector.WorkerManager(target) :
        null;
    ...
    if (target.hasBrowserCapability()) {
      ...
      target.serviceWorkerManager = new WebInspector.ServiceWorkerManager(
  ...
  _observersForTarget: function(target) {
    return this._observers.filter(
        (observer) => target.hasAllCapabilities(
            observer[this._observerCapabiliesMaskSymbol] || 0));
  },
      ...
      for (var i = 0; i < listeners.length; ++i) {
        ...
        if (model)
          model.addEventListener(
              /** @type {symbol} */ (pair[0]), listeners[i].listener,
  ...
  removeTarget: function(target) {
    ...
    var resourceTreeModel = WebInspector.ResourceTreeModel.fromTarget(target);
    var treeModelListeners = resourceTreeModel &&
        resourceTreeModel[WebInspector.TargetManager._listenersSymbol];
  ...
  targets: function(capabilitiesMask) {
    if (!capabilitiesMask)
      ...
      return this._targets.filter(
          (target) => target.hasAllCapabilities(capabilitiesMask || 0));
  },
