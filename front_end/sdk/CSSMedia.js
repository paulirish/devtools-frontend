 ...
 */
WebInspector.CSSMediaQuery =
    function(payload) {
  this._active = payload.active;
  this._expressions = [];
  for (var j = 0; j < payload.expressions.length; ++j)
    this._expressions.push(WebInspector.CSSMediaQueryExpression.parsePayload(
 ...
 */
    WebInspector.CSSMediaQuery.parsePayload =
        function(payload) {
  return new WebInspector.CSSMediaQuery(payload);
     ...
     */
             ...
             */
            WebInspector.CSSMediaQueryExpression =
                function(payload) {
              this._value = payload.value;
              this._unit = payload.unit;
              this._feature = payload.feature;
              this._valueRange = payload.valueRange ?
                  WebInspector.TextRange.fromObject(payload.valueRange) :
                  null;
 ...
 */
                WebInspector.CSSMediaQueryExpression.parsePayload =
                    function(payload) {
 ...
 */
  ...
  _reinitialize: function(payload) {
    ...
    if (payload.mediaList) {
      ...
      for (var i = 0; i < payload.mediaList.length; ++i)
        this.mediaList.push(
            WebInspector.CSSMediaQuery.parsePayload(payload.mediaList[i]));
    }
  },
  ...
  equal: function(other) {
    if (!this.styleSheetId || !this.range || !other.range)
      return false;
    return this.styleSheetId === other.styleSheetId &&
  ...
  rawLocation: function() {
    ...
    return new WebInspector.CSSLocation(
        header, lineNumber, this.columnNumberInSource());
  }
}
