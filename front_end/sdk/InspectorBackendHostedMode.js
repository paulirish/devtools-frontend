...
// This should be executed immediately after InspectorBackend and
...
WebInspector.InspectorBackendHostedMode.loadFromJSONIfNeeded =
  ...
  for (var url of Object.keys(Runtime.cachedResources)) {
    if (url.indexOf('protocol.json') !== -1) {
      ...
      var code = WebInspector.InspectorBackendHostedMode.generateCommands(
          JSON.parse(protocol));
      eval(code);
    }
  }
 ...
 * @return {string}
 */
    WebInspector.InspectorBackendHostedMode.generateCommands =
        function(schema) {
  ...
  for (var i = 0; i < domains.length; ++i) {
    ...
    for (var j = 0; j < commands.length; ++j) {
      ...
      for (var k = 0; parameters && k < parameters.length; ++k) {
        ...
        var text = '{"name": "' + parameter.name + '", "type": "' + type +
            '", "optional": ' + (parameter.optional ? 'true' : 'false') + '}';
    ...
    for (var j = 0; domain.events && j < domain.events.length; ++j) {
      ...
      result.push(
          'InspectorBackend.registerEvent("' + domain.domain + '.' +
