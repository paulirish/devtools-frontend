...
WebInspector.HeapProfilerModel =
  ...
  target.registerHeapProfilerDispatcher(
      new WebInspector.HeapProfilerDispatcher(this));
  this._enabled = false;
  ...
  stopSampling: function() {
    ...
    return this._heapProfilerAgent
        .stopSampling(
            (error, profile) => { currentProfile = !error ? profile : null; })
        .then(() => currentProfile);
  },
  ...
  heapStatsUpdate: function(samples) {
    this.dispatchEventToListeners(
        WebInspector.HeapProfilerModel.Events.HeapStatsUpdate, samples);
  },
  ...
  addHeapSnapshotChunk: function(chunk) {
    this.dispatchEventToListeners(
        WebInspector.HeapProfilerModel.Events.AddHeapSnapshotChunk, chunk);
  },
   ...
   * @override
   */
  resetProfiles: function() { this._heapProfilerModel.resetProfiles(); }
}
