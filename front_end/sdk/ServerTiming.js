 ...
 */
WebInspector.ServerTiming =
    function(metric, value, description) {
  this.metric = metric;
  this.value = value;
 ...
 */
  ...
  var rawServerTimingHeaders =
      headers.filter(item => item.name.toLowerCase() === 'server-timing');
  ...
  function createFromHeaderValue(valueString) {
    ...
    var result = [];
    while (valueString &&
           (metricMatch = serverTimingMetricRegExp.exec(valueString))) {
  ...
  serverTimings.sort(
      (a, b) => a.metric.toLowerCase().compareTo(b.metric.toLowerCase()));
