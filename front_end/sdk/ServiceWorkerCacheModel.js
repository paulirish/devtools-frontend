...
WebInspector.ServiceWorkerCacheModel =
  ...
  WebInspector.SDKModel.call(
      this, WebInspector.ServiceWorkerCacheModel, target);
  ...
  deleteCache: function(cache) {
    ...
    function callback(error) {
      if (error) {
        console.error(
            'ServiceWorkerCacheAgent error deleting cache ', cache.toString(),
            ': ', error);
        return;
      }
    ...
    function myCallback(error) {
      if (error) {
        WebInspector.console.error(WebInspector.UIString(
            'ServiceWorkerCacheAgent error deleting cache entry %s in cache: %s',
   ...
   * @param {function(!Array.<!WebInspector.ServiceWorkerCacheModel.Entry>,
   * boolean)} callback
   */
  ...
  _loadCacheNames: function(securityOrigin) {
    ...
    function callback(error, caches) {
      if (error) {
        console.error(
            'ServiceWorkerCacheAgent error while loading caches: ', error);
        return;
      }
  ...
  _cacheAdded: function(cache) {
    this.dispatchEventToListeners(
        WebInspector.ServiceWorkerCacheModel.Events.CacheAdded, cache);
  },
  ...
  _cacheRemoved: function(cache) {
    this.dispatchEventToListeners(
        WebInspector.ServiceWorkerCacheModel.Events.CacheRemoved, cache);
  },
  ...
  _requestEntries: function(cache, skipCount, pageSize, callback) {
    ...
    function innerCallback(error, dataEntries, hasMore) {
      if (error) {
        console.error(
            'ServiceWorkerCacheAgent error while requesting entries: ', error);
        return;
      }
      var entries = [];
    ...
    this._agent.requestEntries(
        cache.cacheId, skipCount, pageSize, innerCallback);
  },
  ...
  __proto__: WebInspector.SDKModel.prototype
                                    ...
                                    */
                                   WebInspector.ServiceWorkerCacheModel.Entry =
  ...
  this.response = response;
        ...
        WebInspector.ServiceWorkerCacheModel.Cache =
            function(securityOrigin, cacheName, cacheId) {
 ...
 */
                             WebInspector.ServiceWorkerCacheModel.fromTarget =
                function(target) {
