 ...
 * @return {boolean}
 */
WebInspector.TracingModel.isNestableAsyncPhase =
    function(phase) {
 ...
 * @return {boolean}
 */
    WebInspector.TracingModel.isAsyncBeginPhase =
        function(phase) {
 ...
 * @return {boolean}
 */
        WebInspector.TracingModel.isAsyncPhase =
            function(phase) {
  return WebInspector.TracingModel.isNestableAsyncPhase(phase) ||
      phase === 'S' || phase === 'T' || phase === 'F' || phase === 'p';
}
 ...
 * @return {boolean}
 */
            WebInspector.TracingModel.isFlowPhase =
                function(phase) {
 ...
 * @return {boolean}
 */
                WebInspector.TracingModel.isTopLevelEvent =
                    function(event) {
  return event.hasCategory(WebInspector.TracingModel.TopLevelEventCategory) ||
      event.hasCategory(
          WebInspector.TracingModel.DevToolsMetadataEventCategory) &&
      event.name ===
          'Program';  // Older timelines may have this instead of toplevel.
}
 ...
 */
                    WebInspector.TracingModel._extractId =
                        function(payload) {
 ...
 */
                        WebInspector.TracingModel.browserMainThread =
  ...
  var tracingStartedInBrowser = tracingModel.devToolsMetadataEvents().filter(
  ...
  _addEvent: function(payload) {
    ...
    this._backingStorage.appendString(
        this._firstWritePending ? '[' : eventsDelimiter);
    this._firstWritePending = false;
    var stringPayload = JSON.stringify(payload);
    var isAccessible =
        payload.ph === WebInspector.TracingModel.Phase.SnapshotObject;
    var backingStorage = null;
    var keepStringsLessThan = 10000;
    if (isAccessible && stringPayload.length > keepStringsLessThan)
      backingStorage =
          this._backingStorage.appendAccessibleString(stringPayload);
    else
    ...
    if (!event)
      return;
    // Build async event when we've got events from all threads & processes, so
    // we can sort them and process in the
    // chronological order. However, also add individual async events to the
    // thread flow (above), so we can easily
    ...
    switch (payload.name) {
      ...
      case WebInspector.TracingModel.MetadataEvent.ThreadSortIndex:
        process.threadById(payload.tid)
            ._setSortIndex(payload.args['sort_index']);
        break;
  ...
  _addAsyncEvent: function(event) {
    ...
    if (!asyncEvent) {
      // Quietly ignore stray async events, we're probably too late for the
      // start.
      return;
    }
    ...
    if (event.phase === phase.AsyncStepInto ||
        event.phase === phase.AsyncStepPast) {
      var lastStep = asyncEvent.steps.peekLast();
      if (lastStep.phase !== phase.AsyncBegin &&
          lastStep.phase !== event.phase) {
        console.assert(
            false, 'Async event step phase mismatch: ' + lastStep.phase +
                ' at ' + lastStep.startTime + ' vs. ' + event.phase + ' at ' +
                event.startTime);
        return;
      }
      asyncEvent._addStep(event);
  ...
  _parsedCategoriesForString: function(str) {
    ...
    return parsedCategories;
                                                                                    ...
                                                                                    WebInspector
                                                                                        ...
                                                                                        .Event =
                                function(
                                    categories, name, phase, startTime,
                                    thread) {
  /** @type {string} */
 ...
 */
                                WebInspector.TracingModel.Event.fromPayload =
                                    function(payload, thread) {
  var event = new WebInspector.TracingModel.Event(
      payload.cat, payload.name,
      /** @type {!WebInspector.TracingModel.Phase} */ (payload.ph),
  ...
  addArgs: function(args) {
    // Shallow copy args to avoid modifying original payload which may be saved
    // to file.
    for (var name in args) {
      if (name in this.args)
        console.error(
            'Same argument name (' + name +
            ') is used for begin and end phases of ' + this.name);
  ...
  _complete: function(endEvent) {
    if (endEvent.args)
      ...
      console.error(
          'Missing mandatory event argument \'args\' at ' + endEvent.startTime);
 ...
 */
                                        WebInspector.TracingModel.Event
                                            ...
                                            function(a, b) {
  return a.startTime - b.startTime ||
      (b.endTime !== undefined && a.endTime !== undefined &&
       b.endTime - a.endTime) ||
      0;
}
 ...
 */
  ...
  return a.startTime - b.startTime || a.ordinal - b.ordinal || -1;
                                                 ...
                                                 * @param {number} startTime
                                                 * @param
                                                 * {!WebInspector.TracingModel.Thread}
                                                 * thread
                                                 */
                                                WebInspector.TracingModel
                                                    .ObjectSnapshot =
  ...
  WebInspector.TracingModel.Event.call(
      this, category, name, WebInspector.TracingModel.Phase.SnapshotObject,
      startTime, thread);
}
 ...
 */
  ...
  WebInspector.TracingModel.Event
      .call(
          this, startEvent.categoriesString, startEvent.name, startEvent.phase,
          startEvent.startTime,
          startEvent.thread) this.addArgs(startEvent.args);
  ...
  _addStep: function(event) {
    ...
    if (event.phase === WebInspector.TracingModel.Phase.AsyncEnd ||
      ...
      // FIXME: ideally, we shouldn't do this, but this makes the logic of
      // converting
  ...
  return array.sort(comparator);
                                                                     ...
                                                                     * @constructor
                                                                     * @extends
                                                                     * {WebInspector.TracingModel.NamedObject}
                                                                     * @param
                                                                     * {!WebInspector.TracingModel}
                                                                     * model
                                                                     * @param
    ...
    for (var i = 0; i < this._events.length; ++i) {
      ...
      switch (e.phase) {
        case phases.End:
          ...
          var top = stack.pop();
          if (top.name !== e.name ||
              top.categoriesString !== e.categoriesString)
            console.error(
  ...
  _addEvent: function(payload) {
    ...
    if (WebInspector.TracingModel.isTopLevelEvent(event)) {
      ...
      if (this._lastTopLevelEvent &&
          this._lastTopLevelEvent.endTime > event.startTime)
        return null;
