...
WebInspector.Persistence =
    function(workspace, breakpointManager, fileSystemMapping) {
  ...
  this._eventListeners = [
    workspace.addEventListener(
        ...
        this._onUISourceCodeAdded, this),
    workspace.addEventListener(
        WebInspector.Workspace.Events.UISourceCodeRemoved,
        this._onUISourceCodeRemoved, this),
    workspace.addEventListener(
        WebInspector.Workspace.Events.ProjectRemoved, this._onProjectRemoved,
        this),
    this._fileSystemMapping.addEventListener(
        WebInspector.FileSystemMapping.Events.FileMappingAdded, this._remap,
        this),
    this._fileSystemMapping.addEventListener(
        WebInspector.FileSystemMapping.Events.FileMappingRemoved, this._remap,
        this)
  ];
  this._remap();
}
...
WebInspector.Persistence
  ...
  _remap: function() {
    ...
    var networkProjects =
        this._workspace.projectsForType(WebInspector.projectTypes.Network);
  ...
  _createBinding: function(uiSourceCode) {
    if (uiSourceCode.project().type() ===
      ...
      var fileSystemPath =
          WebInspector.FileSystemWorkspaceBinding.fileSystemPath(
              uiSourceCode.project().id());
      var networkURL = this._fileSystemMapping.networkURLForFileSystemURL(
          fileSystemPath, uiSourceCode.url());
      var networkSourceCode =
          networkURL ? this._workspace.uiSourceCodeForURL(networkURL) : null;
    ...
    if (uiSourceCode.project().type() === WebInspector.projectTypes.Network) {
      ...
      var projectId = file ?
          WebInspector.FileSystemWorkspaceBinding.projectId(
              file.fileSystemPath) :
          null;
      var fileSourceCode = file && projectId ?
          this._workspace.uiSourceCode(projectId, file.fileURL) :
          null;
      return fileSourceCode ?
          new WebInspector.PersistenceBinding(uiSourceCode, fileSourceCode) :
          null;
    }
    return null;
  },
  ...
  _unbind: function(uiSourceCode) {
    ...
    this.dispatchEventToListeners(
        WebInspector.Persistence.Events.BindingRemoved, binding);
  },
  ...
  _onWorkingCopyCommitted: function(event) {
    ...
    var other =
        binding.network === uiSourceCode ? binding.fileSystem : binding.network;
    if (Runtime.queryParam('v8only')) {
      other.requestContent().then(
          currentContent => this._syncNodeJSContent(
              binding, other, currentContent, newContent));
      return;
    }
   ...
   */
  _syncNodeJSContent: function(
      binding, uiSourceCode, currentContent, newContent) {
    if (uiSourceCode === binding.fileSystem) {
      ...
      if (newContent.startsWith(WebInspector.Persistence._NodeShebang))
        newContent =
            newContent.substring(WebInspector.Persistence._NodeShebang.length);
  ...
  _moveBreakpoints: function(from, to) {
    ...
    for (var breakpoint of breakpoints) {
      ...
      this._breakpointManager.setBreakpoint(
          to, breakpoint.lineNumber(), breakpoint.columnNumber(),
  ...
  hasUnsavedCommittedChanges: function(uiSourceCode) {
    if (this._workspace.hasResourceContentTrackingExtensions())
      return false;
    if (uiSourceCode.url() &&
        WebInspector.fileManager.isURLSaved(uiSourceCode.url()))
      return false;
  ...
  __proto__: WebInspector.Object.prototype
                                ...
                                */
                               WebInspector.PersistenceBinding =
