...
WebInspector.SCSSParser =
  ...
  parse: function(content) {
    ...
    /** @type {!Array<!{properties: !Array<!Gonzales.Node>, node:
     * !Gonzales.Node}>} */
  ...
  _handleBlock: function(block, output) {
    var selectors =
        block.node.selectors.map(WebInspector.SCSSParser.rangeFromNode);
    var properties = [];
    ...
    for (var node of block.properties) {
      if (node.type === 'declaration')
        ...
        this._handleInclude(node, properties);
          ...
          node.type === 'multilineComment' && node.start.line === node.end.line)
        this._handleComment(node, properties);
    }
    if (!selectors.length && !properties.length)
      return;
    var rule =
        new WebInspector.SCSSParser.Rule(selectors, properties, styleRange);
    output.push(rule);
  },
  ...
  _handleDeclaration: function(node, output) {
    ...
    var property = new WebInspector.SCSSParser.Property(
        range, nameRange, valueRange, false);
  ...
  _handleInclude: function(node, output) {
    ...
    if (!mixinArguments)
      return;
    var parameters = mixinArguments.content.filter(
        node => node.type !== 'delimiter' && node.type !== 'space');
 ...
 */
                                        WebInspector.SCSSParser.rangeFromNode =
        function(node) {
  return new WebInspector.TextRange(
      node.start.line - 1, node.start.column - 1, node.end.line - 1,
      node.end.column);
}
         ...
         */
        WebInspector.SCSSParser.Property =
            function(range, nameRange, valueRange, disabled) {
  this.range = range;
  this.name = nameRange;
  ...
  rebaseInsideOneLineComment: function(commentNode) {
    ...
    function rebaseRange(range, lineOffset, columnOffset) {
      return new WebInspector.TextRange(
          range.startLine + lineOffset, range.startColumn + columnOffset,
          range.endLine + lineOffset, range.endColumn + columnOffset);
    }
  }
}
                             ...
                             /**
                              * @constructor
                              * @param {!Array<!WebInspector.TextRange>}
                              * selectors
                              * @param
                              * {!Array<!WebInspector.SCSSParser.Property>}
                              * properties
                              * @param {!WebInspector.TextRange} styleRange
                              */
                             WebInspector.SCSSParser.Rule =
                function(selectors, properties, styleRange) {
  ...
  this.styleRange = styleRange;
                ...
                /**
                 * @param {!Gonzales.Node} node
                 * @param {!Array<{node: !Gonzales.Node, properties:
                 * !Array<!Gonzales.Node>}>} blocks
                 * @param {!{node: !Gonzales.Node, properties:
                 * !Array<!Gonzales.Node>}} lastBlock
                 */
                WebInspector.SCSSParser.extractNodes = function(
                    node, blocks, lastBlock) {
  ...
  for (var i = 0; i < node.content.length; ++i) {
    ...
    if (child.type === 'include' || child.type === 'declaration' ||
        child.type === 'multilineComment')
      lastBlock.properties.push(child);
    if (child.type === 'declaration') {
      lastDeclaration = child;
      lastDeclaration.declarationRange =
          WebInspector.TextRange.createFromLocation(
              child.start.line - 1, child.start.column - 1);
    }
