...
/******/
	   ...
	   */
            ...
            Node.prototype.toString = function toString() {
              ...
              try {
                stringify =
                    __webpack_require__(2)('./' + this.syntax + '/stringify');
              } catch (e) {
                var message =
                    'Syntax "' + this.syntax + '" is not supported yet, sorry';
            ...
            Node.prototype.traverseByType = function traverseByType(
                type, callback) {
            ...
            Node.prototype.traverseByTypes = function traverseByTypes(
                types, callback) {
              this.traverse(function(node) {
	 ...
	 */
          function parser(css, options) {
            ...
            if (!syntaxParser) {
              var message =
                  'Syntax "' + syntax + '" is not supported yet, sorry';
              return console.error(message);
            }
	   ...
	   */
                toString: function() {
                  return [
                    this.name + ': ' + this.message, '', this.context, '',
                    'Syntax: ' + this.syntax,
          ...
          module.exports = (function() {
            ...
            */
            function markSC(tokens) {
              ...
              // as spaces (set both `ws` and `sc` flags). Mark multiline
              // comments
              // with `sc` flag.
            ...
            */
            function markBrackets(tokens) {
              ...
              // If we then meet a closing (right) bracket, look at the
              // corresponding
              ...
              // this index number) and save right bracket's index as a
              // reference:
              for (var i = 0; i < tokensLength; i++) {
                t = tokens[i];
                switch (t.type) {
        ...
        function(module, exports, __webpack_require__) {
          ...
          var contexts = {
            ...
            'conditionalStatement': function() {
              return checkConditionalStatement(pos) &&
...
*/
          ...
          function getLastPositionForString(content, line, column, colOffset) {
            ...
            var prevLinebreak = linebreaksCount === 0 || linebreaksCount === 1 ?
                -1 :
                content.length - splitContent[linebreaksCount - 1].length -
                    2;  // Line:
            var offset =
                endsWithLinebreak ? linebreaksCount - 1 : linebreaksCount;
            position[0] = line + offset;  // Column:
            if (endsWithLinebreak) {
              offset = prevLinebreak !== -1 ? content.length - prevLinebreak :
                                              content.length - 1;
            } else {
...
*/
          function checkAny(i) {
            return checkBrackets(i) || checkParentheses(i) || checkString(i) ||
                checkVariablesList(i) || checkVariable(i) ||
                checkPlaceholder(i) || checkPercentage(i) ||
                checkDimension(i) || checkNumber(i) || checkUri(i) ||
                checkExpression(i) || checkFunction(i) ||
                checkInterpolation(i) || checkIdent(i) || checkClass(i) ||
                checkUnary(i);
          } /**
* @returns {Array}
*/
...
*/
          function checkArgument(i) {
            return checkBrackets(i) || checkParentheses(i) ||
                checkDeclaration(i) || checkFunction(i) ||
                checkVariablesList(i) || checkVariable(i) || checkSC(i) ||
                checkDelim(i) || checkDeclDelim(i) || checkString(i) ||
                checkPercentage(i) || checkDimension(i) || checkNumber(i) ||
                checkUri(i) || checkInterpolation(i) || checkIdent(i) ||
                checkVhash(i) || checkOperator(i) || checkUnary(i);
          } /**
...
*/
          function checkAtkeyword(i) {
            ...
            if (i >= tokensLength ||
                tokens[i++].type !== TokenType.CommercialAt)
              return 0;
...
*/
          function checkAtrule(i) {
            var l;
            if (i >= tokensLength)
              return 0;  // If token already has a record of being part of an
                         // @-rule,
            // return the @-rule's length:
            if (tokens[i].atrule_l !== undefined)
              return tokens[i].atrule_l;  // If token is part of an @-rule, save
                                          // the rule's type to token:
            if (l = checkKeyframesRule(i))
              ...
              return 0;  // If token is part of an @-rule, save the rule's
                         // length to token:
            tokens[i].atrule_l = l;
            return l;
          } /**
...
*/
          function checkAtruler(i) {
            ...
            if (l = checkTsets(i))
              i += l;
            if (i < tokensLength &&
                tokens[i].type === TokenType.LeftCurlyBracket)
              i++;
            else
              return 0;
            if (l = checkAtrulers(i))
              i += l;
            if (i < tokensLength &&
                tokens[i].type === TokenType.RightCurlyBracket)
              i++;
            else
              return 0;
            return i - start;
...
*/
	 ...
	 */
          function getBrackets() {
            ...
            pos++;
            return newNode(
                NodeType.BracketsType, tsets, token.ln, token.col, end);
          } /**
...
*/
          function checkCombinator2(i) {
            ...
            if (type === TokenType.PlusSign ||
                type === TokenType.GreaterThanSign || type === TokenType.Tilde)
              return 1;
            else
              return 0;
          }
...
*/
          function checkCommentML(i) {
            return i < tokensLength && tokens[i].type === TokenType.CommentML ?
                1 :
                0;
          } /**
...
*/
          function checkCommentSL(i) {
            return i < tokensLength && tokens[i].type === TokenType.CommentSL ?
                1 :
                0;
          } /**
...
*/
          ...
          function _checkCondition(i) {
            return checkVariable(i) || checkNumber(i) ||
                checkInterpolation(i) || checkIdent(i) || checkOperator(i) ||
                checkCombinator(i) || checkString(i);
          } /**
...
*/
          function getConditionalStatement() {
            ...
            var token = tokens[startPos];
            return newNode(
                NodeType.ConditionalStatementType, x, token.ln, token.col);
          } /**
* Check if token is part of a declaration (property-value pair)
* @param {Number} i Token's index number
* @returns {Number} Length of the declaration
...
*/
          function checkDeclDelim(i) {
            return i < tokensLength && tokens[i].type === TokenType.Semicolon ?
                1 :
                0;
          } /**
...
*/
          function checkDefault(i) {
            ...
            var l = undefined;
            if (i >= tokensLength ||
                tokens[i++].type !== TokenType.ExclamationMark)
              return 0;
            if (l = checkSC(i))
              i += l;
...
*/
          function checkDelim(i) {
            return i < tokensLength && tokens[i].type === TokenType.Comma ? 1 :
                                                                            0;
          } /**
...
*/
          function getDimension() {
            ...
            var token = tokens[pos];
            var ident =
                newNode(NodeType.IdentType, getNmName2(), token.ln, token.col);
            x.push(ident);
...
*/
          function getExpression() {
            ...
            return newNode(
                NodeType.ExpressionType, e, token.ln, token.col, end);
          }
...
*/
          ...
          function getExtend1() {
            var startPos = pos;
            var x = [].concat(
                [getAtkeyword()], getSC(), getSelectorsGroup(), getSC(),
                [getOptional()]);
            var token = tokens[startPos];
...
*/
          function checkIdent(i) {
            ...
            var l = undefined;
            if (i >= tokensLength)
              return 0;  // Check if token is part of an identifier starting
                         // with `_`:
            ...
            if (tokens[i].type === TokenType.HyphenMinus &&
              ...
              return 0;  // If token is a character, `-`, `$` or `*`, skip it &
                         // continue:
            if (l = _checkIdent(i))
              i += l;
            else
...
*/
          function getInclude1() {
            ...
            var x = [].concat(
                ...
                getArguments(), getSC(), getKeyframesBlocks());
            var token = tokens[startPos];
...
*/
          function getInclude5() {
            var startPos = pos;
            var x =
                [].concat(getAtkeyword(), getSC(), getIdentOrInterpolation());
            var token = tokens[startPos];
            return newNode(NodeType.IncludeType, x, token.ln, token.col);
          } /**
...
*/
          function checkInterpolation(i) {
            ...
            return tokens[i].type === TokenType.RightCurlyBracket ?
                i - start + 1 :
                0;
          } /**
...
*/
          function getInterpolation() {
            ...
            pos++;
            return newNode(
                NodeType.InterpolationType, x, token.ln, token.col, end);
          }
          ...
          function getKeyframesBlock() {
            ...
            var column = token.col;
            var content =
                [].concat([getKeyframesSelector()], getSC(), [getBlock()]);
            return newNode(type, content, line, column);
          }
	 ...
	 */
          ...
          function checkKeyframesSelector(i) {
            ...
            if (i >= tokensLength)
              return 0;
            if (l = checkIdent(
                    i)) {  // Valid selectors are only `from` and `to`.
          ...
          function getKeyframesSelector() {
            ...
            var keyframesSelector =
                newNode(keyframesSelectorType, content, line, column);
            ...
            while (i < tokensLength) {
              if (l = checkBlock(i)) {
                ...
                break;
                  ...
                  l = checkVariable(i) || checkNumber(i) ||
                      checkInterpolation(i) || checkIdent(i) || checkSC(i) ||
...
*/
          function checkNmName2(i) {
            if (tokens[i].type === TokenType.Identifier)
              ...
              return 0;
            i++;
            return i < tokensLength && tokens[i].type === TokenType.Identifier ?
                2 :
                1;
          } /**
* @returns {String}
*/
          function getNmName2() {
            ...
            if (tokens[pos++].type === TokenType.DecimalNumber &&
                pos < tokensLength && tokens[pos].type === TokenType.Identifier)
              s += tokens[pos++].value;
            return s;
          } /**
...
*/
          function checkNumber(i) {
            ...
            if (i < tokensLength &&
                tokens[i].type === TokenType.DecimalNumber && tokens[i + 1] &&
...
*/
          function checkParentSelector(i) {
            return i < tokensLength && tokens[i].type === TokenType.Ampersand ?
                1 :
                0;
          } /**
...
*/
          function getParentSelector() {
            ...
            var token = tokens[startPos];
            return newNode(
                NodeType.ParentSelectorType, '&', token.ln, token.col);
          }
...
*/
          function getPercentage() {
            ...
            pos++;
            return newNode(
                NodeType.PercentageType, x, token.ln, token.col, end);
          } /**
...
*/
          function checkPropertyDelim(i) {
            return i < tokensLength && tokens[i].type === TokenType.Colon ? 1 :
                                                                            0;
          } /**
...
*/
          function getPropertyDelim() {
            ...
            var token = tokens[startPos];
            return newNode(
                NodeType.PropertyDelimType, ':', token.ln, token.col);
          } /**
* @param {Number} i Token's index number
* @returns {Number}
*/
...
*/
          function checkPseudoe(i) {
            ...
            if (i >= tokensLength || tokens[i++].type !== TokenType.Colon ||
                i >= tokensLength || tokens[i++].type !== TokenType.Colon)
              return 0;
...
*/
          function checkSC(i) {
            ...
            var lsc = 0;
            while (i < tokensLength) {
              if (!(l = checkS(i)) && !(l = checkCommentML(i)) &&
                  !(l = checkCommentSL(i)))
                break;
              i += l;
              lsc += l;
...
*/
          function checkStylesheet(i) {
            ...
            var l = undefined;
            while (i < tokensLength) {
              if (l = checkSC(i) || checkDeclaration(i) || checkDeclDelim(i) ||
                      checkInclude(i) || checkExtend(i) || checkMixin(i) ||
                      checkLoop(i) || checkConditionalStatement(i) ||
                      checkAtrule(i) || checkRuleset(i))
                i += l;
              else
                throwError(i);
            }
...
*/
          function checkTset(i) {
            return checkVhash(i) || checkOperator(i) || checkAny(i) ||
                checkSC(i) || checkInterpolation(i);
          } /**
* @returns {Array}
*/
          function getTset() {
...
*/
          function getUri() {
            ...
            if (checkUriContent(pos)) {
              ...
              token = tokens[pos];
              raw = newNode(
                  NodeType.RawType, joinValues(pos, pos + l), token.ln,
                  token.col);
              uri.push(raw);
              pos += l + 1;
...
*/
          function _checkValue(i) {
            return checkInterpolation(i) || checkVariable(i) || checkVhash(i) ||
                checkBlock(i) || checkAtkeyword(i) || checkOperator(i) ||
                checkImportant(i) || checkGlobal(i) || checkDefault(i) ||
                checkProgid(i) || checkAny(i);
          } /**
* @returns {Array}
*/
          function getValue() {
...
*/
          function getVariablesList() {
            ...
            pos += 3;
            return newNode(
                NodeType.VariablesListType, [x], token.ln, token.col, end);
          } /**
* Check if token is part of a hexadecimal number (e.g. `#fff`) inside
*      some value
* @param {Number} i Token's index number
...
*/
          ...
          function checkCompoundSelector1(i) {
            ...
            if (l = checkTypeSelector(i) || checkPlaceholder(i) ||
              ...
              return 0;
            while (i < tokensLength) {
              var _l2 = checkShash(i) || checkClass(i) ||
                  checkAttributeSelector(i) || checkPseudo(i) ||
                  checkPlaceholder(i);
              if (_l2)
                i += _l2;
              else
          ...
          function checkCompoundSelector2(i) {
            ...
            var start = i;
            while (i < tokensLength) {
              var l = checkShash(i) || checkClass(i) ||
                  checkAttributeSelector(i) || checkPseudo(i) ||
                  checkPlaceholder(i);
              if (l)
                i += l;
...
*/
          ...
          function checkAttributeMatch1(i) {
            ...
            if (type === TokenType.Tilde || type === TokenType.VerticalLine ||
                ...
                type === TokenType.DollarSign || type === TokenType.Asterisk)
              i++;
            else
              return 0;
        ...
        function(module, exports, __webpack_require__) {
          ...
          module.exports = function(css, tabSize) {
            ...
            function pushToken(type, value, column) {
              tokens.push(
                  {tn: tn++, ln: ln, col: column, type: type, value: value});
            }
            ...
            function parseDecimalNumber(css) {
              ...
              pushToken(
                  TokenType.DecimalNumber, css.substring(start, pos--), col);
              col += pos - start;
            }
            ...
            */
            function parseMLComment(css) {
              ...
              // Add full comment (including `/*` and `*/`) to the list of
              // tokens:
            ...
            */
            function parseSLComment(css) {
              ...
              // Add comment (including `//` and line break) to the list of
              // tokens:
            ...
            function getTokens(css) {
              ...
              for (pos = 0; pos < css.length; col++, pos++) {
                ...
                // If current character is a double or single quote, it's a
                // start
                // of a string:
                else if (c === '"' || c === '\'') {
                  parseString(css, c);
                ...
                // If current character is a punctuation mark:
                  ...
                  if (c === '\r' && cn === '\n' || c === '\n') {
                    ...
                    // above
                    // so we push a CRLF token type to the token list and
                    // importantly
                    // skip the next character so as not to double count
                    // newlines or
                    // columns etc
                    if (c === '\r') {
                      pushToken(TokenType.Newline, '\r\n', col);
                      pos++;  // If CRLF skip the next character and push crlf
                              // token
                    } else if (c === '\n') {
                      // If just a LF newline and not part of CRLF newline we
                      // can just
